...
- [ ] Add `DockerAllowlisted bool` with JSON tag `docker_allowlisted` to `RepoDTO` (`apitypes/repo.go`), commented as a computed, caller-scoped boolean (not the list).
- [ ] Compute it in **both** `ListProjects` (`forge.go:549`) and `ListRepos` (`forge.go:594`) loops, alongside `GuardrailBlocked`, by membership-testing the repo id against `h.settings.DockerRepoAllowlist(ctx)` read **once per request** (not per repo). Leave `repoToDTO` and `PatchRepo` unchanged (they omit computed fields, matching `guardrail_blocked`).
- [ ] Add the JSON-tag assertion for the new field to `apitypes/wire_test.go` (sibling of `TestGuardrailOverrideDTOTags`, `:449`).
- [ ] Add `docker_allowlisted: boolean` to the web `Repo` type (`api.ts:346`) and to the repo mock fixtures (`web/src/mocks/`).
- [ ] CLI parity (repo convention): add a `DOCKER` column to `uzi repo list` (`cmd/uzi/repo.go:35`) rendering `boolStr(r.DockerAllowlisted)` (gate behind `--wide` if the row gets too wide).
...
- [ ] Tone rules: neutral/muted by default; **info** (sky) when `docker_blocked`; a quiet **ok** "Ready" when all four are on. Never red/amber. Copy frames off capabilities as *optional*, never "missing". The Docker row, when off and `docker_blocked`, says a queued run is waiting and points to Admin -> Docker workers.
- [ ] Wire into the Repos row badge cell (`Repos.tsx:404`), only for `enabled` repos (matching the existing `!r.enabled` guards). Respect `prefers-reduced-motion`; visible keyboard focus; use ember tokens / the `Badge` tone system.
...
- [ ] Note the Setup indicator and the new queued reason where the repo/admin surfaces are described (`ARCHITECTURE.md` forge/board section and/or the relevant `docs/*.md`), including the re-add-loses-allowlist-seat failure mode. Respect `docs/` frontmatter rules (`web/scripts/check-docs.mjs` runs in the web build).
- [ ] Final cross-cutting pass: `task gate:api`, `task gate:web`, `task gate:controller` (unaffected, must stay green), and the store-it live-DB sweep (`./e2e/run-store-it.sh`) all green together.
