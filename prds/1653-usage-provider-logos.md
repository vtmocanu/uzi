...
- 2026-09-25 (maintainer-approved at the plan gate): in the TUI an unknown or non-positive `limit_window_seconds` is labelled `?`, not `window` (D-T3 amended); the web keeps `window`. Reason: the detail rail is 26 columns with a 10-column bar, so a 6-character label plus the percent and reset countdown overflows it.
