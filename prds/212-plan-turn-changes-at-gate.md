...
- [ ] **M1 — api + DB carry `plan_changed_files` end to end.** Migration adds the
...
- [ ] **M2 — the worker computes and sends the list at the gate, as the runner uid.**
...
- [ ] **M3 — web approval gate shows "Files changed during planning".** `PlanPanel`
...
- [ ] **M4 — `uzi run get` shows the list.** A new `renderRunDetail` block prints the
...
- [ ] **M5 — docs + specs reflect the new gate surface.** Update the gate/approval
...
prior-art convention (bottega / multica / dot-agent-deck) is **not load-bearing** here
