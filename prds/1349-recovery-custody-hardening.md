...
- 2026-09-14: M1–M7 implemented offline against the frozen run base without any mid-run `git rebase`/`merge`/branch switch, per the parallel-dispatch operating rule (active PRD #1332 overlaps `protocol.ts`, `runner.ts`, `workersvc/claim.go`, and the migrations directory). No overlapping conflict arose; migration `00226_recovery_custody_episode.sql` stays a draft number to be renumbered above the live head at landing, which the worker's trusted base-alignment path owns. Landed as seven per-milestone commits (M1 seam → M4/M2/M3 → M5 → M6-go/M6-web → M7), each with its own read-only validator wave.
