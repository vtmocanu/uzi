...
2. **Follow multica's derived-status pattern for the chip.** Precompute a small display enum server-side or in one web helper (`mrChipState(mr_state)`), never scatter raw forge state strings through components — prior art `inspiration/multica/packages/core/github/pull-request-status.ts`, already flagged as the pattern to copy in PRD #24 §4.
...
8. **Migration number is a draft.** Drafted as `00045` (ledger: live head `00043`; `00044` drafted by PRD #32; `00042`+ drafted by PRD #25 is stale — pre-dates `00042`/`00043` landing). Final number assigned at merge time above the live head, per the CLAUDE.md convention.
...
### 2. Deliberate-stop signal (api + web, migration draft `00045`)
