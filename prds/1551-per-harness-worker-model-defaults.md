...
Credential availability never makes settings fail. If D11 cannot resolve a usable harness, or a request explicitly clears `default_harness`, an incoming legacy `default_model` value targets the Claude lane. If both explicit fields and the legacy field are present, the handler first determines that input target and requires the legacy value to equal the corresponding explicit lane; a conflict returns 400 and writes nothing. The stored legacy column and GET response instead project the lane of the harness D11 will actually choose after the save. Thus clearing `default_harness` with only Codex usable routes an incoming legacy value to Claude but mirrors Codex for old app versions. The compatibility behavior, null/default cases and deprecation are contract-tested. Removal, if ever wanted, is a later contract migration.
...
| 2026-09-23 | A post-claim custom-capability requeue retains worker affinity until the configured ceiling | A resumed run may have unpublished work on the prior worker's volume; clearing ownership to accelerate peer placement could lose it |
...
- 2026-09-23: CodeRabbit review on PR #1578 found an incorrect legacy compatibility projection for an unusable harness pin. A regression failed on the original handler and passed with the fix; the live-DB sweep then passed 1,112 tests with no skips. Its affinity-delay finding matches the approved bounded-resume tradeoff, which is now explicit in code, test and hosted-worker docs.
