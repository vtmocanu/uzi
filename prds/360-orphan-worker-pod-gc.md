...
- [ ] Extend the render test (`controller/internal/kube/render_test.go`, near `TestRenderedPodPosture`) to assert `RevisionHistoryLimit` is set to the chosen value on the rendered Deployment. One assertion suffices: the field is set unconditionally at `DeploymentSpec` level, outside the plain-vs-docker branching (which lives entirely in `podTemplate`), so it does not vary by tier.
...
- [ ] Record the worker-Deployment GC behaviour where the controller's worker lifecycle is described (a short note in `ARCHITECTURE.md`'s Agent-runtime/hosted-workers section, or a docstring on `RenderDeployment`), so the next reader knows the limit is deliberate and why.
- [ ] Note in the PRD Decision Log (below) that live-cluster confirmation - watching the superseded RSes and their wedged pods disappear on meta-dev-02 after the controller rolls with the new limit - is a **maintainer follow-up**, out of scope for an offline worker.
