...
1. **Declare (both sources — user decision 2026-07-18):**
   ...
   - **Plan-time inference** by the lead: the planning step scans the clone (`docker-compose*.yml`, `Dockerfile`, testcontainers deps, test scripts invoking `docker`) and emits a structured `required_capabilities` **plus an estimated size class** (mem/CPU/disk) in the plan, alongside the agents it already proposes. The planner emits *requirements*, not a worker pick (Decision 8). **Authoritative and escalation-only** — it can add a requirement the hint missed, never remove a hinted one.
...
- The lead's plan step (`agent/src` planning path) gains a bounded capability-inference pass over the clone and emits `required_capabilities` **+ an estimated size class** in the plan payload/schema, next to the proposed agents (Decision 8 — requirements, not a worker pick).
...
### 5. Surfacing (web + CLI)
- Plan approval shows required capabilities + estimated size and per-worker satisfaction; the block state renders the remediation spectrum.
...
- [ ] **M4 — Plan-time inference + gate.** Lead emits `required_capabilities` **+ estimated size** from a clone scan; the gate blocks on non-empty `unmet` **or size shortfall** with the remediation spectrum; on remediation the run **requeues** to a capable worker (Path 2). Plan schema + approval DTO carry caps + size + satisfaction. **Worker suggestion + select-time mismatch flag** in the UI.
