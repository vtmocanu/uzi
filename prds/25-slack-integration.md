...
- [ ] **M5 — Reply-from-Slack**: thread replies → reject-pending reason / `follow_up` on live runs, ✅ ack reactions, unlinked/finished-run ephemerals (coalesced), per-user inbound rate limit — **plus a per-user rate limit on the two DM-triggering `/me/slack` endpoints (override, test-dm)**: M3 review found an authenticated user can hammer them to DM-spam arbitrary (enumerable) member ids; no content/authz leak (anti-squat holds), but rate-limit it here where the inbound limiter lands anyway.
- [ ] **M6 — Tests passing**: the suites above green (`go test ./...`, `npm test`, e2e unchanged with Slack off).
- [ ] **M7 — Docs & specs**: `docs/slack.md` (audience: user) with the Slack app manifest YAML (scopes: `chat:write`, `im:write`, `im:history`, `users:read`, `users:read.email`; Socket Mode + interactivity + `message.im` event) + setup steps + a privacy note that `users:read.email` lets uzi resolve workspace members' emails and that enabling Slack sends run status metadata to Slack's cloud; ARCHITECTURE.md trust-model addendum; `specs/ai.md` decision record.

### Progress checkpoint (2026-07-09, agent-team session 1)

Resume point: branch `prd-25-slack-integration`, tip `2d4f817`, all gates green (`go build -buildvcs=false ./... && go test ./...` + `-race` on slacksvc/handler; web typecheck + 283 tests + build; gitleaks per commit). Worktree: `../prd-25-slack-integration`.

- **Agent-review status**: M1 + M2 + M3 parts 1-2 fully reviewed AND audited (verdicts: approve / no blocking). M3 parts 3+4 and the injection fix (`d342849`, `8b49978`, `cee0985`, `2d4f817`) are implemented and gate-green but **awaiting reviewer + auditor verdicts** — wave dispatched at EOD; findings land in `.claude/agent-team-tasks/prd-25-m3-final-review.md` / `-audit.md` (gitignored, worktree-only). Resolve any blocking findings before M4.
- **Remaining**: M4 (approval gate), M5 (reply-from-Slack), M6 (e2e + the auditor-recommended raw-DB token byte-check assertion), M7 (docs/specs). M4's inbound-router foundation already exists (`routeInteractive` seam built in M3 to the PRD's inbound rules — M4 only adds action kinds).
