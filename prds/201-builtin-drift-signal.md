...
3. **CORRECTED — compare FOUR mutable columns, not five.** `description`, `model`, `tools`,
   ...
   **Never `Render(def)`.** Two reasons, and only the second one holds for M4a:
   ...
   - #85 M2 stamps `version:` (`prds/done/85-agenttmpl-role-library-sync.md`), which would
...
5. `GET /agent-templates/{id}/builtin` returns the shipped definition to a caller passing
   ...
   **Handler wiring now pinned — see Amendment 11 (issue #223):** M4a's status matrix pinned
   `writeBuiltinDefinition`, but the handler's delegation — that it serves the shipped
   definition, not the stored row — was 0.0% covered until
   `TestGetBuiltinAgentTemplateServesShippedNotStored`.
...
9. **Reset clears the badge with no refetch** — `templateDTO` is the reset response
   ...
   **MET — see Amendment 11 (issue #223).** The CLIENT half is measured twice over: a
   browser run with counters (reset 0→1, all three reads 0→0) and a same-visible-outcome fold
   proving the no-refetch assertion fires on its own channel. **The SERVER half is now pinned
   too:** `TestResetAgentTemplateClearsBadgeServerSide` (issue #223) drives `ResetAgentTemplate`
   through the `agentTemplateWriteStore` seam and asserts `differs_from_builtin` is false after
   reset; folding the handler to return the pre-reset row — so the badge never clears — is
   mutation-fold verified to redden it.
...
### Amendment 12 — 2026-08-23. Issue #223 item 3: the shared drift-predicate fixture ships; #223 closed.

R1 (Amendment 3 §R1) is done, and it is done the way it prescribed: **"the repo already
has the pattern… do it there, not here"**, pointing at `fixtures/run-usage/`.

`fixtures/agent-template-drift/cases.json` is now a shared `(shipped, stored, expected)`
case table pinning all three drift-predicate implementations — `agenttmpl.SameContent`
(server), `mockApi.ts`'s `sameContent` (mock), and `agentTemplates.ts`'s `driftedColumns`
(client) — to one artifact, exactly as `fixtures/run-usage/` pins the two `run_usage`
readers. Two contract tests read it, each folding the table with its own production
predicate rather than diffing Go against JS directly:
`api/internal/agenttmpl/drift_contract_test.go` and
`web/src/lib/agentTemplateDriftContract.test.ts`. Cross-boundary placement means
`-count=1` is load-bearing on the Go half — the fixture sits above `api/`, outside that
module, and contributes nothing to the package's cache key, so a fixture-only edit
otherwise leaves `go test` printing `ok (cached)` over a gutted fixture. `task test:api`
/ `task gate:api` already carry it. See `fixtures/agent-template-drift/README.md` for the
schema, the hand-authored-not-recorded distinction, and the load-bearing case list.

This closes **issue #223's last open item.** Amendment 11 left item 3 deliberately for a
later run, unblocked but not urgent, because the 2026-08-23 triage had already downgraded
the residual risk from data loss to a self-contradicting UI badge (M4b/#275's
`RefreshPristineBuiltin` gates on the `customized`/`origin` flags, not on any content
predicate). Nothing here changes that triage; this is a consistency pin landing on its
own schedule, not against a deadline.

**Not a milestone completion for this PRD.** PRD #201 spans milestones beyond M4a/#223 —
no acceptance-criteria checkbox changes here and the file stays where it is, not moved to
`prds/done/`.

### Amendment 11 — 2026-08-23. Issue #223 items 1 & 2: the handler seam and the two coverage gaps behind it.

The M4a residual §478 recorded — "criterion 5 covered is what must not be said" and criterion 9's
"SERVER half is unpinned" — was ONE structural fact: `Handler.q` is a concrete `*store.Queries`, so no
fake substituted without a live DB and the two DB-touching handlers `GetBuiltinAgentTemplate` and
`ResetAgentTemplate` had no handler-level test. Issue #223 items 1 & 2 close it:

- **Narrow seam, not a widened `Handler.q`.** A two-method interface `agentTemplateWriteStore`
  (`GetAgentTemplate`, `ResetBuiltinAgentTemplate`), satisfied by `*store.Queries`, injected via a
  nil-safe `h.templateWriteStore()` accessor (the `slackVal()` idiom). Production leaves the field nil
  and falls back to `h.q`; only a struct-literal test handler injects a fake. Deliberately narrow to
  bound the blast radius rather than making `Handler.q` an interface (which would enumerate every query
  the package uses). `api/internal/handler/handler.go`, `api/internal/handler/agent_templates.go`.
- **Two mutation-pinned tests**, `api/internal/handler/agent_templates_seam_test.go`. Both were
  mutation-fold verified — RED under the exact fold the issue names, GREEN on revert:
  `TestGetBuiltinAgentTemplateServesShippedNotStored` (folding the handler to serve `templateDTO(t)`,
  the stored row, reddens it) and `TestResetAgentTemplateClearsBadgeServerSide` (folding it to return
  the pre-reset `templateDTO(t)`, so the badge never clears, reddens it). No live DB required.

**Item 3 (the drift-predicate fixture) is NOT in this run** — deliberately left for the next one. It is
unblocked but no longer urgent: the 2026-08-23 triage on issue #223 established that M4b/#275's
`RefreshPristineBuiltin` gates admin-edit protection on the `customized`/`origin` flags (its inline `IS DISTINCT FROM` content check is only a no-op/`updated_at` optimization, and is none of the three drift predicates), so the
three implementations (`agenttmpl.SameContent`, `mockApi.sameContent`, `driftedColumns`) diverging can
no longer overwrite an admin edit; the residual risk dropped from data loss to a self-contradicting UI
badge. The `fixtures/run-usage/` shared-table pattern is still the right fix.
