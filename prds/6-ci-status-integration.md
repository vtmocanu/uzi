...
- [ ] **M1 — Forge: pipeline read methods**: `LatestPipeline` / `LatestMRPipeline` / `ListPipelineJobs` / `JobLogTail` (+ `Pipeline`/`Job` domain types, `ErrNoPipeline`), GitLab driver with client-side tail-truncation and redaction discipline, driver tests against recorded fixtures incl. no-CI, detached-MR-pipeline, and huge-trace cases.
