...
- [ ] **M4 — Docs.** Add a CHANGELOG `[Unreleased]` entry ("Board sort control gains an ascending/descending direction toggle; the chosen sort now applies to the Closed lane too"). Grep `docs/` for any `audience: user` page that describes the board's Sort control and, if one exists, add a sentence about the direction toggle; no new doc page expected. **Validate**: `web/scripts/check-docs.mjs` (run in `npm run build`) passes; CHANGELOG renders.
