...
- `api/internal/schedsvc/scheduler.go:360` and `:524` — fire-time resolution by `catalog_slug` (prompt for prompt targets, labels+guidance for sweeps). A shipped catalog change reaches every enabled default; this PRD's per-schedule mode setting composes at the same point.
...
- Issue [#928](https://github.com/vtmocanu/uzi/issues/928) (refactor-scout) is mode-agnostic and adopts this feature once it lands. Epic #915 is where dedup-by-disposition was established.

## Problem

Proposal-shaped scheduled jobs — feature-bingo today, refactor-scout (#928) next — have exactly one delivery mechanism: write an idea file, open an MR. That has three costs:
