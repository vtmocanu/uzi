...
- [ ] **M1 — Worker forwards usage + semantics verdict**: `mapResult` success+error passthrough; `mapAssistant` per-frame usage attach (Decision 11); the Decision 3 experiment covering BOTH same-process multi-turn resume and cross-process resume from a persisted session id (↳review), recording verdict (a) or (b) in this PRD before M2 starts — the same experiment also confirms subagent assistant frames carry `message.usage` + `subagent_type` on this SDK version; agent tests green. Validation: a real run's result frames show `usage` in `run_messages` payloads and its assistant messages carry per-agent usage; the verdict is written down with evidence.
