...
- [ ] **M4 — Run surfaces**: RunView usage strip + per-phase table + per-agent table (Decision 11) + live ws fold-in, finish-line tokens (success and error), RunsList meta line, `formatTokens`. Validation: matches the mock's §1–2 for a completed, a failed, and a running run (per-agent table per the mock addendum below); a run that used subagents shows one row per agent (lead + each subagent that produced usage-bearing assistant frames — no empty rows for the `worker` pseudo-agent or tool_result-only agents, ↳review); pre-feature runs show no usage UI.
