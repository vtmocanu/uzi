...
Compile the issue snapshot, linked PRD snapshot and proposed plan into stable semantic criteria with evidence requirements. Independently compare that contract with the complete source before approval. Human-gated runs show uncertainties to the owner; an autopilot run with missing/uncertain coverage falls back to `awaiting_approval` rather than auto-approving or failing.

The resulting semantic contract remains visible but enforcement stays on #1226's structural profile until #1231 lands. That limitation must be explicit in the run and PRD status.
...
- Independent plan-source coverage review in a separate session.
...
The lead proposes criteria as part of `submit_plan`. A separate read-only plan-contract session receives the server-provided issue/PRD snapshots and proposed plan/contract, treats repository/forge text as untrusted data, and reports source requirements as represented, missing or uncertain. It cannot implement, edit, approve or waive.
...
Autopilot normally freezes its plan on a running report. If independent source coverage is fully represented, it may continue through the existing auto-approve path. If coverage is missing/uncertain or the contract-review pass is unavailable after bounded recovery, disable the worker-side auto-approve skip and use the existing `SetRunAwaitingApproval` path from `running` with the coverage report. Gate notifications fire through the existing inbox/Slack paths, and wall time is banked while waiting as at other approval parks. The owner decides; the run does not fail or auto-approve uncertainty.
...
| 4 | M4: autopilot fallback and maintainer criteria | M2-M3 | autopilot runner/SQL, PR/run display | Uncertainty cannot auto-approve; non-worker proof stays outstanding. |
| 5 | M5: drift, docs and adversarial tests | M1-M4 | tests, docs/specs/ADR | Omissions and false authority are regression-proof. |

- [ ] **M1: semantic contract DTO and bounds.** Extend the existing contract without breaking #1226: source hashes/snapshots, criteria kinds/text/evidence, coverage verdict and inactive semantic profile marker. Define count/text/snapshot/evidence bounds and legacy absence. Test malformed IDs, duplicate criteria, oversized fields and unknown kinds across API/agent wire. Gates: `task gate:api`, `task gate:agent`.
- [ ] **M2: compiler and independent coverage review.** Add lead proposal fields and a separate read-only reviewer whose authoritative prompt payload comes from the server, not repository instructions. Return represented/missing/uncertain source requirements with bounded citations into snapshots. Retry transport errors under existing recovery policy. Test the #1220-shaped five-milestone source against a plan that omits M5 and a plan that collapses partial M3 criteria. Gate: `task gate:agent`.
...
- A missing/uncertain source requirement cannot pass the human gate or autopilot silently.
- Autopilot uncertainty reaches `awaiting_approval`, not failed or running.
...
- #1226's structural permit remains wire-compatible until #1231 activates semantic enforcement.
