...
A Go API + React SPA web shell with simple email+password registration stored in the DB. **No SSO/OAuth, no email OTP** (user decision, 2026-07-03). Design cherry-picks the best of the two inspiration projects and improves on their weaknesses:
...
| Registration | Auto-provision on first login, allowlist knobs | First-user-only `/register`, rest admin-created | **Open self-registration** + first-user-becomes-admin (bottega's in-tx re-check idea, made Postgres-safe via `pg_advisory_xact_lock` — see Auth design) |
...
| DB | Postgres 17, bespoke migration runner (~130 numbered migrations, 176 up-files) | SQLite, hand-rolled idempotent migrations in code | **Postgres 17 + standard migration tool (goose)** — beats both bespoke approaches |
| Rate limiting | Redis-backed, send/verify | express-rate-limit on register/login | In-process per-IP limiter on register/login (no Redis dep for MVP), real client IP taken from `X-Forwarded-For` set by our nginx (trust only that hop) |
| Local dev | docker-compose (loopback-bound, documented) | None | docker-compose, loopback-bound (copy multica's `127.0.0.1` binding + warning) |

## Technical Design

### Stack (user-confirmed)

- **Backend**: Go + chi + pgx/v5 + sqlc (multica-proven), `golang-jwt/v5`, argon2id via `golang.org/x/crypto/argon2`.
- **Frontend**: React 18 + Vite + TypeScript SPA (simpler than multica's Next.js; no SSR needed). Tailwind for styling.
- **DB**: PostgreSQL 17 (`postgres:17` image), named volume for persistence (plan.md requirement).
- **Migrations**: goose (standard tool; deliberate improvement over both inspirations' bespoke runners).
...
- JWT payload: `{user_id, token_version}`. Every authenticated request compares `token_version` against DB (one PK read; cache later if it matters) and rejects inactive users. Bump triggers: logout, password change, **admin deactivation** — any of these kills all of the user's sessions (real revocation, which multica lacks).
- **Rolling refresh**: authenticated requests re-issue the cookie, sliding the expiry (bottega's rolling-token idea, done cookie-side) — active users never get logged out mid-session, beating both multica (hard 30d) and bottega (localStorage).
...
- Auth design demonstrably ≥ both inspirations: cookie storage (beats bottega), revocation (beats multica), standard migrations (beats both).

## Risks

- **Scope creep toward multica's full platform** — mitigation: milestones frozen to the shell; factory features are separate PRDs.
