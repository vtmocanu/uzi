...
- **No resume path after re-login.** A re-login replaces the alias material (`PATCH /api/me/secrets/codex_auth/{id}` → `BumpCodexMaterialRevision`). The run's binding is write-once (`FreezeRunCodexBinding`), so any run frozen at the old `material_revision` fails `codexCheckMaterialRev` forever after. Holding the run is useless unless a re-login can safely re-admit it.
...
- **Re-login path.**
  1. `PATCH /api/me/secrets/codex_auth/{id}` → `patchCodexSecret` (`api/internal/handler/secrets.go` ~847-873) → `BumpCodexMaterialRevision` (`codex_credentials.sql:91-100`): `material_revision + 1`, status `staging`, and **`provider_account_id` cleared**. While staging, the new identity is unknown.
...
| **hold** | `ErrCodexMaterialRevisionStale` on a subscription run with a frozen identity (`codex_account_key` set) whose **same** alias id is `staging` or `failed` (a re-login in flight, identity not yet known), whether or not the run was already held | park or stay held, no token, pending D5 |
| **terminal** | kind↔mode mismatch; login blob; deleted or incoherent alias (PRD #1332 D3) | `credential_unavailable`, unchanged |
| **terminal** | identity tuple mismatch; `ErrCodexAccountKeyUnfrozen` (as built, nothing freezes a run's identity after create, so an unfrozen subscription run can never pass the predicate) | `credential_unavailable`, unchanged |
...
M1 final-check note: both Claim return paths classify successful assembly and assembly errors in one locked transaction. They check status, worker and generation first, then the attempt's minted epoch/hash before account authority. A stale or superseded attempt returns idle without mutation. A current quarantine, in-flight or verified same-identity re-login parks the exact claim; terminal credential, provisioning and guardrail failures fail it. Each outcome settles only its expected current-generation hold, preserving older holds. If a hold-class assembly error sees a recovered account at this check, the discarded attempt still parks and the account-driven promoter can resume it. An ambiguous mint or transient classification error makes no mutation. The check narrows the handoff race and does not make HTTP delivery atomic with the database. The transaction is the only run-lane assembly writer: with no transaction it refuses with no payload, and a `55P03` from its `NOWAIT` alias/account locks retries the whole transaction a bounded number of times before returning the error unmutated. When lock-time authority decides a terminal outcome, `failure_reason` is that authority error, not the assembly's text. A terminal lock-time authority refusal also overrides a transient assembly outcome (vault locked, empty pool, custom model): the run fails instead of being requeued, because it would fail the same database-only check on its next claim.

Landed with M1 from M2 and M3: the ClaimRun account gate and its peer mirror, and `PromoteRecoveryWaitRuns` / `PromoteRecoveryWaitRunNow` skipping `codex_account_unavailable`. M2 (this branch, reviewed) adds amendment A1's third case to the gate, the `park_codex_account_unavailable` sweeper pass, the health projection, and behaviour tests. M3's account-driven promoter (D3) is also on the branch, reviewed, so it ships in the same MR as M1.

- **ClaimRun predicate.** A standalone predicate, shaped like the custom-model clause, excludes a Codex subscription run (`harness='codex'`, `codex_auth_mode='subscription'`, a hold-opening kind) in any of three cases, all read on the run's **same** alias (`codex_secret_id`) and scoped to its owner:
  - **quarantine:** the linked account exists and `cpa.coord_state='quarantined'`, and the account is still the run's frozen identity at its frozen `credential_revision` (as built, a run with no frozen identity is not held; see the M3 rework note below). A quarantined account with a different identity or a bumped revision is terminal under D1, so the gate lets that claim through to fail in assembly as before.
  - **re-login in flight:** the run is past first link (frozen `codex_account_key` set), its alias is `staging` or `failed`, and the alias `material_revision` is greater than the run's frozen `codex_material_revision`. A PATCH clears `provider_account_id`, so the quarantine case cannot see this one. Without this case, a re-login that starts before the sweeper parks the queued run would let a claim fail on `ErrCodexMaterialRevisionStale`.
  - **A1, verified same-identity relink:** past first link, the alias is `linked` with `material_revision` greater than the frozen one, and the linked account's identity and `credential_revision` equal the run's frozen `codex_account_key` and `codex_account_revision`. D5 re-admits such a run; until then its material revision is stale.

  As built, M3 rework: nothing freezes a run's identity after create. `SetRunCodexFrozenIdentity` is called only from `freezeCodexBinding`, at create, and only when the alias is already linked; no claim path freezes it. A run with a NULL `codex_account_key` therefore fails `evalCodexReleasePredicate` with `ErrCodexAccountKeyUnfrozen` whatever the account does, and could never be promoted. The quarantine case consequently requires the frozen identity (the former `r.codex_account_key IS NULL OR` disjunct is gone from all four copies): such a run is not gated or parked, and the claim fails it `credential_unavailable` as before M2. The shared fixture table no longer fakes a claim-time freeze for these rows.

  As built: the run freezes its identity only as the Go-encoded JSON array `codex_account_key` (`codexAccountKey`, migration 00202), and no SQL expression reproduces that encoding. The predicate therefore decodes the key with a guarded `::jsonb` cast (`pg_input_is_valid`) and compares it structurally to `jsonb_build_array(provider_user_id, workspace_account_id)`; an undecodable key never matches. `coord_state='in_progress'` is not a case (D1). The predicate is run-level and independent of worker capabilities.

  The same predicate text appears four times, because sqlc cannot share a fragment and a SQL function would need a migration: the claimant gate, the peer mirror, the park page, and `ListActiveRunsForHealth`'s `codex_account_gated`. A store unit test pins the four copies byte-identical, after whitespace normalisation, in the generated `runtime.sql.go`. A shared fixture table (every `coord_state` and alias status) pins the SQL predicate against `classifyCodexClaimAuthority`, in a Go unit test and in a LiveDB test.

  `api_key` bindings, Claude runs, chat and judge runs, and unlinked aliases on runs that never reached first link, are unaffected; assembly's existing checks handle them.
- **Sweeper park.** A new pass, `park_codex_account_unavailable`, moves `queued` runs matching any case of the same predicate to `recovery_wait` with cause `codex_account_unavailable`. It moves no generation, opens no hold, leaves `worker_id` alone, and fences on `status='queued'` and the locked row.
  - As built: `ParkQueuedCodexAccountUnavailablePage` bounds the rows **examined**, not only the rows updated. A `MATERIALIZED` page CTE takes at most the page cap of queued Codex subscription runs with `id` past an in-memory keyset cursor, in `id` order, over the partial index `idx_runs_codex_sub_queued`. The `LIMIT` applies before the alias/account predicate runs. The page's still-queued rows are locked `FOR UPDATE SKIP LOCKED`, and the `UPDATE` rechecks `status='queued'`, the hold-opening kinds and the predicate.
...
- As built (M3, reviewed): `promoteCodexAccountAvailable` (`workersvc/codex_account_promote.go`) runs in `Sweep` after the park pass and before the health detector, so a promoted run is health-consistent in the same tick. The park and the promotion cannot undo each other on one account state: every gated run fails the predicate, and a run that passes it matches no gate case. A concurrent re-login landing between the two passes can still park and then promote the same run in one tick. It is best-effort like the park; `SweepResult.CodexAccountPromoted` counts it.
  - As built after the M4 review: the survivor pass (`SweepUnresolvedCodexRefresh`) is back at the tail of `Sweep`, after the detector and auto-stop, because it can make a network call per deferring account and those two decide on the `now` captured at the top of the tick. The promotion pass runs after it, last in `Sweep`, so a survivor promotion of recovery material still resumes a held run in the same tick, absent lock contention (verified: with promotion moved before the survivor, `TestCodexAccountPromoteSurvivorRecoveryLiveDB` fails with the run still held). The park stays before the detector. A promoted run is therefore judged `queued` by the next tick's detector (the promotion itself resets health), and a lease the survivor reaps into quarantine has its queued runs parked on the next tick.
  - One bounded page per tick: `ListCodexAccountWaitRunsPage` reads at most 100 held ids past an in-memory keyset cursor over `idx_runs_codex_account_wait`, wrapping after a short page, like the park's cursor.
  - Per run, one transaction: `LockCodexAccountWaitRunForUpdate` (`FOR NO KEY UPDATE SKIP LOCKED`: status and cause re-checked, the sweeper never waits on a run row, and the promotion changes no key column), then `LockCodexAliasForShareNowait` and `LockCodexAccountForShareNowait` (the M1 classifier's queries), then `GetRunCodexAuthContext`, then the unchanged `evalCodexReleasePredicate`, then `PromoteCodexAccountWaitRun` (the `PromoteRecoveryWaitRuns` field set, fenced on status and the cause). The status change is published after commit.
  - A `55P03`, a missing alias or account row, an unbound run, a `staging` or `failed` alias, or a failed predicate leaves the run held with nothing written; that is not an error. D5 re-admission, failing a held run whose alias was deleted, and failing one whose alias is `linked` to no account (D1's incoherent alias) are M4 (see D5's as-built note). A missing alias state row, a missing account row and an unbound held run are unreachable (no statement deletes a state row except the cascade from its alias, which also nulls the run's alias id; the owner-scoped foreign key keeps a link pointing at an existing account; the freeze writes the alias id and material revision together); they stay held, fail-closed, and no token is released for them.
...
As built (M4, reviewed):
- `ReadmitRunCodexBinding` (`queries/codex_binding.sql`) writes only `codex_material_revision` (and `updated_at`). Its `WHERE` carries every fence above: the hold (status and cause), `codex_secret_id` equal to the caller's observed alias id, `codex_auth_mode='subscription'`, the alias secret's `kind='codex_auth'`, alias `status='linked'` with an account joined through `provider_account_id`, the identity tuple compared with the same guarded `pg_input_is_valid` jsonb expression as the ClaimRun gate, `credential_revision` equality, `coord_state IN ('idle','committed')`, and CAS on the run's observed old and the alias's observed new material revision, with the move strictly forward.
- As built after the M4 review, the frozen key must also equal `@account_key` byte for byte. The promoter passes `codexAccountKey` over the locked account's tuple (read with `GetRunCodexAuthContext` under the locks), which is the text `codexCheckAccountTuple` compares. The jsonb comparison alone accepted a key that is jsonb-equal but not the Go encoding (for example `["u", "w"]`, only writable by direct SQL): such a run was re-admitted and then failed by the Go check in the same transaction, and both committed. With both comparisons the SQL and Go identity checks agree, and the jsonb comparison still keeps the statement safe whatever key a caller passes.
- The `credential_revision` fence is defence in depth. Nothing writes `codex_provider_account.credential_revision` today (it stays at its default 0), so no revocation mechanism bumps it yet. If a future revocation does, the fence keeps such a run from being re-admitted and the classification ends the hold.
- The promoter (`promoteCodexAccountRunTx`) locks the run, then fails a run whose `codex_secret_id` is NULL with a non-null frozen material revision (the alias was deleted), before taking any alias or account lock. Otherwise it locks the alias; a `staging` or `failed` alias leaves the run held, and a `linked` alias with no account (D1's incoherent alias; no writer produces it, since a link sets both columns, a re-login clears both, and deleting the account demotes the alias to `failed` through the orphan trigger) fails the run `credential_unavailable` with `ErrCodexAliasAccountMissing`. It then locks the account, and calls `ReadmitRunCodexBinding` when the alias material is ahead of the run's. On one row it writes the feed line in the same transaction (`InsertCodexReadmitRunMessage`, a `status` message with `text`, `event='codex_binding_readmitted'`, the run's snapshotted `alias_label` and the from/to material revisions, appended past `last_seq` and advancing it, so the worker that resumes the run at its next claim starts after it; a zombie worker of the displaced flight that still holds an unreleased claim at the run's generation keeps its own seq counter, and a frame it appends at the same seq is dropped by `ON CONFLICT (run_id, seq) DO NOTHING`. A zombie frame committed before the feed-line insert's snapshot is part of `MAX(seq)`, so the insert takes the seq after it; only a zombie insert at the same seq that the snapshot does not see (in flight, or committed after it) makes the feed-line insert return no row, and the transaction rolls back with the run still held). It then re-reads `GetRunCodexAuthContext` and classifies it.
- Classification (`classifyCodexAccountHold`, one pure function, unit-tested): terminal only on a definite binding mismatch on the linked alias. That means a kind↔mode mismatch or a non-subscription mode, no frozen identity, an identity tuple that differs (including an undecodable frozen key), or a `credential_revision` that differs. Transient states stay held: quarantine with the same identity and revision, `in_progress`, a material revision still ahead (the CAS lost to a concurrent change, or a lease is live), or missing account columns (`ErrCodexAliasAccountMissing`; unreachable under the account lock, and kept held rather than terminal because it is not a definite mismatch). Promotion still requires the unchanged `evalCodexReleasePredicate`.
- The terminal exit is `FailCodexAccountWaitRun`, a new statement with `MarkRunFailedByID`'s field set plus the capability revoke, fenced on `status='recovery_wait'` and the cause, and touching no custody hold (a failed run retains custody). Its `failure_reason` is `credential unavailable: the Codex login "<label>" bound to this run was deleted …` or `… changed while the run waited for its account: <sentinel>`. The run is published `failed`.
- A re-admission commits only together with its promotion. When a transaction re-admits the run and then classifies it as terminal or stay, `promoteCodexAccountRun` rolls the whole transaction back (no re-admission, no feed line, nothing counted or published) and decides the run again in a fresh transaction with re-admission disabled. Re-admission only moves the material revision, and the classification checks kind, mode, identity and credential revision before it, so the second decision is either the same terminal failure, committed without the re-admission, or a stay with nothing written. Before the review, the terminal branch dropped the re-admission from its result, so nothing was counted or published, but the re-admission and its feed line row were committed with the failure. `SweepResult` gains `CodexAccountReadmitted` (a subset of promoted) and `CodexAccountFailed`, both on the sweeper's tick log line.
...
- [ ] **M1: Sentinel split, exact-claim assembly outcomes and park.**
  - Final-pass evidence (2026-09-24): on the tree merged with `main` in `bae185ce` (up to #1615, including #1611's custody ancestry release), the full LiveDB suite passed (1191 passed, 0 skipped, run with the draft migrations temporarily renumbered above `main`'s `00251`), including `TestClaimObservedCaseReplayParksLiveDB` and `TestClaimCodexJudgeQuarantineStaysTerminalLiveDB`; `task gate:api` and `task gate:web` green. No observation of the M1 and M2 regressions failing on the unfixed code (for example `TestClaimObservedCaseReplayParksLiveDB` red on `df8e1df3`) is recorded, so M1 and M2 stay unticked until it is observed. `task gate:repo` (the migration-milestone gate) stays red on the migration-number collision until the landing renumber.
...
- [x] **M3: Account-driven promotion.** Depends on M2 (same files: `runtime.sql`, `sweep.go`).
  ...
  - Implementation note (reviewed): the pass, its three queries and the LiveDB tests in `codex_account_promote_livedb_test.go` are on the branch:
    ...
    - a relink to a different identity or a material bump written after the page is listed, but before the run's transaction opens, is seen by the in-transaction re-read and does not promote. A separate leg starts `LinkCodexCredentialState` while the promoter holds the alias lock: the relink blocks until the promoter's transaction ends;
...
- [x] **M4: Same-identity re-admission.** Depends on M3.
  ...
  - Implementation note (reviewed): see D5's as-built note. LiveDB tests in `codex_account_readmit_livedb_test.go`:
    ...
    - `TestCodexReadmitStagingOrFailedStaysHeldNoTokenLiveDB`: staging or failed stays held, and a claim delivers nothing.
    - `TestCodexReadmitMaterialCASFenceLiveDB` (renamed from `TestCodexReadmitConcurrentMaterialChangeLiveDB`): a material change injected in the promoter's own transaction just before the CAS matches 0 rows, and the run stays held, not failed. It pins the CAS fences, not concurrency: a real concurrent writer blocks on the promoter's alias `FOR SHARE` lock.
    - `TestCodexReadmitNonCanonicalFrozenKeyLiveDB` (review regression): a frozen key rewritten to a jsonb-equal, non-canonical form fails the run with a tuple mismatch, with no re-admission, no feed line, nothing published but the `failed` state, and `Readmitted` 0. It also asserts that one transaction ran and its `ReadmitRunCodexBinding` matched 0 rows, so the SQL key fence itself refused the re-admission (the promoter's rollback-and-redecide would otherwise mask a missing fence: the outcome is the same terminal failure, from a second transaction). At fbe899ca, before both the key fence and the rollback existed, the failure committed together with the re-admission (material revision 0 to 1).
    - `TestCodexReadmitWithoutPromoteRollsBackLiveDB`: a re-admitting transaction made to classify as terminal (a test wrapper on the re-read) is rolled back whole; the second transaction, without re-admission, leaves the run held, with nothing counted, written or published.
    - `TestCodexReadmitWithoutPromoteRedecidesTerminalLiveDB`: the same wrapper corrupting the re-read in both transactions, so the second transaction, without re-admission, commits the terminal failure: `Failed` 1, two transactions (one re-admission that matched, then none), the frozen material revision kept, and no feed line written or published.
    - `TestCodexA1RelinkOneSweepResumesLiveDB`: a queued A1 run (same-identity relink, alias material ahead) is held by the gate, and ONE full `Sweep` parks, re-admits and promotes it: queued at the alias's material revision, one committed and published feed line, no failure, `CodexAccountReadmitted` 1. The park and promotion pages are scoped to the fixture run.
    - `TestReadmitRunCodexBindingFencesLiveDB`: a control plus one leg per fence, including a non-canonical frozen key and an observed account key that differs.
...
- [x] **M5: Surfaces.** Depends on M1 and M4 (the terminal cases).
  ...
  - Mutation evidence (each applied, observed red, restored):
    - API: the reauth-flag case forced to `reconciling` (`TestDeriveCodexAccountAction` red); each of the three read-handler overlay calls (`GetRun`, `ListRuns`, `AdminListRuns`) removed (`TestCodexAccountActionReadHandlersWiring` red for that handler); the label set on every run (`TestOverlayCodexAccountActions` red).
    - CLI/TUI: the `relogin_required` arm removed; the label drawn raw instead of through `cellText`; the detail width clamp removed; the codex term removed from `transcriptViewport` and from `boardShowSecondLine`; the `run list` / `admin runs` call sites reverted to `displayRunStatus`; the `--follow` notice branch disabled; the `runBandOf`, `runStateToken` and header-counter branches disabled. Each reddened its test in `codex_hold_layout_test.go`, `render_test.go` or `tui_recovery_test.go`.
    - Web: the `relogin_required` branch renamed (3 `RunView.test.tsx` tests red); the `codex:` announcement key reverted to `recovery_wait` (6 red); the shared non-relogin announcement restored (4 red).
