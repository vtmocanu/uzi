...
- As built (M3, pending review): `promoteCodexAccountAvailable` (`workersvc/codex_account_promote.go`) runs in `Sweep` after the park pass and before the health detector, so a promoted run is health-consistent in the same tick. The park and the promotion cannot undo each other on one account state: every gated run fails the predicate, and a run that passes it matches no gate case. A concurrent re-login landing between the two passes can still park and then promote the same run in one tick. It is best-effort like the park; `SweepResult.CodexAccountPromoted` counts it.
  ...
  - A `55P03`, a missing alias or account row, an unbound run, an alias with no linked account, or a failed predicate leaves the run held with nothing written; that is not an error. D5 re-admission and failing a held run whose alias was deleted are M4 (see D5's as-built note).
...
- The promoter (`promoteCodexAccountRunTx`) locks the run, then fails a run whose `codex_secret_id` is NULL with a non-null frozen material revision (the alias was deleted), before taking any alias or account lock. Otherwise it locks the alias; a non-`linked` alias or one with no account leaves the run held. It then locks the account, and calls `ReadmitRunCodexBinding` when the alias material is ahead of the run's. On one row it writes the feed line in the same transaction (`InsertCodexReadmitRunMessage`, a `status` message with `text`, `event='codex_binding_readmitted'`, the run's snapshotted `alias_label` and the from/to material revisions, appended past `last_seq` and advancing it, so a resumed worker starts after it). It then re-reads `GetRunCodexAuthContext` and classifies it.
- Classification (`classifyCodexAccountHold`, one pure function, unit-tested): terminal only on a definite binding mismatch on the linked alias. That means a kind↔mode mismatch or a non-subscription mode, no frozen identity, an identity tuple that differs (including an undecodable frozen key), or a `credential_revision` that differs. Transient states stay held: quarantine with the same identity and revision, `in_progress`, a material revision still ahead (the CAS lost to a concurrent change, or a lease is live), or missing account columns. Promotion still requires the unchanged `evalCodexReleasePredicate`.
...
- A re-admission commits only together with its promotion. Any other outcome rolls the transaction back, so the relaxation never lands alone. `SweepResult` gains `CodexAccountReadmitted` (a subset of promoted) and `CodexAccountFailed`, both on the sweeper's tick log line.
...
- [ ] **M4: Same-identity re-admission.** Depends on M3.
  ...
  - Implementation note (not ticked, pending review): see D5's as-built note. LiveDB tests in `codex_account_readmit_livedb_test.go`:
    ...
    - `TestCodexReadmitTerminalBindingChangesLiveDB`: a bumped `credential_revision`, a different identity, a changed alias kind, a changed run auth mode (alone and together with the kind), and an undecodable frozen key are each terminal `credential_unavailable`, with no re-admission and no feed line.
    ...
    - `TestCodexReadmitConcurrentMaterialChangeLiveDB`: a material change injected just before the CAS matches 0 rows, and the run stays held, not failed.
    - `TestReadmitRunCodexBindingFencesLiveDB`: a control plus one leg per fence.
