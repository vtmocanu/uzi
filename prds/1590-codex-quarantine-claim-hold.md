...
Landed with M1 from M2 and M3: the ClaimRun account gate and its peer mirror, and `PromoteRecoveryWaitRuns` / `PromoteRecoveryWaitRunNow` skipping `codex_account_unavailable`. M2 (this branch, pending review) adds amendment A1's third case to the gate, the `park_codex_account_unavailable` sweeper pass, the health projection, and behaviour tests. Not yet present: M3's account-driven promoter. Until it lands, nothing promotes a parked run back to `queued`; owner cancel still ends it. The promoter therefore ships in the same MR as M1.
