...
**Status:** M1–M6 implemented on the branch `agent/issue-1590` and reviewed (2026-09-24). M3–M6 are ticked; M1 and M2 stay unticked only because their fail-on-unfixed-code regression direction has no recorded observation (see the M1 note). M7 (acceptance on hosted k8s, maintainer) is pending, so the PRD stays open. The branch is merged with `main` up to #1615 (3b802713); its draft migrations `00251`/`00252` collide with `main`'s `00251` and are renumbered at landing (`task migration:renumber`), so `task gate:repo`'s `check:migration-numbering` is red on the branch until then, deliberately deferred to landing.
