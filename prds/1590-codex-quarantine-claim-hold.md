...
- As built (M3, pending review): `promoteCodexAccountAvailable` (`workersvc/codex_account_promote.go`) runs in `Sweep` after the park pass and before the health detector, so a promoted run is health-consistent in the same tick. The park and the promotion cannot undo each other on one account state: every gated run fails the predicate, and a run that passes it matches no gate case. A concurrent re-login landing between the two passes can still park and then promote the same run in one tick. It is best-effort like the park; `SweepResult.CodexAccountPromoted` counts it.
  - The survivor pass (`SweepUnresolvedCodexRefresh`) now runs before the park and promotion passes, not at the end of `Sweep`. It writes only `codex_provider_account` and its refresh intents, and it was already after every early return, so the move changes only what the two Codex passes see in a tick. A survivor promotion of recovery material therefore resumes a held run in the same tick, absent lock contention, and a lease the survivor reaps into quarantine has its queued runs parked in that tick.
  ...
  - A `55P03`, a missing alias or account row, an unbound run, an alias with no linked account, or a failed predicate leaves the run held with nothing written; that is not an error. D5 re-admission and failing a held run whose alias was deleted are M4.
