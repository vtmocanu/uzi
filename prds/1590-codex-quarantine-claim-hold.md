...
- **ClaimRun predicate.** A standalone predicate, shaped like the custom-model clause, excludes a Codex subscription run (`harness='codex'`, `codex_auth_mode='subscription'`, a hold-opening kind) in any of three cases, all read on the run's **same** alias (`codex_secret_id`) and scoped to its owner:
  - **quarantine:** the linked account exists and `cpa.coord_state='quarantined'`, and, once the run is past first link, the account is still the run's frozen identity at its frozen `credential_revision`. A quarantined account with a different identity or a bumped revision is terminal under D1, so the gate lets that claim through to fail in assembly as before.
...
- As built (M3, pending review): `promoteCodexAccountAvailable` (`workersvc/codex_account_promote.go`) runs in `Sweep` after the park pass and before the health detector, so a run moves at most once per tick and a promoted run is health-consistent in the same tick. It is best-effort like the park; `SweepResult.CodexAccountPromoted` counts it.
...
- [ ] **M3: Account-driven promotion.** Depends on M2 (same files: `runtime.sql`, `sweep.go`).
  ...
  - Implementation note (not ticked, pending review): the pass, its three queries and the LiveDB tests in `codex_account_promote_livedb_test.go` are on the branch: a re-logged-in account's held run is promoted by `Sweep` and its claim delivers the re-login's token and generation; quarantined, staging re-login and same-identity relink runs stay held across ticks; a relink to a different identity or a material bump injected between the list and the run's transaction does not promote; a cause changed after the list does not promote; the deadlock interleaving; cancel from the hold; both timer promoters skip the cause with a past retry stamp. Each regression was observed failing under its mutation (blocking account lock: `40P01` deadlock; timer promoter without the cause fence; predicate skipped; run lock and promote without the cause fence).
