...
4. **reset observed by EITHER arm:**
   ...
   - **Arm B — utilization zeroed ahead of schedule** (fixed-grid model): `prev.SevenDayPct.Valid && next.SevenDay.Pct` present, with `int(prev.SevenDayPct.Int16) >= pctResetFloor && next.SevenDay.Pct <= pctResetCeil`. Constants (new): `pctResetFloor = 25`, `pctResetCeil = 5`. Rationale: the 7-day used% is monotonic non-decreasing between resets (anchored, per #310), so a genuine drop only happens at a reset; requiring prev ≥ 25 and next ≤ 5 makes a cross-source ±few-point representation delta unable to fake it, and means the clear returned a *material* quarter-plus of weekly budget (a trivial-headroom clear the user gained nothing from is intentionally not alerted — see D5).
...
- **Trivial-headroom clears are intentionally not alerted** (D5): if the user had used < `pctResetFloor` and the boundary did not move (fixed-grid), the early clear returned nothing worth interrupting them for, and the signal is indistinguishable from jitter. This is a deliberate scope edge, documented, not a bug.
...
- [ ] **M3 — Tests (api).** In `api/internal/usagepoller/engine_test.go` (`TestEarlyReset*`, ~`:566-974`):
  ...
  - Add an **Arm B** case: prev `header_probe` `seven_day_pct=40`, next `pct=0`, `resets_at` unchanged, `now` ≥8h before T → fires. This is the fixed-grid case the current predicate misses.
  - Add **jitter-silent** cases the two margins must reject: (a) boundary moved forward by only 30m (< `resetEpochMoveMargin`) → silent; (b) pct `12→3` with unchanged epoch (< `pctResetFloor`, the maintainer's real trivial-headroom case) → silent.
...
- **D5 — Trivial-headroom fixed-grid clears (prev pct < `pctResetFloor`, boundary unmoved) are intentionally not alerted.** They return nothing worth interrupting the user for and are indistinguishable from cross-source jitter. Documented scope edge, not a blind spot to fix.
...
- **D8 — Jitter margins replace the gate as the false-positive backstop.** Removing the "limiting" gate on a low-utilization token would otherwise let cross-source epoch re-derivation or rounding fire; `resetEpochMoveMargin`=1h (Arm A) and `pctResetFloor`=25 / `pctResetCeil`=5 (Arm B) restore that protection without narrowing the alert to constrained windows.
- **D9 — Thresholds are defaults, tuned by tests, not envs.** `resetEpochMoveMargin`=1h, `pctResetFloor`=25, `pctResetCeil`=5 are code constants (like `earlyResetThreshold`/`exhaustionPct` before them); M3 pins each. Promote to config only if a real capture in M5 shows a default is wrong.
