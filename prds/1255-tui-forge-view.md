...
### D5 — Three additive `Forge` methods and one field, three drivers + `BaseFake`

Every interface addition costs four sites (`gitlab.go`, `forgejo.go`, `github.go`, `forgetest.BaseFake`), which is why the surface is three methods and not six:

| method | GitHub | GitLab | Forgejo |
|---|---|---|---|
| `ListMergeRequests(ctx, projectID, opts{State: opened, Limit})` → `[]MergeRequestSummary{IID, Title, Author, SourceBranch, TargetBranch, HeadSHA, Draft, Conflicts *bool, ReviewDecision, WebURL, Additions, Deletions, Commits, CreatedAt, UpdatedAt}` | `PullRequests.List`; `Conflicts`/`Additions`/`Deletions`/`Commits` via `PullRequests.Get` (memoised per `(iid, head_sha)`, D4); decision via `ListReviews` (memoised per `(iid, updated_at)`, D6) | `ListProjectMergeRequests` (`HasConflicts`, `DetailedMergeStatus`, `Draft`, `SHA`, `BlockingDiscussionsResolved` on the row); `approved` via `MergeRequestApprovals.GetConfiguration`, 403/404 → `none` | `ListRepoPullRequests` (`Mergeable`, `Draft`, `Head.Sha`, `Additions`, `Deletions` on the row); reviews via `rawGetLimited` on `/pulls/{index}/reviews` |
...
- [ ] **M8 — Docs, parity, finalize.** `docs/cli.md` "Watching runs live" gains the two screens, both drill-ins and the keybinding rows; `docs/board.md` cross-links; `docs/configuration.md` documents `FORGE_INTERACTIVE_RATE_MAX`; `task docs:sync` run and the mirror committed (`TestEmbeddedDocsMatchSource`); `specs/ai.md` section for this PRD; `ARCHITECTURE.md` one line under Forge integration ("read-through forge views: pulls / checks / CI runs, memoised, never persisted"). Finalize invariants: zero `.github/workflows/**` entries in `git diff --name-only <base>..HEAD`; sketch registry unchanged.
