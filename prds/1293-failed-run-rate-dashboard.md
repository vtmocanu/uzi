...
- **Tests**: `api/internal/handler/usage_test.go` (a fake `runsStore` carrying a `selfUsage` row; `TestSelfUsageReturnsScopedTotals`, `TestAdminUsageShapesFactoryAndUsers`, the two auth tests); `api/internal/store/run_usage_integration_test.go` (`TestUsageRollupsLiveDB`, the live-DB rollup test; `.claude/rules/go.md` for the `PASS=0` discipline); `web/src/components/UsageCards.test.tsx` (the `bundle()` fixture helper, 10 cases incl. the Share% sum-to-100 case).

### Failure vocabulary (the numerator)

- Terminal `runs.status` values: `completed`, `failed`, `cancelled` (migration `00206` CHECK). Everything else is in flight and must never enter the denominator.
...
- `specs/ai.md` head section is **§633** on `main` and on every sibling worktree (checked 2026-09-12); this PRD uses **§634**. Re-confirm at the landing rebase and bump if a parallel PRD landed first (append-only rule).
...
// RunOutcomesDTO
{
  "finished": 1024,        // completed + cancelled + failed (terminal status; chat/judge excluded)
  "completed": 861,
  "cancelled": 45,
...
- [ ] **1. API: outcome aggregates on the usage endpoints.** Three sqlc queries in `runtime.sql` next to the usage rollups: `SelfRunOutcomes` (per user, both windows), `AdminRunOutcomes` (factory, both windows), `AdminRunOutcomesPerUser` (lifetime, grouped by user). Each counts over `runs` with `status IN ('completed','failed','cancelled') AND kind NOT IN ('chat','judge')`, windows on `created_at`, splits `plan_rejected` out of `failed`, and returns the per-origin counts (a `jsonb_object_agg` over `COALESCE(fail_origin,'unknown')`, or a second `:many` query the handler folds; the worker picks the sqlc-friendlier shape and says why). Add `RunOutcomesDTO` to `apitypes/usage.go`, thread it through `SelfUsageDTO` / `AdminUserUsageDTO`, and merge per-user rows by user id in `AdminUsage` (D5: a user with outcomes and no usage gets a zero-usage row). Tests: extend the handler fakes and the four existing handler tests for the new fields and the merge; a live-DB query test beside `TestUsageRollupsLiveDB` seeding one run per terminal status, one `plan_rejected`, one NULL-origin failure, one `chat` and one `judge` failure, and asserting the exclusions and both invariants; a mutation check that flipping the `plan_rejected` split or the kind filter turns the test red (`.claude/rules/go.md`). Gate: `task gate:api`.
...
- **`jsonb_object_agg` shape vs sqlc.** If sqlc types the aggregate awkwardly, the fallback is a second `:many` query `(scope, origin, count)` folded in Go; M1 leaves the choice to the worker with the reason recorded in the query comment.
...
- **D5: merge per-user rows by user id, zero-filling usage.** Keeps the sum-to-factory invariant and stops a broken-token user from vanishing from the admin table.
