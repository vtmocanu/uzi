...
> ⚠️ **Viability gate.** The precise projected % rests on the 5h/7d windows being **fixed/anchored**. Investigation during PRD creation found that premise is undocumented and the public evidence leans **sliding** (Anthropic's own claude-code#62223 calls the 5h window a sliding window whose "Resets at HH:MM" label is a known misnomer). If the windows are sliding, the projection's denominator is invalid and this feature reframes to a coarse direction-only / trailing-burn-rate signal rather than a projected %. **M0 settles this empirically and offline before any rendering work.** Everything below the M0 line is conditional on M0 confirming anchored.
