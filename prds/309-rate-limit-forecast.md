...
- [ ] **M0 — Settle the window model, reconcile the doc (BLOCKING).** The projection's `elapsed = now − (resets_at − duration)` term is only well-defined for a **fixed/anchored** window that resets to zero at a boundary. The repo currently contradicts itself: `docs/rate-limits.md:9` calls the 5h/7d limits **"rolling"**, while `api/internal/anthropic/client.go:254-255` reads a single reset epoch per window (`anthropic-ratelimit-unified-5h-reset` / `-7d-reset`), which fits an anchored window. Resolve which is true (Anthropic's unified-rate-limit header semantics — a cheap external check), and **fix `docs/rate-limits.md` in the same pass** (CLAUDE.md's doc-correctness rule). If the windows are genuinely rolling, the projected % is not merely imprecise — its denominator is wrong; in that case either reframe the signal to a rolling-window-valid form or reduce it to a coarse direction-only hint before proceeding. M1's boundary tests and M4's copy both depend on this answer. Do not start M1 until it is settled.
- [ ] **M1 — Pure forecast helper + unit tests.** In `web/src/lib/rateLimits.ts`, add `paceForecast(window, nowMs)` returning a small typed result — `{ state: "over" | "on_pace" | "safe"; projectedPct: number }` — implementing the M0-confirmed formula, the strict `> 85` / `> 115` bands, the 2% suppression, the 5h absolute-elapsed floor, and the D8 suppressions (`pct >= 100` and `source === "limit_report"` → `safe`). Unit tests in `web/src/lib/rateLimits.test.ts` covering: band boundaries at exactly **85 (safe), 86 (on-pace), 115 (on-pace), 116 (over)**; first-2% and 5h-floor suppression; `pct>=100` and `limit_report` suppression; past-reset (`elapsed > duration`) under-projecting; **negative `elapsed`** (clock skew / `resets_at` > one duration out) → safe, not a garbage projection; null/zero `resets_at` → safe. Inject `nowMs` (no wall-clock), mirroring `formatCountdown`'s tests (`rateLimits.test.ts:62-69`).
- [ ] **M2 — Forecast meter wrapper.** Add a rate-limit-specific component that composes `MeterTrack` (leaving the shared atom's 72-line contract unchanged — no rate-limit copy or ghost logic inside it) and renders: the translucent ghost to `min(projected,100)%` in the pace tone (Tailwind translucent utilities, `bg-warn/40` / `bg-danger/40`, matching how the atom uses `bg-warn`/`bg-danger` at `Meter.tsx:29-33` — not raw CSS vars), an inset target edge, and the `»` slot shown only when `projected > 100`. Silent when `state === "safe"`. Passes the forecast phrase into `MeterTrack`'s `aria-valuetext`. Respects `prefers-reduced-motion` (static render, no animated growth). Component tests in `web/src/components/Meter.test.tsx` (or a new `RateLimitForecast.test.tsx`): ghost present/absent per state, tone mapping, `aria-valuetext` carries the projection, no `»` when `projected ≤ 100`, and `WorkerStats`'s use of `MeterTrack` is untouched.
- [ ] **M3 — Wire the wrapper into all three surfaces.** `AdminRateLimits.tsx` `WindowRow`, and `RateLimitMeters.tsx` (`SettingsWindowRow` + `MicroRow`), call the wrapper with `paceForecast(win, now)`, gating on `status:"ok"` and skipping when `stale` (per the contract above). Verify the sidebar's narrow width degrades gracefully (ghost always fits; `»` may drop at the tightest width without losing the color signal). Render tests extended in `AdminRateLimits.test.tsx` and `RateLimitMeters.test.tsx`: a forecasting window shows the ghost/marker; a safe/stale/`limit_report` one shows a plain bar; the projected % is reachable only via the accessible description, not printed inline.
- [ ] **M4 — Docs.** Update **`docs/rate-limits.md`** (it exists and is `audience: user`) with a short "reading the forecast" section — three states, silence = safe, the projection is a linear hint ("projected", "at the current rate", not a promise) — following the existing frontmatter conventions so `web/scripts/check-docs.mjs` passes. This edit is also where M0's rolling-vs-anchored wording is corrected.
...
- **Fixed vs. rolling window — now a blocking pre-flight, not a deferred risk (M0).** The repo's own `docs/rate-limits.md:9` says "rolling" while the header data model (one reset epoch per window, `client.go:254-255`) fits an anchored window. If literally rolling, the projection's denominator is wrong, not just imprecise. This internal contradiction — not external uncertainty — is what M0 settles before M1. The display-only nature makes the feature *safe* (it gates nothing) but not automatically *correct*; correctness needs the model confirmed.
...
- **D10 — Window durations are client constants.** `18000`/`604800` are hardcoded on the web with a comment; if Anthropic ever changes a window length this diverges silently. Accepted as minor; revisit only if it happens.
