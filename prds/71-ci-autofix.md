...
**Status**: Draft (reviewed — Opus architecture + fact-check pass, 2026-07-17)
**Priority**: Medium
**Created**: 2026-07-17
...
**(e) The snapshot builder is shared by handler + poller.** Move `snapshotFailedPipeline` + `scrubKnownTokens` out of `handler` into `workersvc` (`workersvc/ci_fix_snapshot.go`) as `BuildFailureSnapshot(...)`, and add `FailureSignature` next to it. `workersvc` gains a `forge` import (correctly layered high→low). Handler and the poller detector both call it.
...
Goose numbers are DRAFTS (live head is `00066`); renumber to the next free numbers above the live head at merge time, per the CLAUDE.md convention.
...
- `ci_autofix.sql` (new):
  - `ListCIAutofixCandidateRefs` — newest run per branch (DISTINCT ON, like `pipeline_statuses.sql`) JOIN `pipeline_statuses ps ON ps.ref = branch AND ps.status='failed'` JOIN `users u ON u.id = run.user_id` WHERE `u.ci_autofix_enabled AND u.anthropic_token_ciphertext IS NOT NULL AND run.mr_iid IS NOT NULL AND branch <> @default_branch AND branch NOT IN (protected refs)`. Returns `ref, mr_iid, user_id, pipeline_id, pipeline_web_url, sha`.
...
**Forge**
- New read `ProjectCIConfigPath(ctx, projectID) (string, error)` on `forge.Forge` + GitLab driver (GET project → `ci_config_path`; empty means the default `.gitlab-ci.yml`). Same neutral-type + redaction discipline as existing methods.
...
**workersvc**
- `CreateAutoCIFixRun(...)` — parallel to `CreateCIFixRun` but sets `auto_approve=true` (mirrors the `CreateAutopilotRun` split); same one-active-fix index + cross-kind branch guards + typed errors (`ErrActiveFixExists`/`ErrBranchInUse`).
- `BuildFailureSnapshot`, `scrubKnownTokens`, `FailureSignature` (moved/added).

**Notifications** (M4 + M6): a `notifsvc` collaborator is wired into `poller/ci_autofix.go` (start + halt) and into the `pipeline_sync` verified-stamp path (landed). Each event enqueues a PRD #46 in-app notification + the issue comment + Slack best-effort.
...
| M2 | Server guard plumbing + shared snapshot/signature | `workersvc/ci_fix_snapshot.go` (moved), `handler/ci_fix.go` (call shared), **`ClaimConfig.CIConfigPaths` producer**, `config.go` (`CI_AUTOFIX_*`), forge `ProjectCIConfigPath` + GitLab driver | — | P1 |
...
| M6 | `CIAutoFix` poller detector + notifications wiring | `poller/ci_autofix.go` (new), `poller/poller.go`, `cmd/api/main.go`, `notifsvc`/`slacksvc` + `CreateIssueNote` on start/halt | M2, M4 | P3 |
...
- Cross-forge parity (Forgejo, PRD #65) — GitLab first; the detector is forge-neutral by construction but validated on GitLab only.
