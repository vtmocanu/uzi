...
- **A 413 arriving *during* bisection tombstones a clean message, and lies about it.** In `bisect`, the oversize verdict refunds the budget and then falls into the shared `hi = mid` — narrowing the poison window on a size signal that is not evidence about the payload. Measured: with the poison at seq 4 and a 413 answering the first probe, the post sequence `[[1,2,3,4],[1,2],[1],[2],[3,4],[3],[4]]` tombstones **seq 2, which is clean**, and its marker tells the operator `message dropped: payload rejected by the api` about a message the api never rejected — a false statement in the run's permanent history, worse than the loss itself. Reachability is low (`takePrefix` caps at 512 KiB against a 1 MiB server cap, so a current worker should never see a 413), but the arm exists precisely for when that accounting is wrong. Two-line fix: treat `oversize` inside `bisect` as `transient` already is — abandon the search, `return batch.slice(lo)`, and let the outer oversize arm split.
- **The reclaim's bail counts a 404 as an outage, so a healthy API stops the sweep and the log blames the API.** `getChatRun` throws on 404, and every throw becomes `skippedStatusUnknown` → `consecutiveFailures += 1`. That conflation is right for the SKIP decision and wrong for the BAIL decision. Measured with six aged HOMEs against a healthy API that 404s three: `{"examined":3,"removed":0,"unexamined":3,"stoppedEarly":"api_unreachable"}` — three reclaimable HOMEs never examined, and an operator sent to investigate an API that was answering correctly. **Not self-correcting**: `readdir` order is stable, so the same three lead every boot and the sweep is permanently stuck at zero. And 404s are likelier here than anywhere, because the sweep targets the oldest stranded HOMEs — exactly the ones whose run rows have since been deleted. Fix: distinguish "the API answered not-found" from "the call failed", and count only the latter.
- **The tombstone drops `agent_instance` and `agent_label`**, so a tombstoned subagent frame leaves its lane and the marker renders in the top-level stream rather than where the loss happened. One line each; cannot break contiguity.
- **A tombstone can be re-tombstoned once** when a marker is re-buffered and then grouped with following messages, escaping the `batch.length === 1` guard. It terminates (the bound is intact — this is not the infinite loop the 122-second test caught), but the second marker reports the *first marker's* kind and size, losing the only forensic content a tombstone carries. Early-return in `tombstone()` when `item.tombstoned` is set.
- **The "57 non-test `DecodeJSON` call sites" tally is 55, across 31 files.** The claim the comment makes is unaffected and correct, but per this repo's own rule an assertion COUNT drifts exactly like a line number — cite the shape ("every other call site"), not the tally.
- **`sanitizePayloadJSON`'s fast path returns the caller's backing array**, so its contract is "sometimes you get your own array back, sometimes a fresh one" — invisible at the call site. Safe today (nothing downstream mutates it); one sentence making the read-only contract explicit would make that durable.
- **SECURITY: `agent` and `kind` are not redacted worker-side, and the api never redacts at all.** Measured on the wire: a secret placed in `kind` reaches the api in the clear. Redaction exists **only** in the worker (`batcher.ts` `emit()`), so an unredacted field reaches Postgres, the WS frame, the browser and `uzi run logs` unscrubbed. `batcher.ts`'s own justification for scrubbing `agent_instance` — *"so the treatment is symmetric and a future id format cannot re-open the hole"* — applies verbatim to both. Likelihood is low (both come from fixed worker-side vocabularies, not model output), which is why it is not blocking; the fix is two lines in `emit()`. **This is distinct from the missing cap** — a capped `kind` is still an unredacted one.
- **`failure_reason` on `/state` is the same unsanitized class, one route over — and the breaker's own report travels it.** `reason.slice(0, 512)` → `reportState` → `textParam(req.FailureReason)` → a Postgres `text` column with **no `stripNUL`**; M2 sanitizes `/messages` only. A NUL there yields 22021 → 500 → `reportState`'s bounded retries exhaust → **the terminal state never lands** and the run is left to the server-side sweeper. Pre-existing rather than opened by this MR, and low reachability (worker prose plus the api's own response body) — but this PRD is explicitly about closing the *class*, and this is the class on a sibling route.
- **`stoppedEarly: "api_unreachable"` names a cause the check never measures** — three 404s from a healthy API produce that exact label. Both validators found it independently and **disagree on the cost, which is worth preserving**: the reviewer measured three reclaimable HOMEs skipped and argued it is not self-correcting across boots (stable `readdir` order), so the sweep sticks at zero; the auditor judged real 404s rare (no `DELETE FROM runs` exists anywhere; runs vanish only by user cascade) and read the cost as a misleading diagnostic sending an operator to investigate a healthy API. Both are right about what they measured. Either way it is the same rule this run applied to test failure messages, applied to an operator-facing log field: name the label after what was actually measured (`status_unknown_streak`), or distinguish not-found from could-not-ask.
- **`f2ddb5ce` is only partly audited and must not be recorded otherwise.** The auditor read its `service.go` half (confirming the `kind` ordering trap is genuinely closed — the post-strip empty check sits in the validation pass, so nothing is written and nothing is logged as laundered) but did **not** run its gates or review `worker_protocol.go`, `sanitize.go`, or its 107 lines of new tests.
