...
| M2 | Version-aware seed: `nixSeedScript` reads the M1 store-hash marker, records it with the sentinel, re-seeds on mismatch **incl. legacy/absent marker**; clears `/data` devbox state on reseed; compose parity decision | `controller/internal/kube/render.go`, `render_test.go`, compose | M1 (marker path contract) | 3 |
...
| M4 | k8s verify on dev-cluster: fresh-PVC worker AND a worker rolled over a stale PVC both heal without manual PVC deletion; a per-run provision succeeds AFTER a reseed over a store that previously held run-provisioned packages; reviewer subagent runs `go test ./...`; compose e2e/smoke green | verification | M2, M3 + release | 4 |
