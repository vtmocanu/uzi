...
**Priority**: High  
**Status**: In progress. Implementation M1-M5 complete and reviewed; M6 code (offline stub seam + e2e phase 74 + registry) delivered and offline gates pass, but the joined offline e2e run (`./e2e/run-e2e.sh`) is deferred to a network-capable environment (see Decision log 2026-09-19). This child is one atomic public-enablement change; it must not be split into separately mergeable backend and frontend PRs.  
