...
- [ ] **M1 — Header restructured to Option B.** The `:431` header block is split
...
- [ ] **M2 — Behavior + a11y parity preserved** *(verification gate, not a separate
...
- [ ] **M3 — Tests updated and green (jsdom-testable claims only).** `WorkersSettings.test.tsx`
...
- [ ] **M4 — Wrap safety asserted in code; visual confirmation is human/browser-gated.**
...
check (bottega / multica / dot-agent-deck, per convention) is not load-bearing for
