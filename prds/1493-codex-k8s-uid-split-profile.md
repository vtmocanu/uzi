...
- `prds/done/1106-codex-harness-phase1.md` M6: record this prerequisite (the issue notes M6 omits it).
...
| D9 | On a read-only kube Secret mount for the join token, **dereference the atomic-writer target** (`busybox stat -L`) and verify the target's exact posture (`uid=0 gid=10001 mode=0440`, plus a positive check that `worker` can actually read it), failing closed instead of aborting; a **dangling link, a stat/dereference failure, and a worker-readability failure are each fatal**, and the projected-Secret symlink's own `0777` lstat is never authoritative. The writable-secret compose reclaim (chown 0:0 → chmod 0400 → hand to `worker`) is unchanged. (Corrects the `stat`-without-`-L` defect shipped in v0.84.0-rc.4, #1523.) | A bare `chown 0:0` on a read-only Secret mount fails `EROFS`, and aborting under `set -eu` crash-loops the pod. Verifying the exact posture the kubelet actually presents — rather than either trusting the mount blindly or refusing to start at all — keeps the same worker-readable/runner-unreadable containment the writable path enforces |
| D10 | Legacy `/data` ownership map on first split enablement (one-time, sentinel-gated, symlink-guarded — a carve-out root or descendant that is a symlink is rejected rather than dereferenced): plain-lane `runner` clones and `provision/` content re-owned to `runner`, **preserved, never purged**; carve-out parents (`runner`, `agent-home`, `provision`) reclaimed and re-owned `worker:runner`; each run's HOME root and its `codex-data` root **stay** `worker:runner`; `codex-session-store` stays worker-private (0700/0600) and is **never touched**; every other per-run HOME descendant (ordinary Claude SDK state, `codex-data/epoch-N` provider trees) is re-owned to `runner`; a hardlink planted from inside a migrated tree into an existing `repos/` file is a documented, narrow, accepted residual | A retained clone or resume state may be the only copy of unpublished work, so nothing under `/data` is ever purged on this path. `codex-session-store` stays worker-private specifically so uid 10003 cannot read or tamper with resume state. The rest of the map is what lets a resumed run's SDK actually read and update its own HOME once it runs as `runner`, without ever widening the worker-only bare-repo cache the migration must not touch |
| D11 | Tmpdirs derive beneath an ambient DinD-shared `TMPDIR` when the docker lane sets one (still one private 0700 dir per uid), else under `/tmp` exactly as before; the sticky bit closes the provider-root/session-store swap uniformly — 3775 on the entrypoint's carve-out parents, 3770 on every `ensureCodexSharedDirectory`-created ancestor; `seed-nix` runs as uid 0 with exactly `CHOWN`, `DAC_OVERRIDE`, `FOWNER`; and advertisement of `codex_harness_v1` is withheld whenever the Landlock probe reports an ERROR (an errno other than `ENOSYS`/`EOPNOTSUPP`, an ABI below 1, or a later create/add-rule/restrict/deny-probe failure — fatal in both modes), granted only when Landlock is available, or the mode is `best-effort` **and** Landlock is specifically `unavailable` | A docker-lane bind source staged outside the shared workdir would resolve to an empty directory inside the daemon's filesystem. Collapsing a Landlock *error* into "unavailable" would let `best-effort` advertise Codex on a worker whose Landlock is actually broken (a seccomp denial, a bad ABI), not merely absent — the exact dishonest-advertisement class this PRD removes |
