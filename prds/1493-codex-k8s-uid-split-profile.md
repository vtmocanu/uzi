...
**Status:** Track U (M1 to M5) implemented on branch `agent/issue-1493`, 2026-09-20. Track M (M6 real-kubelet validation, M7 live verification) remains maintainer work and has not started.
...
- **Track M (maintainer, local):** M6 and M7 — **not started.** A real-kubelet validation and live verification. A uzi worker can do neither. No workflow edit is needed anywhere in this PRD.
...
| D9 | On a read-only kube Secret mount for the join token, verify the exact posture (`uid=0 gid=10001 mode=0440`, plus a positive check that `worker` can actually read it) and fail closed instead of aborting; the writable-secret compose reclaim (chown 0:0 → chmod 0400 → hand to `worker`) is unchanged | A bare `chown 0:0` on a read-only Secret mount fails `EROFS`, and aborting under `set -eu` crash-loops the pod. Verifying the exact posture the kubelet actually presents — rather than either trusting the mount blindly or refusing to start at all — keeps the same worker-readable/runner-unreadable containment the writable path enforces |
