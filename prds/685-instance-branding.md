...
- [ ] **M1 — Backend: settings keys, asset table, public + admin endpoints.**
...
- [ ] **M5 — Docs + CLI parity.** New user-facing `docs/branding.md` (frontmatter `title:`, `audience: user`, unique `order:` — matches `web/scripts/check-docs.mjs`) covering brand/white-label/unbrand and that the credit is fixed. Note the feature in `ARCHITECTURE.md`. Update `CHANGELOG.md` `[Unreleased]`. Confirm whether `api/cmd/uzi/` needs a CLI change (expected none — admin-UI config; record the conclusion per the CLI-parity convention).
