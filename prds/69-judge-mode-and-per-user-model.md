...
- **Build now, self-contained: M1, M2, M3+M5, M6.**
...
- **Coordinate before building: M7a → PRD #123.** Key its trusted closed-enum off #123's classified `failure_reason`, not today's generic timeout string. **#123 is in flight on uzi as of 2026-08-15 — wait for it to land**, so M7a is imminent, not indefinitely blocked. #78's failure mode is still live (worker egress is still 3 FQDNs), so the motivation holds; the judge already *receives* raw `failure_reason`/status/iterations in its prompt header, so M7a's value is "make that input **trusted + closed-enum + prompt-ruled + pre-start-gated**," not "give the judge ground truth it lacks." The pre-start infra-skip gate and the one prompt rule are valid independent of #123.
