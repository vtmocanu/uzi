...
2. **The model is reaped before the credentialed publish.** `executor.run()` reaps in its `finally` (`killAgentTree`, sdk-executor.ts:728); the runner reaps again at the security boundary (runner.ts:3930) before `phasePublish` (wrapped in `withCodexBoundaryOnly({boundary:"finalize"})`, runner.ts:748–759). A repair turn cannot be inserted into `phasePublish`. **But** the existing loop already survives a reap and continues: `ctx.checkpoint({reap:true})` calls `reapForSink` (runner.ts:3814), after which the executor recreates the reaped provider epoch (`startProviderEpoch`, codex-executor.ts:1095/1145/1183) and the next same-session turn keeps its budget, reducer and session. D3 uses that seam.
...
| 1 | **M0** — Exit-capture proof | gate | Provider-neutral exit-capture seam design + a Claude/Codex fixture proving nonzero-Bash → captured-exit (`agent/test`). No store, no state machine. |
...
| 4 | **M3** — `RunContext` final-candidate callback seam | M2 | `agent/src/executor.ts` (the RunContext member) + `sdk-executor.ts`/`codex-executor.ts`/stub loop wiring reusing reap-and-continue; **no new public method**; no runner `phasePublish` edits. |
...
- [ ] **M1: Capture the receipt and freeze the target-repo contract.** Provider-neutral receipt type + adapters, the worker allowlist classifier, the trusted-`T`-frozen validation contract, uzi's own `check:sqlc-drift`, and the D1a sqlc-provisioning decision. Gate: `task gate:agent`, `task gate:repo` (for the new Task target), `task scan:secrets`.
- [ ] **M2: Select and prove the merge primitive; build the replay harness.** Object-level three-way merge, differentially proven against a target-base `git merge` across the enumerated cases; the green/red/typed-conflict classifier; the #1357 fixture green after reconcile and red before. Gate: `task gate:agent`.
...
- [ ] **M4: Wire the runner state machine and refactor the align block.** Callback implementation; pinned-`T_n` refactor of all three align arms with the per-arm tree-equivalence proof; the cap and per-attempt tracking; the MR-producing-kind matrix and exclusions. Gate: `task gate:agent`.
- [ ] **M5: Persist records, add typed origins, unify counters.** Record the validated merge tree OID + `(T_i, H_i, P_n, attempt count)`; new `fail_origin`s with the CHECK-widening two-step migration and the test-path repoint; the resume-durable single-sourced counters + reserve. Gate: `task gate:api`, live-store tests, `task scan:secrets`.
...
- **#1357 fixture**: a fixture where the branch and updated default add colliding migration numbers and the default adds a generated-query input is reconciled and gate-green (on the D1 contract, which includes `check:migration-numbering` and `check:sqlc-drift`) before the publication push.
...
- **Recording and pinning**: uzi records and validates the merge-result tree `tree(M_n)` bound to `(T_n, H_n)`; the published head is `P_n` (`= H_n`, or `H_n` plus the GitHub workflow-overlay commit); uzi pushes `P_n`, never the synthetic merge tree, and only while the final fetched default equals `T_n`. When `P_n != H_n`, `tree(merge(T_n, P_n)) == tree(M_n)` is recorded and proven for the fired arm. The run records `T_n`, `H_n`, `P_n`, the validated `tree(M_n)`, and the reconciliation attempt count.
...
- **#1297 amendment is out of scope for #1359's runs** and is the maintainer-gated pre-dispatch gate (D8).
