...
- [ ] **M1 — Regression tests, written first and shown failing on the unfixed predicate (web).** In `web/src/pages/Dashboard.test.tsx`, beside the PRD #60 checklist test (`describe("Dashboard onboarding checklist (PRD #60)")`, `:259`), using the existing `aWorker` fixture (`:112`, whose default is `status: "online"`, `last_heartbeat_at: null`) and `renderDashboard` (`:150`). Keep steps 1-3 open (the `beforeEach` defaults) so the card renders and the step is assertable:
