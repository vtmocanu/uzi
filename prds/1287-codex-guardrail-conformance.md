...
**Status**: Dispatched as run `40db12f7-01ed-4804-a28f-4d3d323a918d` in Auto mode with MR rework enabled. At the plan gate; no implementation or runtime acceptance is claimed.
...
The U/P execution completeness check rejects missing, duplicate or unknown IDs, an adapter omitted from a required row, a zero-test match, an unmet prerequisite and any required skipped/cancelled/todo case. O reporting and final proof checks remain separate under D8; an `owed` record never counts as executed evidence. A hard-coded list of claimed successes, tests that merely import fixtures, or a grep for test names is not execution evidence. Generate the readable map from the registry or assert the two stay consistent.
...
Model every O row from C1 as exactly one of `inherited`, `receipt-present` or `owed`. `inherited` records the exact previously tested source, image digest, target and case, plus why its enforcement path is unchanged at this candidate; `receipt-present` records fresh maintainer proof on the candidate; `owed` records the missing/new/invalidated case, target, reason and maintainer owner, never a success. The ordinary U/P gate rejects missing or malformed O records, but permits a well-formed `owed` record so the worker can open its PR. A separate lead-run `task check:codex-m4-receipts` rejects every `owed` row; it must pass before merge and parent M4 acceptance. This separates reporting completeness from proof completeness without accepting an unrun OS assertion. Prior totals such as 15/15 or 14/14 are not a blanket coverage waiver. The new HOME-screener regression belongs in U/P, while independent command-root HOME denial may cite existing M3b evidence only when the named case actually proves that invariant.
...
**Parent acceptance:** C1-C5 implementation, current-head CI, the strict macOS Linux-container invocation, and `task check:codex-m4-receipts` with all valid/fresh O receipts must pass before the lead merges and marks parent M4 complete. The worker must leave parent M4 unchecked when any maintainer evidence is pending. Parent M5-M7 remain incomplete.

## Success criteria

1. All required U/P clause/adapter entries are executed, independently asserted and mechanically checked for completeness. A removed case or skipped required runtime makes that strict gate fail. Every O row is fully reported under D8, and no `owed` row may pass final maintainer acceptance.
...
- 2026-09-12 (plan gate): Run `40db12f7-01ed-4804-a28f-4d3d323a918d` was dispatched after publication at `36af73a1`. Plan review identified an O-evidence completion trap: a worker cannot both satisfy a missing maintainer-only receipt and leave that proof to the maintainer. D8 now separates the strict U/P gate's complete tri-state reporting from the lead's all-proven acceptance target. No OS proof is waived and parent M4 remains unchecked.
