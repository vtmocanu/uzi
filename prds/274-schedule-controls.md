...
**Status**: **Draft** (created 2026-08-09; architect-reviewed 2026-08-09 — blocker + should-fixes folded in; owner resolved both open decisions 2026-08-09: Decision 1 = **1a** (thread + default on), sweep default = **bounded 10**)
**Priority**: Medium
...
- [ ] **M1 — wait-on-limit for scheduled runs (Decision 1 = 1a, owner-approved).** Additive `CreateScheduledAutopilotRun` seam; route the auto-approve branch of `createIssueRun`; create-time default on; CLI flag + web toggle default on. **No migration.** Tests: an auto-approve scheduled run honours the schedule's `wait_on_limit`; default is on for a new schedule; existing rows unchanged; **a structural assertion that `poller`'s `CreateAutopilotRun` interface/fake/call site are untouched** (label-autopilot behaviour unchanged).
- [ ] **M2 — sweep cap.** Shared migration `00108` (`max_issues`), `LIMIT sqlc.narg('max_issues')` in `ListSweepCandidateIssues`, scheduler threads the param, DTO `*int` + `mergeSchedule` + `validateScheduleConfig` (non-positive reject, sweep-only scope), CLI `--max-issues` + detail row, web field + `mockApi.ts` parity. Tests: **unit** (param threaded), **LiveDB** (real LIMIT limits to N oldest; NULL = all), validation rejects non-positive / non-sweep.
- [ ] **M3 — guiding prompt.** Shared migration `00108` (`guidance`, no CHECK change), `createIssueRun` composition with delineated owner section + guidance-cap truncation, DTO `*string` + `mergeSchedule` + length cap, CLI `--guidance` + detail row, web textarea + `mockApi.ts` parity. Tests: guidance appears delineated from the issue body; absent when unset; gates unaffected; **near-cap body + guidance truncates, does not skip**.
- [ ] **M4 — docs.** Update `docs/scheduling.md` and `docs/cli.md` for the new flags/fields/defaults, following existing patterns; web modal help text.
- [ ] **M5 — gate green.** `task gate:api` and `task gate:web` pass; the migration renumbered to the next free number above the live head at landing (draft `00108`; head is currently `00107`); all new tests pass under `-race`.
