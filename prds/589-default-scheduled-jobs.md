...
### M2 — Catalog API + prompt-by-reference resolution + Reset
...
### M3 — Multi-repo enablement + clone (api + CLI)
- Enable-across-repos: accept a repo-id list (server fan-out, or client fans out N single-repo `POST`s — pick one and document it) → N independent rows. Custom create likewise supports N repos.
- Clone: `POST /api/schedules/{id}/clone` → a new `origin='user'` row prefilled from the source; a cloned **default** copies its baked prompt into the editable `prompt` column (lock lifted) and sets `catalog_slug=NULL`. Clone-to-a-different-repo is the replication path.
- CLI: `uzi schedule catalog list|enable|reset`, `uzi schedule clone`, multi-repo enable; `docs/cli.md`.
- **DoD tests:** multi-repo fan-out = N rows; clone unlocks prompt (`origin='user'`, `catalog_slug=NULL`, editable prompt copied from catalog).
- **Deps**: M2. **Gate**: `task gate:api`.

### M4 — Sweep-label guardrail
- On enabling a sweep default or creating/editing a sweep schedule, `ListLabels` on the repo; if the selector label is absent, return a warning payload; a follow-up confirm calls `EnsureLabels` to create it. Exposed via api + CLI; the **web** surface for the warn/confirm is owned by M6 (below).
- **DoD tests:** missing-label warn; confirm creates via `EnsureLabels` (fake forge).
- **Deps**: M2. **Gate**: `task gate:api`.

### M5 — #341: report-only prompt opens no MR (agent-only, independent)
...
### M6 — Web: two-tab Schedules page + layout A + sweep-warn UI
...
### M7 — Docs + final integration sweep
...
1. A connected user sees a **Default jobs** tab with 6 catalog entries; enabling one on a repo creates a running schedule with a **locked** prompt.
2. Enabling a default on 2 repos creates 2 independent schedules with per-repo cadence, shown as one expandable row.
3. Cloning a default yields an editable custom schedule (prompt unlocked, `catalog_slug` cleared).
4. A default row stores **no** prompt copy; a shipped catalog prompt change is what fires (proven by the M2 poison test).
5. A report-only scheduled `prompt` run that commits nothing opens **no** MR (#341).
6. Enabling a sweep default whose label is missing warns and offers to create it.
7. `task gate` green across api/web/agent/repo; store-it sweep green. #341 and #303 closed; #317 closed as a duplicate of #396. (Self-improvement + #301/#296/#524 are PRD-B.)
