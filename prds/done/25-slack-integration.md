...
**GitLab Issue**: [vtmocanu/uzi#25](https://github.com/vtmocanu/uzi/-/issues/25)
...
### Inspiration check (audited 2026-07-06; corrected after fact-check)

| Concern | bottega | dot-agent-deck | uzi will do |
|---|---|---|---|
| Slack integration | None (one doc mention) | None (its notification PRDs explicitly keep Slack out) | A **notification/approval bot**, not a chat channel. Uses slack-go, a socket supervisor pattern (per-connection cancellable context, reconnect), ACK-envelope-first inbound handling, base64-sealed BYO tokens, validate-token-on-save |
| Human gate from chat | — | — | Block Kit buttons + threaded reject-reason wired to the existing `approve_plan`/`reject_plan`/`follow_up` steering inputs — neither prior-art project has this |
...
1. **Secret-key class in the registry**: a declared set of secret keys (`slack_bot_token`, `slack_app_token`) that (a) get sealed with `secretbox` and base64-encoded before `UpsertAppSetting` (`app_settings.value` is `TEXT`), (b) are **structurally excluded** from `All()`/`GetSettings` value output — the API emits only `configured: true|false`, so the handler *cannot* forget to redact, (c) get dedicated `Validate` branches, and (d) are readable only through a new decrypt accessor used by `slacksvc`. A test asserts the admin GET response never contains token bytes (sealed or plain). Saving validates live: `AuthTest` for the bot token, a Socket Mode handshake for the app token — validation errors must not echo the submitted token.
...
- Events API / public-URL mode, slash commands, Slack as a full chat surface.
...
- **2026-07-06 — Socket Mode only** (user-confirmed): outbound-only fits the loopback deployment; Events API rejected, "support both" rejected as premature surface. (Corrected after fact-check: this matches, not differs from, prior art's transport choice — uzi's differentiator is the notification/approval shape, not Socket Mode itself.)
