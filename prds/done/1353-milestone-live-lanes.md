...
- **D10 (liveness rule refined at implementation, M3):** D3 said a lane counts as live "within the same freshness window `RunActivity` uses" — but `RunActivity` (`api/internal/runactivity`) applies no staleness window at all; it unconditionally folds the newest-`Seq` frame. What actually shipped in `milestonelanes.Derive`: a lane drops when its dispatch has **completed** (a `tool_result` whose `tool_use_id == agent_instance` — the primary, precise signal) OR its newest frame is older than a new purpose-built constant, `laneFreshWindow` (10 minutes) — a backstop for a subagent that crashed without ever emitting a completion. Recorded in [ADR-1353](../../adr/1353-milestone-live-lanes.md) D2.
- **D11 (no TS derivation twin, M3/M7):** because `milestones_live` is server-authoritative and web-consumed, never client-re-derived (D2/D8) — unlike `current_activity`, which the web does re-derive client-side for its zero-latency now-line (`web/src/lib/runActivity.ts`) — the cross-language fixture originally planned for both halves (M7) is Go-only: `fixtures/milestone-live-lanes/cases.json` is asserted from `api/internal/milestonelanes/derive_test.go` alone. Web parity is instead held by the DTO-shape api-contract compile-time check (`fixtures/api-contract/run.{zero,full}.json`). Recorded in [ADR-1353](../../adr/1353-milestone-live-lanes.md) D3.
...
- [x] **M3 — Server derivation + DTO.** The two-part derivation (D2) producing per-milestone live lanes, exposed as `milestones_live` (D8), with the api-contract fixtures re-recorded (four: `run.{zero,full}.json` + `run_list_item.{zero,full}.json`, since `RunListItemDTO` embeds `RunDTO`) and a **new derivation fixture** (`fixtures/milestone-live-lanes/cases.json`, asserted **Go-side only** from the derivation's Go test — no TS twin, per D11, because `milestones_live` is server-authoritative/web-consumed). Derivation tests: multiple lanes on one milestone, `reviewer ×2` by instance, idle-owner-only (owner shown, no lane), an **out-of-window dispatch frame** (instance live but its dispatch frame is old — the back-join must still resolve), terminal run empty, no-milestones back-compat. `-count=1` applies (cross-module fixture; `.claude/rules/go.md`).
- [x] **M4 — Web RunView render** (parallelizable). Consume `milestones_live`: owner line (from M1) plus a green live line per lane. Tests: multi-lane, repeated role, back-compat unchanged, sanitization via `stripUnsafeChars` on `agent`/`label`/`tool`.
- [x] **M5 — CLI `uzi run get` render** (Go package, serialized with M6). Per-milestone lanes in `milestoneNowRows`; owner demoted; global now-line intact. Parity golden `run_render_attribution_test.go` extended.
- [x] **M6 — TUI crew-rail render** (Go package, serialized with M5). Lanes stacked under the in-progress milestone; register the new lane fields in `d7UntrustedFields` and extend the hostile-value test (D7). Parity goldens `tui_milestone_attribution_test.go` + `milestone_attribution_fixture_test.go` extended.
- [x] **M7 — Cross-surface parity + contract green.** As-built (reconciled with D11): the live-lanes work did NOT change the owner-JOIN output, so the Go/TS join fixture (`api/cmd/uzi/milestone_attribution_fixture_test.go`, `web/src/lib/milestoneAttribution.fixture.test.ts`, `fixtures/milestone-attribution/cases.json`) needed no change and still passes on both halves. The new `fixtures/milestone-live-lanes/cases.json` is asserted **Go-side only** — `milestones_live` is server-authoritative/web-consumed, so there is no TS derivation twin (D11). `task gate` green across api, web, agent, controller; the DTO contract (`fixtures/api-contract/run.{zero,full}.json` + `run_list_item.{zero,full}.json`, Go `contract_test.go` and the web compile-time `keyof` check) passes together; the 5-package live-DB suite (store/handler/forgesvc/schedsvc/workersvc) is green.
- [x] **M8 — Docs + ADR.** ADR for the description-tag binding; update `ARCHITECTURE.md` where the run-view surfaces are described and this PRD's Decision Log; `task docs:sync` if a `docs/*.md` changed.
...
- **Derivation (M3):** table-driven Go tests over `run_messages`, including the out-of-window dispatch back-join and `reviewer ×2` by `agent_instance`; the new `fixtures/milestone-live-lanes` fixture is the Go-side pin (asserted from `derive_test.go`; no TS twin, per D11).
