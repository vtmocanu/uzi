...
**GitLab Issue**: [#112](https://github.com/vtmocanu/uzi/-/issues/112)
**Status**: **Complete** (created 2026-07-22, merged 2026-07-25 via [MR !106](https://github.com/vtmocanu/uzi/-/merge_requests/106), merge commit `8feb27ec`)
**Completion note**: M1-M5 shipped. **M6 is NOT done** and was deferred out of the branch by necessity — it needs a deployed image and live `uzc_`/`uza_` tokens on dev-cluster, which a pre-merge branch cannot provide. It is the one open item from this PRD, tracked as [#133](https://github.com/vtmocanu/uzi/-/issues/133) so it stays visible in open work rather than only inside this archived file, and described in its own milestone entry below. Evidence at merge: `./e2e/run-e2e.sh` 186 PASS / 0 FAIL measured post-`origin/main`-merge on a harness byte-identical to the merged tip; GitLab pipeline [20010](https://github.com/vtmocanu/uzi/-/pipelines/20010) green across all 15 jobs. Open follow-ups: [#124](https://github.com/vtmocanu/uzi/-/issues/124) (the same `Cf` gap on the web, a more exposed surface than the terminal), [#129](https://github.com/vtmocanu/uzi/-/issues/129) (CHANGELOG missing five release sections, pre-existing).
**Priority**: Medium
**Related**: [#64](https://github.com/vtmocanu/uzi/-/issues/64) (the `uzi` CLI this extends), [#99](https://github.com/vtmocanu/uzi/-/issues/99) (per-agent invocation id/label — the data the lanes render), [#95](https://github.com/vtmocanu/uzi/-/issues/95) (steer queue), [#47](https://github.com/vtmocanu/uzi/-/issues/47) (run health), [#94](https://github.com/vtmocanu/uzi/-/issues/94)/[#46](https://github.com/vtmocanu/uzi/-/issues/46) (judge review + triage), [#102](https://github.com/vtmocanu/uzi/-/issues/102) (the web board this is the terminal analogue of)
...
- **`bottega`** has no terminal UI of any kind — nothing there to match or
  beat here.
