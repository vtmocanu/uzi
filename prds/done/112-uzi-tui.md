...
**Status**: **Complete** (created 2026-07-22, merged 2026-07-25 via [MR !106](https://gitlab.example.com/vtmocanu/uzi/-/merge_requests/106), merge commit `8feb27ec`)
**Completion note**: M1-M5 shipped. **M6 is NOT done** and was deferred out of the branch by necessity — it needs a deployed image and live `uzc_`/`uza_` tokens on dev-cluster, which a pre-merge branch cannot provide. It is the one open item from this PRD, tracked as [#133](https://gitlab.example.com/vtmocanu/uzi/-/issues/133) so it stays visible in open work rather than only inside this archived file, and described in its own milestone entry below. Evidence at merge: `./e2e/run-e2e.sh` 186 PASS / 0 FAIL measured post-`origin/main`-merge on a harness byte-identical to the merged tip; GitLab pipeline [20010](https://gitlab.example.com/vtmocanu/uzi/-/pipelines/20010) green across all 15 jobs. Open follow-ups: [#124](https://gitlab.example.com/vtmocanu/uzi/-/issues/124) (the same `Cf` gap on the web, a more exposed surface than the terminal), [#129](https://gitlab.example.com/vtmocanu/uzi/-/issues/129) (CHANGELOG missing five release sections, pre-existing).
**Priority**: Medium
