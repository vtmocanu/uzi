...
> 1. **"One M7 operator item is only PARTLY delivered (commit `8190069`)"** — *no work was outstanding*. Commit `8190069` **made** the `specs/human.md` edit; what it recorded is that the *feature* described in the Deferred list is partial. `specs/human.md:471-476` reads: the dedicated operator exists and provisions per-user workers **on user request**; still deferred are the **when-queued-work-appears trigger**, autoscaling, scale-to-zero, and the chat-agent case. Every one of those was **explicitly out of #58's scope from the first draft** (§ Feature #58 in `specs/human.md:400`: "spawn-on-queued-work is NOT in scope here"). A milestone that delivered a doc edit saying "the feature is partial" was read, for a week, as a doc edit that was itself partial.
> 2. **Issue #82 is not a #58 residual** — it belongs to **PRD #18's** per-run tool-provisioning path (`agent/src/provision.ts`, the `tool_allowlist` table, the image `/nix` seed). #58 only *surfaced* it, because hosted workers are where egress is locked to three FQDNs; a compose worker runs the identical code and merely happens to reach github. Tracked forward in **PRD #123** — see `prds/done/123-provisioning-without-github-egress.md`.
>
> **The rule this earns: a residual belongs to the PRD that owns the code, not the PRD that was running when it fired.** Attaching #82 here kept a shipped feature open and put the fix on the wrong roadmap. (Audit 2026-07-25 — `glab issue view 58` already read `state: closed`; only the repo side was lagging.)

*(Status corrected 2026-07-18 — the header still read "Draft" long after v0.3.0 shipped; see git history. Created 2026-07-16; revised same day after parallel design review / security audit / fact-check — mechanism changed from api-managed Deployments to a dedicated controller, TLS on the worker hop pulled into scope, v1 admin surface trimmed.)*
**Priority**: Medium
...
No prior art in the inspiration submodules (bottega, dot-agent-deck — neither
does in-cluster worker provisioning); the bar is this repo's specs and
existing code style.
