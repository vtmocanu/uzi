...
- **Priority**: Medium
- **Status**: Done (2026-08-22). All four milestones landed on `agent/issue-534`: **M1** relocated the four per-repo routes to `/repos/{id}/github-project-sync*` under `RequireAuth` with the owner-or-admin `GetRepoForUser` preflight (existence-hiding 404); **M2** added the admin instance kill-switch card in Admin → Instance settings; **M3** added the per-repo Provision/Adopt/Disable/status controls to the Boards page (GitHub-only, `forge_type`-gated); **M4** refreshed `docs/github-project-sync.md` and recorded the authorization split in `specs/ai.md` §567. Verification: `task gate:api` (incl. `-race`, deadcode, lint ratchet) and `task gate:web` green; M1's owner/non-owner/admin authorization tests (`github_project_sync_livedb_test.go`) executed and passed via `./e2e/run-store-it.sh` (RUN>0, zero SKIP). All success criteria SC-1…SC-5 met.
