...
**Decision 10 — Reverted-drift cordon recovery: clear `draining_since` on the no-drift path (issue #458).**
Decisions 4/8 cordon a busy drifted worker and clear `draining_since` only when it actually rolls (`RegisterWorker` on re-register). A drift REVERTED before the roll — an operator resets `workers.image.tag` mid-rollout — leaves the worker cordoned with nothing to roll and no clear path, so it heartbeats `online` but claims no runs forever, and neither the drain deadline nor force-roll (both inside the drift branch, after the no-drift early return) can recover it. Fixed as a post-hoc review finding on PR #455: `reconcileWorker` now clears `draining_since` on its no-drift branch via a new symmetric uncordon control-write (`DELETE /api/controller/workers/{id}/drain`, the mirror of Decision 8's cordon `POST`). Safe because `draining_since` has a single writer and a single meaning — the drift-cordon — so "no drift + draining" is always a reverted drift or a just-committed roll, both correct to clear; the clear is fail-safe (retry next tick on error) and idempotent. This does NOT complete M9 — live-cluster validation of the cordon/drain/roll behavior (now including this recovery path) remains human-run, so M9's checkbox stays unchecked. (This PRD was moved to `prds/done/` and issue #422 closed once code milestones M1-M8 landed; M9 is tracked as an outstanding human-run ops step, not a blocker on closure.)
...
- [x] **M1 — Decouple worker image tag from appVersion (flip the default).**
...
- [x] **M2 — Preserve PRD #113's upgrade badge under a decoupled tag (Decision 9).**
...
- [x] **M3 — Worker `draining` state + busy/draining on the poll wire (api/db, atomic cross-module).**
...
- [x] **M6 — N-1 worker↔API compatibility contract (Decision 2).**
...
- [x] **M7 — Docs, ARCHITECTURE, ADR, release runbook (docs only).**
...
- [x] **M8 — CLI surface for drain/cordon/force-roll (code) OR explicit deferral.**
  Resolved by **explicit deferral** (the branch this milestone permits), recorded in `adr/0422-decouple-worker-version.md` §Maintainer hand-off (e) with the reason: there is nothing for a CLI verb to call. The cordon control-write is `RequireController`-scoped (shared controller token, not the CLI's user auth) and no admin/user endpoint stamps `draining_since`; force-roll and the drain deadline are deploy-time controller env / chart values (`workers.forceRoll`, `workers.drainDeadline`) with no runtime API; and the CLI is deliberately read-mostly for workers (`docs/cli.md`: "no admin writes"). A CLI verb would require new admin product surface the controller-driven model does not need — a clean follow-up if operator-initiated manual drain is later wanted. No code shipped for this milestone.
