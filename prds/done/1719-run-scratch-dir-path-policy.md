...
- **D6. Lifecycle.** Preserve scratch only while the identical runner clone remains retained through parks and resumes. Recreate an empty scratch directory on fresh reseed or cross-worker recovery; scratch contents are not checkpointed or durably recoverable. Never delete scratch during active execution or routine checkpoint publication. Settled clone retirement performs cleanup.
- **D7. Review snapshots are fresh plain exports.** Resolve the reviewed SHA, create `mktemp -d .uzi/scratch/snap.XXXXXX` for each review, and run `git archive "$sha" | tar -x -C "$snap"` with `set -o pipefail` or equivalent checks of both commands. Remove the export after review; never reuse it or create a nested `git worktree add`. An export has no Git metadata and no installed dependencies, so Git-dependent gates run in the real checkout under the frozen integration-gate discipline.
...
- [x] **M6. Tests.** Regression tests ship with M2 to M5 and are demonstrated red against the corresponding unfixed seam, then green with the fix. M6 integrates the complete suite and required component gates. Coverage:
...
- 2026-09-26: Implementation keeps scratch only with the identical retained clone. Approval waits retain that clone; fresh reseed and cross-worker recovery start empty. Regression tests for newly introduced publication and guidance seams were red before their fixes and green after them; lifecycle characterization tests verify the existing clone custody paths. Review exports use a fresh random directory and check archive and extraction failures.
