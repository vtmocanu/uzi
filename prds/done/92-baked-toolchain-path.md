...
**GitLab Issue**: [#92](https://github.com/vtmocanu/uzi/-/issues/92) (judge-filed from a retrospective of run on vtmocanu/uzi#90)
**Status**: COMPLETE (2026-07-20). Shipped in **v0.8.4** (MR !79, merged; #92 auto-closed). All milestones landed: version-aware `/nix` seed (render.go), immutable `/opt/uzi-toolchain` + indirect GC root, boot preflight, base/jvm lockstep, plus the seed/preflight/guardrail tests. **M4 verified LIVE on dev-cluster**: worker `uzi-hw-1011e7dd` rolled 0.8.3→0.8.4 over its stale 0.8.1-seeded PVC; the seed detected the legacy sentinel (no marker) and reseeded (`want=zhlki707w… have=`), and `python3/go/gcc/pip` now resolve via `/opt/uzi-toolchain/bin` (gcc included), with the recorded marker matching the image so subsequent boots skip reseed. Fable/architect review (2 rounds) folded in — it caught a wrong mechanism in the first draft (see Decision Log). One residual watch-item: a *per-run provision after reseed* is unit-tested (data-clear) but not yet exercised by a real run.
...
**Related**: Issue [#91](https://github.com/vtmocanu/uzi/-/issues/91) (runner-writable, persistent `/nix` — same store; the GC-root work here intersects it). PRD #84 (pre-run readiness preflight — a boot-time toolchain preflight (M3) is the natural runtime analogue and DOES catch this, since the PATH is dead from pod start).
...
Bake the resolved toolchain identity into the image as `/etc/uzi-toolchain-profile` = the `readlink -f` of the devbox global `default` profile store path (the **store hash**, NOT the image `appVersion`: appVersion changes every release, so keying on it would wipe-and-reseed on every roll and destroy the warm-start cache of per-run provisioned packages the volume exists to hold — key on the store hash so a reseed fires only when the toolchain actually changed). `nixSeedScript` records that value alongside the sentinel and **re-seeds on mismatch** (full wipe + re-tar — the script already wipes-before-extract on a partial seed, and the store is a cache by its own doctrine). **A legacy sentinel with no recorded marker MUST count as a mismatch (⇒ reseed)** — that is exactly how the currently-broken live worker heals on its next roll; an "absent marker = skip" implementation would silently fail the primary DoD. A roll that changes the toolchain then re-seeds automatically; a roll that doesn't stays a no-op. One side effect to handle: a reseed wipes store paths that per-run provisioning realized at runtime, and the devbox/nix state under `/data/agent-home` (profiles, gcroots, generation links) survives and would dangle into the wiped store — so the reseed must ALSO clear that `/data` devbox state, or M4 must prove `devbox install` re-realizes cleanly after a reseed. `render_test.go` gets cases for match (skip), mismatch (reseed), fresh (seed), and **legacy-sentinel-without-marker (reseed)**. The compose `agentnix` path (auto-seeds only when empty) needs the same guard or a documented `down -v` + the boot preflight below; decide in-PR.

### 2. Expose the toolchain immutably (HARDENING) — both Dockerfiles, lockstep

Make the image self-consistent and PATH independent of devbox's mutable global symlink: after `devbox global install`, `readlink -f` the realized profile and create a stable `/opt/uzi-toolchain → <store profile>`, registered as an **indirect nix GC root** (`/nix/var/nix/gcroots/… → /opt/uzi-toolchain`) so the seed tar carries it and a runner-triggered `nix-collect-garbage` (see #91) can't reap it. Point PATH at `/opt/uzi-toolchain/bin` instead of `…/.devbox/…/default/bin`. Per-run `devbox shellenv` still **prepends** provisioned tools (buildSdkEnv folds `toolEnv.PATH`; `filterShellenv` resolves the `$PATH` back-ref against the base/runner PATH), so run-specific toolchains still win with the baked set as a durable floor. End both Dockerfiles with a **fail-closed** `RUN command -v python3 go gcc pip && go version && python3 --version && gcc --version && pip --version` so a broken toolchain fails the build instead of shipping silent (this is the guard the "UNVERIFIED" stanza never had). **Guardrail prohibition to record in the entrypoint:** `/opt/uzi-toolchain/bin` must NEVER be added to the worker's stripped PATH (`agent/templates/entrypoint.sh` lines ~74-84) — it dereferences into runner-writable `/nix` and would pierce the PRD #51 M2-audit invariant; it belongs only on the image PATH that becomes `UZI_RUNNER_PATH`.
...
| M1 | Dockerfile exposure: `/opt/uzi-toolchain` symlink + GC root + PATH re-point + fail-closed assertion + bake toolchain-id marker (`/etc/uzi-toolchain-profile`); both templates in lockstep | `agent/templates/base/Dockerfile`, `agent/templates/jvm/Dockerfile` | M0 | **DONE** (this MR) |
| M2 | Version-aware seed: `nixSeedScript` reads the M1 store-hash marker, records it with the sentinel, re-seeds on mismatch **incl. legacy/absent marker**; clears `/data` devbox state on reseed; compose parity decision | `controller/internal/kube/render.go`, `render_test.go`, compose | M1 (marker path contract) | **DONE** (this MR) |
| M3 | Boot toolchain preflight (fail registration on miss) + record the entrypoint guardrail prohibition | `agent/src/*` (worker startup), `agent/templates/entrypoint.sh` (comment) | M1 | **DONE** (this MR) |
| M4 | k8s verify on dev-cluster: fresh-PVC worker AND a worker rolled over a stale PVC both heal without manual PVC deletion; a per-run provision succeeds AFTER a reseed over a store that previously held run-provisioned packages; reviewer subagent runs `go test ./...`; compose e2e/smoke green | verification | M2, M3 + release | **DEFERRED** — post-merge operational validation (needs cluster access) |
...
**Implementation note (this MR).** M1–M3 landed. M2 chose: (a) `/data` clear performed in the seed init container via a new `/data-seed` mount (keeps the heal atomic and M2/M3 scopes disjoint); (b) compose parity = documented `down -v` after a toolchain-changing roll + the M3 boot preflight as the loud backstop (no compose reseed machinery). The reseed clears the SHARED provisioning state only — `agent-home/.local/{share/devbox,state/nix,share/nix}` and `/data/provision` — never per-run SDK homes. A FUTURE-COUPLING comment on the `/data-seed` mount ties its best-effort clear to the PRD #51 M4 runner-uid split when that lands on k8s. M4's live-heal verification is deferred (no cluster access in the worktree); the fix is build-verifiable (fail-closed per-tool `command -v` assertion) and unit-verifiable (`render_test.go` fresh/match/mismatch/legacy/data-clear cases + preflight tests).
