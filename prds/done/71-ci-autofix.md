...
| An ungated autopilot is a known failure mode | The plan gate still holds for CI-config fixes; code fixes are bounded by the cap + opt-in, not by nothing. Scope is narrowed to agent-owned MR branches, never `main`. |
...
- **2026-08-10 (implementation landed — M1–M7 on branch `agent/issue-71`)**: shipped as specified, with these design notes worth recording. (1) **CIConfigPaths producer = Option B** (architect decision): the protected set (`CI_AUTOFIX_CONFIG_PATHS` defaults ∪ the project's fetched `ci_config_path`) is computed at run-creation via `workersvc.MergeCIConfigPaths` and PERSISTED on a new `runs.ci_config_paths` column (migration `00115`), then read into `ClaimConfig.CIConfigPaths` at claim assembly — chosen over a claim-time forge fetch because it is fail-closed on the auto path and keeps `assembleClaim` forge-free and the snapshot byte-identical. (2) **Requeue-while-parked hole closed** (found by adversarial review of M5): an auto ci_fix run whose CI-config plan parked but was re-queued by a worker restart before human approval resumed as `plan_approved=true` (from `run.auto_approve`) and skipped the gate. Fix: `SetRunAwaitingApproval` now clears `auto_approve = false` when a run parks — symmetric with the existing seeded `plan_source='agent'` decouple — so a parked auto run requires genuine human approval on resume; the worker's `ciFixHumanApproved` derives from `auto_approve !== true`. (3) **`halt_notified` is reset on every proceed** so the no-progress and cap halts each get their own truthful comment (a single shared latch conflated them). (4) Migration numbers as landed: `00114` (user toggle), `00115` (`runs.ci_config_paths`), `00116` (`ci_autofix_attempts`). (5) Stale PRD references corrected in implementation: the entrypoint is `cmd/server/main.go` (not `cmd/api/main.go`), and notifications are **in-app (`notifysvc`) + backing-issue comment** for start/halt/landed — **Slack is deliberately not wired** for these events (the run-failure notifier already DMs the last attempt's failure). (6) The M-b active-run swallow caps `last_pipeline_id` to `min(pipeline_id, active-run target)` via `GetActiveCIFixTargetForRef`. Validation: full `task gate:api` / `task gate:agent` / `task gate:web` green; LiveDB `TestCIAutofixLiveDB` + `TestSetRunAwaitingApprovalClearsAutoApproveLiveDB` pass on postgres:17; the four `main`-never-touched guardrail layers pass for auto ci_fix runs (never-main candidate selection proven by the LiveDB candidate test). Docs: `docs/ci-autofix.md`, `docs/configuration.md`, `ARCHITECTURE.md`, `CHANGELOG.md`; specs: `specs/ai.md` §510–517, `specs/human.md` Feature #71.
