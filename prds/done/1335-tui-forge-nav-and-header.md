...
- [x] **M1 — Nav regression tests, written first and shown failing on the unfixed key handling.** In `api/cmd/uzi/tui_pr_test.go` and `tui_cirun_test.go`, beside the existing `esc`-back tests, drive the model with a `keyLeft` press:
