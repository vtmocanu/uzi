...
- **Status**: Implemented (M1–M3 complete). api-only: `ListMRWatchCandidates` Lane B + tests + `specs/ai.md` §90.
...
- [x] **M1 — Add the closed-issue terminal-recording lane.** In `ListMRWatchCandidates`: rewrite the WHERE as Lane A (unchanged) OR Lane B (`i.state='closed' AND mr_state IS NULL OR IN ('opened','locked')`); add `r.created_at` to the `latest` CTE; add `ORDER BY (i.state='opened') DESC, l.created_at DESC` and `LIMIT 100`; update the query doc comment. Update the two stale cross-references (`forge.sql:537-543`, `prd_link_patch_query_test.go:17-19`). Run `sqlc generate` and commit the regenerated `forge.sql.go`. Add the clarifying comment in `mr_watch.go`. Verify `TestMRStateIsWatcherOwned` and `TestPRDLinkPatchCandidatesReadsNoIssueState` still pass (no new writer, no signature change).
- [x] **M2 — Tests: candidate selection (incl. the existing LiveDB test) + move-free recording.** Done; coverage was expanded beyond the original sketch (see divergence note below).
  1. **Rewrote `TestListMRWatchCandidatesLiveDB`** (`api/internal/store/mr_watch_integration_test.go`) — fixture **104** (closed issue, completed run, MR 204, `mr_state=NULL`) moved out of `absent` into the present set. **DIVERGENCE from the original plan:** rather than flip only 104 (set `{101,104,106}`, count 3), the branch adds four more Lane B fixtures for explicit branch coverage — **108** (closed, `mr_state='opened'` → candidate, pins `IN ('opened')`), **109** (closed, `mr_state='locked'` → candidate, pins `IN ('locked')`), **110** (closed, `mr_state='merged'` → absent, decay bound) and **111** (closed, `mr_state='closed'` → absent, decay bound). Final **present set `{101,104,106,108,109}`, count 5**; `absent` = `{102,103,105,107,110,111}`. Fixture 105's comment strengthened as the explicit "Lane A unchanged / Lane B closed-only" guard. Candidate fact-checks added for 104/108/109 (mr_iid + mr_state). Header doc comment updated to describe Lane B. Self-skips without `UZI_TEST_DATABASE_URL`; run is CI's job (`test:api-store-it`).
  2. **Watcher unit tests** (`api/internal/forgesvc/mr_watch_test.go`): `TestMRClosedIssueMergedRecordsNeverMoves`, `TestMRClosedIssueMergedBootstrapRecords`, `TestMRClosedIssueClosedRecordsNeverMoves` — a completed run on a **closed** issue records the terminal `mr_state` (`merged`/`closed`) and **moves no card** (`assertNoMove`), via both the `default` (merged) arm and the NULL bootstrap arm. All PASS.
  3. **Multi-tick backfill/decay** (`TestMRClosedIssueBackfillDecayIsIdempotent`): tick 1 backfills `merged` (one write, one forge read); tick 2 with `stored='merged'` is a no-op (observed==stored → idempotent). PASS.
  4. Non-vacuity proven by mutation of `mr_watch.go` in a detached throwaway worktree (never the shared tree), restored from a `cp` backup: removing `recordMRState` in the `default` arm reddens `TestMRClosedIssueMergedRecordsNeverMoves`; removing it in the bootstrap arm reddens the other three (`expected exactly one mr_state write, got 0`). The LiveDB query test cannot be folded offline (no Postgres) — its non-vacuity is CI-verified (`test:api-store-it`); the offline procedure a maintainer with a DB would use is folding the generated const in `forge.sql.go`, not the `.sql`.
- [x] **M3 — Gate green + spec note.** `task gate:api` passes (fmt-check, vet, build, lint, deadcode, test with `-race -count=1`). Added a `specs/ai.md` entry (§90, the `ListMRWatchCandidates` bullet list) describing the closed-issue terminal-recording lane, the move-free guarantee, the backfill/decay behaviour, and the `LIMIT 100` rationale — including the accepted R4 caveat that the literal `LIMIT` now bounds the total returned set (Lane A included), previously unbounded. The rewritten `TestListMRWatchCandidatesLiveDB` needs Postgres (not in the worker's gate); it is compile-verified here and its run is CI's job (`test:api-store-it`) — a maintainer with a DB confirms it with a positive control (named test `--- PASS`, `RUN > 0`, zero `SKIP`).

---

## Success criteria

1. **Discriminating fixture (offline, gate-adjacent):** in `TestListMRWatchCandidatesLiveDB`, a completed run on a **closed** issue with `mr_state=NULL` and MR present is now a candidate. The fixtures were strengthened beyond the original sketch to pin both Lane B branches and the decay bound: closed-issue runs with `mr_state` NULL (104), `'opened'` (108) and `'locked'` (109) are candidates, while terminal `'merged'` (110) and `'closed'` (111) are not — present set `{101,104,106,108,109}`, count 5. The watcher unit test shows one `SyncMRStates` tick records `mr_state='merged'` for a closed-issue merged MR with **no board-card move**. Falsifiable: on `main` today fixture 104 is asserted absent and the set is `{101,106}`.
...
3. **Backfill + decay:** historical merged runs record `merged` over subsequent ticks and then stop being candidates; the `LIMIT 100` bounds forge reads, and the open-issues-first `ORDER BY` guarantees a closed-issue backfill burst never defers a Lane A candidate ahead of it. (Note: the `LIMIT` does cap the *total* returned set — see R4 — so it bounds Lane A too; accepted because 100 far exceeds any realistic Lane-A count.)
