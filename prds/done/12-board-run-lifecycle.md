...
- **DTO**: each card gains `latest_run: { id, status, mr_iid, failure_reason, owner_name, worker_name, created_at, updated_at } | null` (newest by `created_at`, via `DISTINCT ON (issue_iid) ... ORDER BY issue_iid, created_at DESC` rather than the `LEFT JOIN LATERAL` originally proposed here — sqlc v1.30 doesn't propagate LATERAL nullability, typing the joined run columns non-nullable and panicking on a no-run issue; DISTINCT ON rides the same composite index and returns only issues-that-ran, mapped onto cards Go-side, see Implementation notes). Runs are owner-scoped today (`ListRunsForUser`), so on multi-user boards the latest run may belong to someone else: `owner_name` is shown ("started by X") and the run-view link renders only for the owner (a non-owner clicking would 403 on `GetRunByIDForUser`).
