...
- [x] **M1: structural contract, schema and hard claim clause.** Add additive fields/tables and bounds; stamp `completion_contract_version=1` in `CreateRun` before the first claim when rollout is enabled; freeze `profile=structural` content with `milestones_frozen` on human approval and autopilot running reports. Add the unconditional ClaimRun compatibility clause outside both override paths. Surface a capable-worker wait. LiveDB tests must prove a fresh interlocked row is not claimable by an incapable worker before approval, and that owner override plus `capability_aware=false` cannot bypass it, while a legacy run remains claimable. Gates: `task gate:api`, `task gate:agent`, `task scan:secrets`.
  - Landed (this run): migration `00211_completion_contract.sql` (nullable `runs.completion_contract_version`/`contract_revision`/`completion_contract`; `workers.protocol_capabilities text[] NOT NULL DEFAULT '{}'`). Separate protocol vocabulary + `capability.FilterProtocol`, threaded through worker register (`agent`→`api`) into `workers.protocol_capabilities`. Non-bypassable `ClaimRun` clause (main WHERE + peer fleet-spread mirror), outside `fn_worker_can_claim`/`required_capabilities`/`ClearRunRequiredCapabilities`/`capability_aware`. Rollout switch `completion_interlock_rollout` (default + fail-safe OFF) stamps the version in `createRun`. Contract frozen (Go builder) at the two existing freeze sites (`CreateApprovePlanInput`, `SetRunRunning`), idempotently and tied to the milestone-source freeze. LiveDB + unit tests added; `task gate:api`, `task gate:agent`, `task scan:secrets` green.
  - Landed (this run, with M5): **capable-worker wait surfaced** — a new `queuedReason` rung (`api/internal/workersvc/health.go`, `CountOnlineWorkersSatisfyingProtocol` over `workers.protocol_capabilities`) returns `reasonNoCompletionCapableWorker` when an interlocked queued run has no online worker carrying `completion_interlock_v1`; a legacy run never triggers it. This closed M1's only open item, so M1 is now complete.
- [x] **M2: attempts, permit route and completion transaction.** Implement claim-fenced idempotent permit requests, exact head/revision identity, bounded denial summaries and the extensible optional audit/finding fields. Implement transactional permit consumption plus completed state, with a seam for later generation activation. LiveDB tests cover duplicate request, stale claim, wrong head/revision, decision race, retry after response loss and legacy passthrough. Gates: `task gate:api`, relevant LiveDB tests, `task scan:secrets`.
  - Landed (this run): migration `00212_completion_attempts_permits.sql` (`runs.completion_attempts int NOT NULL DEFAULT 0` + `runs.latest_completion_attempt jsonb`; `run_completion_attempts` bounded log; `run_completion_permits` with `UNIQUE (run_id, contract_revision, head)` idempotency key, `issued_by_worker_id` claim fence, NULL `audit` / `'{}'` `finding_ids` reserved for #1231). sqlc queries `RecordCompletionAttempt` (single-statement insert+increment+summary+prune-to-50 CTE), `UpsertCompletionPermit` (idempotent `ON CONFLICT DO UPDATE` returning the same permit), `GetUnconsumedCompletionPermit`/`GetConsumedCompletionPermit`/`ConsumeCompletionPermit`/`GetRunOwnedByWorkerForUpdate`, regenerated + threaded into the hand-written `Store` interface. Service `completion_permit.go`: server-authoritative `computeUnmetCriteria` (fail-closed for the contract-NULL split-state), `RequestCompletionPermit` (denial taxonomy `stale_claim`/`not_interlocked`/`revision_drift`/`contract_not_frozen`/`missing_milestones`), `RecordCompletionAttempt` nudge endpoint, and `completeRunWithPermit` — a real pgx transaction (`SELECT run FOR UPDATE` → consume permit → `SetRunCompleted`), idempotent on retry-after-response-loss, non-terminal without a matching permit, with the documented `#1229/#1214` generation-activation seam before commit. `SetState` `case "completed"` routes interlocked runs through it (legacy path byte-for-byte unchanged); `StateRequest.Head` added; `SetTxBeginner(pool)` wired in `main.go`. Worker routes `POST /worker/runs/{id}/completion/{permit,attempt}`. Fake-store + LiveDB tests (all listed cases + attempt-log bound + split-state + gated-completion-without-permit); mutation self-check confirmed the "stays non-terminal" LiveDB assertion reddens when the permit requirement is neutered. `task gate:api` and `task scan:secrets` green.
- [x] **M3: same-lead attempt loop and stall/budget routing.** Turn gated `signal_done` into a checkpoint-first structural attempt before `turn.done` breaks. Feed missing IDs back through the existing follow-up path. Add the distinct completion fingerprint; route `REASON_NO_PROGRESS`, `REASON_MAX_ITERATIONS`, `REASON_WALL`, `REASON_IDLE` and the served post-attempt `budget_exhausted` flag to `enterCompletionHold(reason)`; exempt live post-attempt rows from `SweepRunningTimeout`; keep pre-attempt legacy failures unchanged. Tests prove decreasing unmet sets continue, three identical attempts hold, and a live post-attempt row past its wall is not server-failed. Mutating away the sweeper carve-out must redden that LiveDB assertion. Gate: `task gate:agent`, `task gate:api`.
  - Landed (this run): worker-only claim signal `completion_contract_version` on `ClaimConfig` (`api/internal/workersvc/claim.go`/`claim_assembly.go` + agent `protocol.ts`; NOT the web RunDTO, so no api-contract fixture change). Extended the attempt endpoint to **persist the declaration**: `RecordCompletionAttempt` union-merges the lead's `milestones_completed` (subset-validated by `progressParams`, unioned via the verbatim `SetRunRunning` jsonb CASE inside the single `RecordCompletionAttempt` statement) and recomputes unmet over the merged set in one call — removing the M2-review "persist THEN attempt" ordering hazard. Sweeper carve-out: `SweepRunningTimeout` now EXCLUDES `completion_attempts > 0 AND a live worker heartbeat` (same `staleCutoff` the requeue path uses), leaving stale-worker post-attempt rows to `RequeueRunsOfStaleWorkers`; sqlc regenerated (no new `SweepResult` counter). Agent client `recordCompletionAttempt`; executor RunContext `completionInterlock` / `recordCompletionAttempt` / `enterCompletionHold` (+ `ExecutorResult.completionHeld`) wired in `runner.ts`. Executor `turn.done` runs the checkpoint-first attempt loop: checkpoint (reap) → attempt → empty unmet finalizes; non-empty re-prompts the SAME session via the mid-loop follow-up injection (no reset, no forge call); the completion fingerprint `(sorted unmet, head, worktree)` shares `STALL_LIMIT=3` and routes 3 identical attempts to the hold. Post-attempt `MAX_ITERATIONS`/`WALL`/`IDLE`/`NO_PROGRESS` route to `enterCompletionHold`; pre-attempt behavior unchanged. **`enterCompletionHold` is deliberately LEFT UNWIRED in `runner.ts` for M4** (SetRunCompletionHold + captureHoldContext + park order); the executor falls back to the legacy throw when it is absent. Agent executor unit tests (`agent/test/runner-completion-attempt.test.ts`, all seven cases a–g) + LiveDB tests (attempt union-merge; sweeper carve-out with the not-failed mutation reddening) added; `task gate:agent` and `task gate:api` green.
  - Still open (M4/M5): the served post-attempt `budget_exhausted` running-ack flag (D3) is NOT yet wired — the worker-side wall/idle/max-iter/no-progress routing lands, but the SERVER's one-shot `budget_exhausted` acknowledgement that lets a live worker enter the hold on the server's steer is deferred with the M4 hold writer. `enterCompletionHold` itself is the M4 seam (see above).
- [x] **M4: final-head publication and recoverable hold.** Integrate the permit after today's scan/alignment result, keep security failures typed, add final PR-head reads for all three forges, and make `Closes` conditional on the permit. Add the dedicated completion-hold writer and fixed capture hook order. A failed/unverified Git capture keeps the worker/session live. Tests cover alignment head change, PR-head race, worker death around permit/PR, refused hold ACK and no cleanup before positive capture. Gates: `task gate:api`, `task gate:agent`, relevant LiveDB tests, `task scan:secrets`.
  - Landed (this run): the three must-fix permit-path fixes first (`UpsertCompletionPermit` rebinds `issued_by_worker_id` on the A→B requeue conflict; `RequestCompletionPermit` normalizes head+branch identically to the consume side + a new `empty_head` denial; `GetUnconsumed/GetConsumedCompletionPermit` are branch-bound and the branch is taken from the worker-reported `StateRequest.Branch`, never the NULL-at-lookup `run.Branch`), each with a fail-first LiveDB regression. Then migration `00215_completion_hold.sql` (`runs.hold_reason`/`hold_captured_head`/`completion_budget_exhausted_at`) + `SetRunCompletionHold` (sibling of `SetRunPaused`; `running|awaiting_input`→`paused`, `hold_reason='completion_blocked'`, guarded on `completion_attempts>0`) + its `POST /worker/runs/{id}/completion/hold` endpoint; the served `budget_exhausted` steer (`StampCompletionBudgetExhausted` = the sweeper carve-out's complement, `RunDTO.completion_budget_exhausted`, `SweepResult` counter in the sweeper guard+emit). Agent: `forge.getMergeRequestHead` on the interface + all three drivers (GitLab prefers `diff_refs.head_sha`); `client.requestCompletionPermit`/`requestCompletionHold`; `phasePublish` captures the exact landed head `H` (post-align `trackingTip`), requests the permit, creates/adopts the MR only when granted with `Closes` gated on it, reads + verifies the PR head == `H` (mismatch/denied → hold), and reports `head: H` on completion; `enterCompletionHold`+`captureHoldContext` implement the D6 park order (clone/HOME retained on every uncertain path, cleared only when kept-live-terminal). `gate:api`/`gate:agent`/`scan:secrets` + LiveDB green; each unit reviewer+tester (+auditor on the permit fixes), mutation-verified.
- [x] **M5: owner continue, web and CLI.** Add the completion question discriminator, claim-delivered `completion_hold_window_seconds` (default 900), exemption from the normal question-timeout failure, `completion_decision=continue`, resume semantics and guidance injection. Expiry enters the verified hold and never reports failed. Render interlock/attempt/hold/capability/context states in web and CLI JSON; add `uzi run decide <id> --continue [--guidance]`. Carry hold state through resume and clear it only when the decision is acted on, not prematurely in `ResumePausedRun`. Gates: `task gate:web`, `task gate:api`, `task gate:agent`.
  - Landed (this run): `completion_decision` run-input kind (migrations `00216`/`00217`, two-step CHECK) + the dedicated `POST /runs/{id}/completion/decision` owner endpoint (`{decision:"continue", guidance?}`; `awaiting_input` live window → delivers an `answer`-kind input resolving the run's `open_question_id`; `paused` hold → `ResumePausedRun`→`queued`+guidance follow-up; the D3 steer clears here; the hold clears on the first `SetRunRunning`, not in `ResumePausedRun`). Honest-state `RunDTO` (`completion_interlock`/`completion_attempts`/`completion_unmet`/`hold_reason`/`hold_context`=`unavailable(same_worker_only)`/server-computed `completion_phase`) + api-contract fixtures; claim `completion_hold_window_seconds` (default 900). The completion-question marker (migration `00218` `runs.completion_question_at`) so the endpoint/phase key on a real discriminator, not the interlock+attempts proxy. Agent: `askCompletionQuestion` — the 900s live window at the completion-stall point (awaiting_input + `completion_question:true`, races `steering.awaitAnswer` against the window timer, expiry→verified hold, never `REASON_QUESTION_TIMEOUT`; continue→resume the same session with guidance). Web: three derived pseudo-statuses via `effectiveRunStatus`/`RUN_STATUS_TONES`/`RUN_STATUS_LABELS` (blocked-over-paused precedence) + a RunView completion panel (unmet ids/attempts/hold_context via `stripUnsafeChars`). CLI: `uzi run decide` + `renderRunDetail` completion rows via `sanitizeTTY`. Capable-worker wait (M1 leftover) landed alongside. `gate:web`/`gate:api`/`gate:agent` + LiveDB green; each unit reviewed (one blocking review-finding — SetRunRunning wrongly cleared the served steer — fixed in `a32d3264` with a mutation-verified regression).
- [x] **M6: docs, specifications and adversarial regression.** Add ADR 1225 for the structural contract, hard capability clause, permit, exact head, same-lead rejection and hold ordering. Update run lifecycle/autopilot/pause/CLI docs and run `task docs:sync`. Add the user-approved structural requirement to `specs/human.md`: "An issue run may open a closing PR only after every in-scope approved milestone is declared complete against a frozen contract for the exact final head, or the owner records an explicit later decision. An incomplete attempt returns to the same lead and otherwise holds without discarding its work. [user, #1226]" Add technical requirements to `specs/ai.md`. Reproduce run `b0ffd0ea`: frozen five, declared four, no scope cap, 17 iterations remaining. The fixed path checkpoints then names M5 to the same session and makes zero forge-create calls; deleting the missing-milestone denial makes the mutation call create with `Closes`. Run relevant component gates once to logs.
  - Landed (this run): `adr/1225-structural-completion-interlock-invariants.md` (the negative-space invariants, modeled on `adr/1190`) linked from `ARCHITECTURE.md`; `docs/run-completion-hold.md` (new, `audience:user`) + cross-refs in `docs/run-pause.md`/`docs/autopilot.md` + `docs/cli.md` (`uzi run decide` + completion states), stating the interlock is rollout-gated (`completion_interlock_rollout`, off by default) and the context limitation is honestly `same_worker_only` (no cross-worker durability claim); `task docs:sync` mirrored into `api/internal/uzidocs/embed/` (`TestEmbeddedDocsMatchSource` green). `specs/human.md` Feature #1226 with the verbatim `[user, #1226]` requirement; `specs/ai.md` §633 technical contract (cross-refs ADR-1225, migrations 00211, 00212, 00215-00218). The adversarial #1220 regression (`agent/test/runner-completion-1220-regression.test.ts`): reproduces `b0ffd0ea` (5 frozen, 4 declared, no scope cap) and asserts checkpoint-first + same-session rework naming the missing milestone + ZERO `createMergeRequest` calls; mutation-verified (neutering the missing-milestone gate reddens it). Docs fact-checked against the landed code. Component gates run to logs.
