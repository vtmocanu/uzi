...
**Completion note**: All milestones landed on branch `agent/issue-362`. Notable
deltas from the plan, recorded per convention: M4 needed no api change (the list
DTO already carries the summaries via the embedded `RunDTO` populated by `runToDTO`);
M2 wired the per-user override end-to-end (`user_settings.go` GET/PUT + the Run
Defaults page), not just the queries, so the admin "user can override" copy is true;
M3c added a `summary_intent_present` claim signal to implement the skip-if-set rule;
M3b's detector was de-nested + input-capped to eliminate a ReDoS on the untrusted
issue description. Timeout default `SUMMARY_MODEL_TIMEOUT_MS`=60s.
**Priority**: Medium
**Author**: Vlad Mocanu
**Created**: 2026-08-18
...
- [x] **M1 — Data model, DTO, persist endpoint, live-update.**
