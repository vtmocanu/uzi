...
- [x] **M0 — Migration.** Add `customized BOOLEAN NOT NULL DEFAULT false`; backfill builtins from `updated_at > created_at`. (Goose number assigned at merge, above the live head.) — `api/internal/store/migrations/00111_agent_template_customized.sql` (provisional `00111`; renumber above the live head at merge).
