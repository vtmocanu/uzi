...
bottega stores tokens plaintext-equivalent (host ambient creds); dot-agent-deck delegates to ambient credentials. Neither does password-wrapped envelope encryption. Prior art is Bitwarden/1Password key hierarchy (master-password-derived KEK → DEK → data), which this follows.
