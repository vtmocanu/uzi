...
- **Issue**: #1251
- **Status**: Complete — all milestones (M1a–M1c, M2, M3, M4) landed on `agent/issue-1251`.
- **Priority**: Medium
...
- [x] **M1a — Update-prompt render (real `tuiModel`, no sketch).** Modal overlay: box via `provenanceBox`, three choices with the `▸`/tungsten/`selBg` selection convention, latest in `sage`, security variant as the amber band. Deterministic View/Update assertions per state (normal / security-band / non-brew / selected-row) + the colorprofile-Ascii text fallback. Landed in `api/cmd/uzi/tui_update_prompt.go` (`renderUpdatePrompt`) with `case m.updatePrompt.showing:` in `View()`; tests in `tui_update_prompt_test.go`.
- [x] **M1b — Detection, gating, dismissal.** Widened `buildInfoMsg`/`fetchBuildInfoCmd` to carry `Latest`; gate on `skewCheck`/`showVersion` (folds `IsStampedVersion` + `UZI_VERSION_CHECK`) + `Latest` present + not-prerelease + behind + not-dismissed; per-version dismissal persisted in the local store (`versioncheck.go` `DismissedUpdateTag`/`RecordDismissedUpdate`). `Latest.Version`/`Name`/`NotesURL` sanitized (D7, hostile-render test). NOTE: the CLI axis is recomputed with `uzicli.CompareServerVersion` (`cmp < 0` = behind), NOT `releasecheck.UpdateAvailable` — the anchor's suggestion, because `releasecheck` imports `internal/store` (the pgx server stack) and `cmd/uzi` must not depend on it (`TestNoServerDeps`); `CompareServerVersion` is the CLI-side twin with the same normSemver+IsValid+Compare discipline.
- [x] **M1c — Brew detection + foreground-exit upgrade.** Detect brew and run `brew upgrade uzi-cli` both through a func-typed `Env.Brew` seam (exemplar `Env.Git`); "Update now" hands off on foreground exit via `runPendingUpgrade` after `p.Run()`; non-brew info variant with the release-notes link. Tests assert the constructed argv (`["upgrade","uzi-cli"]`) and `foreground=true` via the seam without running brew.
- [x] **M2 — Vault detection + tier-1 hint (foundational, backend-free).** Decode `vault.unlocked` from `GET /api/auth/me` via a NEW additive `Client.WhoamiVault` (leaving `Whoami`/`uzi whoami` `{user}`-only), thread `locked` (and the viewer identity for M3) into the model on the strip cadence (Init / 60s strip tick / `r`, so it auto-clears on unlock, D3); render the quiet faint `🔒 vault locked` hint on its OWN line beside `boardRateLimitStrip` (D5), with an ascii-safe `[locked] vault locked` form under `colorprofile.Ascii` (D4); reserve its row in `boardCapacity`. `vault.unlocked` decoded as a `*bool` so an absent/pre-vault server reads unlocked (no spurious lock). Landed in `api/internal/uzicli/client_account.go` (`WhoamiVault`) + `fake_account.go`; model wiring + `vaultIndicatorLine()` in `api/cmd/uzi/tui.go` / `tui_board.go`. Tests: `WhoamiVault` decode (`client_test.go`), View/capacity/ascii/coexist-with-strip (`tui_vault_test.go`).
- [x] **M3 — Tier-2 escalation band (builds on M2, optional).** When locked and ≥1 of the viewer's OWN runs is parked on the vault (best-effort lowercased `"vault is locked"` substring on `HealthReason`, regression-pinned to the exact server string), the tier-1 hint escalates to the steady amber needs-you band on the SAME one optional line (mutually exclusive with the hint, so `boardCapacity`'s single-row reservation is unchanged), leaving `boardRateLimitStrip` on its own line (D5). Count scoped by `OwnerEmail` on the admin board and suppressed when `selfEmail` is unknown (D11); degrades to the M2 tier-1 hint when run-health is off or the count is 0 (R6). The band shows a COUNT, never the raw reason, so no untrusted text is drawn; the hostile-render test proves the count path cannot inject and `HealthReason` is already in `d7UntrustedFields`. Landed in `api/cmd/uzi/tui_board.go` (`ownParkedOnVaultCount` + escalated `vaultIndicatorLine` + `vaultBand`, reusing the `attentionBanner` fill+ink convention); ascii-safe band under `colorprofile.Ascii` (D4). Tests in `api/cmd/uzi/tui_vault_band_test.go` (escalation, R6 fallback, admin scoping/D11, count correctness, D5 coexistence, capacity, ascii) and the extended hostile-render case in `tui_model_test.go`.
- [x] **M4 — Docs + CLI parity + finalize invariants.** Documented the startup update prompt (new `### Startup: an available update` subsection) and the vault-locked indicator (Board chrome) in `docs/cli.md`, plus a `docs/updates.md` note that the TUI prompt is the one client-side CLI-vs-latest-release axis; regenerated the `api/internal/uzidocs/embed/` mirror with `task docs:sync` (`TestEmbeddedDocsMatchSource` green). CLI parity confirmed: no other `api/cmd/uzi/` command needed a change (`uzi version` already prints the update row; `uzi whoami` left `{user}`-only) — and the shipped band/hint copy was corrected to drop the non-existent `uzi vault unlock` command (vault unlock is web/API-only, `POST /api/vault/unlock`), now "unlock in the web app to resume". Finalize invariants verified: `git diff --name-only <base>..HEAD` shows zero `.github/workflows/**`, and the sketch registry is unchanged (only `template`, D8).
