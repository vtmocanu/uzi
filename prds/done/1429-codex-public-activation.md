...
- 2026-09-19: M1-M5 landed and reviewed clean, all their named gates green (`gate:api`, `gate:web`, `gate:agent`, `test:codex-m3b`, `run-store-it.sh` incl. Bearer LiveDB). M6's code also landed and its offline gates pass: the harness-neutral stub seam (`agent/src/main.ts` `buildRunExecutor`, stub short-circuits the codex branch), the offline Codex activation phase (`e2e/phases/74-codex-activation.sh`, hardened with the stub self-guard + settled cancel), the regenerated `e2e/README.md` phase registry, and `gate:agent`/`test:codex-m3b`/`gate:repo` (incl. `check:e2e-registry-doc`/`lint:shell`/`scan:secrets`/`check:token-literals`/`sast:semgrep`) all green. **M6 is left unchecked** because its gate list includes a green `./e2e/run-e2e.sh`, which is NOT obtainable in this sandbox: `run-e2e.sh` fails at the Docker image BUILD step, because an isolated build has no network egress — the `api` image's `go build ./cmd/server` (`api/Dockerfile:53`) exits 1 fetching Go modules, and the agent image's `agent/codex/install-codex.sh` network fetch is a second such blocker. The joined offline e2e PROOF is therefore deferred, not skipped.
- 2026-09-19: **Pre-merge exit criterion for M6 / parent #1106 M5B.** Before merge, a maintainer-local (or nightly `e2e.yml`, which has network egress) green `./e2e/run-e2e.sh` — exercising the new phase 74 — is required. M6's checkbox and the parent #1106 M5B "complete" marking are to be set only after that green run is observed. This PRD stays in `prds/` (not `prds/done/`) until then, and parent #1106 is not touched by this run.
- 2026-09-19: **Review fixes on the MR (PR #1449).** Two model pickers keyed their vocabulary on the raw harness pick instead of the effective harness, so an "inherit" pick that resolves to Codex (a Codex-only owner, or a usable Codex default) was offered Claude aliases; the API accepted the pick and D6 then dropped it at claim. The schedule modal (found by the review bot) and the Run Defaults "Worker model" picker (found by a follow-up review of the same pattern) now both follow `effectiveHarnessIsCodex`, the helper the adjacent Anthropic token pickers already used. Each has a regression test for the Codex-only owner, observed failing on the old expression and passing on the new one. The judge-model picker has the same shape but is a design question (one judge model serves runs on either harness) and is tracked separately in #1454, not changed here.
- 2026-09-19: **M6 exit criterion met: joined e2e green on the MR head.** A maintainer-local `./e2e/run-e2e.sh` (stub executor) on head `4706607a` ended `All E2E checks passed`, 53 phases PASS and 0 FAIL. Phase 74 `codex-activation` passed all 15 of its assertions: explicit-unavailable 422 with no run created; a runtime-assembled dummy `openai_api_key` saved through the boot-unlocked vault; an explicitly Codex-selected run observed `harness=codex` on the wire, reached the plan gate, and completed through the harness-neutral stub seam with a branch and MR while staying Codex; `cost_status=unreported` with tokens folded and never a metered $0; the run bound `codex_secret_id` with no `anthropic_secret_id` and no `run_credential_epochs` row; the Claude control run completed metered with no Codex binding (no fallback or cross-contamination either way); a schedule pinned to Codex fired a Codex run; a Codex-only user's plain run resolved to Codex implicitly (D11 rule 3); secrets ended exactly as found; and the dummy key appeared in no container log and nowhere on the worker's `/data`. An earlier attempt on head `f2ecdb38` already had phase 74 passing, but the harness as a whole was red on two phases inherited from `main` (`api-outage-readoption`, `api-outage-outbox`); those were fixed on `main` in PR #1456 and this run is the repeat after merging `main`. The three `LEAK` rows in the summary are leftover runs the driver cancelled during cleanup, the same ones `main` shows, and are not phase failures. M6 is ticked, this PRD moves to `prds/done/`, and parent #1106 M5B is marked complete; parent M6 (chart egress, fleet roll, live dev-cluster run and the real API-key repeat) and M7 remain open.
