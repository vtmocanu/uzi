...
|---|---|---|---|
| Skills storage | Nothing | Repo-local `.claude/skills` for its own tooling only — not a product feature | DB `skills` with builtin/global/user scopes + allocation overlay; builtins Go-embedded from `skilltmpl/builtins/` with parse/validity tests (PRD #17's decoupled convention) |
| Delivery to agent | N/A | N/A | Synthesized **local plugin dir outside the clone** + explicit `skills` enable-list — works with `settingSources: []`, so the injection defense never loosens; a native filesystem-discovery delivery mechanism would have forced weakening it |
| Repo-borne skills | N/A | Its whole model — but it's a dev-tool repo, trusted by definition | Explicit per-repo opt-in, default off, skills-only (hooks/settings/commands from the repo never load) |
| Per-agent allocation | N/A | N/A | DB many-to-many + per-user overlay (uzi runs belong to users with private skills, so allocation splits shared from per-user) |
...
- 2026-07-05 (AI, verified against installed SDK): delivery via local plugin dir + explicit `skills` enable-list + `AgentDefinition.skills` — the only channel that works under `settingSources: []`; a native-discovery materialization approach was rejected because it would require loosening that isolation.
