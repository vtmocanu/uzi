...
**GitHub Issue**: [#104](https://github.com/vtmocanu/uzi/issues/104)
**Status**: **COMPLETE** (created 2026-07-21; completed 2026-07-21) — merged on the former GitLab instance as MR !89, 21 commits from `b94c45b4`. All seven milestones landed; every code milestone reviewed **and** security-audited; full `run-e2e.sh` green including the new binding phase. Also folds in [#105](https://github.com/vtmocanu/uzi/issues/105).
**Priority**: Medium
**Related**: [#53](https://github.com/vtmocanu/uzi/issues/53) (rate limits — this PRD repoints its table), [#32](https://github.com/vtmocanu/uzi/issues/32) (user vault — whose rewrap path this PRD must fix before it can store a second token), [#50](https://github.com/vtmocanu/uzi/issues/50) (LLM egress proxy — must learn which token to inject), [#69](https://github.com/vtmocanu/uzi/issues/69) (completed judge mode + per-user model work; overlapping settings surface)
