...
`specs/ai.md` §482-484 appended; (3) the MR — posted, and two CVEs the new gates caught
in CI (Go 1.26.4→1.26.5, `x/net` v0.55.0→v0.56.0) fixed in the same MR before merge.
Issue #103 closed. **The original block is left verbatim below** because its warning —
6/6 ticked is not the same as shipped — is the reason this PRD did not report itself
done prematurely, and deleting it would erase that lesson.
