...
**Status**: Done (2026-09-25, branch `agent/issue-1645`); design direction approved by the maintainer on 2026-09-25 after a two-agent UX brainstorm
...
- Reset (`ResetDefaultSchedule` in `api/internal/store/schedules.sql.go`, behind `uzi schedule reset` and the web Reset action) restores cron, timezone, model, auto-approve, wait-on-limit, max issues and output mode to the catalog values, sets `override_subagent_model = false`, **clears** MR rework (to inherit: the handler passes no value), guidance, the harness pin and the credential override, clears `customized`, and sets `status = 'active'` with the next fire recomputed from the catalog cadence (the pause switch, `enabled`, is untouched). The sealed prompt and labels are not touched. The `schedule reset` help text in `api/cmd/uzi/schedule.go` and the embedded CLI skill describe only part of this (they omit guidance, harness, credential override and output mode); M5 corrects them.
...
- **2026-09-25, M7 screenshots are run evidence, not PR attachments (plan review).** The worker cannot attach images to a PR and image files must not be committed, so M7 is checked on worker-side evidence (captures taken, keyboard walk done, the 390px scrollWidth check passed); the landing session re-captures the listed views from the PR branch in mock mode and attaches them.
- **2026-09-25, enable guard in two layers.** The enable dialog marks a newly checked sweep repo `checking` in the same state update that selects it (so the first render already disables Enable), and also treats any repo without a `done` report as checking; `SweepLabelWarn.onCheckStateChange` reports `done` when the current check settles. Either layer alone holds Enable; tests pin the dialog holding Enable without the warn's help.
- **2026-09-25, Reset scope corrected.** Implementation verified that Reset clears MR rework to inherit (the handler passes no value, so there is no catalog MR-rework value) and re-activates a parked schedule (`status = 'active'`, next fire recomputed) without touching its pause switch. The CLI help, the embedded skill and both docs state this; *Verified current behavior* above is amended to match.
- **2026-09-25, implementation notes.** The grouped components were deleted with M3 rather than M4 (knip); `AddAnotherRepo` moved to its own file for the user-row menu. The partial-enable page error ("Enabled … on X of N repos; K failed.") never showed before this change because the following reload cleared it; it is now set after the reload. Browser acceptance found the More actions menu clipped off the left edge at 390px; it is now clamped to the viewport. Non-blocking browser notes left for follow-up: Pause all is pushed mostly out of view in the narrow tab row by the "K not enabled" pill; the enable dialog never flips upward; inline Reset on customized rows shifts Run now/Edit left relative to other rows; narrow-screen action targets are small (32x28px); dimmed paused-row text in the light theme is about 2.45:1; a paused/resumed row re-sorts without scrolling its focused switch into view; the tab row's horizontal scroller clips the tabs' focus ring; "Enable on another repo" from the Schedules tab leaves the user on the Job catalog after Cancel.
- **2026-09-25, landing browser check; screenshot attachment waived.** The landing session re-captured all twelve views (both tabs, light and dark, 1440px and 390px, the open More actions menu and the open enable dialog) from the PR branch in mock mode with headless Chrome. It found the enable dialog of a first-column card sliding under the desktop sidebar (its clamp used the viewport, not `<main>`) and fixed it with a regression test. No horizontal scroll at 390px on either tab. The maintainer waived attaching the images to the PR.
