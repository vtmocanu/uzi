...
paragraph was wrong): **no auto-retry on provider limits anywhere**, which is the
load-bearing half and still verified — bottega retries once on a stale-OAuth 401
(`retryOn401.ts`), dot-agent-deck fails on any non-200, and neither retries on a
provider quota/rate limit specifically. Prior art does, however, document a
usage-limit classification pattern elsewhere (a hard quota-exhausted reason kept
distinct from a soft capacity/rate-limit reason, matched via structured failure
reasons rather than free text), giving Decision 8's "opt-out runs still get a
better death" a real precedent. **Deliberate design choice here: matching error
TEXT is fragile — we pin to structured SDK signals instead (Decision 1), which does
not rot when a provider rewords a message.** PRD #42 Decision 12 explicitly punted
with "the SDK's own retry/backoff is the handler" — correctly scoped to
...
8. **Opt-out runs still get a better death — and the SERVER composes the sentence
   ...
   **↳M0 — if the WORKER composes that string, this PRD ships a Success Criterion its
   ...
   error_during_execution`. (Prior art exists for this half — see the Prior-art
   paragraph.)
...
- 2026-07-27 — Staleness re-verification against HEAD `ca597779` (1343 commits since
  ...
  argument no longer covers PRD #108's auto-stop pass; the prior-art paragraph
  understated prior art on usage-limit classification, corrected in place. **Dropped as
