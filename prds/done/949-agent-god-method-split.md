...
- **D5 — `execute()` extracted in two commits** (`c6a25c72`, `2cee2d2d`), not one-phase-per-commit. Front half = `buildFlight` (P0 prologue) + `phaseClone` (P1) + `phaseResume` (P2) + `phasePreflightHandoff` (P3+P4 incl. the executor handoff), over an unexported `RunFlight` carrier declared before the `try`. Back half (P5-P7) = a single `phasePublish`. The carrier's real cross-seam set was derived by dataflow and is a superset of the illustrative list above (adds `runnerClone`, `result`, `observedSessionId`, `ciFixHumanApproved`); the `addSecret`/`removeSecret` pair and the `try`/`catch`/`finally` stay in `execute()` (D2).
- **D6 — the terminal-return contract was satisfied structurally, not by signal-threading, in `execute()`.** Because the P5-P7 back half is the *terminal* region of the `try` (nothing follows it before the `catch`), extracting it as one `phasePublish` method keeps every internal `return;` verbatim: a return from `phasePublish` reaches the end of `execute()`'s `try` and the `finally` runs exactly as before. No `return;` was converted to a boolean/signal, so the fall-through risk the epic flagged (a `not_code`/`report_only` run falling through into the push/MR stack) cannot arise. M1 added a characterization test pinning "a `ci_fix` `not_code` run performs NO push and NO MR" (`runner-push-mr.test.ts`, commit `e7451d15`) as a belt-and-braces guard.
- **D7 — `run()` extracted in one commit** (`530274f9`) into `phaseSetup` (P1-P4, before the `try`, returning the unexported `DriveState` carrier), `phasePlanGate` (P5), and `phaseRunLoop` (P6-P8). `run()` has exactly one early return in its `try` (the `not_code` result), so — unlike `execute()` — the signal pattern was used there and only there: `phasePlanGate` returns `ExecutorResult | undefined`, forwarded by `if (early !== undefined) return early;`. `phaseRunLoop` (no early returns) ends with the verbatim `return result`. `onSignal` is carried as the same reference (so `removeEventListener` still matches) and `baseOptions` (with `settingSources: []` and both PreToolUse hooks) is built once in `phaseSetup` and carried, never reconstructed.
- **D8 — M4 found ZERO vacuous negatives.** All ~80 negative-assertion lines in `prompt.test.ts` (the PRD's "75" undercounts overlapping forms) were positive-controlled by local, reverted builder mutations and shown to redden (directly or via a same-test sibling that throws first). No test edit was needed, so `prompt.test.ts` stays byte-identical. Three lines are non-vacuous-in-intent but un-positive-controllable *in isolation* because an earlier assertion in the same test/loop shadows them (`node:assert` throws on first failure): `prompt.test.ts:1937` (a lone-surrogate check the SAFE-class assert at :1928 always trips first — genuinely unreachable, guaranteed instead by the `clampToDirCharset` allowlist upstream), `:1452` and `:1809` (structural tautologies whose real property is live in siblings :1448/:1456/:1458 and :1807/:1808). Left as-is: the PRD's over-tighten risk (below) applies and each property is already covered. Verified independently by a fact-checker (zero refuted).
