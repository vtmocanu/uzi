...
bottega does not implement this either — it was audited and the audit fact-checked (2026-07-03, see decision log). dot-agent-deck was also checked: it is a TUI for monitoring agent sessions with no forge/label/kanban layer — N/A here.

| Concern | bottega does | uzi will do |
|---|---|---|
| Forge credentials | None — agents use the server's ambient `gh`/git identity (single shared identity, weakness) | **Per-user bot PAT**, encrypted at rest (AES-256-GCM via a `secretbox` utility), scoped to that user's connection |
| Forge abstraction | None — GitHub-only `gh` CLI shell-outs | **`Forge` interface from day one**; GitLab driver first, Forgejo later without touching callers |
| Repo registry | Local filesystem paths typed by user, no forge discovery | **Normalized `repos` table keyed by stable forge project id**, discovered via membership API, user enables per repo |
| Issue source | Local SQLite tasks; never fetches issues | **Forge is the source of truth**; uzi caches PRD-labeled issues, never forks them |
| Kanban columns | Local 4-value status enum (board renders 3 columns) | **Columns = forge labels** (GitLab board lists semantics), configurable per repo, synced two-way |
| Issue↔work mapping | Regex on branch name `task/{id}-` (brittle, weakness) | **Explicit FK rows** `(repo_id, forge_issue_iid)` — no text-convention parsing |
| Sync | Webhook for PR re-trigger only; board never syncs | **Polling with `updated_after` + periodic full reconcile + on-demand refresh** (laptop MVP can't receive webhooks); webhook support deferred, designed-for |
| Rate limits/caching | None | Honor `RateLimit-*`/429+`Retry-After`, per-connection poll budget, persisted issue cache |

Patterns copied: bottega's per-user credential isolation, membership authz, and refuse-to-start
secret guard (already in PRD #1). Secretbox encryption-at-rest and persisted-truth conflict
handling were reasoned independently against the same constraints.
...
- All calls go through one wrapper handling 429/`Retry-After`, `RateLimit-*` headers, timeouts (never an untimeouted `http.DefaultClient`), pagination, and **secret redaction: the PAT and `Authorization`/`PRIVATE-TOKEN` headers must never reach logs or error strings** (redaction unit test required, M5).
...
- **Incremental pull**: per enabled repo, poll `ListIssues(labels=PRD, state=all, updated_after=HWM)` on `FORGE_POLL_INTERVAL` (default `60s`). High-water-mark = max `updated_at` **returned by the forge** (never client clock — skew would drop updates); GitLab's `updated_after` is inclusive at second granularity, so boundary rows are re-fetched and deduped by upsert. **Verified 2026-07-03 against gitlab.example.com (bot PAT, E2E): GitLab throttles issue `updated_at` bumps to roughly one per ~60s window, independent of whether the change is a label add or a label remove.** Consequence: changes made at normal human cadence (spaced more than the throttle window apart — add OR remove) bump `updated_at` and are caught incrementally within one poll; multiple changes landing inside the same throttle window (e.g. rapid successive moves) share a single bump, so only the last is guaranteed incrementally and the earlier ones are caught by the reconcile tier. The default `FORGE_POLL_INTERVAL` (60s) is the same order of magnitude as the throttle window, so at normal editing cadence nearly everything is caught incrementally.
...
- **Conflicts**: last-writer-wins at the forge (its native semantics). If a poll shows the forge changed an issue uzi displays, forge state replaces cache (persisted-truth-over-event-claim).
- **Freshness contract**: content edits and label changes (add or remove) made at normal cadence appear within one poll interval; changes bunched inside GitLab's ~60s `updated_at` throttle window, plus de-labeling/close/reopen visibility gaps and deletions, are caught within one reconcile interval.
...
- Demonstrably ≥ bottega: real forge-issue kanban with two-way label sync (bottega has none), per-user revocable bot identity (bottega has shared ambient identity), and reconciliation-by-design sync (not webhook-only).
