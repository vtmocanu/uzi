...
3. Stub-line count in the six files drops; `task gate:api` green; docs updated (M3). **Actual (2026-09-01): the six migration commits remove 360 lines against 16 insertions (net −344)** (`git diff --shortstat 67b284b6..3a2a5b35`). The "roughly 3000-line order" figure in the draft was an overcount — it read the six files' *total* size (129+278+779+689+754+723 = 3352 lines, mostly real test logic) as if it were all stubs; the genuinely removable no-op-stub lines were ~360, and that is what dropped. The tax-reduction goal (criterion 1) is unaffected.
4. No `.github/workflows/**` in the branch diff.

## Outcome (2026-09-01)

All three milestones delivered; branch `agent/issue-922`. New package `api/internal/forge/forgetest` (`basefake.go` + `basefake_test.go`); the six fakes migrated one commit per package (poller ×2). Zero test bodies modified and **no `ErrNotStubbed` replication was needed** — every deleted no-op stub was genuinely uncalled, so behavior is preserved without an explicit stub anywhere (criterion 2 met with an empty replication list). `task gate:api` and `task deadcode:api` both green.

Acceptance experiment (criterion 1), run locally and reverted — throwaway `DummyAcceptanceProbe(ctx) error` added to `forge.Forge`:
- interface only → the `forge` package fails to build at its three `forge.New` driver returns (`*gitLab`/`*forgejo`/`*github` "missing method DummyAcceptanceProbe");
- + the three driver stubs → the sole remaining break is `internal/forge/forgetest/basefake.go:52` (the `var _ forge.Forge = (*BaseFake)(nil)` assertion) — the 4th and only other site;
- + the `BaseFake` stub → `go vet` over `forgetest` and all six migrated test packages exits **0** with **no `_test.go` file edited**.

So the tax is exactly the three drivers + `BaseFake` (four files), and none of the six test files, confirming criterion 1.
