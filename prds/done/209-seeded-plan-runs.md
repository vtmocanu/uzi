...
**Status**: **COMPLETE — all M1-M8 done 2026-08-04** (agent-team build; each milestone reviewer+auditor+tester validated, M5 also browser-validated, M6 mutation-proved). **M1 and M2 are the whole feature**; M3-M8 are surface and proof. **Open Question 1 remains a documented FOLLOW-UP** (not resolved by this PRD): `--agent-source repo` against a clone with no `.claude/agents/` silently runs with zero subagents rather than falling back to `own` — M7 documents the current behavior; the PRD's recommended worker-side "well-formed but unsatisfiable ⇒ fall back to own + note" arm has not landed.
