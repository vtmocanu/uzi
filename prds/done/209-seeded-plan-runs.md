...
**Issue**: [#209](https://github.com/vtmocanu/uzi/-/issues/209) · **Label**: PRD · **Priority**: Medium
...
**Status**: **COMPLETE — all M1-M8 done 2026-08-04** (agent-team build; each milestone reviewer+auditor+tester validated, M5 also browser-validated, M6 mutation-proved). **M1 and M2 are the whole feature**; M3-M8 are surface and proof. **Open Question 1 was RESOLVED by issue #231** (post-PRD follow-up): `--agent-source repo` against a clone with no `.claude/agents/` now falls back to `own` with a feed note, via the recommended worker-side "well-formed but unsatisfiable ⇒ fall back to own + note" arm in `resolveAgentSelection`'s `"ok"` case.
...
- [x] **M3 — CLI surface.** *(Done 2026-08-04, commit `08b4b195`, merged to 209-seed. `uzi run create --plan-file <path>` (`-` for stdin) + `--agent-source`/`--exclude-agents` reusing `approveSelection`'s validation; a `uzicli.CreateRunSeed{PlanMD, Selection}` mirrors the server's `SeededPlan` so a nil seed sends neither field (SC2 byte-identical, pinned by a raw-wire-body test). Usage errors fire before any HTTP call (roster flag without `--plan-file`; `--exclude-agents` without `--agent-source`; unreadable file). SKILL.md reference half updated, drift test green. `task gate:api` green; reviewer clean, tester mutation-proved all four usage/stdin/drift/SC2 tests live. **Noted (pre-existing, out of scope):** a 422 (plan too-large/empty) surfaces as exit 1 "generic" not a distinct user-input code, so an agent branching on exit codes can't separate a fixable oversized plan from a server fault — affects every 422 CLI-wide, a candidate follow-up rather than an M3 fix.)* `uzi run create --plan-file <path>` (`-` for stdin), plus `--agent-source` / `--exclude-agents` reusing `approveSelection`'s existing validation. Exit codes per the documented contract; `--exclude-agents` without `--agent-source` stays a usage error. Bundled `SKILL.md` updated, drift test green.
- [x] **M4 ⟨R⟩ — Staleness guard.** *(Done 2026-08-04, merged to 209-seed. Commits `8f443c62` (migration `00096`: `planned_base_commit` + `require_base_match`; server persist + claim + CLI `--planned-commit`/`--require-base`; worker `evaluateBaseStaleness` prefix-tolerant compare + `BaseCommitDivergedError`) → `b1f130f7` (validation hardening from the wave). **Open Question 3 decided: WARN on divergence by default; `--require-base` FAILS the run** before any implement/push. `task gate:api` + live-DB sweep + `task gate:agent` all green; the four worker cases (silent/match/warn/fail) mutation-proved. Validation: reviewer clean (fail path throws before implement/push, requeue re-runs the guard, only seeded runs carry it), auditor confirmed additive + no prompt-injection (planned_commit reaches only the compare + feed, never the model prompt) + fail path clean, tester all-live. **Hardening the wave added:** `planned_commit` is validated at create time as hex 7–64 chars (`^[0-9a-fA-F]{7,64}$`) — without it a 1-char `--planned-commit` prefix-matches and silently disarms `--require-base`; this also caps the field (was only bounded by the 1 MiB body limit). **Follow-up (consistent with M1):** the defense-in-depth handler-layer 400s are CLI/service-tested, not handler-tested — needs a live forge mock.)* The client sends the commit it planned against; the worker compares it to the clone's resolved base and surfaces a divergence. **Open question 3** decides refuse-vs-warn.
- [x] **M5 ⟨R⟩ — Web surface.** *(Done 2026-08-04, merged to 209-seed. Commits `a302517c` (expose `plan_source` on the read DTO `apitypes.RunDTO` + `runToDTO`, golden `TestRunDTOTags` updated — the DTO gap M1 left, since M1 only wired the CLAIM wire) → `8cf19a65` (web: read-only `SeededPlanPanel` gated on `plan_source === "seeded"`, disjoint from the approval PlanPanel; the roster-pending state keyed on `plan_source === "seeded" && agent_source === "repo" && empty roster`, replacing the misleading "used the repo's own agents" over an empty chip list; `run-seeded` mock fixture). `task gate:api` + `task gate:web` green. Validation: reviewer clean (both new tests mutation-confirmed non-vacuous, DTO additive, board unchanged); auditor CLEAN (the untrusted plan renders through the shared hardened `<Markdown>` with no rehype-raw → structurally no HTML→DOM path); web-ux PASS in a real browser (the seeded-plan collapsible renders, the misleading-empty-roster bug is fixed in the DOM, non-seeded runs unchanged). api.ts types `plan_source?` optional for rollout skew (absent ⇒ not-seeded, fail-safe).)* Bigger than it reads. `<PlanPanel>` renders only under `run.status === "awaiting_approval"` (`web/src/pages/RunView.tsx:639`) and `run.plan_md` is read only inside it (`:982-984`) — so a seeded run's plan body is currently **unreachable** on the run page. This is a new renderer, not a copy tweak. Second, smaller item in the same milestone: `AgentRosterSummary` is gated `run.status !== "awaiting_approval" && run.agent_source` (`:680`), and a seeded run has `agent_source` from creation, so for `source: "repo"` the card appears on first load saying *"This run used the repository's own agents"* over an **empty chip list** until the post-checkout report lands `repo_agents` (`:1139`). The board start button is unchanged (no start-run modal — the user ruled against one on 2026-07-27, `handler/workers.go:683-685`).
- [x] **M6 ⟨R⟩ — Proof, end to end.** *(Done 2026-08-04, commits `208db5f5` (the proof) → `e75568ad` (fix-the-doc on the judge comment). Two agent/test proofs: the STUB full-path test drives the real runner + StubExecutor + fakeGitlab on a genuine seeded row-2 claim (plan_approved+plan_source=seeded+session_id=null) and asserts create→claim→implement→MR with NEVER an awaiting_approval state, exactly one MR (`Closes #90`), zero plan-feed messages — reviewer independently mutation-proved the "never awaiting_approval" assertion is non-vacuous (flipping plan_source→'agent' makes it appear). The JUDGE planless-gate tests assert the seeded plan reaches the judge prompt, the empty steering log is omitted, the verdict parses, and no recommendation penalizes the missing gate (the plan-delivery assertion mutation-checked). Success Criterion 1 proven end-to-end through the stub; the ~30min shell e2e was correctly skipped (no coverage beyond the stub). Fix-the-doc: the stale `sampleMessages` head/tail comment in judge-runner.ts, which claimed the head holds "the plan gate", was corrected for the seeded case.)* e2e coverage through the stub executor (which already honours the pre-approved path, `executor.ts:605`) for a full seeded run: create → claim → implement → MR, never passing `awaiting_approval`. Claim contract has a home already: `agent/test/claim-skills-contract.test.ts:67` asserts `claim.plan_approved === true`. **The judge criterion is stated concretely, because "without degrading" has no predicate**: `judge-runner.ts:352` does deliver `t.plan_md`, so the seeded plan reaches the judge; but `trace.inputs` (`:357`) is **empty** for a seeded run (no `approve_plan`), so the steering log vanishes, and `sampleMessages` (`:392-393`) claims its head/tail keeps "the plan gate and the delivery" when the head now holds no gate. Assert: the verdict schema parses, and no recommendation cites a missing plan.
- [x] **M7 — Docs.** *(Done 2026-08-04, commits `5ad61b94`/`25c0eb55`/`816b6111`, merged. New user page `docs/seeded-plans.md` (the write→base-commit→roster→create→PRDLESS walkthrough with D7 self-sufficiency stated up front), `docs/cli.md` flag additions, an `ARCHITECTURE.md` Run-lifecycle bullet (a run can be born past the gate; the checkpoint moves to the MR). check-docs green. Fact-checked: all 6 load-bearing claims CONFIRMED against the code — including a fix-the-doc catch (an earlier draft wrongly said a bad `--agent-source repo` is rejected at create time; it is NOT — it silently runs with zero subagents, PRD Open Question 1).)* `docs/cli.md`, `ARCHITECTURE.md` (Agent runtime), and the D7 constraint written where a plan author will read it before writing one.
- [x] **M8 — The skill teaches plan AUTHORING, not just the flag.** *(Done 2026-08-04, commit `9ab9091f`, merged. `api/internal/uzicli/skill/SKILL.md` gained an "Authoring a seeded plan" section: self-sufficiency (D7), reading the roster from `.claude/agents/`, the base commit from `git rev-parse HEAD` (M4), the exact `uzi run create --plan-file` invocation, and the PRDLESS both-labels + promote-or-wait path. Drift test green (prose only, no new flags).)* M3 covers the reference half (a new flag gets a line). This is the other half, and it is the one that decides whether the feature works in practice: the bundled `SKILL.md` gains an authoring section covering what makes a plan self-sufficient and why (D7), how to read the run's roster from the clone's `.claude/agents/`, where the base commit comes from (M4), and the exact `uzi run create --plan-file` invocation — including the `PRDLESS` path below. **Validated by**: a Claude Code session holding only the installed skill can take a written PRD and reach an implementing run with no further instruction.
