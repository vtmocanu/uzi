...
- **multica** uses `Collapsible` for one sidebar nav group (the "Pinned" section, `packages/views/layout/app-sidebar.tsx:681-712`) but has no whole-sidebar collapse in `dashboard-layout.tsx`. Its board hides columns **manually per-status** (`hideStatus` via an EyeOff menu item, `packages/views/issues/components/board-column.tsx:104-107`) and deliberately renders empty columns with a placeholder (`board-column.tsx:170`) — there is no automatic empty-column filter. uzi's icon-rail collapse and derived auto-hide are therefore different mechanisms, not ports; multica's choice to keep empties visible is the trade-off the tick box makes user-controlled.
...
- `collapsed` state lifted into `AppShell`, initialized **lazily** (`useState(() => prefs.get(...))`, not a post-mount effect — avoids an expanded→collapsed first-paint flash) and persisted via a new `web/src/lib/prefs.ts`: typed get/set, JSON, `try/catch` + `typeof window` guard (multica's localStorage helpers do exactly this — `onboarding/source-backfill-dismiss.ts`, `editor/extensions/mention-recency.ts`; we skip their `useSyncExternalStore` reactivity since no two components watch one key here).
...
- 2026-07-05: Collapse = icon rail, not fully hidden — destinations stay one click away and the pattern matches the multica-derived shell language.
