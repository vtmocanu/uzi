...
**D-T2. Account always named.** On the board strip, every shown Claude token and Codex account renders its label after its accent bar `▎`, regardless of how many are readable (`showLabel` becomes unconditional for the strip; keep the helpers' selection behaviour). Labels stay capped and routed through `m.renderer.Plain`. The detail rail's Codex block names its account(s) likewise; its `CODEX` rail header stays. The TUI Codex account label carries no ` (default)` suffix, on the strip or the rail (maintainer-approved amendment, 2026-09-25; see the Decision Log).

**D-T3. Window labels and buckets.** Codex window cells are labelled by `limit_window_seconds` in the same `5h` / `7d` / `3h` form as the web (`formatCodexWindowLabel`'s rules: whole days → `Nd`, whole hours → `Nh`, whole minutes → `Nm`, else `Ns`; unknown/≤0 → `window` on the web), never `P` / `S`, on the board strip and the detail rail. In the TUI (board strip and rail), an unknown or non-positive `limit_window_seconds` is labelled `?` instead of `window` (maintainer-approved amendment, 2026-09-25; see the Decision Log); the web keeps `window`. The bucket with id `"codex"` draws no bucket name; any other bucket keeps its name (Plain-routed) before its windows.
...
- 2026-09-25 (landing): the maintainer skipped attaching M8 screenshots to the PR; the landing session's local visual check stands as M8's evidence. The sidebar selection refetch now ignores a stale settings response (CodeRabbit finding on the PR), with a regression test.
...
- 2026-09-25 (maintainer-approved at the plan gate): in the TUI an unknown or non-positive `limit_window_seconds` is labelled `?`, not `window` (D-T3 amended); the web keeps `window`. Reason: the detail rail is 26 columns with a 10-cell bar; to keep the percent and reset countdown whole, a 6-character `window` label would cost 3 of the 10 bar cells, while `?` keeps the bar full.
- 2026-09-25 (maintainer-approved at the plan gate): the TUI Codex account label drops its ` (default)` suffix on the strip and the rail (D-T2 amended). Reason: every account is now always named, so the common single-account case would read `codex ▎default (default)`; the web sidebar shows no default marker (the Settings Codex card keeps its default badge).
- 2026-09-25: the merged sidebar component is `SidebarUsageLimits` (`web/src/components/SidebarUsageLimits.tsx`); `SidebarRateLimits` and `SidebarCodexRateLimits` are removed.
- 2026-09-25: `selectedCodexRateMeters` no longer returns a `showLabel` flag, since the strip and the rail both always name Codex accounts; the Claude `selectedRateMeters` keeps it for the out-of-scope Claude rail (`railRateMeters`). The web mirrors the server's main bucket id as `CODEX_MAIN_BUCKET_ID` in `web/src/lib/codexRateLimits.ts`, and the TUI as `codexMainBucketID`.
- 2026-09-25: M2's mock fixture is the EXISTING default Codex account (`cdx-acct-default`) with its first bucket changed to the main `"codex"` bucket (empty display name), not a new account: a non-default account is hidden from the sidebar unless selected, so only the default one exercises D-W3 in the default mock. The TUI demo fixture (`api/cmd/uzi/demo.go`) likewise uses the main bucket with 5h/7d window lengths.
- 2026-09-25: "Every run" means every run whose row or header renders the harness chip at its unchanged placement (D-W5). A run-list row that shows the "waiting for vault unlock" badge shows that badge in place of the chip cluster, as before; the run header always shows the chip.
