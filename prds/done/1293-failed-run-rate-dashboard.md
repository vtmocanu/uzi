...
**Status:** Complete (2026-09-19). Shipped on branch `agent/issue-1293`; API + web + CLI + specs, no migration, no `.github/workflows/**` touched.
...
- `specs/ai.md` head section is **633** on `main` and on every sibling worktree (checked 2026-09-12); this PRD uses **634**, written as the heading `## 634. PRD #1293 — Failed-run rate on the dashboard (global and per user)` (the file's heading form is `## NNN. PRD #…`; the `§` prefix is only for inline cross-references). Re-confirm at the landing rebase and bump if a parallel PRD landed first (append-only rule). *(Superseded 2026-09-13, issue #1317: `specs/ai.md` is frozen; record the design in this PRD's Decision Log or an ADR instead.)*
