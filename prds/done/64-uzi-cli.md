...
Neither prior-art project offers a directly comparable surface: bottega has no CLI at all (no
`cmd/`, no CLI); dot-agent-deck is itself a Rust binary with no server/auth surface to broker a
credential against. The design below is therefore reasoned from first principles against the
constraints already established elsewhere in this repo (the credential posture in PRD #1/#4, the
open-redirect closure in PRD #45 Decision 3):

- **CLI exists in-repo, same module.** Go + cobra at `api/cmd/uzi/`, in the existing api Go
  module rather than a separate repo (**this reversed our original separate-repo decision**,
  Decision Log 11) — a same-repo CLI can import `api/internal/…` directly for a compile-time
  contract with no public package, no pseudo-versions, no `GOPRIVATE`, no `vendor/`.
- **Browser login: poll, not a loopback listener.** The browser only *approves*; the CLI polls
  for the mint. No port, no binding heuristics, no tunnel, no token ever rides a URL — works over
  SSH and in containers, where a loopback-listener design does not (it needs outbound-IP
  detection, a `--callback-host` escape hatch, `tcp4` pinning for a macOS IPv6 bug, and `ssh -L`
  instructions, and still binds `0.0.0.0` on LAN).
- **Redirect safety: no redirect target exists at all.** A session JWT never leaves the browser;
  PRD #45 Decision 3's closed open-redirect stays closed. (A callback URL that accepted any RFC
  1918 address would hand a full session JWT to any private-network address in the URL — that
  failure mode is exactly why this design has no redirect target to attack.)
- **Credential model: one credential, two acquisition paths.** A static token minted in the
  webui (agent/CI path) and a browser-brokered `uzi login` (human path); both mint the same row.
- **Token table**: `token_prefix` + `revoked`, diverging from uzi's own `workers` table
  (`token_hash` only, `00020_workers_runs.sql:13`) — plus a `scope` column for the read-only-admin
  class.
- **Prefix convention**: `uzc_` (user) / `uza_` (admin-read); uzi's own `uzw_` (worker) already
  fits the convention of a prefix per credential class.
- **Auth dispatch**: one `Authorization` header, presence-dispatched in `RequireUser`.
- **Agent-facing docs: `go:embed`ed skill ships inside the binary** ⇒ skew structurally
  impossible, works offline, works for a **private** repo (a skill fetched live from a public
  GitHub `main` is unusable for us).
- **Skill drift control: a test**, not manual discipline — our skill documents our own cobra
  tree, so CI asserts every documented command/flag exists in it.
- **Self-update: rejected.** brew is our only distribution channel, so a self-update command
  would be a pure alias for `brew upgrade uzi-cli`.
- **Token in shell history**: `uzi auth token` reads stdin when piped, prompts on a TTY — never
  accepted as a plain argument.

**Net**: each decision above was reasoned from the failure modes of the rejected alternative, not
copied from an external project. The one place a same-repo CLI genuinely wins — importing
internal packages directly for a compile-time contract — is exactly what reversed the original
...
CREATE TABLE cli_tokens (
    ...
    token_prefix text NOT NULL,                  -- "uzc_a1b2…" display stub
...
`token_prefix` and `revoked` **diverge from uzi's own `workers` precedent**, which stores
`token_hash` alone and hard-deletes. The divergence is
...
**The server sets every lifetime; the client never proposes one** — a client choosing its own
credential lifetime is backwards.
...
api/internal/uzicli/         # client, config, authflow, output, skill
...
`allowed-tools: Bash(uzi *)` and `user-invocable: false`.
...
- **Drift control**: a test asserts every command/flag the SKILL.md documents exists in the cobra
  tree. This is the same discipline a manual source-map doc would enforce by hand, done
  **mechanically** instead — possible for us precisely because our skill documents our own CLI.
...
3. **Poll, not a loopback listener** — the decisive evidence is what a loopback-listener design
   costs in practice. Such a listener needs outbound-IP topology-detection heuristics (a UDP-dial
   ...
   plaintext HTTP**. Worse, a naive implementation of that pattern typically accepts **any RFC
   1918 address**, handing a **full session JWT** to any private-network address in the URL.
   Polling deletes every line of that. — architect.
4. **One credential, two acquisition paths** (webui-minted static + browser-brokered), not two
   ...
   confirmed against prior art. — architect.
...
8. **Expiry: 90 days, unless it is a webui-minted `user`-scope token.** *(Q5: architect proposed the
   acquisition-path split as a refinement of a flat 90-day expiry; **user adopted it** over the
   ...
   The server sets every lifetime; a client choosing its own credential lifetime is backwards.
...
10. **`token_prefix` + `revoked`, diverging from uzi's own `workers` precedent.**
...
11. **The CLI lives in this repo, at `api/cmd/uzi/`.** *(**User reversal** of their own earlier
    ...
    zero-commit moment.)* The evidence was that a same-repo CLI importing internal packages
    **directly** gets a compile-time contract for free, with no public package,
...
14. **Skill: embedded, not fetched; content-hashed, not version-keyed; test-guarded, not
    source-mapped.** A skill fetched from GitHub `main` describes main rather than the installed
    binary — and needs a **public** repo, which we are not. An embedded skill cannot describe a
    version other than the one running. A manual source-map evidence discipline is the right idea
    done by hand; ours is a test, because our skill documents our own cobra tree. — architect.
...
20. **Runtime skew is a distribution problem, and co-locating the code does not fix it.** In-repo
    ...
    reviewable in one MR), and the exact version handshake from Decision 13. — architect.
