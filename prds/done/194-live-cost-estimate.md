...
Prior art per the inspiration-first convention: a comparable model-pricing table
(four rates per model — `input`/`output`/`cacheRead`/`cacheWrite`) was reviewed for
its resolution strategy. Its resolver does exact-match-then-strip-trailing-date and
explicitly refuses `startsWith` matching, which is the resolution rule adopted
below. Its own documented gap — no future-dated pricing, and a single `cacheWrite`
rate (the 5m rate only) — is the concrete precedent for the 7.7% error described
below; effective dating and a fifth rate (1h cache write) are this PRD's genuine
deltas on top of that resolution rule.
...
- Resolution is **exact-match first**, then a documented normalisation (strip a
  ...
  `claude-opus-5-something` priced differently.
...
question; and corrections to the prior-art characterisation, the 7.66% figure, the
