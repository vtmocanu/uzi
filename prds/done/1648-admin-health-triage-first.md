...
| 2026-09-25 | M8 closed on the maintainer's own mock-mode visual pass of `3866d07d` instead of before/after screenshots attached to the PR | The maintainer checked every page M8 names in a local mock build (Health degraded and silent on Dawn only; incident also on Ember; Health at 390px), found no regressions, and said M8 can be ticked; the agent run cannot attach images. Accepted residual: the Unknown badge was not seen rendered in any mock (no `?mock=` scenario serves an Unknown check; `unknownDoc` is test-only); it is covered by component and unit tests only. |
