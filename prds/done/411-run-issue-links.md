...
**Priority**: Medium
**Status**: Complete — landed on branch agent/issue-411, 2026-08-20
...
**Which issue-less kinds actually reach the runs list matters, and it is not all of them.** `ListRunsForUser` (the query behind the list, dashboard, and board strip) filters `AND r.kind NOT IN ('chat', 'judge')` (`store/queries/runtime.sql:450`) and INNER JOINs `repos`, so the repo-less kinds never appear. The issue-less kinds that **do** appear as run cards are exactly three: **`task`** (PRD #400 `uzi handoff` — repo + branch, no issue), **`ci_fix`** (CI-autofix, tied to a pipeline; titled "Failing pipeline on `<ref>`" by `web/src/components/CIFixRunHeader.tsx:16`), and **`prompt`** (a scheduled free-form prompt run). `chat` and `judge` live on their own pages and are excluded here; `self_improve` has a real issue and links.

These three must show their **kind** where the issue number would sit, never a broken/empty link. **Today the meta line is unguarded — `RunsList.tsx:263` is `{run.repo_path} #{run.issue_iid}`, so an issue-less run currently renders a dangling `vtmocanu/uzi #`** (React renders `#{null}` as a lone `#`). Labeling by kind also fixes that pre-existing cosmetic bug.
...
4. **Issue-less runs are labeled by KIND, not a generic flag** (user decision, 2026-08-20). Where the issue ref would render but `issue_iid` is NULL, show the run's **kind** as a muted neutral chip — `task`, `ci fix`, `prompt` (the only three issue-less kinds that reach the list; `ci_fix` renders as "ci fix"). This is more informative than a generic "no issue" and self-documents what the run is. Per-surface rule: `issue_iid == null` → kind chip; else if `issue_web_url` is a valid https URL → forge link; else → plain number (rare edge: issue cached-out). This replaces today's dangling `repo #` for those kinds. (Schedule surfaces already render `"prompt"` for null `issue_iid`, so this is a runs-surface concern only.)
...
- [x] **M1 — Runs carry the issue web URL (server + FE type).** List path: `LEFT JOIN issues` on `(repo_id, forge_issue_iid)` in `ListRunsForUser` and `ListActiveRunsAll`, select `i.web_url`; `sqlc generate`; map the nullable column in the list handlers (`handler/runs.go`) where `RepoPath` is already set. Detail path: in `GetRun` (`handler/workers.go:1049`), call `GetIssueByIID(repo_id, issue_iid)` best-effort beside `GetForgeTypeForRepo` (`:1076`) and stamp `dto.IssueWebURL` — **no join on `GetRunByID*`**. Add `IssueWebURL *string` (json `issue_web_url`) to `RunDTO` (`apitypes/run.go`; `RunListItemDTO` embeds it, so list+detail are covered by one field). Add `issue_web_url: string | null` to `Run` (`web/src/lib/api.ts:1227`). **Validate**: list DTO includes `issue_web_url` for an issue-run, `null` for a task-run and for a run whose issue is uncached; detail DTO likewise; the `GetRunByID*` return type is unchanged (no caller ripple).

- [x] **M2 — Issue links + kind labels on all run surfaces (web).** Render `#<iid>` as an `isHttpsUrl`-guarded forge anchor (board-card style, `ExternalLinkIcon`, `target=_blank rel=noreferrer`, `stopPropagation` inside card `<Link>`s) at surfaces 1, 3, 4, 5; when `issue_iid == null`, render the run's **kind chip** (`task` / `ci fix` / `prompt`) in its place — a small shared helper mapping `run.kind` → label, reused across the surfaces — which also replaces the current dangling `repo #`. Add `issue_web_url` to the run fixtures in `web/src/mocks/data.ts` (e.g. `:698,:705,:807,:882`) and add at least one issue-less (`task`/`ci_fix`/`prompt`) run fixture so both the link and the kind chip render under `VITE_UZI_MOCK=1` and the tests have positive fixtures for each branch. **Validate**: an issue-run row links to its `issue_web_url`; a `task`/`ci_fix`/`prompt` row shows its kind chip and no dangling `#`; a run with null `issue_web_url` (issue cached-out) shows a plain number, no anchor; links render in mock mode.

- [x] **M3 — Schedule last-fire carries + renders the issue URL.** Server: add `WebURL` to `schedsvc.Started`/`Skip` (`outcome.go:8,:18`), thread `issue.WebURL` through `createIssueRun`/build sites (`scheduler.go:410,:434` + fetched-skip sites), populate in `marshalLastFire` (`last_fire.go:58-69`), add `web_url` (json `web_url`) to `apitypes.LastFireStarted`/`LastFireSkip`, and to the FE `LastFireStarted`/`LastFireSkip` types (`api.ts:884,:892`). Web: render the started (`Schedules.tsx:476`) and skip (`:538`) row `#<iid>` as a guarded forge link; add `web_url` to the mock schedule `last_fire` (`web/src/mocks/mockApi.ts:943-970`). **Validate**: a recorded fire's persisted summary includes `web_url` for a started/fetched-skip issue; the row renders a link; an `already_running`/`fetch_failed` skip and an old summary render a plain number.

- [x] **M4 — Tests.** Go: a **live-DB/integration** test in the store package (alongside `store/run_usage_integration_test.go` / `schedules_livedb_test.go`) that actually executes the joined `ListRunsForUser`/`ListActiveRunsAll` and asserts `issue_web_url` is populated for a run with a cached issue and NULL for one without — a fake-store handler test proves only mapping, not that the SQL join runs (per `.claude/rules/go.md`). A `schedsvc` unit test (`scheduler_test.go`) that `marshalLastFire` writes `web_url` for a started row and leaves it empty for an unfetched skip. Web (vitest): the issue link renders with the correct href for an issue-run; the kind chip (`task`/`ci fix`/`prompt`) renders for the matching issue-less run and no dangling `#` appears; **no** anchor renders when `issue_web_url`/`web_url` is null — each negative paired with a positive on the same wording (per `.claude/rules/web.md`), and every new assertion proven non-vacuous by a call-site mutation. **Validate**: `task gate:api` and `task gate:web` pass.

- [x] **M5 — Docs.** CHANGELOG `[Unreleased]` entry ("run issue numbers now link to the forge issue; issue-less runs flagged"). Check whether any `audience: user` doc under `docs/` describes the runs list / run view / schedules and, if so, add a sentence. No new doc page expected.
...
2. A `task` / `ci_fix` / `prompt` run never renders a dead issue link or a dangling `#` — it shows its kind chip.
...
- **2026-08-20**: Issue-less runs labeled by **kind** (`task` / `ci fix` / `prompt`), not a generic "no issue" chip (user decision) — more informative and it also fixes the pre-existing dangling `repo #` at `RunsList.tsx:263`. Only those three issue-less kinds reach the list; `chat`/`judge` are excluded by `ListRunsForUser` (`runtime.sql:450`). Chip driven by `issue_iid == null`, link rendering by a valid `issue_web_url`.
