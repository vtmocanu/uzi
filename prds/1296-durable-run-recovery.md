...
- [ ] **M1: Durable identity and storage foundation.** Implement immutable capture/claim-generation identity, states, quota reservations and ordered encrypted-chunk storage contracts. Define the common custody predicate and idempotency before consumers fork. Prove fresh/stale/foreign claims, concurrent reservations and no metadata/byte cascades that destroy custody. Gate: `task gate:api`, live-store tests through the existing harness, `task scan:secrets` with the canary detected.
  - _Partial (2026-09-12, left unchecked):_ the M1 SEAM/contract landed in the branch working tree — migration `00219_recovery_archive.sql` (`runs.claim_generation`, `recovery_custody_holds`, `recovery_captures`, `recovery_capture_chunks`), the complete `queries/recovery.sql` store layer, the `ClaimRun` custody-CTE + admission predicate + `CountUnresolvedCustodyHoldsForOwner`, the `recovery_archive_v1` protocol capability, the frozen owner DTOs (`apitypes/recovery.go` + api-contract fixtures + `apiTypes.ts`), the worker RPC types (`apitypes/recovery.go` + `agent/src/protocol.ts`), and the `DesiredWorker.CustodyHeld` controller wire field. `task gate:api`/`gate:web`/`gate:agent` and `controller` build+test are green; live-DB tests prove the claim opens a hold, capture reserve/manifest-CAS/release, and no custody cascade. STILL OWED for the box: `task scan:secrets` with the canary, the concurrent-reservation/foreign-claim proofs beyond what landed, and the lead's live seam execution + commit. M2/M3/M5 consume these frozen files without editing them.
...
- 2026-09-12: M1 SEAM/contract implemented on `agent/issue-1296` (uncommitted; lead to review, execute the seam live, and commit). Migration numbered 00219 above the live head 00218 (renumber above the live head at merge). `ClaimRun` became leading `target`/`hold` CTEs before the `UPDATE ... RETURNING *` so the custody hold opens atomically in the claim while the query still returns `store.Run` (a `SELECT * FROM claimed` CTE tail made sqlc emit a bespoke `ClaimRunRow` that rippled across ~40 existing claim tests; the leading-CTE form avoids it). Custody-admission uses `workersvc.custodyHoldLimit=8`, with a non-positive limit disabling the gate so the existing claim tests (which pass the zero value) still claim. Recovery capability is the additive `recovery_archive_v1` protocol capability (D9). Worker-facing archive RPC types were kept out of the SPA api-contract fixtures (owner DTOs only) and pinned via wire_test tag tests + a new `agent/test/recovery-protocol-contract.test.ts`.
