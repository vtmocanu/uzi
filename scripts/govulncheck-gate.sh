...
# 🔴 THE ANALYSIS IS PINNED TO THE IMAGE'S BUILD CONFIGURATION --
# `CGO_ENABLED=0 GOOS=linux GOARCH=amd64` -- ON THE govulncheck RUN BELOW, NOT
# ON THE HOST. This gate's predicate is "is a vulnerable function reachable from
# SHIPPED code", so it must scan the file set the images ship, not the file set
# the invoking machine's platform defaults happen to select. The shipped values
# come straight from the Dockerfiles: `api/Dockerfile:50` and
# `controller/Dockerfile:19` both read `RUN CGO_ENABLED=0 GOOS=linux go build`,
# and the published images are linux/amd64 only (`.github/workflows/release.yml`,
# "platforms: linux/amd64 -- amd64-only for now"). amd64-only is sufficient today
# because there is exactly one published platform; add axes here if that changes.
#
# WHY THE PIN IS ON THE RUN AND NOT THE INSTALL. The govulncheck binary itself
# must be host-native -- it runs on this machine, it only ANALYSES the target
# configuration. And `go install` refuses to cross-install when GOBIN is set:
#
#   GOBIN=<tmp> GOOS=linux GOARCH=amd64 go install <pkg@version>
#     -> rc=1, "go: cannot install cross-compiled binaries when GOBIN is set",
#        $GOBIN never created
#
# Measured twice independently on go1.26.5. This script always sets GOBIN (the
# throwaway bin three lines below the install), so a GOOS/GOARCH on the install
# line would break it outright. The pins therefore live inline on the RUN command
# and nowhere else; do NOT hoist them to a top-of-script `export`, which would
# also hit the install and reintroduce that failure. (Corrected 2026-08-04, Unit B
# review: an earlier note claimed the cross-built binary lands in
# $GOBIN/<goos>_<goarch>/ and that loosening the `-x` install check would open a
# hole. Both halves are wrong -- the install refuses outright as shown above, so
# the exit 2 comes from the `irc -ne 0` arm and never from the `-x` arm.)
#
# PINNING CGO_ENABLED=0 OURSELVES ALSO CLOSES THE OLD `CGO_ENABLED=1`-IN-ENVIRONMENT
# DISARM. `cgo` is a build constraint, so before this pin an inherited
# `CGO_ENABLED=0` (or `=1`) in the environment silently changed which files
# compiled and therefore which calls the gate saw -- measured through this script
# unmodified on a fixture whose call to a vulnerable symbol sat behind
# `//go:build cgo`: default env -> exit 1 with GO-2021-0113 named, `CGO_ENABLED=0`
# -> exit 0 "No vulnerabilities found." Our inline value now OVERRIDES any
# inherited one, so that disarm is gone: the setting is no longer the environment's
# to choose.
#
# WHY PINNING CHANGES NOTHING TODAY BUT IS STILL CORRECT (measured, not assumed).
# Both modules read 0 called vulnerabilities under either setting. On darwin/arm64
# api's package graph is IDENTICAL under both settings (418 packages, no file-count
# delta, 0 CgoFiles). On linux/amd64 the stdlib file set DOES move -- `net`
# 5 -> 0 CgoFiles, `os/user` GoFiles 4 -> 5 -- so the two configurations are not
# interchangeable in general; it was latent here only because no file under `api/`
# or `controller/` carries `import "C"` or a `//go:build cgo` constraint today. If
# this pin ever surfaces a new CALLED vulnerability, that finding is REAL --
