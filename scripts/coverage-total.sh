...
# The `total:` line was what the retired GitLab pipeline's `coverage:` regex read.
# Its shape is `total:\t(statements)\t75.4%` -- tab-separated, which is why that
# regex did not assume spaces. GitHub Actions has no `coverage:` equivalent, so
# nothing in CI parses this line today; it is still printed for local/log inspection.
