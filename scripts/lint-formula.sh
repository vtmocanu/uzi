...
# SHIPPED IN THE WRONG PLACE. This guard was originally one variable set in the
# retired GitLab pipeline's `lint:repo` `variables:` block, which the audit showed
# is displaceable: GitLab ranked PIPELINE variables (manual run, trigger, schedule)
# at 3 and PROJECT variables at 4, against job variables at 8 -- so a same-named
# manual-pipeline variable displaced the job's value and the job went green having
# checked nothing. That precedence hazard was GitLab-specific; the two-signal guard
# is kept regardless, since resting the whole guarantee on one line in one file was
# the real defect.
