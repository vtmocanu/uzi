...
- **format** — `task fmt-check` is a check, not a sweep: a `gofmt -w` sweep that pulls in foreign files still passes. On an un-rebased branch a red may be pre-existing drift, not yours; say which. Each `gate:<c>` runs `fmt-check:<c>` first, so a component gate already covers it.
- **lint** — `task lint` runs seven targets: the four components plus `lint:shell`, `lint:yaml`, `lint:formula`. Go is golangci-lint v2.12.2 via `../scripts/golangci-lint.sh` (a pinned, sha256-verified release binary, not `go run`); npm is oxlint 1.80.0. In the Go gates lint runs after build, so a non-compiling tree shows as a lint "typechecking error".
