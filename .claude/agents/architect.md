...
boundaries are `internal/forge` (interface + neutral types; `gitlab.go` is the
only driver, no other package imports a driver), `internal/forgesvc` (shared
...
is never touched - is enforced by four independent guardrail layers (GitLab
Developer role + protected branch; worker-held PAT with env-scoped git config;
