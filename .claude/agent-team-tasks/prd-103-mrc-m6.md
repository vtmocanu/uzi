...
| coder | **UNIT A COMPLETE 2026-08-04, 9 code commits, tip `f47961e6`, `task gate` rc=0** (archived `probes/prd-103-mrc-coder/c8-full-gate-at-tip.txt`). All 4 amendments against its work reproduced before it acted; 2 self-caught non-controls; 1 deviation accepted (`node` over `jq`, better reasoning than my prescription). **Unit B not started.** |
| tester | **REPORTED 2026-08-04 — 73 full-suite runs**, matched pairs + Clopper-Pearson intervals. Refuted "60000 fixes it" (1.20x real margin), found a **second** flaky test with a different mechanism, killed the split by the stronger design, and corrected my Amendment 7. Evidence: `probes/prd-103-mrc-tester-tt/` (173 files). |
