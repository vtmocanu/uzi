...
- **Tag that commit and push the tag:**
  ...
  ```
  The tag push triggers `release.yml` AND `brew.yml` (plus `kind-smoke.yml`) — watch and approve all of them (step 4), not just `release.yml`. Never tag a commit whose message carries a `[skip ci]`/`[ci skip]` marker — GitHub skips the tag's workflows too, so nothing publishes.
...
- **Once `brew.yml` is green, refresh the local CLI:** `brew update && brew upgrade uzi-cli`, so the machine you release from runs the version you just shipped (the formula publishes to the owner's tap, `<owner>/homebrew-tap`; if the CLI is not brew-installed here, skip this). Local convenience, not a release gate — do it after the formula publishes, never before.
