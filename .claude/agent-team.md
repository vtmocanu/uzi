...
lint           task lint           # composite over SEVEN targets, not four: the four
               ...
               task gate:repo      # its own lint:<c>, so a COMPONENT GATE ALREADY
                                   ...
                                   # observable exit code, and an automated
...
- Release flow: tag-driven (PRD #52). `v*` tags publish the api/web/controller/agent images +
  the OCI Helm chart to GHCR (Model B: chart `version`/`appVersion` == the
...
- CI: real (`.github/workflows/`, PRD #52) — validate/test across **api, controller,
