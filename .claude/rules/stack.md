...
|---|---|---|
| restricted / standard | `uzi-workers` | FQDN allowlist: default-deny `NetworkPolicy` floor (`deploy/chart/templates/worker-networkpolicy.yaml`, `networkPolicy.enabled` default true) plus the Antrea FQDN ANNP (`worker-fqdn-egress.yaml`), including the Codex hosts `api.openai.com`, `chatgpt.com`, `auth.openai.com` on 443 (PRD #1106 D12). Enforced per IP learned from the allowlisted names, not per hostname: another host sharing an allowed IP is reachable (ADR-0285 residual, #1651); destinations at other IPs are blocked and time out |
...
- The standard tier does complete to its own allowlist (`cache.nixos.org`, `*.anthropic.com`, `api.openai.com`, `chatgpt.com`, `auth.openai.com`, `search.devbox.sh`, `api.github.com`, `ghcr.io`, `pkg-containers.githubusercontent.com`, the forge), so a `200`/`404` from one of those says nothing about tier. A completed HTTP response from a destination confirmed OUTSIDE the allowlist's learned IP addresses (e.g. `codeload.github.com`) is evidence of the docker tier; an off-list hostname alone is not (it may share an allowed IP, ADR-0285). `uzi admin workers` stays authoritative.
