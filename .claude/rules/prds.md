...
- The committed tip may be recoverable from a **persistent** worker's bare repo at `refs/uzi-runner/agent/issue-N`, per the `uzi-watcher` skill's *Recovering a failed run's work from the worker PVC*, until that branch ref is overwritten by a later retry or its PVC is deleted. Ephemeral-worker terminal cleanup can remove that PVC; this is not durable recovery. Issue #377 makes a real-workflow-edit failure graceful (pre-push detection, typed reason); the workflow-scope/base-align paths preserve a best-effort, capped, redacted diff in `preserved_patch` on the failed-run card, but a push-secret-blocked run intentionally omits it (the diff would leak the detected secret).
...
- A branch merely *behind* `main` on those files is a distinct case needing no authoring change: GitHub gates the push on the pushed tip's tree, not on what the branch's own commits changed. uzi realigns at finalize (GitHub-only, only when the workflow trees differ): a fast-forward overlay of just the default's workflow subtree when the branch touched no workflow file, else a SHA-preserving merge falling back to a rebase, per [ADR-456](../../adr/0456-rebase-before-finalize-push.md) (amended for the overlay), failing cleanly with the diff preserved if that conflicts.
...
4. **This is now mechanically enforced** by the `check:token-literals` gate (`scripts/check-token-literals.sh`, in `gate:repo`): it flags any NEW complete provider-token literal in tracked source *even when* an inline `//gitleaks:allow` sits on the line, so a run reddens offline instead of at the doomed push. Only the two gitleaks canaries are allowlisted; the assembled `"glpat-" + "<20 chars>"` form passes.
