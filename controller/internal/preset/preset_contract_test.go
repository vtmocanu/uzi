...
// The memory sibling of the PVC guard above (issue #625). A preset's MemoryLimit larger
// than its tier's limitRange.max.memory is rejected by the limitranger admission plugin
// exactly as an over-large PVC is: the chart renders clean, the controller boots clean,
// and the worker provisions and never appears. This pins every preset's memory ceiling
// under both tiers' limitRange.max.memory read out of values.yaml, so raising a preset's
// MemoryLimit past the chart's ceiling is a red build here rather than a silent strand.
func TestPresetMemoryLimitsFitTheChartsLimitRangeMax(t *testing.T) {
	ceilings := chartMaxMemory(t)
	// Vacuity guards, the same idiom the PVC test above states: a check that iterates an
	// empty collection passes while asserting nothing.
	if len(ceilings) == 0 {
		t.Fatal("no tier ceilings were read from values.yaml; every assertion below would pass vacuously")
	}
	if len(sizes) == 0 {
		t.Fatal("the preset size table is empty; the MemoryLimit assertions below would pass vacuously")
	}
	for tier, max := range ceilings {
		for name, s := range sizes {
			if s.MemoryLimit.Cmp(max) > 0 {
				t.Errorf("preset %q: MemoryLimit = %s exceeds %s = %s, so a worker of that size would provision "+
					"and never appear. Raise it in deploy/chart/values.yaml.",
					name, s.MemoryLimit.String(), tier, max.String())
			}
		}
	}
}

// chartMaxMemory reads both tiers' limitRange.max.memory out of deploy/chart/values.yaml,
// keyed by the values path so a failure names the key to edit. Same parse-and-Fatal
// contract as chartMaxPVCStorage above: PARSED not grepped (`memory` appears under two
// blocks a regex cannot tell apart), and FATAL on anything unexpected — unreadable file,
// missing key, unparseable quantity — because a skip or a default would silently defeat
// the guard.
func chartMaxMemory(t *testing.T) map[string]resource.Quantity {
	t.Helper()
	// controller/internal/preset -> repo root.
	path := filepath.Join("..", "..", "..", "deploy", "chart", "values.yaml")
	raw, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read %s: %v (the memory ceiling is read from the chart; it must not fall back to a hardcoded constant)", path, err)
	}
	var v struct {
		Workers struct {
			LimitRange struct {
	...
	out := map[string]resource.Quantity{}
	for key, val := range map[string]string{
		"workers.limitRange.max.memory":        v.Workers.LimitRange.Max.Memory,
		"workers.docker.limitRange.max.memory": v.Workers.Docker.LimitRange.Max.Memory,
	} {
		if val == "" {
			t.Fatalf("%s is empty in %s; there is no ceiling to check the preset memory limits against", key, path)
		}
		q, err := resource.ParseQuantity(val)
		if err != nil {
			t.Fatalf("%s = %q in %s is not a resource quantity: %v", key, val, path, err)
