...
type Config struct {
	...
	// WorkerDinDLimitCPU / WorkerDinDLimitMemory override the DinD sidecar's resource
	// LIMITS (PRD #89 0.8.1). Empty ⇒ the render side's built-in default ("2" / "2Gi").
	// A cluster raises them because in-daemon image builds OOM the 2Gi default (uzi's own
	// e2e builds 5 images inside the daemon). Optional even when the docker tier is on —
	// validated as k8s Quantity strings when set, so a typo fails at boot, not at admission.
...
func validateDockerTier(cfg *Config) error {
	...
	// Optional DinD limit overrides (PRD #89 0.8.1). Validated as k8s Quantity strings
	// when set so a typo fails at boot, not at the far end when the apiserver rejects the
	...
	for _, l := range limits {
		v := strings.TrimSpace(os.Getenv(l.key))
		if v == "" {
			continue
		}
		if _, err := resource.ParseQuantity(v); err != nil {
			return fmt.Errorf("%s=%q is not a valid resource quantity (e.g. \"4\" or \"6Gi\"): %w", l.key, v, err)
		}
		*l.dst = v
	}
	return nil
