...
const (
	...
	// effect as the plain tier. Issue #1757 raised this from 4Gi to 6Gi to cover ~2
	// concurrent heavy runs plus that cache, and to rank a busy worker later under node
	// disk pressure. It ranks and places; it never limits, and it cannot save a pod on a
	// node whose root disk is too small. Re-derive what is actually left on ephemeral
	// storage before raising it again.
	workerDefaultDockerEphemeralRequest = "6Gi"
)
...
type RenderConfig struct {
	...
	// adding to it. Empty ⇒ workerDefault{,Docker}EphemeralRequest ("512Mi" / "6Gi").
