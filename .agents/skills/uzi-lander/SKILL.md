...
- `scripts/land-prep.sh` rebase / renumber / gate / lease push; `scripts/merge.sh` the
  ...
  `scripts/lib/pr-checks-classify.sh`). `scripts/{cr-rate-limit,land-prep,watch-pr}.test.sh`
  are the hermetic regressions, wired through `task test:uzi-lander` and `gate:repo`.
