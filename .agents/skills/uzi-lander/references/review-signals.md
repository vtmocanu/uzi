...
| PR body | `pulls/N` `.body` | Greptile may rewrite the description between `<!-- greptile_comment -->` markers: `Confidence Score: N/5`, a summary, a mermaid diagram, and `<sub>Reviews (K) · Last reviewed commit: [...](…/commit/<full-sha>)</sub>`. Not on every PR, and user-editable, so never a verdict alone. With no completed review run on the head, `greptile_paired_verdict` (`scripts/lib/greptile-verdict.sh`) counts the head reviewed when that SHA (full 40 hex, parsed only inside the block) or a head review object names it AND a completed `Greptile Review` run on some PR commit finished at or after the head's committer date; that run's M is the tally. This also overrides a stale `in_progress` duplicate run on the head. The edit bumps the PR `updated_at`. |
...
3. Greptile: check-run on head → absent / in_progress / completed(+M); else the body- or review-paired run → completed(+M).
