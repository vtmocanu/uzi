...
branch (a resumed/continued run), which excludes already-pushed commits — with the pinned
gitleaks, its three silencers (`.gitleaks.toml`, `.gitleaksignore`, inline
`//gitleaks:allow`) forced OFF — GitHub Push Protection honours none of them, so a scan
that did would clear a range GitHub still rejects. A finding fails
