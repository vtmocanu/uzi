...
[Anthropic token](./anthropic-token.md), and shows them as live meters.

## Where to look

| Surface | What you see |
|---|---|
| **Settings → Claude limits** | A card under your token, with both windows as bars, current percentage, and a reset countdown. |
...
uzi polls Anthropic in the background, by default every 5 minutes. A
reading can be a few minutes old — fine for windows measured in hours and
days. If your vault is locked, uzi can't open your token to poll it: your
...
at the default 5-minute poll interval, that's roughly 300 tokens a day for
an affected account — negligible next to normal usage, but an instance
...
planning factory work, not just a personal gauge.
