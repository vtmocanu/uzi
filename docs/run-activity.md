...
| stalled (amber) | The newest speaker, but the run's health has flagged it `stalled` or `looping` — a looping agent never reads as healthy green. A `slow` (near-timeout) flag is a budget fact about the run, not evidence the speaker is unhealthy, so it does **not** turn the lane amber. |
