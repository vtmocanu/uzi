...
There's no auto-provisioning of a fresh worker when a run's requirements
go unmet — the remediation today is to switch to an eligible worker you
already have, or provision a persistent one yourself (see
[Hosted workers](./hosted-workers.md)); spawning one on demand needs
on-demand worker provisioning, which hasn't shipped. There's also no way to
declare a requirement from an issue label, and no finer-grained
per-tool "needs admin to install this" classification — both remain future
work.
