...
| `UZI_PRIVILEGE_CHECK_INTERVAL` | `24h` | Cadence of the background PAT least-privilege re-check sweep (Go duration). `0` **disables** the sweep entirely — the one duration knob where `0` is honored, not treated as "unset". The save-time check (blocks an over-privileged token at connect) and the on-demand "Check privileges" button run regardless. A boot pass back-fills never-checked connections shortly after start, so a grandfathered over-privileged token surfaces without waiting a full interval. A negative or otherwise malformed value falls back to the default. |
