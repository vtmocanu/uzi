...
| `HANDOFF_RUN_TIMEOUT` | `4h` | Wall-clock cap for a **non-interactive** `uzi handoff` task run, decoupled from `RUN_TIMEOUT`. Persisted onto the run at create and enforced by the sweeper exactly as `RUN_TIMEOUT` is; clamped to the 8h wall ceiling. `--interactive` handoffs are exempt (bounded by `WORKER_TASK_IDLE_TIMEOUT`). |
...
| `HANDOFF_RUN_MAX_ITERATIONS` | `10` | Implement ⇄ review loop cap for a non-interactive `uzi handoff`, decoupled from `RUN_MAX_ITERATIONS`, so the longer wall clock is not wasted by the iteration cap. |
