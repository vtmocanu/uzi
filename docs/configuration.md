...
**Mid-turn tick (issue #1597).** Both tiers above fire only at a turn boundary, so a single very long turn — the case this closes — could previously commit real work and reach no checkpoint at all until the turn ended. A repeating mid-turn tick, `CHECKPOINT_TICK_INTERVAL` (default `5m`; `0` disables it), fetches the runner clone's committed work back into the worker's bare repository at most once per tick, when the branch tip has moved and the clone is not busy, and publishes it to origin through the same credential-free broker once `CHECKPOINT_INTERVAL` has elapsed since the last publish attempt and the branch tip has moved — the same publish cadence as the iteration-boundary tier, just triggered without waiting for the turn to end. Worst-case loss while at least one publish sink is reachable is therefore roughly `CHECKPOINT_INTERVAL` plus one tick interval; there is no durability claim while every sink is down, and a fetch-back with no successful remote publish is never reported as durable. A per-flight sink gate serializes the tick against every other path that moves the run's durable state (checkpoint closure, pause/wall/completion-hold parks, credential switch), and a stat-only busy probe skips the tick whenever a git lock or merge/rebase marker is present in the clone. Scanning covers every checkpoint publish that goes through the checkpoint body without an overlay context — the mid-turn tick, the time-gated iteration-boundary tier, and milestone publishes on non-GitHub forges — over the pinned commit range being packed, minus content already public, before that range ever reaches origin; a finding or an untrusted scan skips the remote publish and keeps only the local fetch-back. On a Codex run a milestone publish is instead deferred (`scan_deferred`) rather than scanned inside the boundary permit. The park/shutdown/pause/capture checkpoint publishes and every GitHub milestone (`reap:true`) publish remain unscanned, unchanged from before this issue — a GitHub milestone always carries an overlay context, whether or not an overlay commit is actually built. Neither `CHECKPOINT_TICK_INTERVAL` nor `CHECKPOINT_INTERVAL` is exposed via the Helm chart or the controller, so hosted workers run the defaults. See [ADR-1597](../adr/1597-midturn-checkpoint-durability.md) for the full design, including the scan's size caps, merge-commit handling, and the Codex-permit deferral.
...
| `CHECKPOINT_INTERVAL` | `20m` | Minimum time between the time-gated iteration-boundary checkpoint's origin publishes (PRD #267 — see [Checkpoints: two tiers](#checkpoints-two-tiers-prd-122-m8-prd-267) above); only fires when the branch tip has moved since the last publish *attempt* — not the last success, so a persistent broker failure is retried at most once per interval. `0` disables the time-gated path, restoring milestone-only checkpoint behaviour. Only committed work is captured — a lead that doesn't commit isn't protected by a shorter interval. Also gates the mid-turn tick's origin publish (issue #1597) — the tick itself runs on `CHECKPOINT_TICK_INTERVAL`, but still only *publishes* once this interval has elapsed. A milestone publish deferred out of a Codex permit (`scan_deferred`) is owed and bypasses this gate once; after that attempt the normal gate governs retries again. Not exposed via the Helm chart or the controller — hosted workers run the default. |
| `CHECKPOINT_TICK_INTERVAL` | `5m` | How often the mid-turn checkpoint tick (issue #1597 — see [Checkpoints: two tiers](#checkpoints-two-tiers-prd-122-m8-prd-267) above) fetches the runner clone's committed work back into the worker's bare repository while a turn is still in progress. `0` disables the mid-turn tick's fetch-back and origin publish; milestone and iteration-boundary checkpoints still happen as before, and with the tick disabled a Codex milestone's deferred (`scan_deferred`) scan+publish runs inline right after the permit ends, before the milestone checkpoint returns, rather than restoring pure turn-boundary-only behaviour. Otherwise, a fetch-back happens at most once per tick, when the branch tip has moved and the clone is not busy; an origin publish still additionally requires `CHECKPOINT_INTERVAL` to have elapsed and the branch tip to have moved. Not exposed via the Helm chart or the controller — hosted workers run the default. |
