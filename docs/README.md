...
docs. `audience: user` pages are bundled at build time into the in-app
`/docs` section for everyone (PRD #7); `audience: operator` pages are
bundled there too, but shown only to admins (issue #75). `design` and
`contributor` pages stay repo-only. There is no separate docs site or
duplicated content: the same file you edit here is what both a browser
and a `git blame` see.
...
1. Create `docs/<slug>.md`. The slug is the filename; it becomes the route
   (`/docs/<slug>`) if the page is `audience: user` (everyone) or
   `audience: operator` (admins only).
2. Add the frontmatter (a leading `---` fence, at byte 0):
   ...
   - `order` is required, and must be unique within its audience, for
...
4. `audience: user` pages are automatically searchable from the `/docs`
   index (PRD #28) — no separate registration needed. `audience: operator`
   pages are likewise searchable, for admins only (issue #75).
...
| `operator` | Someone deploying/configuring the stack | In-app `/docs` (admins) + repo |
...
missing/invalid frontmatter, a missing/duplicate `order` among `user` or
`operator` pages (each its own namespace), a broken relative link (doc→doc
or doc→img), reference-style links, and an oversized image; it warns
(without failing) on a `user` page over the line budget.
