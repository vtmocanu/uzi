...
- **New outbound egress.** Installing tools reaches the **devbox resolver** (`search.devbox.sh`, hit by `devbox install` to turn `name@version` into a nix ref) and then **nix substituters** (`https://cache.nixos.org` plus any you configure) to fetch it — the *new* egress this feature adds. A worker also reaches the forge directly for git (clone/fetch/push), so its full outbound set is `api` + the forge + the resolver + the substituters. Allow the resolver and the substituters through an egress firewall if you run one; on a hosted kube-native worker both are already on the shipped FQDN allow-list.
