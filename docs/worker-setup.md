...
- **A repo's own `devbox.json` is opt-in.** Off by default. On the **Boards** page, a repo's **Tools** panel has a "trust this repo's `devbox.json` packages" toggle. When on, a run also installs the packages that repo's `devbox.json` lists, unioned with your own list (yours wins a version conflict). Only the `packages` field is read: the repo's `shell.init_hook`, `scripts`, flake references, and every other key are ignored and never run. It bypasses the admin allowlist, so enable it only for a repo whose review discipline you trust.

The worker image installs a **pinned** devbox binary and nix at build time (no floating installer, no first-run download). Storage layout: the nix store is the `agentnix` volume at `/nix`; devbox/nix per-user metadata (profiles, GC roots) lands HOME-derived under `/data` (the `agentdata` volume). Both persist across `docker compose down`/`up`, so only a fresh `down -v` re-downloads packages.
