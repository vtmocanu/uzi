...
- **The upstream-release check (PRD #836) gives `api` a new outbound-egress destination: `api.github.com`.** On a schedule (`api/internal/releasecheck`, default 6h) `api` — the only component that makes THIS release-check request (its own egress; the agent worker's outbound reach is the separate tiered egress of ADR-0285) — polls `GET https://api.github.com/repos/vtmocanu/uzi/releases/latest`, persists the remote facts to `app_settings`, and derives `update_available`/`far_behind`/`security` at **read time**, with zero egress on every subsequent read. `GET /api/version` then serves those derived fields alongside the existing build info, so `web` and the CLI still never call GitHub themselves. This is unrelated to the worker egress tiers below (ADR-0285): it is `api`'s own egress, gated off entirely by an admin runtime toggle for an air-gapped install, and it targets a compile-time-constant URL rather than a user-supplied one, so it does not go through the forge/agent-source SSRF allowlists. See [ADR-836](adr/0836-upstream-release-check.md) and [docs/updates.md](docs/updates.md).
- **Instance branding (PRD #685) is the other deliberately public endpoint pair.** `GET /api/branding` and `GET /api/branding/logo/{slot}` are unauthenticated by design, same as `GET /api/version` above, because the app mark and the optional "POWERED BY" brand they carry must render for a signed-out visitor too. Config (modes, flags, company text) lives in the existing `app_settings` KV store, admin-writable via `PUT /admin/settings`; logo bytes live in a separate `branding_assets` table and are served only from the cacheable image route, never inlined into the JSON, keeping the anonymous response small. A `MIT © Vlad Mocanu` credit is gated on a build-time source constant `SHOW_LICENSE_CREDIT` (`web/src/lib/flags.ts`), default OFF/hidden, and when enabled renders in the signed-in sidebar version row, the signed-out `PublicShell` footer, and the Admin → Branding live preview; it is still not admin-editable. See [docs/branding.md](docs/branding.md).
...
Read-through forge views in the TUI (open PRs, live PR checks, CI runs — PRD #1255) are a third read path alongside the polled caches below: served on demand through the same connection PAT, memoised server-side for seconds not minutes, never persisted to a table — see [docs/cli.md](docs/cli.md#the-forge-screens-pulls-and-ci).
...
`api/internal/poller.Engine` (`main.go`) is started as a single background goroutine alongside the HTTP server. Each tick, for every enabled repo, it either runs an incremental pull — two fetches, the `uzi`-labelled set (`state=all`, PRD #764 M5 keys this on the configured `uzi_label` rather than the old `PRD` label) plus PRD #102 M6's additive open, no-label set, each bounded by its own high-water mark in `forgesvc.Marks{PRD, Open}` (the struct field is still named `PRD` in code; never the client clock; the max `updated_at` that fetch itself returned) — or, every `FORGE_RECONCILE_EVERY`-th tick (and always on a repo's first poll after being enabled), a full reconcile: fetch both complete sets with no lower bound, upsert everything, and evict cache rows absent from their union. Eviction is the only way to observe de-labeling, closing-with-label-removed, or deletion, which an `updated_after`-filtered query structurally cannot report. Per-repo state (`marks forgesvc.Marks`, poll count) lives only in the `Engine`'s in-memory map — a disabled repo simply drops out and a re-enabled one starts over with a fresh full reconcile.
...
A firing detector calls `workersvc.CreateAutoMRReworkRun`, which lands a new **`mr_rework`** run kind (`runs.kind`, an eighth beside `issue`/`ci_fix`/`chat`/`judge`/`self_improve`/`prompt`/`task`) shaped by `runs_kind_shape` as `repo_id` + `pipeline_ref` + `mr_iid` + `target_run_id` all required (`target_run_id` points at the completed run whose MR is watched, mirroring `judge`'s use of the same column; `pipeline_ref` is the `agent/issue-N` branch it folds fixes onto). Two guards run at INSERT, both create-time rather than reactive: a same-kind partial unique index (`uq_runs_one_active_mr_rework` on `(repo_id, mr_iid)`) rejects a second concurrent rework on one MR, and a **cross-kind** partial unique index (`uq_runs_one_active_branch_ref` on `(repo_id, pipeline_ref)`, spanning both `ci_fix` and `mr_rework`) keeps the two features off the same branch worktree at once. The cross-kind guard is the one place this diverges from a naive reuse of `ci_fix`'s existing branch check: for an `mr_rework`/`ci_fix` run, `runs.branch` is NULL for its entire active life (only a *completed* run's `SetRunCompleted`/`ReconcileRunMR` ever populate it), so two freshly-created runs of either kind would both read NULL and never see each other — `pipeline_ref` is written at INSERT specifically so the guard has something to count against immediately. **The same exclusion now also spans `issue` runs** (#1626): each of the three create paths (`issue`, `mr_rework`, `ci_fix`) takes a transaction-scoped Postgres advisory lock keyed on `(repo, branch)` for the target `agent/issue-N` branch, which only serializes the creates against each other, and checks the other kinds before inserting — visibility comes from each check's own key (`pipeline_ref` for the `ci_fix`/`mr_rework` side, `issue_iid` for the issue side, mapped only from a branch of the exact form `agent/issue-N`), not from the lock itself. An **unforced** `issue` create for an issue whose previous MR is still open (always the case while an `mr_rework` is working it, unless that MR was merged or closed meanwhile) still gets the existing open-MR refusal first; a `--force` issue create, an unforced one with no open MR (e.g. against an active manual `ci_fix`), or a create arriving from the other kinds gets the typed `ErrBranchInUse` surfaced as a 409 by each handler with its own wording. An `mr_rework`/`ci_fix` create refuses when a non-terminal `issue` run for that issue number exists. Callers without a user in the loop never surface that `ErrBranchInUse` as a 409: the autopilot poller skips quietly and retries on its next tick, and the scheduler records the fire as `already_running` and advances to its next cadence. `ci_fix` still separately refuses any ref whose branch an active run of any kind already holds; `mr_review_watch`'s poller retries the candidate on its next tick, while `ci_autofix` instead records the failed pipeline as seen, so that pipeline is not retried and the next failing pipeline on the branch is a fresh candidate. An `issue` run's own `runs.branch` is set only by its terminal report, never at worktree creation — the branch column was never a reliable signal to check against before that report lands. The MR's review-comment snapshot rides this create path explicitly (`fetchReviewCommentsSnapshot`, a sibling of PRD #381's issue-comment snapshot, reusing its 32 KiB cap and bot self-filter) rather than through the generic `CreateRun` issue-comment fetch, which would have fetched nothing (an `mr_rework` run has no `issue_iid`). The run is therefore also **issue-less on the board**: queuing it fires no board-column move and it never appears in the runs list's issue-scoped lanes, only via its own `kind`.
...
A background engine (`api/internal/usagepoller`, the same `Boot()` immediate
tick plus interval-ticker shape used elsewhere for a single-instance
background actor, e.g. the scheduler below) ticks on
...
`RUN_KIND_PROFILES` table (`agent/src/run-kind.ts`). Adding a ninth kind follows the
...
- **running → paused (structural completion hold)** ([PRD #1226](prds/done/1226-structural-completion-interlock.md),
  rollout switch `completion_interlock_rollout`, **on by default** for a new `issue`
  run resolved to the `claude` harness). A distinct reason a
  ...
  final head gates every `Closes #N`; the worker learns the frozen
  `contract_revision` it needs to request that permit from either the claim
  response or the ordinary `/state` acknowledgement's `completion_revision`, so a
  run claimed once before its plan was approved still resolves its identity once
  the contract later freezes. A plan with no milestones still freezes an empty
  (`criteria: []`) contract at the same two sites the contract always freezes at
  (plan approval for a gated run, the plan-bearing `running` report for an
  autopilot run) rather than never freezing, so it passes
  the check vacuously instead of never being able to complete. A seeded-plan
  run (`--plan-file`) never sends a plan-bearing report at all, so it is never
  stamped interlocked in the first place and this freeze does not apply to it. A run's resolved
  harness gates the stamp too: only a `claude` run is stamped, since the Codex
  executor does not yet run the completion-attempt nudge loop this hold depends on
  (#1627) — a Codex run is created unstamped and this whole mechanism stays inert
  for it. The owner has three decisions (`uzi run decide <id>`):
  continue-with-guidance (`--continue`), reduce scope to a kept milestone set
...
- **→ completed / failed** — the **worker**, not the agent, pushes the branch
  ...
  the separate, duplicate `run_failed` notification), gated on `stop_kind` so
  a deliberate cancel or plan-rejection stays silent and only genuine
  breakage notifies.
...
and the pending hosted-worker ephemeral-request
...
**general-purpose autoscaled/spawn-on-demand hosted workers** (PRD #58
delivered the static-provisioning subset of the item decided 2026-07-10,
specs/ai.md §168 — a user-triggered click provisions a persistent worker via
the dedicated controller described [above](#worker-controller-k8s-only); PRD
#529 (Path 1, see [Run lifecycle](#run-lifecycle)) has since delivered
spawn-on-demand for the narrow capability-unmet trigger — an ephemeral,
run-bound worker, auto-provisioned and torn down with the run it serves —
but the controller still never spawns one on its own for ordinary queue
depth, a persistent worker still has no scale-to-zero, and #529's own Path 2,
an approve-time capability denial, is deferred), per-user
