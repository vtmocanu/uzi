...
- **running → paused (structural completion hold)** ([PRD #1226](prds/done/1226-structural-completion-interlock.md),
  ...
  for it. The owner's only decision today is continue-with-guidance
  (`uzi run decide <id> --continue`);
  provider-context durability is honestly `same_worker_only`. An admin can turn
  the switch off from Admin → Settings → **Completion check**; a run created
