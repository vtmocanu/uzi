...
- **Docs**: `docs/*.md` need leading-fence frontmatter (`title`, `order`, `audience`); only `audience: user` pages render in-app at `/docs/:slug`. `web/scripts/check-docs.mjs` (runs in `npm run build` and as `task check-docs:web`) fails on bad frontmatter, duplicate `order`, broken relative links, and a backticked `prds/…`/`adr/…` path that does not exist, in `docs/`, `prds/**` and `adr/` alike (see the PRDs bullet). See `docs/README.md`.
