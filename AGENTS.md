...
- `task gate` runs everything, `gate:repo` first: the checks with no component of their own (shellcheck, yamllint, GitHub Actions lint incl. embedded scripts, the Homebrew formula, migration numbering and additivity, spec numbering, no-binary-text, `check:token-literals` (a NEW complete provider-token literal in tracked source, flagged even under an inline `//gitleaks:allow` — an independent check beside, not a replacement for, `scan:secrets`), `scan:secrets` with gitleaks, `sast:semgrep`). `task --dry gate:repo` lists them. `gate:repo` / `gate:api` / `gate:controller` / `gate:web` / `gate:agent` run one slice each.
...
- **Specs contract**: `specs/human.md` is the requirements record. **For this repo, AI has full authority over it and never needs approval to edit it**: form and substance alike, including adding a requirement, rewording one, or changing what one means. The format is already established, so shape entries as you see fit; keep them terse and tag each edit `(AI-synced YYYY-MM-DD)` for traceability. This line wins over any `.claude/agents/*` text that gates `human.md` edits (spec-keeper, architect). **`specs/ai.md` is FROZEN at §637 (issue #1317): never append, renumber or edit it**; `task check:spec-numbering` enforces that. AI design decisions live in the originating PRD's Decision Log and, when they outlive the work, an ADR. Goal: rebuild-from-specs from `human.md` + `adr/` + `prds/done/`.
- **PRDs**: active work in `prds/*.md`, completed ones move to `prds/done/`. They are the design-rationale record (Decision Logs); link them from `ARCHITECTURE.md` rather than duplicating.
  - **A PRD is CI-checked: run `task check-docs:web` before committing one, and never backtick a `prds/…` or `adr/…` path that does not exist yet.** `check-docs:web` (in `validate-web`) scans `prds/**` (incl. `prds/done/`) and `adr/` and fails on a backticked artifact path it cannot find, so naming the ADR a milestone *will create* as `` `adr/NNNN-slug.md` `` reddens `main`. Write it un-backticked. A skip-ci PRD commit does not dodge this, it defers it: the failure lands on the next contributor's push (PR #1267 went red on `prds/1265`, 2026-09-12).
  - **The literal skip-ci token, `[skip ci]` with the space, skips CI from anywhere in the commit message, body included.** Quoting it in prose skips the very run meant to prove a fix; write `skip-ci` when referring to it.
- 🔴 **This is a public repo: never post sensitive data in issues, PRDs, commits, or docs.** No real usernames or emails, no internal hostnames, cluster/namespace names, tokens, or customer data. Redact real values to placeholders (`user A`, `example.com`, `<host>`) first. Remediation differs by surface: a leaked credential must be rotated (removal alone does not help); a value committed to a doc needs a history rewrite; a value in an issue/PR needs delete-and-refile, since editing it out does not erase GitHub's edit history.
...
- **Builtin agent templates**: `api/internal/agenttmpl/builtins/*.md` is the single source of truth for the twelve builtin product roles (`lead` plus eleven subagents), `go:embed`-shipped and boot-seeded into the DB. Parse/validity tests guard them, not a byte-match against another directory. `.claude/agents/*.md` is this repo's dev-team roster: decoupled, free to drift, and product changes must never touch it (`lead` lives only in `builtins/`).
  ...
  - **Change a dev-team role upstream, then sync.** Edit the role in the vtmocanu/skills `roles.yaml` (bump its `version:`, cut a skills release), refresh the installed skills, then run the agent-team `sync.py apply <role>` here. Land the synced `.claude/agents/*.md` by a direct push to `main` or a `[skip-cr]` PR. Never hand-edit a role's generic body here; only its `## For this repo` tail is local.
- **Docs**: `docs/*.md` need leading-fence frontmatter (`title`, `order`, `audience`); `audience: user` pages render in-app at `/docs/:slug` for everyone and `audience: operator` pages for admins only (`web/src/lib/docs.ts`), the other audiences are repo-only. `web/scripts/check-docs.mjs` (runs in `npm run build` and as `task check-docs:web`) fails on bad frontmatter, an `order` duplicated within its audience (`user` and `operator` each keep their own namespace), broken relative links, and a backticked `prds/…`/`adr/…` path that does not exist, in `docs/`, `prds/**` and `adr/` alike (see the PRDs bullet). See `docs/README.md`.
...
- **Three project skills split the run lifecycle**: `uzi-watcher` dispatches an issue, steers the plan gate, and recovers a lost run's work; `uzi-lander` takes over any run or PR after that and lands it (review bots, CodeRabbit rate limits, uzi's `mr_rework`, local fixes, rebase and migration renumbering, admin merge, post-merge CI), one status line per state change, and also lands the whole open-PR set, Renovate included; `uzi-release` only cuts the release. The review, merge and CI-watch mechanics have one home, `.agents/skills/uzi-lander/`; the other two point there.
- **Cutting a release is driven inline by the lead** via the `uzi-release` skill's scripts (`release-cut.sh` / `release-watch.sh` / `release-verify.sh`); read `.claude/agents/release.md`, the mechanics reference, before tagging. `deploy/README.md` is still GitLab-worded and is not the authority for publish mechanics; `.github/workflows/release.yml` is.
  - **RC-first (PRD #1265, ADR-1265).** Releases are cut as candidates `vX.Y.Z-rc.1` by default and promoted to stable from the candidate's own commit in lockstep with the next candidate. `release-cut.sh <X.Y.Z> [--promote|--promote-only|--skip-promote|--stable] [--changelog-file <draft>]` is a tag-derived state machine: it DERIVES the coverage base itself (no `<prev-tag>` positional; `--prev-tag` overrides), folds `[Unreleased]` into the stable-keyed `## [X.Y.Z]` section, bumps `Chart.yaml`, autobumps the worker tag if the agent runtime changed (the pin may legitimately stay at a PUBLISHED RC tag inside a stable chart, D11; an unpublished pin is refused by the release guards), commits direct to `main`, and runs the oracle. Default cuts the next RC or refuses (exit 3, with the facts) when a lower RC is in flight; `--promote` tags the stable locally from a throwaway release branch, then cuts the next RC; `--promote-only <in-flight base>` tags the stable the same way and stops (no next RC, `main` untouched, push the stable tag only), for when `main` carries work not wanted in a candidate yet; it skips the rest of this sentence (nothing lands on `main`, so there is no `main` push, no `ci.yml` wait and no candidate tag). For every other verb, push `main`, watch `ci.yml` green (`watch-run-ci.sh`, background), tag the candidate `vX.Y.Z-rc.1`, and push the tag with the user's go-ahead (without one the harness classifier blocks a `v*` tag push; then hand them `! git push origin <tag>`). **On a promote the push order is the stable tag, then `main`, then the RC tag.**
  - **A `vX.Y.Z` stable tag publishes images, chart, Release-marked-latest and the Homebrew formula unattended; a `vX.Y.Z-rc.N` tag publishes images + chart + a pre-release Release, and only the separate opt-in `uzi-cli-rc` formula (PRD #1378), never the stable `uzi-cli` one. There is no approval gate.** Access control is the `protect-release-tags` ruleset, so only a repo admin can create a `v*` tag and the push itself is the authorization.
  - `release-watch.sh` watches the publish in the background (every tag waits on `release.yml` and `brew.yml`: an RC publishes the `uzi-cli-rc` formula, a stable `uzi-cli`), auto-rerunning a transient publish flake up to twice. `release-verify.sh` then confirms five image tags plus the chart on GHCR, cosign signing proven from the publish job log (cosign 3.x signs via the OCI referrers API, not a `sha256-<digest>.sig` tag, so do not grep the tag list for `.sig`), and the Release channel (a stable is marked latest; an RC is flagged pre-release with `releases/latest` unchanged).
