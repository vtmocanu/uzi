...
function makeRig(canaries: CodexCanaries, opts: { responder?: Responder } = {}): Rig {
  ...
  const deps: CodexExecutorDeps = {
    launchProviderRoot: async (_spec, credential): Promise<CodexLaunchRootResult> => {
      // The FRESH credential rides into the launcher here (and NOWHERE else). Stash it so a
      // test asserts BOTH that it reached the launcher AND that it never rode any other surface.
...
function makeExecutor(rig: Rig, binding: CodexBinding, log: Logger): InstanceType<typeof CodexExecutor> {
...
function surfacesOf(rig: Rig, emitted: EmittedMessage[], log: RecordingLog): Surfaces {
  return {
    emitted: JSON.stringify(emitted),
    // The provider-visible prompts/requests are the params the harness put on the transport
    // (thread/turn start + every child request). The credential is NOT among them.
    providerRequests: JSON.stringify(rig.transport.requests),
    ...
    commandState: JSON.stringify({ commands: rig.spawnCommandCalls, fileop: rig.fileopSpawns }),
  };
}
/** Assert the two INJECTED secrets (the fresh provider credential + the claim capability) are
 *  absent from all four public/provider/log/command surfaces. */
...
describe("codex-m3b packaged lifecycle (injected fakes)", () => {
  ...
  it("new-root resume → implement: a clean turn returns the branch, releases a FRESH token, and keeps every canary off all four surfaces", async () => {
    ...
    // the executor resumes the prior session and drives one implement turn.
    rig.transport
    ...
    // The fresh credential rode into the launcher — and NOWHERE else.
    assert.equal(rig.transport.credential, canaries.credential, "the fresh credential reached the launcher env");
  ...
  it("plan gate (dark): with no pre-approval the plan turn fails closed — submit_plan folding is not wired in m3, so no plan is produced (documents the dark state)", async () => {
    counts.tests += 1;
    const canaries = codexCanaries();
    const rig = makeRig(canaries);
    // A plan turn that streams a root submit_plan then completes. The Codex harness does NOT
    // fold signals into the reduced result yet (foldSignals is unreached for a Codex turn), so
    // planResult.plan is undefined and the executor fails closed rather than gating on nothing.
    rig.transport
      .push(threadStarted())
      .push(toolCall(1, "submit_plan", { plan: canaries.planArg }, "th-1", "tn-1", "c-plan"))
    ...
    await assert.rejects(
      withTimeout(makeExecutor(rig, bindingOf(subscriptionBlock(canaries, "t")), log.log).run(ctx), 5000, "plan run"),
      /produced no plan/,
    );
    assert.equal(gated, false, "the gate is never reached because the plan turn produced no plan");
    // Even on the failed-closed path the credential/capability never leaked.
...
describe("codex-m3b packaged lifecycle (real launch → loopback provider)", { skip: PACKAGED ? false : "image-only (set CODEX_M3B_PACKAGED=1 inside the worker image)" }, () => {
  it("drives the packaged CodexExecutor through the REAL supervisor/fileop/codex against the loopback fake, keeping the credential/capability canaries off every public surface", async () => {
    // 🔴 MAINTAINER-VERIFIED. Authored best-effort: the exact real app-server item framing is
    // unconfirmed (codex-harness.ts marks the item types provisional), so this asserts the
    // SECURITY-critical invariant that holds regardless of how the tool sequence plays out —
    // the injected credential/capability never reach a public/log surface — plus that the real
    // Codex actually reached the loopback (proving the baked binaries + launch wiring). The
    // outer run-lifecycle.sh watchdog bounds a wedged real process.
    const { FakeProvider, lifecycleResponder, codexCanaries: freshCanaries } = await import("./fake-provider.js");
    const canaries = freshCanaries();
    const fake = await FakeProvider.start({ credential: canaries.credential, respond: lifecycleResponder(canaries) });
    try {
      const rig = makeRig(canaries); // reused ONLY for its fake client + session store seams
      // Drive the REAL launcher: drop the injected launchProviderRoot/transport so the packaged
      // defaultLaunchProviderRoot runs the real supervisor + Codex, pointed at the loopback.
      const realDeps: CodexExecutorDeps = {
        sessionStore: rig.deps.sessionStore,
        idleMs: 15000,
        wallMs: 45000,
        boundaryDeadlineMs: 2000,
      ...
      const exec = new CodexExecutor(
        ...
        { binding: bindingOf(subscriptionBlock(canaries, "claim")), client: rig.client as never, provider: loopbackProvider },
        realDeps,
      );
      const { ctx, emitted } = makeCtx({ worktreePath: worktree, planApproved: true, approvedPlan: "plan" });
      let ran = true;
      try {
        await exec.run(ctx);
      } catch {
        ran = false; // a non-clean terminal is acceptable here; the invariant below is the point
      }
      // The real Codex authenticated with the fresh credential and reached the loopback.
      assert.ok(fake.requests.length >= 1, "the real app-server reached the loopback fake provider");
      assert.deepEqual(fake.errors, [], "the loopback fake saw no auth/transport errors");
      // The credential/capability never rode a public message or a log line.
      assert.doesNotMatch(JSON.stringify(emitted), new RegExp(escapeRe(canaries.credential)));
      assert.doesNotMatch(JSON.stringify(emitted), new RegExp(escapeRe(canaries.capability)));
      assert.doesNotMatch(log.lines.join("\n"), new RegExp(escapeRe(canaries.credential)));
      console.log(`CODEX_M3B_PACKAGED_REAL requests=${fake.requests.length} ran=${ran}`);
    } finally {
      await fake.close();
    }
  });
