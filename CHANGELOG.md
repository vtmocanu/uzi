...
- **Codex now has its own restricted-tier egress allowlist entries, and its docs reflect the split ([#1106](https://github.com/vtmocanu/uzi/issues/1106), [#1623](https://github.com/vtmocanu/uzi/issues/1623)).**
  The chart's restricted-tier worker egress allowlist and its CI render assertion now carry `api.openai.com`, `chatgpt.com` and `auth.openai.com` alongside the existing `*.anthropic.com` entry, fleet-wide since the policy is namespace-level with no per-harness split; ADR-0285's tier table records the addition. User docs now describe truthful metered/subscription/unreported Codex cost accounting, that the run judge follows the reviewed run's own harness and credential (with the Anthropic-only token/model pickers explicitly scoped), the `no_usable_credential` schedule-skip reason, `uzi run/schedule create/edit --harness`, and the widened worker egress set.
