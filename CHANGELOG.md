...
- **GitLab merge requests no longer show as review-approved when nobody approved them.**
  The forge-view pulls screen trusted GitLab's `approved` flag, which is vacuously `true` on Enterprise Edition (gitlab.com and the default self-managed build) when a project has no approval rules, so every clean merge request banded READY; the driver now requires at least one real approver, matching the GitHub/Forgejo behavior and the `approved` review-decision contract.
