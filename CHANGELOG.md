...
- **uzi's three PriorityClasses now rank the database above the control plane above the hosted workers, and ArgoCD replaces them in place when a value changes (a7782441).**
  The order used to be workers 1000, control plane 999, database 998, so on a full node a Pending hosted worker could evict uzi's own api or the single Postgres instance and take every run down with it. It is now database 1002, control plane 1001, workers 1000: a worker is disposable and its run requeues, the api and the database are not. Because `value` and `preemptionPolicy` are immutable on a live PriorityClass, all three class templates carry `argocd.argoproj.io/sync-options: Replace=true`, so the upgrade deletes and recreates each class instead of wedging on a rejected patch; running pods keep the priority recorded at admission and no manual cluster step is needed. Operators who tuned the values keep their overrides.
- **Dependency updates.**
