...
## 276. uzi CLI — in the api module at `api/cmd/uzi/`, importing only stdlib-leaf `apitypes`
...
- **The CLI imports only leaf packages; the shared DTOs live in a stdlib-only `api/internal/apitypes/`
  ...
  **no `pgx` and no `chi`** (Success Criterion 8). Handlers serialize the `apitypes` structs; a broken
  ...
  - **The token-metadata DTO is the one exception: it lives in `handler/cli_tokens.go`, not
    `apitypes`.** It is never decoded by the CLI (token CRUD is cookie-only, §278), so it earns no place
    in the shared leaf.
...
- **Command tree** (noun-verb), current realization:
  ...
  uzi worker list|rm            # NO create — minting a join token is a webui action (§278)
  uzi repo list
  ...
  endpoints are cookie-only (§279), so there is nothing to call.
...
- **`uzi logout` is local-only** (deletes the stored credential, points at the webui to revoke
  server-side) — a consequence of the token CRUD being cookie-only (§278, PRD #64 D16).
...
## 277. `RequireUser` — parse-on-Bearer dispatch (never fallback), and the `IsAdmin=false` scope ceiling
...
## 278. Route disposition — enumerated per-route, never a wholesale group swap
...
## 279. Admin over the CLI — read/write split by ROUTING, reach gated by SCOPE (both required, orthogonal)
...
- **`RequireAdminRO` is JUST `user.IsAdmin`** — do not re-check `scope` there. The `RequireUser`
  masking (§277) already reduced any non-`admin_ro` token to `IsAdmin=false`, so a second scope check
...
## 280. CLI-token storage — two tables, the NULL trap, coarse forensic writes, and the server-set expiry matrix
...
- **`cli_auth_requests`**: `code_challenge` (base64url(S256(verifier))), `client_desc`, `user_code
  ...
  `expires_at NOT NULL` (~5 min). See §281.
...
- **No plaintext token is stored, not even sealed** — minted inside the poll transaction and returned
  once (§281). Rejected: mint-at-approve holding `secretbox(token)` for ≤5 min — legitimate but
...
## 281. Browser-brokered, poll-based auth flow — no loopback listener, claim-first mint, roll-back-not-denied
...
## 282. CLI agent selection is Option B (`--agent-source`/`--exclude-agents`), superseding the PRD's `--agents`; `run review` is read-only
...
## 283. Bundled skill — `go:embed`, content-hash self-upgrade, `.bak` rescue, never-fatal, drift-tested
...
## 284. Release — the CLI rides uzi's `v*` tags; `publish_brew` is one tag-only job; brew builds from source
...
## 285. The `uz[caw]_` credential family is taught to BOTH scrubbers when it is minted (M5)
...
## 286. Config + credentials — hardcoded `~/.config/uzi`, 0600 creds, env override, and a context map from day one
...
## 287. Accepted CLI security residuals — recorded, not fixed
...
- **`last_used_ip` is XFF-spoofable under an over-broad `TRUSTED_PROXIES`, and is forensic-only.** It
  ...
  misleads an investigator at worst — it grants nothing. It rides the coarse ≤1/min write (§280) and
...
- **Password change / logout does NOT revoke CLI tokens** (PRD #64 Decision 6, GitHub-PAT semantics,
  ...
  which is why `revoke-all` + `last_used_ip`/`token_prefix` (§280) are the load-bearing forensics.
