...
numbers continue past PRD #28's #135. Builds on PRD #4 (worker runtime, claim
payload, guardrails), #16 (skills scope+allocation shapes reused here), and #17
(claim plumbing, decoupled builtins). Migrations landed `00044`–`00048` (live head
...
## 136. Worker image templates in git (agent + compose)
...
- **`workers.template_declared` (UI, at join-token issuance) + `workers.template_reported`
  (register)** (migration `00044`). Register payload gains optional `template`
...
## 137. Devbox provisioning engine in the worker (M3)
...
## 138. Tool profiles + admin allowlist (M4)
...
## 139. Tier-2 repo `devbox.json` opt-in (M5)

Serves human: repo-carried toolchains, safely.

- **`repos.repo_devbox_opt_in BOOLEAN NOT NULL DEFAULT false`** (`00046`); per-repo
...
## 140. Agent template scopes migration + user CRUD (M6)

Serves human plan.md 44 (global/user agents), extended from skills to the templates.

- **`agent_templates` grows `scope` (builtin|global|user) + `user_id`** (`00047`),
...
## 141. Template allocation + claim filtering (M7)

Serves human: per-agent allocation (which templates ride which runs).

- **`agent_template_allocations`** (`00048`), same shape as `agent_skill_allocations`
