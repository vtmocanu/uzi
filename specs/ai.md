...
## 188. Worker ↔ run concurrency model — ADR-42 (PRD #42, landed)
...
## 223. run_usage snapshot table + greatest-wins rollup centralized in a VIEW

- **Storage** (`00062_run_usage.sql`): `run_usage(run_id FK ON DELETE CASCADE, session_id, model,
...
- **Every rollup reads a `run_usage_totals` VIEW** (`00063`) that computes `MAX` per `(run_id, model)`
...
## 224. Per-agent attribution is client-derived per-call usage — a different data path
...
## 225. Client surfaces derive from the stream; one shared formatter; strip vs list-row can diverge
...
## 226. M6 — the isolated e2e stub stands in for the SDK's usage frames
...
# PRD #70 — Status favicon (brand mark + live tab-icon status badge)
...
- **What.** A brand favicon (the ember `FactoryIcon` mark) plus a client-side dynamic status overlay
  that badges the tab icon from the *signed-in user's own* run states + unread notifications. Purely
  presentational and per-session; no server involvement.
- **Files.**
  - `web/public/favicon.svg` — the stroked ember mark (base icon).
  - `web/public/favicon-32.png` — Safari <26 fallback, generated by `web/scripts/gen-favicon-png.mjs`
    (a pure-Node zlib PNG encoder, no image deps). The PNG is a **filled-silhouette** rendition of the
    stroked SVG — an intentional small-size deviation (a stroked mark reads poorly at 32px).
  - `web/index.html` — two `<link rel="icon">` tags: PNG first (Safari picks it), SVG `sizes="any"`
    with `id="favicon-svg"` (the element `applyFavicon` swaps at runtime).
  - `web/src/lib/favicon.ts` — pure `deriveFaviconState` / `failedRunIds` (state logic, unit-testable)
    plus DOM `renderFavicon` / `applyFavicon` (canvas → `data:` URL → `<link href>` swap).
  - `web/src/lib/useFavicon.ts` — the poll hook.
  - Mounted **once** in the top-level `AppShell` component.
- **Priority ladder: `failed > attention > running > idle`, first match wins.** A single derived tier
  drives the badge dot color; the highest-priority live signal wins.
- **`failed` is session-scoped to FRESH failures.** The hook captures a baseline of already-failed run
  ids on its **first poll**; only failures appearing after that baseline redden the tab. A stale
  historical failure never sticks red nor masks a live gate, and **red clears on reload**.
- **A deliberate stop never reddens.** `isStoppedRun` (status `cancelled`, or `failed` with a
  `stop_kind`) is excluded from the failed tier — consistent with the cross-surface deliberate-stop
  neutral styling (§74).
- **Running tier = `queued` | `claimed` | `running`** — all three count as running-tier.
- **Theme-independent: always ember.** The favicon does not track `data-theme`; one brand color.
- **No animation — static dot.** A background rAF loop would burn throttled cycles for a tab the user
  is not looking at; the badge is a static overlay.
- **No new endpoint / no second poll.** Reuses `listRuns` + the shared unread count already polled
  elsewhere; the `attention` tier keys on unread notifications.
- **Runs poll runs WHILE THE TAB IS HIDDEN** — plain `setInterval`, deliberately NOT
  `usePollWhileVisible` (§85), so a run failing on a backgrounded tab still badges the icon; a
  `visibilitychange` handler forces a catch-up poll on re-show.
- **Safari degrades to the static mark.** Safari does not honor live `<link>` href swaps, so it shows
  the brand favicon with no live overlay — accepted, not worked around.
...
- **Decision — the CLI lives in this repo, in the existing api Go module, at `api/cmd/uzi/`
  (cobra tree, one file per noun) + `api/internal/uzicli/` (client, config, authflow, output,
  skill).** *(User reversal of an earlier "separate repo" choice, PRD #64 D11.)*
  - Why: the decisive evidence was multica's same-repo CLI importing `server/internal/…` **directly**
    for a compile-time contract with no public package, no pseudo-versions, no `GOPRIVATE`, no
    `vendor/`. A separate module is not merely worse but **impossible without that ceremony** — a
    module rooted below `api/` cannot import `api/internal/…` under the Go internal rule. Counter-case
    answered: PRD #58's `controller/` is a separate module *on purpose* (keeps the kube client out of
    `api/go.mod`, and deliberately imports no api internals); the CLI is the inverse on both counts.
  - Accepted cost: cobra now lives in `api/go.mod`, so a cobra bump re-runs the api's gates — real but
    small against a k8s client tree.
- **The CLI imports only leaf packages; the shared DTOs live in a stdlib-only `api/internal/apitypes/`
  leaf, guarded by a build-time layering assertion.** In-module the compiler carries the contract for
  free, so `apitypes` exists for **binary hygiene**, not contract-carrying: importing `internal/handler`
  for a DTO would drag chi + pgx into a CLI binary. `go list -deps ./cmd/uzi` (a test) must contain
  **no `pgx` and no `chi`** (Success Criterion 8). Handlers serialize the `apitypes` structs; a broken
  CLI-consumed field fails `validate:api` in the same MR (Criterion 7 — the whole point of same-module).
  - **The token-metadata DTO is the one exception: it lives in `handler/cli_tokens.go`, not
    `apitypes`.** It is never decoded by the CLI (token CRUD is cookie-only, §279), so it earns no place
    in the shared leaf.
- **Zero new CI jobs.** `api/cmd/uzi` sits under the existing `.go_job` (`cd api`); `validate:api`
  (`go vet && go build ./...`) and `test:api` (`go test ./...`) cover it automatically, and it
  auto-joins `.gate_needs` (Criterion 8).
- **Command tree** (noun-verb), current realization:
  ```
  ...
  endpoints are cookie-only (§280), so there is nothing to call.
- **`run approve/reject/cancel/follow-up` all map to the one endpoint `POST /api/runs/{id}/inputs`**
  with `kind ∈ {approve_plan, reject_plan, cancel, follow_up}`; the server validates the agent
  `selection` against the run's real roster.
- **`uzi logout` is local-only** (deletes the stored credential, points at the webui to revoke
  server-side) — a consequence of the token CRUD being cookie-only (§279, PRD #64 D16).
- **Output**: human tables on a TTY, `--json` for agents. A non-TTY stdout disables colour/spinners
  and honours `NO_COLOR`, but **does not auto-switch to JSON** — a pipe silently changing format is a
  footgun; agents pass `--json` explicitly.
- **Exit codes** (documented in SKILL.md so agents branch without parsing prose):
  `0` success · `1` generic · `2` usage · `3` auth required/invalid/wrong scope · `4` not found ·
  `5` conflict (e.g. run finished) · `6` server unreachable / 5xx. The classifier defaults an unknown
  error to **1**, detects cobra usage errors explicitly, and maps network errors to **6**; the
  `HTTPClient` returns only typed `*ExitError`, pinned by a test.

## 278. `RequireUser` — parse-on-Bearer dispatch (never fallback), and the `IsAdmin=false` scope ceiling

Serves human: Feature #64 headless-agent requirement (Success Criterion 1 — an agent drives uzi with a
Bearer token, no browser/cookie/`$HOME`) and Criterion 4 (a token's authority is capped by its scope).
PRD #64 Decisions 1, 6, 7 (as amended); realized in `api/internal/middleware/cli_auth.go`.

- **Decision — one new middleware `RequireUser` populates the SAME `userKey` every existing handler
  already reads.** No parallel route tree, no handler rewrites. It composes *above* an unmodified
  `RequireAuth`, so the existing browser-path tests still pin the cookie flow byte-for-byte.
- **Dispatch is a single explicit branch on the credential PARSING as `Bearer <non-empty>`, never a
  fallback-on-failure:**
  ```
  Authorization parses as Bearer <non-empty>  → CLI-token path ONLY (sha256 → cli_tokens → users;
                                                 no CSRF; cookie ignored)
  otherwise                                   → existing RequireAuth cookie path, byte-identical
                                                 (CSRF enforced on writes)
  ```
  - Why not "try cookie, on failure try Bearer" (or the reverse): that is the classic CSRF-bypass
    shape — an attacker adds a junk `Authorization` header to skip the CSRF-checked branch.
    Presence-dispatch makes each request take **exactly one** path. Pinned by a test: a cookie request
    carrying a bogus `Authorization` header is rejected on the bearer path and **never** silently
    falls back to the cookie path.
  - **Precisely: dispatch-on-parse, not on raw presence.** The branch keys on
    `jointoken.FromAuthorizationHeader(...)` returning `ok`; an `Authorization: Basic …` header falls
    to the **cookie** path (still CSRF-enforced, therefore safe). "Presence" is a shorthand that is not
    literally true.
- **The scope ceiling — the correction that makes `scope` real. When the credential is a CLI token
  with `scope != 'admin_ro'`, a COPY of the user row with `IsAdmin=false` is put into the context.**
  - Why it is load-bearing, not cosmetic: `IsAdmin` is consulted by **owner-or-admin handlers outside
  ...
  - **Admin-ness resolves live from the row, never from the credential.** `RequireUser` loads the user
    fresh per request via `cli_tokens.user_id`, exactly as `RequireAuth` does. So demoting an admin
    **instantly neuters their `uza_` token** — no revocation step, no cache to bust. Scope is a
    **ceiling, not a grant**: `admin_ro && !user.IsAdmin` ⇒ refused; `!admin_ro` ⇒ indistinguishable
    from a non-admin **everywhere**, not merely under `/api/admin/*`.
- **Ruled consequence, documented not accidental**: `GET /api/auth/me` over a `uzc_` reports
  `is_admin:false` (it reports *this credential's* effective authority, not the human's résumé), so
  `uzi whoami` shows admin only for a `uza_` token.
- **Retracted claim recorded** (PRD #64 D7 correction): an earlier design held that a `scope` column
  *alone* made the token a ceiling. It did not — with `scope` consulted only by `RequireAdminRO`, the
  trio reads still leaked. The ceiling is real **only** because of the `IsAdmin=false` context copy.
  The two mechanisms (the column + the mask) are one design.

## 279. Route disposition — enumerated per-route, never a wholesale group swap

Serves human: Feature #64 (least-privilege CLI surface); Success Criterion 4. PRD #64 route-disposition
table + Decisions 5, 16, 18, 21. M2 splits existing chi groups so only the enumerated routes get
`RequireUser`.

- **Decision — the swap is decided per ROUTE, not per group.** `handler.go`'s single `RequireAuth`
  group at `:458` also contains PAT-writing, secret-deleting and run-minting endpoints; swapping it
  wholesale would hand a Bearer token `POST /api/forge/connections` (writes a forge bot PAT),
  `DELETE /api/me/secrets/anthropic_token` (no vault gate — a factory kill) and `POST /api/chats`
  (mints token-spending runs). M2 is real surgery in `handler.go` (and the `/auth` + `/runs`
  sub-groups) scoped to exactly the routes below.

  **`RequireUser` (Bearer-reachable):**
  - `GET /api/auth/me` (`whoami`)
  - `GET /api/runs`, `GET /api/runs/{id}`, `GET /api/runs/{id}/messages`, `POST /api/runs/{id}/inputs`
    (list/get/logs/approve/reject/cancel/follow-up)
  - `GET /api/runs/{id}/review` (`uzi run review`, PRD #64 D21 — a read whose payload is more
    machine-actionable than human-readable; adds the third admin-widening consumer to the masked set)
  - `GET /api/repos`, `POST /api/repos/{id}/runs` (`repo list`, `run create`)
  - `GET /api/workers`, `DELETE /api/workers/{id}` (`worker list` / `worker rm` — `DELETE` stays
    swapped deliberately: destroying a worker exfiltrates nothing and the loss is the owner's own;
    the asymmetry with `POST /api/workers` is *mint vs unmint*, not *read vs write*)
  - `/api/admin/*` **9 GETs**, additionally behind `RequireAdminRO`

  **Cookie-only (Bearer rejected at the middleware, structurally):**
  ...
  - **`POST /api/workers`** — mints a plaintext `uzw_` join token whose claim response carries
    **decrypted** `ForgePAT` + `AnthropicOAuthToken`. A non-admin `uzc_` reaching it could exfiltrate a
    ...
    credentials **outlive** revoking the `uzc_`. `uzi worker create` is therefore a webui action
    (PRD #64 D18). This inverts the table's own logic — reading a PAT is worse than writing one, and
    `uzc_`→`uzw_` is **upward** escalation, not lateral.
  - `/api/me/cli-tokens` (all — list/create/delete/revoke-all) — a Bearer-reachable token CRUD lets a
    stolen `uzc_` mint replacements (revocation becomes whack-a-mole) and lets an admin's stolen
    user-scope token mint a `uza_`, escalating past the ceiling (the mint keys off the *user*, not the
    presenting credential's scope). PRD #64 D16; this is why `uzi logout` is local-only.
  - `/api/vault/*` (the password surface), `/api/me/secrets/*`, `/api/forge/*`, `/api/admin/*` **4
    writes**, and every other per-user surface with no v1 verb (`/api/chats/*`, `/api/usage`,
    `/api/notifications`, `/api/me/{settings,slack,autopilot,judge,rate-limits}`, `/api/agent-templates`,
    `/api/skills`, `/api/tool-allowlist`) — least privilege: the surface widens when a verb needs it.
  - `/api/ws` — WS-follow is deferred; leaving it unswapped keeps `handler/ws.go`'s "runs inside the
    session-authenticated group" comment true.
  - `PATCH /api/repos/{id}` etc. — no v1 verb; also the F1 admin-write path.
- **Limiter ordering is a silent-failure trap.** `POST /api/repos/{id}/runs` is wrapped in
  `forgeLimiter.PerUserMiddleware`, whose contract requires it to run **after** the auth middleware
  that sets the user. It keeps working under the swap precisely because `RequireUser` populates the
  same `userKey` — but it **fails soft**: `PerUserMiddleware` falls back to client-IP keying when the
  user is absent, so a split that leaves a limiter above `RequireUser` collapses every CLI caller into
  one IP bucket with **no failing test**. Keep auth first, limiter second.

## 280. Admin over the CLI — read/write split by ROUTING, reach gated by SCOPE (both required, orthogonal)

Serves human: Feature #64 admin read verbs (PRD #64 D5 — user override of the architect's "exclude
admin entirely"); Success Criterion 4 admin surface. Realized as a two-group `/admin` route split +
`RequireAdminRO`.

- **Decision — routing enforces read-only, scope enforces which tokens reach admin at all; both are
  required.** The `/admin` route splits into a **reads** group (`RequireUser` + `RequireAdminRO`,
  9 GETs) and a **writes** group (`RequireAuth` + `RequireAdmin`, cookie-only, 4 writes:
  `PATCH /users/{id}`, `PUT /users/{id}/judge`, `PUT /settings`, `PUT /selfimprove`).
- **"A read-only-admin token reaches a write handler" is structurally impossible, not policy-checked**:
  the write group's `RequireAuth` is cookie-only, so a Bearer request is rejected by the middleware
  chain before any handler exists to hold a flag.
- **`RequireAdminRO` is JUST `user.IsAdmin`** — do not re-check `scope` there. The `RequireUser`
  masking (§278) already reduced any non-`admin_ro` token to `IsAdmin=false`, so a second scope check
  would (a) force `RequireUser` to export `scope` into context and (b) risk drifting from the mask.
  One mechanism, one place.
- **Live demotion applies here too**: `RequireAdmin`/`RequireAdminRO` read `IsAdmin` from the
  per-request-loaded user, so setting an owner's `is_admin=false` makes their `uza_`'s admin reads fail
  on the very next request.

## 281. CLI-token storage — two tables, the NULL trap, coarse forensic writes, and the server-set expiry matrix

Serves human: Feature #64 credential model; Success Criteria 3, 4, 9. PRD #64 Storage section +
Decisions 7, 8, 10, 19. Migrations `00067_cli_tokens.sql` / `00068_cli_auth_requests.sql` (numbers
assigned at merge above the live head `00066`, per the goose convention).

- **`cli_tokens`**: `token_hash bytea UNIQUE` (sha256 of `uzc_…`/`uza_…`, mirrors `workers.token_hash`),
  `token_prefix text` (4 body chars, e.g. `uzc_a1b2` — 24 bits, leaves 2^232 of the token, enough to
  identify a row), `scope ∈ {user, admin_ro}`, `revoked bool` (soft delete — keeps the incident trail
  and permanently poisons the unique hash against reuse), `last_used_at`, `last_used_ip inet`,
  `expires_at` (nullable, server-set). Index `(user_id, revoked)`.
  - Why `token_prefix` + `revoked` (diverging from `workers`, which stores `token_hash` alone and
    hard-deletes): a worker is disposable infrastructure; a CLI token is a human-held credential with
    an incident story. Lifted from multica's `personal_access_token`.
- **`cli_auth_requests`**: `code_challenge` (base64url(S256(verifier))), `client_desc`, `user_code
  UNIQUE`, `status ∈ {pending, approved, denied, consumed}`, `user_id` (set at approve), `scope`,
  `expires_at NOT NULL` (~5 min). See §282.
- **The NULL trap (fail-closed but exactly on the agent path).** The lookup MUST be
  `WHERE token_hash=$1 AND NOT revoked AND (expires_at IS NULL OR expires_at > now())`. A bare
  `expires_at > now()` is NULL — hence false — for precisely the never-expiring webui-minted `uzc_`,
  i.e. it would silently reject the agent/CI token the NULL exists to protect.
- **`last_used_at` + `last_used_ip` are coarse: one write, both columns, at most once per token per
  minute** (skip when `last_used_at > now() - interval '1 minute'`). A write per request would put a DB
  write on every CLI call against a single-replica api. `last_used_ip` uses `ClientIP(r,
  trustedProxies)` (the same helper the limiters use, so `TRUSTED_PROXIES` semantics are not
  re-invented) and is therefore **zero-marginal-cost**. Accepted precision limit: at ≤1/min it records
  *an* IP in the window, not every IP — a **signal for a human deciding to revoke**, not an audit log.
- **`cli_auth_requests` needs a sweeper**: opportunistic `DELETE … WHERE expires_at < now()` and/or
  fold into the existing stale-run sweeper.
- **`user_code` shape**: 8 chars of Crockford base32 (`0-9A-Z` minus `I L O U`), rendered `XXXX-XXXX`,
  ~40 bits. It is a **display + typing** confirmation checked *alongside* a `request_id` the client
  already holds — NOT a lookup key — so 40 bits suffices. **Invariant recorded so nobody crosses it:**
  if anyone later adds a canonical device-flow entry ("enter your code at /device"), the code becomes a
  lookup key and its entropy becomes the whole boundary — that endpoint would need its own rate limit
  and 40 bits reconsidered. `UNIQUE` makes a collision a loud insert failure; retry the insert with a
  fresh code on a unique violation rather than 500.
- **Expiry matrix — "90 days, unless it is a webui-minted `user`-scope token", and the SERVER sets
  every lifetime** (a client choosing its own credential lifetime is backwards — multica's mistake):

  | Token | Mint path | `expires_at` | Why |
  |---|---|---|---|
  | `uzc_` (user) | webui static | **none (NULL)** | the agent/CI path — a token dying mid-pipeline is the one footgun it cannot absorb |
  | `uzc_` (user) | browser (`uzi login`) | **90 days** | human-held; re-running `uzi login` is trivial, so a bounded lifetime is free security |
  | `uza_` (admin_ro) | either | **90 days** | scope wins over acquisition path — the only control bounding a *stolen* factory-wide-read token |

- **No plaintext token is stored, not even sealed** — minted inside the poll transaction and returned
  once (§282). Rejected: mint-at-approve holding `secretbox(token)` for ≤5 min — legitimate but
  strictly worse, since this way a DB dump contains no CLI token in any form.
- **`/api/me/cli-tokens` list returns `token_prefix` + `last_used_at` + `last_used_ip` (the entire
  forensic surface, not decoration)** and carries a **`POST …/revoke-all`** panic button
  (`UPDATE … SET revoked=true WHERE user_id=$1 AND NOT revoked` — one query over the existing index;
  idempotent). With no per-request audit log and `token_version`-exemption both ruled in, explicit
  revocation is the *only* remaining control, so "revoke each one" must be one button, not an N-click
  enumeration (PRD #64 D19). Envelopes (M2-chosen, web must match): list `{"tokens":[…]}`; mint (POST)
  `{"token":"<plaintext once>","cli_token":<metadata dto>}` (mirrors `CreateWorker`).

## 282. Browser-brokered, poll-based auth flow — no loopback listener, claim-first mint, roll-back-not-denied

Serves human: Feature #64 human login path (Success Criterion 2 — `uzi login` works on a password-only
stack AND an OIDC stack with no IdP configuration). PRD #64 auth-flow section + Decisions 2, 3, 17,
15(f). Endpoints `/api/auth/cli/{start,poll,request,approve,deny}`; realized in `handler` + M5.

- **Decision — the CLI is NOT an OIDC client; it brokers a token off uzi's existing session
  chokepoint**, which password AND OIDC both converge on. So it inherits every current and future login
  method with no IdP client registration. Rejected alternatives: direct PKCE to the IdP (needs a second
  client, a new "accept IdP assertion as login" trust surface, and is dead on OIDC-disabled compose
  stacks) and round-tripping uzi's callback (reopens the open-redirect PRD #45 closed on purpose).
- **Decision — poll, not a loopback listener** (PRD #64 D3, decided against multica's own source). A
  listener needs outbound-IP detection, `--callback-host` escape hatches, `tcp4` pinning for a macOS
  IPv6 bug, `ssh -L` instructions (it breaks over SSH), binds `0.0.0.0` on LAN, and receives the token
  as a URL query param over plaintext HTTP — and multica's `validateCliCallback` accepts any RFC 1918
  address, handing a full session JWT to any private-network address in the URL. **Polling deletes
  every line of that**, works over SSH and in containers, and no token ever rides a URL.
- **The flow**: CLI generates `verifier = rand(32)`, `challenge = S256(verifier)`; `POST /cli/start
  {challenge, client_desc}` → `{request_id, user_code, expires_in, interval}`; CLI prints the
  `user_code` and opens `<frontend>/cli-auth?request=<id>`; the browser (RequireAuth — password OR OIDC
  login happens HERE) shows the consent screen; the user **types** the `user_code`, picks scope, and
  `POST /cli/approve {request_id, user_code, scope}` (RequireAuth + CSRF + per-user limiter) marks the
  row approved and **mints nothing**; the CLI polls `POST /cli/poll {request_id, verifier}` until the
  mint returns `{token, user}` once.
- **The mint is claim-first, not read-then-write** — "inside a transaction" is NOT sufficient under
  READ COMMITTED (two concurrent polls both read `approved`, both mint ⇒ two tokens from one approval,
  invisible to a single-threaded test). The conditional
  `UPDATE cli_auth_requests SET status='consumed' WHERE id=$1 AND status='approved' AND expires_at>now()
  RETURNING code_challenge, user_id, scope` **is** the guard (the same pattern as run claims'
  `FOR UPDATE SKIP LOCKED`); mint only on a returned row, zero rows ⇒ 410. Verify
  `S256(verifier)==code_challenge` in the same tx and **roll back on mismatch**, leaving the row
  untouched.
- **On mismatch: roll back, mark NOTHING `denied`, return 4xx, `slog.Warn`** (PRD #64 D15(f),
  retracted-then-corrected). "Roll back" and "mark denied" cannot both happen — the rollback undoes the
  marking. And marking buys nothing (the verifier is 32 random bytes, so there is no brute-force to
  harden) while it **costs**: the `request_id` is not a secret (it rides the browser URL, history and
  access logs), so anyone who learns one could poll with a junk verifier and kill a victim's in-flight
  `uzi login`. Rolling back keeps the legitimate poll alive.
- **`GET /cli/request/{id}` (RequireAuth cookie-only) returns ONLY `{client_desc, status,
  expires_at}` — deliberately NOT `user_code`**: the human must TYPE the code from their terminal;
  auto-fill would defeat the anti-async-phishing property, and typing (not merely comparing) interrupts
  the approve-the-tab reflex that auto-opening a browser trains. `status` reports `expired` for a
  stale-but-pending row. `approve` errors: 400 code-mismatch, 403 non-admin picking `admin_ro`, 404
  unknown, 409 not-pending, 410 expired. `deny {request_id}` (CSRF) → 200 / 409 not-pending.
- **`poll` gets its own limiter — a correctness requirement, not tidiness.** `authLimiter` is 10/min
  keyed on (path, client IP); an RFC 8628 poll at a 5s interval is 12/min, so `uzi login` would trip
  its own rate limit at poll #11 and every caller behind one NAT shares the bucket. Hence a dedicated
  `cliPollLimiter`, sized so the server-returned `interval` cannot exceed it. New env keys:
  `CLI_POLL_RATE_LIMIT_MAX` (default 60) / `CLI_POLL_RATE_LIMIT_WINDOW` (default 1m), in
  `api/internal/config/config.go`. `Routes()` gains the limiter parameter.
- **`approve` rides the per-user limiter** (`authLimiter.PerUserMiddleware`): making the user *type*
  the `user_code` turned it into a credential-checking endpoint, the same shape uzi already
  rate-limits for vault unlock.
- **`poll` is POST, not GET**: the verifier must never land in access logs.

## 283. CLI agent selection is Option B (`--agent-source`/`--exclude-agents`), superseding the PRD's `--agents`; `run review` is read-only

Serves human: Feature #37 (either/or agent source with per-agent exclusions, no mixing) as reached
through the CLI; Feature #64 (`uzi run review`, PRD #64 D21). Reconciliation of the PRD command-tree
line, recorded per the phase-2 follow-ups.

- **Decision — the PRD's `uzi run … --agents` flag does NOT exist as built; the CLI ships Option B:
  `--agent-source own|repo` + `--exclude-agents a,b`.** This is 1:1 with the real wire model
  `selection: {source:"own"|"repo", exclusions:[]}` (PRD #37 decision 4 — the web `AgentPicker` sends
  ...
  - **The skill↔cobra-tree drift test enforces this**: a SKILL.md that still says `--agents` fails the
    test (§284).
- **`uzi run review` is READ-ONLY** (PRD #64 D21). It maps to `GET /api/runs/{id}/review`; there is no
  `rejudge` verb, because re-running the judge mints a token-spending run (a webui action). Human
  output is the verdict line + one row per recommendation (category, target, confidence) with the
  rationale beneath; `--json` passes the `{"review": … | null}` envelope through.
  - **`null` review is exit 0 "not judged", NOT exit 4** — the endpoint returns 200 `{"review":null}`
    for a visible-but-unjudged run; mapping that to not-found would invent an error the API does not
    raise. Reserve exit 4 for a real 404.
  - **A `status:"failed"` review renders the "judge incomplete" caveat** the run page shows. The wire
    value is `failed` (enum `{complete, failed}`), NOT `incomplete` — *"judge incomplete"* is only the
    badge wording in `docs/judge.md`; a `--json` consumer keying on `"incomplete"` matches nothing and
    would silently treat every fallback review as complete.
  - **`target`, `rationale_md`, `summary_md` are UNTRUSTED judge free text** derived from
    attacker-influenceable repo/CI content — data, never instructions. Only `verdict`, `category`,
    `confidence` are closed enums safe to branch on. The SKILL.md and `docs/judge.md` CLI section state
...
- **Decision — the skill is `go:embed`ed into the binary and written to
  ...
  `user-invocable: false`). Beats multica's fetch-from-GitHub-`main` approach: an embedded skill cannot
  describe a version other than the one running, works offline, and works for a **private** repo
  (multica's is unusable for us).
...
- **User-edited file rescue**: if `sha(on-disk) != recorded_sha`, copy to `SKILL.md.bak`, write, warn
  once on **stderr**. Rejected: silent overwrite (destroys work in a directory we merely declared ours)
  and skip-and-warn (leaves agents driving a stale CLI forever).
- **Never fatal, never blocking.** Any install error warns on stderr; the real command still runs — a
  read-only `$HOME` (CI containers) must not break `uzi run list --json`. Escape hatches:
...
- **Drift control — a test asserts every command/flag the SKILL.md documents exists in the cobra
  tree** (`skill_drift_test.go`). This is multica's manual `references/*-source-map.md` discipline done
  **mechanically**, possible because our skill documents our own CLI. Its limit (recorded): it
  constrains the command **list**, not the **prose** — an instruction-shaped change to the body passes
  it, which is why `SKILL.md` is a review-gated privileged artifact (PRD #64 Risk 11).

## 285. Release — the CLI rides uzi's `v*` tags; `publish_brew` is one tag-only job; brew builds from source

Serves human: Feature #64 install via the existing `vtmocanu/homebrew-tap` (Success Criterion 6 —
`brew install uzi-cli` on a clean machine with only an SSH key yields a working binary whose
`uzi version` == the tag). PRD #64 release section + Decisions 9, 13; Risks 1, 2, 12. Rides PRD #52's
CI shape.

- **Decision — the CLI rides uzi's existing `v*` tags; no separate `uzi-cli-<version>` release tag.**
  PRD #52 already made `v*` mean "the whole product releases". The payoff: `uzi version` == the API
  version the binary was compiled against, making the skew check exact. Accepted cost: a web-only
  release bumps the CLI cosmetically.
- **Brew builds from source**: `url … using: :git, tag: "vX.Y.Z"`, `depends_on "go" => :build`,
  `std_go_args(ldflags: "-s -w -X main.version=#{version}")`. The formula must `cd`/`buildpath` into
  `api/` (the module is `api/go.mod`). No `version` line (brew audit flags it redundant); Homebrew
  strips the leading `v`, and the CLI stamps `v#{version}` so `uzi version` == the tag.
  `Formula/uzi-cli.rb` in this repo is the **source of truth**; release CI copies it into the tap.
- **No `vendor/`** (PRD #64 D9, reversing an earlier decision): once the CLI imports only its own
  module (D11), the private-module problem that made vendoring near-mandatory is gone — and `go mod
  vendor` is module-wide, so it would drag pgx/chi/goose into the repo and flip the whole module to
  `-mod=vendor`, changing how the api server itself builds. A large invasive change for a problem that
  no longer exists.
- **CI — one new job `publish_brew`** (tag-only, modelled on example-app): `needs: *publish_needs` (atomic
  release — a failing gate blocks the brew publish), `rules: - if: '$CI_COMMIT_TAG'` (SELF-GATED, never
  `workflow.rules` alone). `TAP_WRITE_TOKEN` is injected PROTECTED + MASKED, exactly like the Harbor
  creds, so it is **absent on MR refs** (agent-authored MRs can edit `.gitlab-ci.yml`, and an MR
  pipeline runs the MR's own CI); the self-gate means a `$UZI_CI_DEMO_FAIL=="1"` pipeline never runs it
  (inherited free — it only runs on tags). It bumps ONLY the `tag:` line in the copied formula and
  `mkdir -p tap/Formula` defensively. Zero new validate/test jobs.
- **Brew spike finding (PRD #64 Risk 1, resolved in M4 before Phase 2)**: a from-source Go formula
  installs cleanly under Homebrew's sandbox with **no GOPATH override** — Homebrew points `HOME` at a
  sandboxed temp dir, so `$HOME/go/pkg/mod` (the writable GOMODCACHE) already works. The
  `ENV["GOPATH"] = buildpath/"gopath"` fallback was prepared but is **not needed**; do not add it.
  Accepted `brew audit --strict` finding: "Git should specify revision: when tag:" — documented in the
  formula header as a CI-owned-tag tradeoff, not a defect.
- **Tap provisioning (one-time, release-owner)**: the `vtmocanu/homebrew-tap` default branch must be
...
- **Precondition (PRD #64 Risk 12), invisible in this repo**: `v*` protected-tag creation must stay
  **Maintainer-only** so the Developer-role bot cannot cut a `v*` tag and reach `TAP_WRITE_TOKEN`.
  Blast-radius asymmetry worth carrying: `TAP_WRITE_TOKEN` writes a **shared** tap whose formula
  `install` blocks are arbitrary Ruby executed on every engineer's laptop — worse than a leaked Harbor
  password. Scope the token to the tap repo; prefer a project access token over a personal one.

## 286. The `uz[caw]_` credential family is taught to BOTH scrubbers when it is minted (M5)

Serves human: the primary directive's spirit (no credential leaks) and Feature #64's own instruction to
put `UZI_TOKEN` in GitLab CI. PRD #64 Risk 14 / Decision 18's error class. Realized in
`slacksvc/redact.go` and `handler/ci_fix.go`.

- **Decision — M5 (where the token family is born) extends BOTH redactors with the final shape
  `uz[caw]_[A-Za-z0-9_-]{16,}`** (covering the pre-existing `uzw_` while there), with a test sealing a
  literal `uzc_` through the review ingest and the Slack path.
- Why non-optional and non-deferrable: this PRD *creates* the echo-into-a-trace path — it tells users
  to put `UZI_TOKEN` in CI → a `set -x` job echoes it → the trace enters a run → the judge quotes it
  into `summary_md` → it is served over the newly-swapped `/review` and copied into the Slack DM path.
  Neither `slacksvc.ScrubSecrets` nor `snapshotSecretPatterns` knew the `uzc_`/`uza_`/`uzw_` shape
  before. Introducing a credential family without teaching the redactors is Decision 18's error class
  in a new coat — a rule this repo applies in two places and skipped in the third because it did not
  look like a token surface.

## 287. Config + credentials — hardcoded `~/.config/uzi`, 0600 creds, env override, and a context map from day one

Serves human: Feature #64 headless-agent requirement (agents/CI have no interactive login and often no
`$HOME`, so env must override files). PRD #64 config section. Realized in `api/internal/uzicli/config.go`.

- **`~/.config/uzi/config.toml` (0644) and `credentials.toml` (0600, dir 0700; refuse to read if
  group/world-readable).** `UZI_URL` / `UZI_TOKEN` override the files — mandatory for agents/CI. In
  GitLab CI, `UZI_TOKEN` must be a **masked** variable.
- **The path is HARDCODED and deliberately does NOT honour `$XDG_CONFIG_HOME`.** This is the obvious
  "improvement" a future reviewer will request, and it is a trap: on at least one machine in this team
  `XDG_CONFIG_HOME` points into a **git-tracked, mackup-synced repo**, so honouring it would write a
  never-expiring `uzc_` into version control. If XDG support is ever genuinely wanted, it must refuse to
  write a credential into a directory inside a git worktree.
- **The config is a map of contexts from day one** (`[contexts.default]`) so `uzi context use` is later
  additive, not a file-format migration (the user runs two instances: compose laptop +
  `uzi.example.com`). Rejected: macOS Keychain — cross-platform complexity, and containerised
  agents need the file path anyway, so it would be a second mechanism, not a replacement.
- **There is deliberately no `--token` flag** — it would put the credential on `argv`, readable via
  `ps` and `/proc/<pid>/cmdline`, contradicting `uzi auth token`'s stdin discipline and the worker's
  own "not `git -c`" rule. `UZI_TOKEN` + the 0600 file cover every case, including CI.
- **M7 attaches the credential only over https** (carried finding): `--url`/`$UZI_URL`/config URL must
  be rejected if non-https before a `Bearer <uzc_>` is attached, with a loopback-http exception for the
  compose laptop (127.0.0.1/localhost) — the server's `FORGE_ALLOWED_BASE_URLS` https-only posture,
  applied client-side to stop a cleartext token leak.

## 288. Accepted CLI security residuals — recorded, not fixed

Serves human: Feature #64 credential model; the trusted-team / single-laptop threat model uzi assumes.
PRD #64 Risks 8, 10 + phase-2 security-audit findings. Consistent with §210's "operability aid, not a
guardrail" posture for forensic controls.

- **`POST /api/auth/cli/deny` has no owner binding.** On a multi-user instance, an authenticated peer
  who learns a pending `request_id` (~5-min window) can cancel a victim's not-yet-approved login (the
  victim re-runs; no credential is exposed). **By design** — there is no owner to bind to before
  approval, and it is not reachable unauthenticated. Optional future hardening: log the denier
  (forensic only; ownership cannot be bound pre-approval). Not a fix.
- **`last_used_ip` is XFF-spoofable under an over-broad `TRUSTED_PROXIES`, and is forensic-only.** It
  is a signal for a human deciding whether to revoke, **never** an auth decision, so a spoofed value
  misleads an investigator at worst — it grants nothing. It rides the coarse ≤1/min write (§281) and
  uses the same `ClientIP` helper as the limiters, so the `TRUSTED_PROXIES` semantics are not
  re-invented.
- **The `admin_ro` blast radius is bounded, not eliminated.** Within its 90-day cap a stolen `uza_`
  reads the whole factory (every user's runs, workers, usage, rate-limits, settings) and **nothing
  detects it** — there is no per-request audit log, only `last_used_at`/`last_used_ip`. Controls that
  make it acceptable under the trusted-team model: least privilege by default (`scope='user'`), live
  demotion, the distinct `uza_` prefix (recognizable to humans and secret scanners), writes
  structurally unreachable, `slog.Info` on every `admin_ro` mint, and the 90-day cap.
- **Real-time consent phishing (PRD #64 Risk 10).** `POST /api/auth/cli/start` is unauthenticated by
  necessity, so anyone can mint a `request_id` and send a logged-in victim the consent URL. The design
  is RFC 8628-conformant, and the RFC is explicit that conformance does not stop a real-time phisher.
  Mitigations: the consent screen names the requesting host and requires the user to **type** the
  `user_code`; the 5-minute window bounds it. The countermeasure that would close it (binding to a
  pre-authenticated session) defeats the point — the CLI has no session.
- **Password change / logout does NOT revoke CLI tokens** (PRD #64 Decision 6, GitHub-PAT semantics,
  user-ruled twice). The accepted consequence, stated in these words in the docs page: *a password
  change is NOT an incident-response control for CLI tokens; you must enumerate and revoke each one* —
  which is why `revoke-all` + `last_used_ip`/`token_prefix` (§281) are the load-bearing forensics.
...
## 289. Second `forgejo.go` driver behind the existing `Forge` interface, full parity (D1/D2)
...
- **Terminology stays internal (D2, user, overruling the architect).** Go types, DB
  ...
  GitLab-flavoured. Only user-visible copy switches per forge (§294).
- **The blocker that deferred this was false (headline correction, see §16/§15).** It
  ...
  evidence.
- **Migration `00069_forgejo_and_mr_web_url.sql`** carries this PRD's schema (mr_web_url,
  §294). Number assigned at landing above the live head `00068` (F4); renumber if a
  sibling lands first.

## 290. Version gate ≥ 16.0.0, in `VerifyToken`, strict semver (D4/D4a)
...
## 291. Client library `code.gitea.io/sdk/gitea` v0.25.1, with two deliberate hand-rolls (D5)
...
## 292. Label writes: full-set replace + accepted lost-update window (D3)
...
## 293. Guardrails on both drivers: branch-read, per-forge scopes, `Role` enum; merge finding = Violation (D6/D6a/D6a-1/D6b/D7)
...
## 294. MR web URL persisted, per-forge UI copy, and the pipeline-status classifier (D8 + D2-web + Feature #6)
...
## 295. Worker forge seam: per-forge REST driver keyed on the claim's `forge_type` (D9)
...
## 296. Connect picker, save-time validation, and the e2e db-flip lane (D11 + D10/D10a)
...
## 306. Filing a forge issue from a judge recommendation is deterministic templating — no second LLM call (D1, D11)
...
- **Works on every recommendation ever judged, with no backfill or re-judge (D11, user question).**
  ...
  no token on old runs any more than new. The category→repo default (§309) reads *current* settings
...
## 307. Two endpoints, split by CSRF/CLI posture; owner-or-admin read, caller-owns-repo write (D2, D8)

Serves human: the fileable-recommendation requirement (§306) plus the user decision to **keep
...
- **Draft and file are separate endpoints.** `GET /api/runs/{id}/review/recommendations/{recID}/
  ...
  - Why a **server** GET rather than rendering the draft in the browser from review data the panel
    ...
    scan (§312), the category→repo resolution (§309, reads settings + the caller's repos), and the
...
- **Residual, out of uzi's hands:** the human-edited *description* can carry GitLab quick-action
  ...
  (§312), not by a keyword blocklist. The `prdless_enabled` instance setting (default on) is a
...
## 309. The target repo is category-derived and always overridable (D4)
...
- **The link lives in its own table, not on the recommendation row.** Migration `00071`
...
## 311. Claim-first concurrency, forge-first tx, created-with-warning, and a clamped stranded-claim sweeper (D7, D9)
...
## 312. Write-boundary sanitizer, re-run server-side on the POST body — breakout-proof fence, CRLF-aware `/`-strip, best-effort secret scan (D10)
...
- **`recommendation_dispositions`** (migration `00073`), UNIQUE `(review_id,
  category, target)` — the identical coordinate `recommendation_filed_issues`
  (PRD #68 `00071`) already uses. `status CHECK IN ('done','dismissed')`,
  `dismiss_reason CHECK IN ('wont_do','not_an_issue')` with a table `CHECK`
  that a reason is present **iff** `status='dismissed'`, `rationale_hash`
  (§333), `set_by_user_id`/`set_at`/`updated_at`. FKs: `review_id` **ON DELETE
  CASCADE** (dies with the review), `set_by_user_id` **ON DELETE SET NULL**
  (an unrelated user's deletion must not delete the row).
- **Two hosts rejected, for two different reasons.** Not columns on
  `review_recommendations`: that row is deleted-and-reinserted on every
  re-judge (`UpsertRunReviewWithRecommendations`, `queries/judge.sql`), so a
  per-row status would be wiped on re-run — the same trap `recommendation_filed_issues`
  exists to dodge (§310). Not columns on `recommendation_filed_issues`: that
  row's lifecycle is entangled with claim-first filing (`filed_issue_iid`/
  `filed_at` nullable, in-flight vs settled), and a disposition routinely
  exists with **no filed issue at all** — dismiss-without-filing and
  done-without-filing are the common cases.
- **Survives the recommendation delete-reinsert for the same reason
  `recommendation_filed_issues` does**: the `run_reviews` row is stable across
  a re-judge (`UNIQUE target_run_id`, upserted not replaced), so keying on the
  review's own id makes the disposition outlive the recommendation rows under
  it. `category` carries no `CHECK` (identical to `00071`): it is always
  copied server-side off an already-`CHECK`-constrained recommendation row,
  never accepted from the request body.

## 332. One Go ladder (`BucketOf`) buckets both counters — deliberately no SQL `CASE`; "filed" means settled (D2)

- A recommendation carries two independent facts on its coordinate — **filed**
  (D#68) and **disposition** (this PRD) — that compose freely (filed-then-done,
  dismissed-without-filing). The one bucket a row falls into is the ladder
  **`dismissed > done > filed > todo`**, implemented **once**:
  `workersvc.BucketOf(dispositionStatus string, filedSettled bool) string`
  (`workersvc/triage.go`), consumed by **both** the per-review DTO
  (`reviewToDTO`, `handler/judge.go`) and the global stats aggregate
  (`JudgeTriageStats`, `workersvc/judge_disposition.go`).
- **Why not a SQL `CASE` mirroring it**: a Go helper cannot back a SQL `CASE`,
  so a SQL-side aggregate would have re-implemented the ladder a second time —
  reintroducing exactly the drift risk a single source of truth prevents. The
  global query (`ListJudgeTriageRowsForUser`, `queries/dispositions.sql`) is a
  plain flat join returning one row per recommendation with its raw
  `disposition_status` + `filed_settled`; `BucketTriage` (`workersvc/triage.go`)
  loops those rows through `BucketOf` in Go.
- **"filed" means a SETTLED link everywhere** — `filed_at IS NOT NULL`, never
  an in-flight claim. The per-review path already skips unsettled claims
  (`reviewToDTO`, `!f.FiledAt.Valid → continue`, mirroring PRD #68's own
  panel-facing filter), and the global join's `(f.filed_at IS NOT
  NULL)::bool AS filed_settled` matches it explicitly — an in-flight or
  stranded claim must not read "filed" globally while the panel shows "to do".

## 333. The stale flag keys on a `rationale_hash`, not a timestamp — sticky across re-judge (D3)

- Because the disposition is coordinate-keyed on a stable review row, a
  done/dismissed disposition survives a re-judge **untouched** — the point:
  dismiss a false positive, re-run the judge, it does not silently reopen.
  What must still warn the human is a re-judge that changes the
  recommendation *underneath* the disposition. Each row stores
  `rationale_hash` (`workersvc.RationaleHash`, `sha256` hex of
  `rationale_md`) at set-time; `reviewToDTO` recomputes the hash of the
  **current** recommendation and flags `Stale` iff it differs
  (`workersvc.RationaleHash(rc.RationaleMd) != d.RationaleHash`).
- **A `set_at < review.updated_at` predicate was rejected** (load-bearing
  correction from review): `UpsertRunReview...` sets `updated_at = now()` on
  **every** conflict, byte-identical re-judge included (`queries/judge.sql`),
  so that predicate means "any re-judge has happened at all" and would
  false-flag every quietly-dismissed row after any re-run — gutting the
  flagship "dismiss, re-judge, stays quietly dismissed" behavior. The hash
  makes an unchanged re-judge produce **no** flag and a changed rationale
  produce one, on both sides of the comparison, testably.
- **Inherits the `target=''` collapse** (PRD #68 §310): several empty-target
  recommendations share one coordinate and thus one disposition and one hash
  (the first such row's) — accepted, identical to the filed case, and
  inherent to the survive-re-judge key.

## 334. Enum-only dismiss reasons — no free-text note in v1 (D4)

- `dismiss_reason ∈ {wont_do, not_an_issue}`; no free-text field. A note would
  be a new **untrusted-text-storage-and-render surface**, and this panel
  already refuses to render even judge text as anything but escaped plain
  text (`RunView.tsx`) — an enum needs no sanitization and renders as a fixed
  label. `not_an_issue` is the **false-positive** tally surfaced by both
  counters (`TriageDTO.FalsePositives`, the `not_an_issue` sub-count of
  Dismissed, `workersvc.BucketTriage`).

## 335. Set/undo is `RequireUser` + strict OWNER-ONLY — not the owner-or-admin viewer helper (D5)
...
- **The load-bearing authz correction, found on review**: an owner-**or**-admin
  ...
  `IsAdmin=true`** (`middleware/cli_auth.go`, `if row.Scope !=
  clitoken.ScopeAdminRO { user.IsAdmin = false }`), and `uza_` is documented
  read-only **across the whole factory** (`docs/cli.md`). An owner-or-admin
  mutation here would be the *first* admin-reaching write on the `RequireUser`
  path, letting a read-only `uza_` token mutate any user's triage. So the
  service resolves the review by **strict caller-ownership**
  (`resolveOwnedRecommendation`, `workersvc/judge_disposition.go`,
  `s.GetReviewForTarget(ctx, ownerUserID, false, runID)` — `isAdmin`
  **hardcoded false**, never the caller's real `user.IsAdmin`) — the exact
  `SubmitInput(ctx, user.ID, …)` pattern `CreateRunInput` already uses —
  **not** `GetRunForViewer`/`GetReviewForTarget`'s owner-or-admin form that
  backs the review *read* and `FileIssue`, since reusing that would readmit
  the cross-user write. `ErrRunNotFound` (not owner-visible) and
  `ErrRecommendationNotFound` (visible, but recID absent from the *current*
  review) both map to the **same 404**, so a write leaks no ownership
  oracle (`writeDispositionError`, `handler/review_disposition.go`).
- **Practical consequence, stated precisely**: since there's no scope gate a
  handler can read, a `uza_` token is refused (404) only on **another**
  user's review — on its **own** review `caller == owner`, so it can write,
  exactly like `CreateRunInput` today. `uza_`'s read-only reach is therefore
  *cross-user*, not "cannot write anything": an admin can always triage their
  **own** judge runs, and is blocked only from triaging **other** users'.
- **PUT is an idempotent upsert on the coordinate; DELETE is undo, no
  claim-first dance (D6).** Re-clicking done, switching done→dismissed, or
  changing the reason is the same `PUT` (`ON CONFLICT (review_id, category,
  target) DO UPDATE`, re-stamping `rationale_hash`/`set_at`), so two
  concurrent writes converge on the last value. `DELETE` returns 0 rows on
  an absent coordinate (`ErrRecommendationNotFound` → 404); there is no
  external side effect to make exactly-once, so none of PRD #68's
  claim-first machinery is needed. The row keeps `set_by_user_id`/`set_at`
  for forensics, but the panel shows only a relative time, never an actor —
  under owner-only the setter is always the owner, so a name would be
  redundant, and an admin *reading* another user's review must not see a
  misleading "resolved by you".

## 336. Per-review DTO and the global stats aggregate both ride the shared ladder; the denominator is recommendation rows, all-time (D7/D8)

- `ReviewDTO` (`apitypes/review.go`) gains `dispositions []DispositionDTO`
  (`{category, target, status, reason, set_at, stale}` — `stale` computed
  server-side, the browser never sees a hash) and `triage TriageDTO`
  (`{total, todo, filed, done, dismissed, false_positives}`), both built in
  `reviewToDTO` from the shared `BucketOf`/`BucketTriage` helpers. The web
  renders `triage` **directly**, never re-deriving counts in TS, so the bar
  and `uzi review show` cannot disagree.
- `GET /api/me/judge/stats` (`RequireUser`, mirrors `/me/memory`) joins the
  caller's `run_reviews` (`user_id = caller`, `idx_run_reviews_user`) →
  `review_recommendations` → LEFT JOINs both side-tables on the coordinate,
  returning **one flat row per recommendation** (`ListJudgeTriageRowsForUser`)
  bucketed through the same helper (`JudgeTriageStats`). The denominator is
  **recommendation rows**, not coordinates — the `target=''` collapse means
  shared-coordinate rows share a status, accepted exactly as the per-review
  count. **All-time, not windowed**: "to do"/"filed" is a true backlog an old
  row still belongs to. Renders on `RunsList`'s header and deliberately does
  **not** respect the list's active filters — it's a global backlog, not a
  filtered view.

## 337. Self-improve backlog exclusion — a second `NOT EXISTS`, row-existence is the exclusion (D9)

- `ListOpenImproveUziRecommendations` (`queries/selfimprove.sql`) already
  excluded any coordinate claimed-or-filed in `recommendation_filed_issues`
  (PRD #68 §310); this PRD adds a **second, independent** `NOT EXISTS`
  against `recommendation_dispositions` on the same coordinate, in the
  **same migration** that creates the table (folds into M1, avoiding a
  second migration on the same path). **Row-existence is the exclusion** —
  any disposition excludes, regardless of `status`: a `dismissed` improve_uzi
  is a human "no", a `done` one is handled, and either way the engine must
  not fold it into its aggregated tracking issue. Undo (row deleted)
  re-includes it next cycle — no claim-first machinery needed, since the
  upsert is atomic and there's no forge write to make exactly-once. The
  query comment names **both** exclusions so a future reader isn't surprised
  by an empty tracking issue despite backlog rows existing.

## 338. CLI: a `uzi review` group absorbing the old read; short rec ids; the `uzi run review` deprecated alias; orphan dispositions (D10, Interactions)

- **`uzi review show|resolve|dismiss|undo|stats`** (`cmd/uzi/review.go`).
  `show` absorbs the old `uzi run review` read (verdict + recommendations +
  the new triage line) and prints a **git-style short rec id** (first 8 hex
  of the rec UUID) per row; the mutation verbs accept that short id,
  resolving an unambiguous prefix against the run's **current** review
  (`resolveRecID`) so the verbs are usable straight off `show`'s own output —
  an ambiguous prefix is a usage error, no match is a not-found with a
  "refresh, the review may have changed" hint. `resolve`→`PUT status=done`,
  `dismiss --reason wont-do|not-an-issue`→`PUT status=dismissed`,
  `undo`→`DELETE`, `stats`→`GET /me/judge/stats`. `uzi run review` is kept as
  a **hidden, `Deprecated`-annotated** alias sharing `runReviewShow` with
  `show`, so the two render byte-identical output; cobra's deprecation
  notice is forced onto stderr (`review.SetOut(env.Stderr)`) so `--json`
  stdout stays uncontaminated.
- The `RequireUser` + owner-only mount (§335) is exactly what lets a `uzc_`
  token drive all five verbs on its own runs, and lets a `uza_` token
  `show`/`stats` across the factory while a mutation against another user's
  review is refused — exit 4 (`ExitNotFound`), verified end-to-end in
  `cmd/uzi/review_test.go` (`TestReviewMutationRefusedForReadOnlyToken`).
  `undo` on a coordinate with no disposition is softened client-side to
  `ErrNoDisposition` (a plain error, not an `*ExitError`) so it reports
  "already undone" at exit 0 rather than a spurious failure — the same
  posture D6's idempotent-DELETE takes server-side.
- **Orphaned ("zombie") dispositions (Interactions).** When a re-judge stops
  emitting a coordinate, the disposition row persists (review-keyed, the
  review survives) but is **inert while orphaned**: `reviewToDTO` only emits
  a `DispositionDTO`/counts it for coordinates with a **current**
  recommendation row (`dispByCoord` matched against `rw.Recommendations`),
  so an orphan is absent from the panel, absent from both counters, and not
  undoable (both endpoints are recID-addressed and no current recID resolves
  to it). If a later re-judge re-emits the coordinate, the old disposition
  reappears and the rationale-hash stale flag (§333) fires iff the
...
*(§367 is reserved by PRD #108 Phase 2, in flight on its own branch at the time this
was written — 2026-07-25. The gap is deliberate, not a lost section.)*
