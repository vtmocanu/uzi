...
numbers continue past PRD #32's #141. Builds on PRD #4 (worker runtime, claim
...
## 142. Worker image templates in git (agent + compose)
...
## 143. Devbox provisioning engine in the worker (M3)
...
## 144. Tool profiles + admin allowlist (M4)
...
## 145. Tier-2 repo `devbox.json` opt-in (M5)
...
## 146. Agent template scopes migration + user CRUD (M6)
...
## 147. Template allocation + claim filtering (M7)
