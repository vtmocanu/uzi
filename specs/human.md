...
- A gated plan (or plan-revision) turn that ends with prose only — no `submit_plan`, no `ask_user`, just lead text — gets exactly one corrective nudge, resuming the same session with fixed text; no second nudge is given on the same planning attempt. [user, #1593]
  - A planning attempt is one call of the plan or revision turn. Anything that runs that turn again (a requeue, a recovery or limit-wait resume, a wall-park extend, a credential-switch re-drive) starts a fresh attempt with its own nudge and plan-missing-park budget, as the clarification-park budget also restarts when the executor is re-entered. The run's wall-clock limit is the overall backstop. [AI-synced 2026-09-24, #1593]
