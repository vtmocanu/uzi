...
- A gated plan (or plan-revision) turn that ends with prose only — no `submit_plan`, no `ask_user`, just lead text — gets exactly one corrective nudge, resuming the same session with fixed text; no second nudge is given on the same planning attempt. [user, #1593]
  - A planning attempt is one call of the plan or revision turn within an executor invocation. A requeue, a wall-park extend or a credential-switch re-drive re-enters the executor and starts a fresh attempt with its own one-nudge, one-park budget; the clarification-park budget likewise restarts on re-entry (though it spans the whole invocation, not one turn). Each re-drive is bounded by its own cap or by requiring an owner action. [AI-synced 2026-09-24, #1593]
