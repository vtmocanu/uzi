...
- A gated plan (or plan-revision) turn that ends with prose only — no `submit_plan`, no `ask_user`, just lead text — gets exactly one corrective nudge, resuming the same session with fixed text; no second nudge is given on the same planning attempt. [user, #1593]
  - A planning attempt is one executor invocation of the plan or revision turn. A requeue, a wall-park extend or a credential-switch re-drive re-enters the executor and starts a fresh attempt with its own one-nudge, one-park budget, as the clarification-park budget already does; each re-drive is bounded by its own mechanism's cap. [AI-synced 2026-09-24, #1593]
