...
export default defineConfig({
  ...
  test: {
    ...
    // `text-summary` prints the totals block the retired GitLab pipeline's `coverage:`
    // regex read; `cobertura` was the artifact that annotated the MR diff. GitHub
    // Actions has no equivalent for either, so both are informational today.
