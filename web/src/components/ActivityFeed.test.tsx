...
describe("ActivityFeed crew roster", () => {
  ...
  it("summarizes a steer_ack (scope cap) as its worker-authored text in the lane header (PRD #634)", () => {
    // The worker acknowledges an operator scope directive by finalizing at the ceiling;
    // its steer_ack `text` becomes the lane header one-liner, so a scope-capped run whose
    // newest message is the ack shows a meaningful summary instead of an empty one. The
    // same text also renders in the feed row below (RunEvent's steer_ack MetaLine), so
    // this assertion targets the header one-liner span specifically.
    const now = Date.now();
    const fresh = new Date(now - 500).toISOString();
    const msgs = [
      m(1, "text", { text: "implementing" }, "worker", fresh),
      m(
        2,
        "steer_ack",
        { text: "scope capped: finalized 2 of 4 milestones", directive: "scope", ceiling: 2, completed: 2 },
        "worker",
        fresh,
      ),
    ];
    const { container } = renderFeed(msgs, { status: "running", health: "ok" });
    const oneLiners = Array.from(container.querySelectorAll("span.truncate.text-xs.text-muted"));
    expect(oneLiners.some((el) => el.textContent === "scope capped: finalized 2 of 4 milestones")).toBe(true);
  });
...
// PRD #634: a scope-cap finalization (steer_ack) is a quiet, consequential transition —
// the peer of a usage-limit park — so it must reach the polite live region.
describe("ActivityFeed — scope-cap steer_ack (PRD #634)", () => {
  const region = () => document.querySelector('[aria-live="polite"]') as HTMLElement;

  it("announces a steer_ack to the live region with FIXED text (payload is worker-authored)", () => {
    // Before this, `steer_ack` was not in the meaningful set, so a screen-reader user got
    // silence at the exact moment the run capped at the operator's ceiling. The phrase is
    // fixed and interpolates nothing from the worker-authored payload — it is read aloud
    // out of context, matching the limit_* treatment.
    const hostile = "<img src=x onerror=alert(1)> scope capped 2/4";
    renderFeed(
      [
        m(1, "status", { event: "init", model: "claude-opus-4-8" }, "lead"),
        m(2, "steer_ack", { text: hostile, directive: "scope", ceiling: 2, completed: 2 }, "worker"),
      ],
      { status: "running" },
    );
    const text = region().textContent ?? "";
    expect(text).toBe("Run finalized at the operator scope ceiling");
    expect(text).not.toContain("onerror");
  });
});
