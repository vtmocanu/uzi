...
function agentOneLiner(latest: RunMessage | undefined): string {
  ...
  switch (latest.kind) {
    ...
    // PRD #634: the worker acknowledged an operator scope directive and finalized the
    // committed slice at the ceiling. `text` is worker-authored; it renders here as a
    // plain escaped string child (same as the `status`/`text` cases above), truncated to
    // one lane-header phrase, with a fixed fallback when it is empty.
    case "steer_ack": {
      const t = (latest.payload as { text?: string } | null)?.text ?? "";
      return truncate(t.trim(), 80) || "Finalized at the scope ceiling";
    }
    default:
      return "";
  }
  ...
  useEffect(() => {
    ...
    for (const mm of messages) {
      if (
        ...
        mm.kind === "limit_wait" ||
        mm.kind === "limit_hit" ||
        // PRD #634: a scope-cap finalization is the quiet, consequential peer of a
        // usage-limit park — the run stops taking new milestones and nothing else on
        // the page says so at the moment it happens. Announce it so a screen-reader
        // user is not left in silence at the cap.
        mm.kind === "steer_ack"
      )
        meaningful = mm;
    }
                ...
                : // Fixed strings for the same reason agentOneLiner uses them: the
                  ...
                  : meaningful.kind === "limit_hit"
                    ...
                    : // Fixed phrase, matching the limit_* treatment: the steer_ack
                      ...
                      meaningful.kind === "steer_ack"
                      ? "Run finalized at the operator scope ceiling"
