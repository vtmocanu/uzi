...
export const RunEventRow = memo(function RunEventRow({
  ...
  switch (msg.kind) {
    ...
    // so it never reaches a raw-HTML sink. Closes the m4 regression where a scope-capped
    // run's steer_ack fell through to the "unrenderable steer_ack event" fallback.
    case "steer_ack": {
      const text = asString(rec?.["text"]) ?? "";
      return text ? <MetaLine text={text} usage={phaseUsage} /> : <UnrenderableRow kind={msg.kind} rec={rec} />;
    }
    case "error":
      return (
