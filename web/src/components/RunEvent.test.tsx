...
// PRD #634 M5: a scope-capped run emits a `steer_ack` message when the worker finalizes
// its committed slice at the operator's scope ceiling. Before this unit the web feed had
// no consumer for it, so it fell through to "unrenderable steer_ack event".
describe("RunEventRow — steer_ack (PRD #634)", () => {
  const payload = {
    text: "scope capped: finalized 3 of 5 milestones at the ceiling",
    directive: "scope",
    ceiling: 3,
    completed: 3,
  };

  it("renders the worker-authored text as a status-style line", () => {
    const { container } = render(
      <RunEventRow msg={msg({ seq: 30, kind: "steer_ack", payload })} live={false} />,
    );
    expect(container.textContent).toContain("scope capped: finalized 3 of 5 milestones at the ceiling");
  });

  it("no longer falls through to the 'unrenderable steer_ack event' fallback (m4 regression)", () => {
    const { container } = render(
      <RunEventRow msg={msg({ seq: 31, kind: "steer_ack", payload })} live={false} />,
    );
    expect(container.textContent).not.toContain("unrenderable");
  });
});
