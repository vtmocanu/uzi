...
describe("Repos — skipped columns + Resync (PRD #576 M3)", () => {
  ...
  it("shows the Column-by nudge after Auto-create succeeds, not before (PRD #593)", async () => {
    mockApi.getProjectSyncStatus.mockResolvedValue({ ...linkedWithUnmatched });
    mockApi.autocreateProjectSyncColumns.mockResolvedValue({ status: "columns_created" });
    const panel = await openSyncPanel("vtmocanu/gh");
    await within(panel).findByText("Linked");

    // The nudge follows the action, so it is absent on a plain panel load.
    expect(within(panel).queryByText(/uzi now writes your columns/i)).toBeNull();

    fireEvent.click(
      within(panel).getByRole("button", { name: /Auto-create the missing columns/ }),
    );

    // After auto-create switches the link to a "uzi Status" field, the nudge appears.
    expect(await within(panel).findByText(/uzi now writes your columns/i)).toBeTruthy();
  });

  it("clears the Column-by nudge on the next reload so it doesn't bleed across panels (PRD #593)", async () => {
    mockApi.getProjectSyncStatus.mockResolvedValue({ ...linkedWithUnmatched });
    mockApi.autocreateProjectSyncColumns.mockResolvedValue({ status: "columns_created" });
    mockApi.resyncProjectSync.mockResolvedValue({ status: "resynced" });
    const panel = await openSyncPanel("vtmocanu/gh");
    ...
    // Raise the hint via auto-create.
    fireEvent.click(
      within(panel).getByRole("button", { name: /Auto-create the missing columns/ }),
    );
    expect(await within(panel).findByText(/uzi now writes your columns/i)).toBeTruthy();

    // A later reload (Resync calls loadSyncStatus) must RESET the hint — the state lives
    // on the always-mounted page, so without the reset it would bleed onto a panel where
    // the user did not just create a field.
    fireEvent.click(within(panel).getByRole("button", { name: /Resync/ }));
    await waitFor(() =>
      expect(within(panel).queryByText(/uzi now writes your columns/i)).toBeNull(),
    );
