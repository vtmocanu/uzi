...
export function Repos() {
  ...
  // After uzi creates its own "uzi Status" field (provision or auto-create), the
  // GitHub board still groups by the built-in Status, so the values uzi writes stay
  // invisible until the user switches the board's "Column by" to "uzi Status". Set
  // on those actions; a persistent panel hint tells them to make that switch. Reset
  // whenever the panel reloads (loadSyncStatus) so it only follows the action.
  const [columnByHint, setColumnByHint] = useState(false);
  ...
  const loadSyncStatus = useCallback(async (repoId: string) => {
    ...
    // Only the provision/auto-create success paths re-raise this (right after this
    // load); a plain (re)load must clear it so the hint follows the action, not the board.
    setColumnByHint(false);
    setSyncLoading(true);
    try {
  ...
  const provisionSync = async (repoId: string) => {
    ...
    try {
      ...
      // Provision creates a "uzi Status" field; nudge the user to group the board by it.
      setColumnByHint(true);
    } catch (err) {
  ...
  const autocreateSyncColumns = async (repoId: string) => {
    ...
    try {
      ...
      await loadSyncStatus(repoId);
      // Auto-create switches the link to a fresh "uzi Status" field; nudge the user to
      // group the board by it, or the values uzi writes stay hidden behind built-in Status.
      setColumnByHint(true);
    } catch (err) {
      setSyncError(syncErrorMessage(err));
    } finally {
              ...
              >
                ...
                {syncLoading ? (
                  ...
                  <div className="space-y-3">
                    ...
                    {/* Column-by nudge (PRD #593): after uzi creates its own "uzi Status"
                        field, the board still groups by the built-in Status, so the values
                        uzi writes stay hidden until the user switches Column by. See
                        docs/github-project-sync.md ("point the board view at uzi Status"). */}
                    {columnByHint && (
                      <div
                        role="status"
                        className="space-y-2 rounded-md border border-ok/30 bg-ok/5 p-3 text-sm"
                      >
                        <p className="text-fg">
                          uzi now writes your columns to a &ldquo;uzi Status&rdquo; field. Your board
                          still groups by GitHub&rsquo;s built-in Status, so set{" "}
                          <span className="font-medium">Column by</span> to{" "}
                          <span className="font-medium">uzi Status</span> to see them: open the board,
                          click <span className="font-medium">View</span> &rarr;{" "}
                          <span className="font-medium">Column by</span> &rarr;{" "}
                          <span className="font-medium">uzi Status</span>, then{" "}
