...
describe("forgeNoun", () => {
  it("returns the per-forge Title-Case noun", () => {
    ...
    expect(forgeNoun("github")).toBe("Pull Request");
  });
  ...
  it("gives the compact abbreviation and reference sigil per forge", () => {
    ...
    // GitHub renders "PR" and "#" (its PRs and issues share one number namespace), and
    // must be named explicitly so a missing arm never falls back to GitLab's "MR"/"!".
    expect(mrAbbrev("github")).toBe("PR");
    expect(mrRefSymbol("gitlab")).toBe("!");
    expect(mrRefSymbol("forgejo")).toBe("#");
    expect(mrRefSymbol("github")).toBe("#");
  });

  it("derives GitHub's lower and sentence casings from the single noun literal", () => {
    expect(forgeNounLower("github")).toBe("pull request");
    expect(forgeNounSentence("github")).toBe("Pull request");
  });
  ...
  it("names the destination platform per forge, defaulting to GitLab", () => {
    ...
    expect(forgePlatform("github")).toBe("GitHub");
