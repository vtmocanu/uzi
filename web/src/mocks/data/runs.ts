...
export const mockRuns: Run[] = [
  ...
  },
  {
    ...
    health_reason: null,
    health_since: null,
    // PRD #1202 D11: the owner-only automatic-rework guard readings. The automatic loop
    // spent all 5 of its 5 allowed cycles, so the past-cap line reads "· stopped".
    mr_rework_auto_cycles: 5,
    mr_rework_auto_cap: 5,
    plan_md: SAMPLE_PLAN(),
    repo_agents: null,
    agent_source: null,
    agent_exclusions: null,
    ...
    recovery_wait_cause: null,
    codex_account_action: null,
    codex_secret_id: "sec-codex-linked",
    codex_secret_label: "chatgpt-plus",
    recovery_retry_not_before: null,
    forge_park_count: 0,
    forge_park_max: 0,
    claimed_at: minsAgo(140),
    started_at: minsAgo(139),
    finished_at: minsAgo(96),
  ...
  {
    // PRD #1190: a run its OWNER paused — the only fixture whose status is `paused`, so
    // the only one that renders the `‖ paused` info pill, the conditional Resume header
    // action, the "· clock stopped" elapsed, and the PausedPanel. It parked after
    // milestone 3 of 6 (mock frame C): the three pause-request columns are NULL (a landed
    // pause has consumed them), checkpoint_tip_at is set (the checkpoint it parked on), and
    // updated_at is when it paused, which the elapsed and panel heading read.
    id: "run-paused",
    repo_id: "repo-uzi",
    issue_iid: 33,
    ...
    stop_kind: null,
    stop_reason: null,
    // `ok` and cleared, like the limit_wait fixture: SetRunPaused resets the health
    // columns on entry, and the health detector's allowlist never revisits a paused run.
    health: "ok",
    health_reason: null,
    health_since: null,
    plan_md: null,
    ...
    agent_exclusions: null,
    own_agents: null,
    // 3 of 6 milestones done — the mock frame C shape.
    milestones: [
      { id: "tr-1", title: "Define the shared TriageRow shape" },
      { id: "tr-2", title: "Fold the Judge backlog onto it" },
      { id: "tr-3", title: "Fold the run page recommendations onto it" },
      { id: "tr-4", title: "Fold the Findings list onto it" },
      { id: "tr-5", title: "Shared bucket/severity ladder" },
      { id: "tr-6", title: "One rendering, three mounts" },
    ],
    milestones_completed: ["tr-1", "tr-2", "tr-3"],
    milestones_in_progress: null,
    milestones_candidate: null,
    // A landed pause has consumed the request columns; the checkpoint it parked on stays.
