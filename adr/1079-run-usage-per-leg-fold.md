...
- `BumpRunLineageEpoch` and the `resume_lineage_break`-driven bump loop are retired, and nothing reads or writes `runs.lineage_epoch` any more. The **column itself is kept this release, not dropped**: `check:migration-additive` (PRD #422 Decision 2 / M6) forbids dropping an api-facing column in a forward migration, because sqlc emits explicit column lists and an N-1 api replica still `SELECT`s it by name during a rolling release. The column is dead (sqlc still generates an unread `LineageEpoch` field); a follow-up migration ([#1087](https://github.com/vtmocanu/uzi/issues/1087)) drops it a release later, once no N-1 replica remains — the contract step of an expand/contract change. The `resume_lineage_break` event itself is unaffected and stays persisted as a dropped-resume diagnostic — a break is always followed by a fresh `init`, which the position-absolute count already handles without needing to know a break happened.
