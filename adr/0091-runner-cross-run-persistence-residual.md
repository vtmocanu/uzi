...
**Status**: Accepted (records the accepted residual and its bound; the structural fix is tracked by [PRD #529](https://github.com/vtmocanu/uzi/blob/main/prds/529-ephemeral-workers.md) / [issue #79](https://github.com/vtmocanu/uzi/issues/79))
**Date**: 2026-08-22
...
The clean fix is #529's ephemeral workers: a run-bound worker that boots, serves one run, and is torn down. In #529's v1 design each gets its own `-nix` and `-data` PVCs (Decision 7), so the shared-path vector does not exist. When #529 lands, verify the vector is gone and close #91 citing this ADR. **Caveat for the #79 cost path:** if a later optimization shares one nix store across ephemeral workers to cut cold-start (the tradeoff issue #79 weighs), the `/nix` half of this vector partially returns — content-addressed `/nix/store` paths are hash-immutable (planting one needs a hash preimage), but the mutable profile/gcroots would again be shared. So a shared-nix-store variant must re-check this residual rather than assume ephemerality closed it. The middle option (fresh provision HOME + fresh nix profile per run on the shared worker) is explicitly **not** chosen: it pays Decision 5's cold-start cost on every run and is superseded by #529 anyway.

## What this ADR does NOT decide
- Whether to build ephemeral workers, or when — that is [PRD #529](https://github.com/vtmocanu/uzi/blob/main/prds/529-ephemeral-workers.md) (planned, explicitly not near-term) and the cost tradeoff in [issue #79](https://github.com/vtmocanu/uzi/issues/79).
...
- #91 is closed (this ADR is its durable record); the live reminder to switch lives on #529, whose milestones include verifying and closing out this residual.
