...
- **The peel is provenance-gated (review F1).** `runnerCloneForBranch` peels an overlay ONLY when `seededFrom === "checkpoint"` — the worker-built, api-brokered ref an agent cannot push to. A `ckpt(overlay):` subject on any agent-controllable base (a tracking/origin tip) is a forgery and is left intact, so the peel (which discards the tip's tree) can never silently drop the agent's own commit. A subject prefix is not provenance across the agent trust boundary; `seededFrom` is.
