...
- Documented residuals (recorded, not solved):
  ...
  5. A **lost publish ACK** (the broker accepts the overlay push but the HTTP response is lost) leaves `flight.lastCheckpointRefTip` stale, so the next overlay after new work chains from the wrong `parent[0]` and is skipped `not_descendant` — that branch stops getting overlay checkpoints for the rest of the run. The already-up-to-date=success path already reconciles a re-attempt of the SAME tip; a lost-ack-then-new-work needs a two-tip reconcile (chain from the attempted tip, keep the pack-exclude at the last-confirmed tip). Tracked in #1086; deferred because its correctness is only verifiable against the broker's depth-1 store (the harness added for this ADR, below).
- GitHub-only; GitLab/Forgejo skip the whole path.
- **The peel is provenance-gated on TWO checks (review F1).** `runnerCloneForBranch` peels an overlay only when BOTH hold: (a) `seededFrom === "checkpoint"` — the worker-built, api-brokered ref an agent cannot push to (so a forged `ckpt(overlay):` subject on a tracking/origin tip is never peeled); AND (b) a **structural** check — the diff from the overlay to its last parent is non-empty and confined to `.github/workflows/`. (a) alone is insufficient: an agent can commit its own realTip with a forged subject and, if that tip ships as a PLAIN checkpoint (not behind on workflows ⇒ no overlay built), it lands on the checkpoint ref too, so `seededFrom` is `"checkpoint"` there as well. (b) is the unforgeable discriminator — a genuine overlay swaps only `.github/workflows`, while a real-work commit differs in other files, so the peel (which discards the tip's tree) can never silently drop the agent's own committed work. A subject prefix is not provenance across the agent trust boundary; the checkpoint-ref origin plus the workflow-only diff shape are.
- **The base-first accept property has an executable depth-1 guard.** `api/internal/pushbroker/overlay_ancestry_internal_test.go` reconstructs the broker's depth-1 store (the branch-fork parent absent) and asserts a base-first overlay is accepted while a realTip-first one is rejected `not_descendant` — the regression guard against flipping the parent order back (the agent-side shape tests use a full-graph merge-base that cannot catch it).
