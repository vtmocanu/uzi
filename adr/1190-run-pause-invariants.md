...
**`SweepRunningTimeout` no longer belongs on this list; it no longer exists.** [PRD #1497](../prds/1497-park-at-wall.md) replaces it with two statements, `RequestWallParks` and `ParkRunsAtWall`, that move a wall-deadline run to non-terminal `paused` (`hold_reason='budget_exhausted'`), never to `failed` — the wall stopped being a terminal transition. `RequestWallParks` *sets* `pause_mode = 'wall'` (it is the request, not a clear); `ParkRunsAtWall` clears the three pause columns as part of settling that request and consuming its `run_user_inputs` row (D18), the same "a park closes its own request" reasoning I4 and I6 already give for a worker-driven park, not the terminal root-cause clearing this paragraph describes. A wall-parked run can still carry a pending pause after the fact if a fresh owner pause races it — I4's re-arming argument applies to it exactly as to the other involuntary parks, since it is no longer terminal at all.
