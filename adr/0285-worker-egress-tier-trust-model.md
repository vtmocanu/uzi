...
- **Restricted / standard** (`uzi-workers`): egress is a default-deny `NetworkPolicy` floor (`deploy/chart/templates/worker-networkpolicy.yaml`, `networkPolicy.enabled` default true) plus an Antrea FQDN allowlist ANNP (`worker-fqdn-egress.yaml`). Antrea permits TCP only to the IP addresses learned from the allowlist's DNS names (not per hostname: other hosts sharing those addresses stay reachable; see "IP-level, not host-level" below). The names are `cache.nixos.org`, the forge, `*.anthropic.com`, the three Codex hosts `api.openai.com`, `chatgpt.com` and `auth.openai.com` (PRD #1106 D12, below), `search.devbox.sh` (PRD #808), `api.github.com` (allowed since #818), and the CNPG-chart pair `ghcr.io` + `pkg-containers.githubusercontent.com`. Destinations at other addresses (e.g. `codeload.github.com`) are **blocked** (measured TIMEOUT; `.claude/rules/stack.md`, `ARCHITECTURE.md` hosted-worker section).
...
The single most repeatable mistake here is reading a docker-tier egress result as a standard-tier control failure — made at least twice. The mitigation is procedural and cheap: name the tier (`uzi admin workers` → `docker:`), re-measure on the intended tier, and treat a completed HTTP response from a destination confirmed outside the allowlist's learned IP addresses as evidence of the docker tier (an off-list hostname alone may share an allowed IP). The same rule lives in `ARCHITECTURE.md` (hosted-worker section) and `.claude/rules/stack.md` (which loads when an agent touches `deploy/**`), so the reader meets it before measuring.

### Codex endpoints on the restricted tier (PRD #1106 D12)
The restricted tier's allowlist gains three exact hosts, each on TCP 443 only: `api.openai.com`, `chatgpt.com` and `auth.openai.com` ([PRD #1106](../prds/done/1106-codex-harness-phase1.md) D12, confirmed by the user 2026-09-04; see also [ADR-1106](1106-codex-harness.md)).
...
- **Accepted exfiltration class.** A request to either vendor carrying an attacker's own API key or session can store data there for later retrieval; `*.anthropic.com`, which the tier already allows, has the same property, so the widening is accepted as symmetric with it.
- **Exact names only.** The DNS selector does not match `chatgpt.com` subdomain names, and there is no `*.openai.com` (packet reachability can still arise through a shared IP, per the next bullet). The list grows only from a real provisioning failure that names a host (M6), never from prediction.
- **IP-level, not host-level (accepted residual).** Antrea's FQDN policy matches the names at DNS time and then allows the resolved A/AAAA addresses ([Antrea FQDN-based filtering](https://github.com/antrea-io/antrea/blob/main/docs/antrea-network-policy.md#fqdn-based-filtering)); it does not check TLS SNI or the HTTP Host header. An allowed name behind shared CDN or anycast addresses (`chatgpt.com` among them) therefore opens TCP 443 to every other virtual host served from those addresses, and a poisoned or rebound DNS answer for an allowed name can add an arbitrary IP. This holds for every allowlist entry, not only the Codex hosts. It is accepted for D12 because the tier already carries the same exfiltration class through the vendor endpoints; a host-level boundary needs L7 enforcement (an SNI/Host-checking egress proxy), tracked in [#1651](https://github.com/vtmocanu/uzi/issues/1651).
...
- Every restricted-tier worker, including one that only runs Claude, can reach `api.openai.com`, `chatgpt.com` and `auth.openai.com` on TCP 443 (PRD #1106 D12). `scripts/assert-chart-render.sh` asserts, in both the CI render and the chart defaults, that each of the three has exactly one Allow entry, on TCP 443 only, and that no other Allow entry (literal or wildcard) equals or ends with `openai.com` or `chatgpt.com`, or is a wildcard covering a probe OpenAI/ChatGPT host; fqdns are compared lowercased with one trailing dot stripped; a deployment that overrides `allowFQDNs` must carry the same three.
