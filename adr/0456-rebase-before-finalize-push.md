...
## 2026-08-23 amendment — workflow-subtree overlay adopted as the primary base-align strategy (issue #627)

Issue #627 adds a **narrower** strategy that now runs **before** the merge→rebase chain this ADR describes. For a GitHub run whose `.github/workflows/` subtree differs from the default branch's, and which never itself modified a workflow file, uzi overlays just the default tip's `.github/workflows/` subtree onto the agent tip instead of reaching for a whole-tree merge or rebase. The gate does not require the rest of the branch tree to equal the default; unrelated default-branch changes are deliberately left outside the agent tip. The merge→rebase→preserve-and-fail chain above is retained **unchanged** as the fallback: it is entered whenever the overlay's gate does not hold, the overlay hits an unexpected git error, or the overlaid push is still workflow-scope/non-fast-forward-rejected.

**Added to "Alternatives considered."** The original ADR weighed rebase-only, a server-side checkpoint base-refresh, and a continuous-rebase policy — but never a narrower, path-scoped alignment. That missing option is what #627 adopts:

- **Workflow-subtree overlay (adopted as primary, #627).** Where D2's SHA-preserving merge and its rebase fallback both operate on the **whole tree**, the overlay touches only `.github/workflows/`. It precedes merge/rebase for three reasons: it **cannot conflict**, because the branch provably never touched those paths; it is a **fast-forward** (every original agent commit SHA is preserved; one overlay commit is added on top, so none of D2's SHA-rewrite hazards — tracking-ref invalidation, checkpoint divergence — apply); and it makes the branch's workflow tree **byte-for-byte equal** to the default's, deletions included, which is all GitHub's tip-vs-default workflow-scope check requires, **without** importing main's unrelated changes. Dragging in those unrelated changes is exactly what made the whole-tree merge/rebase fail on an ordinary conflict (a second PRD's Go file, `specs/ai.md` section numbering, a goose migration number) and abort the run as `finalize_base_align_conflict` — a failure mode this narrower strategy sidesteps entirely for the common case it targets.

**Gating (the correctness pin).** The overlay runs only when a freshly recomputed workflow-change signal is non-null **and** empty — i.e. the branch provably modified no workflow file. This defends against #377's null-diff fail-open: a branch that *did* modify a workflow file can otherwise reach the base-align block, and overlaying there would silently clobber the agent's own workflow edits while falsely reporting success. A null or non-empty signal falls straight into the unchanged merge→rebase→preserve chain above. An empty-default edge is also handled: if main has deleted its entire `.github/workflows/`, equalization is rm-only (a bare `git checkout <defaultTip> -- .github/workflows` would error in that case).

**Scope unchanged from the rest of this ADR.** GitHub-only. No new `fail_origin`, no new DB column, no migration; #377's early-fail path and the `finalize_base_align_conflict` + `runs.preserved_patch` terminal are unchanged — the overlay only adds an earlier, narrower attempt in front of the existing chain.
