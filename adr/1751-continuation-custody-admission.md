...
A queued run with `claim_generation >= 1` that still has at least one owner-scoped `open` custody hold for that exact run is admitted by `ClaimRun` even when its owner is at or above the custody hold limit (`workersvc.custodyHoldLimit`, 8). A fresh run (`claim_generation = 0`), or a previously claimed run whose holds are all settled, is gated exactly as before.
...
- An owner's open-hold count can grow past the limit while interrupted runs resume. Old generations are still cleaned up by capture/publication release, the completed-run ancestry settlement (#1582), and the live same-worker settlement added in the second milestone of #1751.
