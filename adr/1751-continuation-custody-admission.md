...
- **Worker.** A confirmed publish records a live leg (its exact request, MAC-covered) on the hold's settlement record and sends it in the background. Every read-modify-write on a settlement record runs under a queued per-hold lock and re-reads first; a sent leg is never replaced and survives completion, supersede and newer-generation adoption until answered; the candidate and recovery evidence (source/adopted/pushed pins, the predecessor's recovery pin and journal) is removed only after an exact `released`, under the lock, with the settlement record last; only the leg's own `published` pin is dropped when a leg is cleared or superseded unsent. Each live request is bounded by 10 s, which is the most it can delay a same-hold completion step.
