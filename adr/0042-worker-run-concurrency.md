...
Both prior-art projects multiplex many runs per host process; neither isolates a run in its own worker process or container:
...
Notable: neither handles Anthropic-API 429 contention across concurrent sessions — each delegates backoff to the child SDK/CLI. Shared-token contention under concurrency has no prior art to copy.
...
A bounded slot-semaphore model, applied to a codebase whose server side already supports it. The cap is kept **worker-side config** rather than server-side, because capacity is a property of the machine the worker runs on, which only the worker's operator knows. To preserve the observability a server-side cap would otherwise give for free, the worker **advertises** its cap at registration (one nullable column; the server never enforces it) so the UI can show `active/cap` instead of a bare count.
...
- ✅ Matches industry queue-worker precedent (Sidekiq, Celery, Temporal, BullMQ — §3 above); the required safeguards are small and enumerable.
