...
**Status**: Accepted (implemented, issue #1247, M1–M10; maintainer M0 dev-cluster evidence pending)
**Date**: 2026-09-17
...
What **is** implemented, so this gap is DB residue only — never a stuck run, a stale render, or a signal leak: (a) every terminal transition (`SetRunCompleted`, `SetRunFailed`, `CancelRunServerSide`, and the rest of the eleven terminal writers) clears both switch columns alongside the pre-existing PRD #1190 pause-columns clear; and (b) the run-detail read (`GetRun`, PRD #1247 Step A) suppresses the *derived* `credential_switch` DTO field whenever an applied `run_credential_epochs` row exists at a generation strictly greater than `credential_switch_generation` — so the surfaces that render it (CLI/web/TUI, all fed by the detail read) show `null` after the reclaim, not a stale value. Even the raw `credentialSwitchState` reads `"requested"`, not `"released"`, once `ClaimRun` clears `claim_released_at` on reclaim. What is genuinely deferred is only the pair of DB columns themselves: on a still-running run's same-run release-then-reclaim they are left un-cleared until #1422. That residue is invisible to every render surface (only a caller reading the raw columns and comparing against `credential_epochs` could notice), and `ClaimRun` never filters on the stamp, so it cannot wedge a claim.
