...
const nonRootChildEnv = "SAFETREE_TEST_NONROOT_CHILD"
...
// euid, because root bypasses the 0555/0444 permission checks that a cleanup test
// exists to exercise, and would let it pass for the wrong reason.
//
...
func requireNonRootCommandUID(t *testing.T) bool {
	...
	stage, err := os.MkdirTemp("", "safetree-nonroot-")
	if err != nil {
		t.Fatal(err)
	}
	...
	bin := filepath.Join(stage, "safetree.test")
