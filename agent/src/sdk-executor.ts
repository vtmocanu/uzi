...
// Issue #281: consecutive work turns that are unchanged-tree + no-subagent + verbatim-
 ...
 * turn, if any; `pid` is the dead CLI's pid (its process-group id), whose group must be
...
/** Issue #281: normalize a turn's lead text for verbatim-repeat comparison — trim and
 *  collapse internal whitespace so trivial reflow does not defeat the match, while a
 *  genuinely different wording still resets the streak (biased toward continuing). */
function normalizeStallText(s: string | undefined): string {
...
export class SdkExecutor implements Executor {
      ...
      if (preApproved) {
        ...
        if (isCIFix) {
          ...
          planPrompt = buildSelfImprovePlanPrompt({
            ...
            // PRD #686 D11: the open self-improve MRs' "what was proposed" text, rendered
            // in its own untrusted fence with a trusted non-overlap instruction outside it.
            openSelfImproveMRs: ctx.openSelfImproveMRs,
  ...
  private async phaseRunLoop(
    ...
    let maxServedWallMs = drive.maxServedWallMs;
      ...
      // Issue #281: no-progress / terminal-refusal detection state.
      let stallStreak = 0;
      let lastStallFingerprint: string | undefined;
      let lastStallText: string | undefined;
      // The detector trips only on STALL_LIMIT CONSECUTIVE qualifying refusal turns. The
      // checkpoint and clarification/follow-up paths `continue` past the detector below, so
      // each must clear the streak first — otherwise a checkpoint or clarification turn
      // between two refusals leaves the streak intact and three NON-consecutive refusals
      // trip it (CodeRabbit #655). A bypassing turn is progress or new input, never a repeat.
      ...
      for (;;) {
        ...
        // transition frames, and enqueues the immediate push. Built fresh per turn so a
        // milestone reported in_progress then completed WITHIN the turn (two SDK messages)
        // diffs correctly across the turn's several observations.
        const onProgress = this.makeProgressObserver(
          ctx,
          latestProgress,
          frozenMilestones,
        );
        ...
        if (asked?.length) {
          ...
          if (verdict.parked) {
            ...
            iteration--;
            resetStallState(); // a clarification park is not a refusal turn → breaks the streak
            continue;
          }
          ...
          followUp = verdict.followUp;
          iteration--;
          resetStallState(); // a clarification turn is not a refusal → breaks the streak
          continue;
        }
        ...
        // Issue #281: stop a lead that declared a terminal refusal and is making no
        // progress, instead of re-prompting identically until the iteration budget
        // drains. Trips only on the CONJUNCTION of the issue's three signals across
        // STALL_LIMIT consecutive turns: the worktree fingerprint (commit tip + porcelain)
        // is unchanged from the previous turn, no subagent produced a frame this turn, and
        // the lead's response is verbatim-identical (whitespace-normalized) to the previous
        // turn's. Any of a new commit, a working-tree change, a subagent frame, or a
        // differently-worded response resets the streak — so a run making real progress
        // never trips. Inert unless the runner wired worktreeFingerprint (stub/chat/non-
        // issue executors leave it absent), so the loop is unchanged for them.
        if (ctx.worktreeFingerprint) {
          const fingerprint = await ctx.worktreeFingerprint();
          const text = normalizeStallText(turn.finalText);
          const capable =
            fingerprint !== null && !turn.subagentActivity && text.length > 0;
          const repeated =
            capable &&
            lastStallFingerprint !== undefined &&
            fingerprint === lastStallFingerprint &&
            text === lastStallText;
          stallStreak = repeated ? stallStreak + 1 : capable ? 1 : 0;
          lastStallFingerprint = capable ? fingerprint : undefined;
          lastStallText = capable ? text : undefined;
    ...
    return (progress: MilestoneProgress): void => {
      ...
      for (const id of progress.completed) {
        if (!prevCompleted.has(id))
          this.emitTransitionFrame(ctx, id, "complete", titles);
      }
      for (const id of progress.in_progress) {
        if (!prevInProgress.has(id))
          this.emitTransitionFrame(ctx, id, "started", titles);
      }
