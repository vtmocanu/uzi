...
const REPO_AGENT_MAX_BYTES = 64 * 1024;
...
const REPO_AGENT_MAX_TOOLS = 64;
const REPO_AGENT_MAX_TOOL_LEN = 64;
...
/** Conventional non-role documentation basenames that may sit beside agent files in a
 *  `.claude/agents/` directory (a README describing the roster, a LICENSE, …). When
 *  such a file FAILS to parse as an agent (a README has no frontmatter, so
 *  parseAgentFile returns invalid), detectRepoAgents treats it as documentation and
 *  drops it SILENTLY rather than surfacing an `invalid` note (issue #715). The name
 *  alone is NOT disqualifying: a reserved name that DOES parse as a valid agent is kept
 *  — the drop applies only on the invalid path. Matched case-insensitively on the whole
 *  filename. Mirrors the API's reservedDocNames in
 *  api/internal/agentsource/parser.go; a set-level convenience, not part of the
 *  per-ParseFile parity corpus, so the two need not stay byte-identical. */
const RESERVED_DOC_NAMES: ReadonlySet<string> = new Set([
  "readme.md",
  "license.md",
  "contributing.md",
  "changelog.md",
