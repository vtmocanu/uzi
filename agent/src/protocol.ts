...
export type MessageKind =
  ...
  | "limit_hit"
  /** PRD #634 M4: the worker's acknowledgement that it acted on an operator scope
   *  directive (payload `{ text, directive: "scope", ceiling, completed }`), so the
 ...
 *  @public — dynamically-parsed wire payload (no static consumer); documents the
 *  worker↔api contract shape. Kept exported deliberately (issue #597). */
...
export interface IterationBudget {
   ...
   * is `run.milestones_completed.length`; `scopeCeiling` absent ⇒ unbounded. Both optional,
   * carried through the same delivery seam as the budget so a scope directive rides the live
   * ACK and a re-claim alike.
   */
  scopeCeiling?: number;
...
export interface JudgeTraceTarget {
   ...
   *  null on a normal run. 'scope_capped' marks an operator-directed partial. */
...
export interface MessagesRequest {
  ...
  /** PRD #1247 M5 (INERT seam): the claim-lane generation the reporting worker holds. A CAPABILITY
   *  worker stamps it on every message batch so the server's per-query fence can engage — a
   *  released/reclaimed old flight's batch then persists nothing (the api decodes it as
   *  `ClaimGeneration *int64`). Optional so a legacy worker omits it and the append stays unfenced.
   *  A LATER behavior unit stamps it; nothing sets it today. */
  claim_generation?: number;
}
 ...
 *   - ok      → the parsed selection, as sent — UNLESS it names `source: "repo"`
 *               against a clone with no roster (`!repoAvailable`), which is
 *               well-formed but unsatisfiable: fall back to `own` plus a note
 *               (issue #231 / PRD #209 Open Question 1). Passing such a selection
 *               through would hand implement an empty subagent map — a broken run,
 *               not a deliberate lead-only one. Keyed on `repoAvailable` only: a
 *               present roster all of whose members are excluded is a confused
 *               request, not this case, and is out of scope.
...
export function resolveAgentSelection(
  ...
  switch (parse.status) {
    case "ok":
      if (parse.selection.source === "repo" && !repoAvailable)
        return {
          selection: { source: "own", exclusions: [] },
          note: "this clone has no repo agent roster; using your own agent templates",
        };
...
export interface StateRequest {
  status: RunState;
  /** PRD #1247 M5 (INERT seam): the claim-lane generation the worker holds. Stamped by a capability
   *  worker on every mutating report so the server's fence can engage; optional so a legacy worker
   *  omits it. A LATER behavior unit stamps it; nothing sets it today. */
  claim_generation?: number;
  ...
  /** PRD #212: the `git status --porcelain` line list the plan turn produced, computed
   *  as the RUNNER uid. Sent on EVERY awaiting_approval report (Decision 3): `[]` when
   *  the plan turn was clean, so each gate round replaces the server's list. OMITTED by
   *  a pre-#212 worker (the api COALESCE then preserves the column). */
   ...
   *  server must CLEAR it (ClearRunMilestonesCompleted) rather than union onto it.
   *  Additive + optional and EMITTED ONLY when true, on a single dedicated run-start
   *  `running` report — NEVER on an iteration heartbeat (a clear on every heartbeat would
   *  wipe live progress). Omitted entirely otherwise (a pre-#628 worker, and a resume that
   *  recovered its tree via `seededFrom` "tracking"/"checkpoint"), so the wire shape is
   *  unchanged. Keyed on the TREE signal, NOT the session signal `resume_lineage_break`,
...
export interface StateAck {
   ...
   *  unbounded); `completedCount` is `run.milestones_completed.length`. m3's loop-top honor
