  ...
  private async *runTurn(request: RunTurnRequest, rendered: RenderedCodexRun): AsyncGenerator<HarnessEvent> {
    ...
    try {
      ...
      for (;;) {
        ...
        // LIVENESS for the root's idle watchdog: while the parent callback is admitted,
        // the watchdog is suspended. Outside that interval, the root driveCodexTurn
        // re-arms idle on every event it consumes (codex-executor.ts). After routing,
        // yield a CONTENT-FREE `activity` carrying ONLY the ROOT thread id (no items,
        // no child text) so child traffic remains visible to the root watchdog without leaking any child
