...
export interface RunContext {
   ...
   *  MRs, carried only on a self_improve run so the picker chooses a NON-OVERLAPPING
   *  improvement. Absent/empty ⇒ no block injected. UNTRUSTED-content advisory data —
   *  rendered nonce-fenced, never as instructions. */
  ...
  /** M4: dequeue the next queued follow-up to inject into the next loop turn. Called at the end of
   *  every ordinary work turn and at a cooperative checkpoint (#1152): one per turn, FIFO. The
   ...
   *  state (branch tip + `git status --porcelain`), for no-progress detection in the
   *  implement loop. Returns null when unresolvable — the loop then cannot assert
 ...
 */
const STUB_NOT_CODE_SENTINEL = "UZI_STUB_NOT_CODE";
 ...
 */
const STUB_LIMIT_SENTINEL = "UZI_STUB_LIMIT";
...
const STUB_RESULT_USAGE = {
  input_tokens: 21400,
  cache_read_input_tokens: 188000,
  cache_creation_input_tokens: 0,
  output_tokens: 6100,
};
const STUB_RESULT_MODEL_USAGE = {
  "claude-fable-5": {
    inputTokens: 21400,
    outputTokens: 6100,
...
const STUB_RESULT_COST_USD = 0.24;
const STUB_CODER_USAGE = {
  input_tokens: 12000,
