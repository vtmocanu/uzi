...
describe("reclaimStrandedRunHomes (PRD #108 M6)", () => {
  ...
  // PRD #556 M3 (D4) — a shutdown-preserved run HOME is reclaimed by the EXISTING GC.
  //
  // PRD #556 M1 preserves `agent-home/<runId>` across a worker-shutdown interrupt —
  // the same two fs carve-outs a usage-limit park already makes — so a same-worker
  // re-claim within the affinity grace can resume the SDK session. The reclamation of
  // such a preserved HOME (when the run does NOT resume here — it resumed on another
  // worker, or ran to completion elsewhere) must reuse `reclaimStrandedRunHomes` and
  // add NO new reap criteria. That works BY CONSTRUCTION because the GC is
  // provenance-agnostic: it has no input describing HOW a HOME came to be stranded, it
  // only reaps a HOME whose run the api positively reports terminal and skips every
  // non-terminal or unknown status. So a shutdown-preserved HOME is modelled here
  // exactly as any other stranded HOME — that identity IS the point, and it is why
  // this milestone adds a TEST rather than code. The PRD explicitly REJECTS the two
  // alternatives that would have needed code (D4): eager "reap because owned by
  // another worker" reaping (deletes a non-terminal run — catastrophic on the
  // shared-volume compose lane the GC models as live, PRD #108 Risk 5) and a
  // TTL-delete-on-unknown (a limit park legitimately lives up to 8 days).
  describe("PRD #556 M3 — shutdown-preserved HOME reuses the existing terminal-status GC (D4)", () => {
    // Every fixture here uses `makeHome(...)` with its default `OLD` age (24h), which
    // clears the 3h DEFAULT_RECLAIM_MIN_AGE_MS the same way the matrix test above does,
    // so the age guard never silently skips these before the status check runs.
    it("reaps the preserved HOME once its run is terminal (resume completed, here or elsewhere)", async () => {
      // The same-worker resume ran to completion, or it resumed on another worker and
      // finished there: either way the api reports the run terminal, so the preserved
      // HOME is now genuine garbage and is reclaimed.
      for (const terminal of TERMINAL_RUN_STATUSES) {
        fs.rmSync(root, { recursive: true, force: true });
        fs.mkdirSync(root, { recursive: true });
        const preserved = makeHome(RUN_A);
        const summary = await reclaimStrandedRunHomes(
          root,
          statuses({ [RUN_A]: terminal }),
          nullLogger(),
        );
        assert.strictEqual(fs.existsSync(preserved), false, `status ${terminal}: preserved HOME must be reaped`);
        assert.strictEqual(summary.removed, 1, `status ${terminal}`);
        assert.strictEqual(summary.skippedNotTerminal, 0, `status ${terminal}`);
      }
    });

    it("NEVER reaps the preserved HOME while its run is non-terminal — incl. `running` on ANOTHER worker", async () => {
      // The requeued run has not gone terminal. `running` is the load-bearing case: the
      // run resumed on a DIFFERENT worker and its SDK may be actively writing that
      // HOME's transcript on a shared volume (PRD #108 Risk 5), so deleting it here
      // would corrupt a live resume. `limit_wait` (parked, past the age floor for its
      // whole park), `claimed` and `queued` (requeued-but-not-yet-resumed) must all be
      // kept too.
      for (const nonTerminal of ["running", "claimed", "queued", "limit_wait"]) {
        fs.rmSync(root, { recursive: true, force: true });
        fs.mkdirSync(root, { recursive: true });
        const preserved = makeHome(RUN_A);
        const summary = await reclaimStrandedRunHomes(
          root,
          statuses({ [RUN_A]: nonTerminal }),
          nullLogger(),
        );
        assert.ok(fs.existsSync(preserved), `status ${nonTerminal}: preserved HOME must be KEPT`);
        assert.strictEqual(summary.removed, 0, `status ${nonTerminal}`);
        assert.strictEqual(summary.skippedNotTerminal, 1, `status ${nonTerminal}`);
      }
    });

    it("skips the preserved HOME when its status is unknown (api down → throw, or 404 → undefined)", async () => {
      // No TTL-delete: an unreadable status is never treated as "reap". A throw (api
      // down) and a returned undefined (404, the run row gone) both KEEP the HOME.
      for (const lookup of [
        async () => {
        ...
        fs.mkdirSync(root, { recursive: true });
        const preserved = makeHome(RUN_A);
        const summary = await reclaimStrandedRunHomes(root, lookup, nullLogger());
        assert.ok(fs.existsSync(preserved), "an unknown status must never authorise a delete");
