...
describe("assembleAgents", () => {
  ...
  it("PRD #702 M5: appends the worker-runtime deps note to a role whose body lacks it (non-vacuous)", () => {
    // The reviewer body has no worker-runtime/deps guidance, so a distinctive phrase
    // from WORKER_RUNTIME_APPEND appearing in the COMPOSED prompt can only come from
    // the append, not the body. (A coder-based assertion would be vacuous until M6
    // strips that note from the builtin coder body.)
    const marker = "races that background install";
    assert.ok(
      !reviewer.prompt_body.includes(marker),
      "precondition: the raw reviewer body does NOT carry the worker-runtime note",
    );
    // Also exercise a synthetic template with an empty body — nothing but the appends.
    const emptyBodied: AgentTemplate = { ...reviewer, name: "blank", prompt_body: "" };
    const { subagents } = assembleAgents([reviewer, emptyBodied]);
    assert.ok(
      subagents.reviewer?.prompt?.includes(marker),
      "the composed reviewer prompt carries the worker-runtime note from the append",
    );
    assert.ok(
      subagents.blank?.prompt?.includes(marker),
      "even an empty-bodied role's composed prompt carries the worker-runtime note",
    );
    // Decision 7: the note is generic — it names no specific repo, only "the worked repo".
    assert.ok(
      /the worked repo's JS dependencies/.test(WORKER_RUNTIME_APPEND),
      "the note refers to the worked repo generically",
    );
    for (const literal of ["uzi", "vtmocanu", "skills"]) {
      assert.ok(
        !WORKER_RUNTIME_APPEND.includes(literal),
        `the note must name no specific repo (found "${literal}")`,
      );
    }
  });
