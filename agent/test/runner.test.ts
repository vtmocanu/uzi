...
describe("RunRunner — usage-limit park (PRD #35)", () => {
  ...
  // error result.
  function limitFactory(homeRoot: string, iid: number): { factory: ExecutorFactory; paths: () => Paths } {
    ...
    return { factory, paths: () => ({ pluginDir, runHome, worktree: worktreeDirFor(iid) }) };
  }
  interface Paths {
    pluginDir: string;
    runHome: string;
  ...
  const assertAllGone = (p: Paths) => {
    assert.strictEqual(fs.existsSync(p.worktree), false, "runner clone must be removed");
    assert.strictEqual(fs.existsSync(p.pluginDir), false, "skills plugin dir must be removed");
    assert.strictEqual(fs.existsSync(p.runHome), false, "run HOME must be removed");
  };
  ...
  it("preserves the clone, the plugin dir and the run HOME when the server parks the run", async () => {
    ...
    try {
      ...
      await runnerWith(factory, gitlab).execute(gitlabClaim(91, { wait_on_limit: true }));
      const p = paths();
      ...
      assert.strictEqual(fs.existsSync(p.worktree), true, "runner clone must survive a park");
      assert.strictEqual(fs.existsSync(p.pluginDir), true, "skills plugin dir must survive a park");
      assert.strictEqual(fs.existsSync(p.runHome), true, "run HOME must survive a park");
  ...
  it("cleans up fully when the server answers 200 but failed the run instead of parking it", async () => {
    const { gitlab } = fakeGitlab();
    const homeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uzi-park-declined-"));
    try {
  ...
  it("fails with the structured limit fields, and does not park, when the run is opted out", async () => {
    ...
    try {
      ...
      assert.strictEqual(api.states.some((s) => s.body.status === "limit_wait"), false, "must not park");
