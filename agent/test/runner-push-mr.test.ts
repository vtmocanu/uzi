...
describe("RunRunner — worker-performed push + MR", () => {
  ...
  // issue #341: a zero-commit `prompt` run (schedule-fired ad-hoc work) now has its OWN
  // report-only terminal, separate from the issue-kind guard (ADR-0279 §2 forbids
  // broadening that gate). It still does NOT trip the issue-kind FAIL guard — the input
  // here is the exact CONFIRMED-empty [] that fails an issue run — but instead of pushing
  // an EMPTY merge request (the pre-#341 bug this test used to assert), it completes
  // report-only: status completed + report_only, with NO branch pushed and NO MR opened.
  // (Historical #279 intent: the empty-diff guard must stay issue-only; a prompt run must
  // not hit the issue FAIL path. #341 adds what the prompt kind does INSTEAD.)
  it("completes a zero-commit prompt run report-only — NO push, NO MR (issue #341)", async () => {
    const { gitlab, calls } = fakeGitlab();
    let pushed = false;
    git.pushBranch = (async () => {
      pushed = true;
    }) as typeof git.pushBranch;
    // A CONFIRMED-empty diff ([], not null): the prompt run committed nothing. The
    // StubExecutor still commits a real branch so fetchAgentBranch succeeds; the stub
    // drives the terminal's view of the diff, not the real (non-empty) one.
    git.changedFiles = (async () => []) as typeof git.changedFiles;
    ...
    const statuses = api.states
      .filter((s) => s.runId === claim.run_id)
      .map((s) => s.body.status);
    // It must NOT fail (the issue-kind FAIL guard stays issue-only)...
    assert.ok(
      !statuses.includes("failed"),
      `a zero-commit prompt run must not fail (got ${statuses.join(",")})`,
    );
    // ...and it completes report-only rather than opening an empty MR (#341).
    const completed = api.states.find(
      (s) => s.runId === claim.run_id && s.body.status === "completed",
    )!.body;
    assert.strictEqual(completed.report_only, true);
    assert.ok(
      !("mr_iid" in completed) || completed.mr_iid === undefined,
      "no MR iid on a zero-commit prompt report-only completion",
    );
    assert.strictEqual(calls.length, 0, "no MR opened on a zero-commit prompt run");
    assert.strictEqual(pushed, false, "no branch pushed on a zero-commit prompt run");
  });

  // issue #299 / #341: the prompt zero-diff terminal carries the SAME checkpoint-orphan
  ...
  it("fails a zero-commit prompt run that published a checkpoint (orphan guard), opening NO MR (issue #299)", async () => {
    ...
    assert.strictEqual(calls.length, 0, "no MR opened on the guarded prompt run");
    assert.strictEqual(pushed, false, "no branch pushed on the guarded prompt run");
  });

  // issue #341 companion: a `prompt` run that DID commit is UNCHANGED — it still pushes
  // and opens exactly one MR. This pins the zero-diff terminal to the confirmed-empty case
  // only, so it cannot suppress a real prompt deliverable.
  it("a prompt run that DID commit still pushes + opens exactly one MR (issue #341)", async () => {
    const { gitlab, calls } = fakeGitlab();
    let pushed = false;
    git.pushBranch = (async () => {
      pushed = true;
    }) as typeof git.pushBranch;
    // A non-empty diff — the prompt run committed real work.
    git.changedFiles = (async () => ["file.ts"]) as typeof git.changedFiles;
    const claim = gitlabClaim(25, { kind: "prompt" });
    await runner(new StubExecutor(nullLogger()), gitlab).execute(claim);
    ...
    assert.ok(
      !statuses.includes("failed"),
      `a committing prompt run must not fail (got ${statuses.join(",")})`,
    );
    const completed = api.states.find(
      (s) => s.runId === claim.run_id && s.body.status === "completed",
    )!.body;
    assert.strictEqual(completed.mr_iid, 42);
    assert.strictEqual(calls.length, 1, "the MR was opened on the committing prompt run");
    assert.strictEqual(pushed, true, "the branch was pushed on the committing prompt run");
  });
