...
test("claude C2 reducer: an empty turn projects only its terminal and leaves done=false, finalText undefined", async (t) => {
...
test("claude C2 reducer: a garbage/activity-only turn adds no messages and preserves the fallback result", async (t) => {
...
test("claude C2 reducer: signals from a subagent-origin frame are not folded", async (t) => {
...
test("claude C2 reducer: signals from a main-origin frame are folded once", async (t) => {
