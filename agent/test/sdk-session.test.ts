...
describe("sessionTranscriptResolvable (issue #105)", () => {
  ...
  it("finds it regardless of WHICH project dir holds it (glob is a deliberate superset)", async () => {
    // This is the guard against re-narrowing to one computed path. The CLI encodes
    // realpath(cwd), so a single computed dir false-absents on any symlinked data dir
    // and silently discards a live session; the glob cannot. See sdk-session.ts header.
