...
- **Forge layer**: `api/internal/forge` defines the `Forge` interface + neutral domain types; `gitlab.go` and `forgejo.go` are the drivers (ADR 0065). No other package imports a driver directly. All errors pass through a PAT-scrubbing redactor; outbound base URLs are allowlisted (`FORGE_ALLOWED_BASE_URLS`, https-only — SSRF guard).
