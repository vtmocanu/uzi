...
**This was not hypothetical when it was written — it had already corrupted an audit.** The auditor's independent `gitleaks:allow` count for the same MR came back **7 repo-wide**, produced by a grep carrying `--include='*.go' --include='*.yml'`: an accurate count of non-ignored `.go`/`.yml` files, reported as repo-wide, missing the two occurrences in the ignored-but-tracked brief. **Its instrument had exactly this defect three commits before this entry existed to name it.** So the cost is not a hypothetical missed sweep; it is a wrong number that entered the record through the team's own verification step, which is the step nobody re-audits.
