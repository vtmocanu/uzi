...
Two consequences worth knowing before you debug the wrong thing. **`brew link --overwrite` does NOT repair it**: brew's own bookkeeping still reads "already linked" and it refuses; `brew unlink agent-browser && brew link agent-browser` does. And the repair does not *hold* — the next `npm ci` in `agent/` undoes it. If you only need to drive a browser, call the Cellar binary directly and skip the whole cycle: `/opt/homebrew/Cellar/agent-browser/<version>/libexec/bin/agent-browser`, which no npm postinstall touches.
...
**A brace interval is a second instance — but ONLY under `-E`/`-P`, and the first version of this paragraph got it wrong.** Measured 2026-07-27 on a fixture with exactly 3 literal occurrences, ugrep 7.5.0:

```
grep -c  'tabIndex={0}'  -> 3   (correct: POSIX BRE requires \{...\} for an interval, so a bare {0} is LITERAL)
grep -cF 'tabIndex={0}'  -> 3
grep -cE 'tabIndex={0}'  -> 4   <- the trap
grep -cP 'tabIndex={0}'  -> 4   <- and here
```

*(This paragraph originally read "a reviewer verifying a mutation had been restored ran `grep -c 'tabIndex={0}'` and got a wrong count". **That did not happen.** Plain `grep` was right; the reviewer had asserted `'^2$'` expecting two occurrences when the true literal count was three, because the file also contains the string inside the comment arguing against re-adding it. An off-by-one expectation, blamed on the tool, written up here by the lead as a mechanism, and retracted by the reviewer who filed it — three days' worth of this file's own lesson in one paragraph.)*

**The durable rule survives the retraction, for a better reason than the one it was filed under.** Use `-F` when you mean a literal. And **verify restores with `git status`/`git diff`, not with a grep count** — not because grep miscounts, but because a literal count only tells you anything if you already know how many occurrences ought to exist. The VCS does not require you to know that, which is exactly why it is the right instrument for "is the tree back where it was".
