...
// TestWorkerListMarksEphemeral asserts the STATUS column marks an auto-provisioned,
// run-bound throwaway hosted worker `(ephemeral)` (PRD #529 M7), and that a normal
// worker carries no such marker.
//
// The fixture stages one ephemeral worker and one normal one so the failure that matters
// — a renderer that drops the marker, or stamps it on every row — is caught. Non-vacuous:
// dropping the `if w.Ephemeral` suffix in statusCell reddens the ephemeral assertion, and
// stamping it unconditionally reddens the normal-worker assertion.
func TestWorkerListMarksEphemeral(t *testing.T) {
	fc := &uzicli.FakeClient{Workers: []apitypes.WorkerDTO{
		{ID: "w1", Name: "ephem", Status: "online", AnthropicBindMode: "default", Ephemeral: true},
		{ID: "w2", Name: "standing", Status: "online", AnthropicBindMode: "default"},
	}}
	out, _, code := runCLI(t, fakeEnv(fc), "worker", "list")
	if code != uzicli.ExitOK {
		t.Fatalf("exit = %d, want 0", code)
	...
	if got := rowOf("ephem"); !strings.Contains(got, "(ephemeral)") {
		t.Errorf("ephemeral worker row = %q, want it to contain %q", got, "(ephemeral)")
	}
	if got := rowOf("standing"); strings.Contains(got, "(ephemeral)") {
		t.Errorf("standing worker row = %q must not carry the ephemeral marker", got)
	}
}
