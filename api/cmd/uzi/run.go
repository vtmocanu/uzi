...
func newRunCmd(env Env, gf *globalFlags) *cobra.Command {
	...
	cmd.AddCommand(newRunListCmd(env, gf), newRunGetCmd(env, gf), newRunLogsCmd(env, gf), wait, newRunReviewCmd(env, gf), newRunCreateCmd(env, gf), newRunApproveCmd(env, gf), newRunRejectCmd(env, gf), revise, newRunCancelCmd(env, gf), newRunStopCmd(env, gf), scope, followUp, answer, inputs, expedite, resumeNow, mrRework)
	return cmd
}
