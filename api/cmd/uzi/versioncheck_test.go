...
// D8's stronger claim: `uzi docs` is offline on EVERY invocation, not just when
// unconfigured. Even with a stamped binary and a configured server URL — the exact
// setup under which `run list` DOES probe (see TestSkewWarningGoesToStderrNotStdout)
// — a `docs` verb fires zero probes and prints no skew warning, because the whole
// `docs` subtree is exempt. Both leaves that take flags/args are covered.
func TestSkewWarningExemptCommandsDoNotProbe_Docs(t *testing.T) {
	withVersion(t, "v0.11.8")
	...
	for _, tc := range []struct {
		...
		{"docs search", []string{"docs", "search", "forge"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			fc := behindServer()
			args := append(append([]string{}, tc.args...), "--url", skewURL)
			_, errOut, code := runCLI(t, skewEnv(t, fc), args...)
			if code != uzicli.ExitOK {
				t.Fatalf("exit = %d, want 0", code)
			}
			if fc.BuildInfoCalls != 0 {
				t.Errorf("BuildInfoCalls = %d, want 0 — a configured `uzi %s` must not probe (D8)",
					fc.BuildInfoCalls, strings.Join(tc.args, " "))
			}
			if strings.Contains(errOut, "is behind") {
				t.Errorf("`uzi %s` printed a skew warning:\n%q", strings.Join(tc.args, " "), errOut)
			}
		})
	}
}
...
func TestExemptFromVersionCheck(t *testing.T) {
	...
	exempt := [][]string{
		...
		// `docs` and its leaves read the embedded corpus and make no network call of
		// their own (D8), like `version`.
