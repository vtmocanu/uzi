...
//
// This asserts on renderSteerQueue() alone (SGR-stripped, PER ROW), anchored on the
// KIND cell being the FIRST token of each row — NOT on m.View().Content, over which
// both substrings appear independently of the cell: "scope" is inside the scope row's
// state label "active (scope ceiling set)" and "follow-up" is in the footer keybind
// hint "f follow-up". A whole-view Contains therefore passes even with the P3c cell
// deleted (proven vacuous by a reviewer). Anchoring on the row PREFIX is what gates
// P3c: strip the cell and the scope row starts with its state label, so
// HasPrefix(row, "scope") turns false.
func TestSteerQueueLabelsTheScopeKind(t *testing.T) {
	runID := "r-own"
	scopeBody := "3"
	followBody := "please also update the docs"
	m := ownerModel(t, &uzicli.FakeClient{}, runID, ownedRun(runID))

	next, _ := m.Update(runInputsMsg{runID: runID, inputs: []apitypes.SteerInputDTO{
		{ID: 2, Kind: "scope", Body: &scopeBody, CreatedAt: time.Now().Add(-time.Minute)},
		{ID: 1, Kind: "follow_up", Body: &followBody, CreatedAt: time.Now().Add(-2 * time.Minute)},
	}})
	m = next.(tuiModel)

	// The queue block ONLY — View().Content would also carry the footer hint. SGR-strip
	// so the KIND cell is the leading printable token of each row.
	...
	for _, line := range lines {
		...
		if strings.Contains(line, followState) {
			followLine = line
		}
	}
	if scopeLine == "" {
		t.Fatalf("no scope row rendered in the steer queue:\n%s", block)
	}
	if followLine == "" {
		t.Fatalf("no follow_up row rendered in the steer queue:\n%s", block)
	}

	// The mutation-proof gate: the scope row must BEGIN with the KIND cell "scope".
	// Without the P3c cell the row begins with its state label "active (scope ceiling
	// set)", so a plain Contains(line, "scope") — which that label also satisfies — is
	// not enough; the prefix anchor is what removing the cell fails.
	if want := steerKindLabel("scope"); !strings.HasPrefix(strings.TrimSpace(scopeLine), want) {
		t.Errorf("scope row does not begin with the KIND cell %q; without the P3c cell it begins with the state label\nrow: %q", want, scopeLine)
	}
	if want := steerKindLabel("follow_up"); !strings.HasPrefix(strings.TrimSpace(followLine), want) {
		t.Errorf("follow_up row does not begin with the KIND cell %q\nrow: %q", want, followLine)
	}
}
