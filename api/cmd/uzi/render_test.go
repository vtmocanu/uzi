...
// TestRenderRunDetailChangedFiles pins `uzi run get`'s "files changed during planning"
// block (PRD #212 M4): the CLI twin of the web PlanPanel section, printed below the table
// like report_md, rendered only when the list is non-empty.
//
// The load-bearing assertion is that the porcelain lines go through sanitizeTTY and NOT
// cellText: sanitizeTTY spares the LEADING space of the porcelain XY status code, where
// cellText's trailing strings.TrimSpace would eat it and collapse " M path" (modified in
// worktree) into "M path" — losing the exact status-vs-index distinction M1 stored the
// porcelain lines to preserve. A cellText implementation of this block fails the present
// case below on the leading-space check.
func TestRenderRunDetailChangedFiles(t *testing.T) {
	base := apitypes.RunDTO{
		ID: "run-1", Kind: "issue", Status: "awaiting_approval",
		IssueTitle: "do the thing", ForgeType: "gitlab", Health: "ok",
	}

	// Present: the header renders, and each porcelain line survives verbatim — crucially
	// WITH its leading status-code space (" M src/app.ts"), which only sanitizeTTY spares.
	// The truncation marker (Decision 5) is a synthetic non-porcelain line and must render
	// verbatim too, un-parsed.
	present := base
	present.PlanChangedFiles = []string{" M src/app.ts", "?? notes.md", "… (+42 more)"}
	out := renderDetail(t, present)
	if !strings.Contains(out, "files changed during planning:") {
		t.Errorf("a run with plan-turn writes must render the header, got:\n%s", out)
	}
	// The leading space proves sanitizeTTY, not cellText, rendered this line.
	if !strings.Contains(out, " M src/app.ts") {
		t.Errorf("the porcelain status prefix (leading space) was stripped — cellText, not sanitizeTTY? got:\n%q", out)
	}
	if !strings.Contains(out, "?? notes.md") {
		t.Errorf("an untracked porcelain line was dropped, got:\n%s", out)
	}
	// Decision 5: the truncation marker renders verbatim, not parsed as porcelain.
	if !strings.Contains(out, "… (+42 more)") {
		t.Errorf("the truncation marker must render verbatim, got:\n%s", out)
	}

	// Sanitized: a control/bidi rune in a repo-controlled path must be stripped before it
	// reaches the terminal. U+202E is RIGHT-TO-LEFT OVERRIDE (written as the Go escape,
	// not a raw format rune, to stay clear of the lint ratchet — staticcheck ST1018).
	hostile := base
	hostile.PlanChangedFiles = []string{" M src/a\u202eb.ts"}
	if out := renderDetail(t, hostile); strings.Contains(out, "\u202e") {
		t.Errorf("a hostile path reached the terminal carrying the bidi override, got:\n%q", out)
	}

	// Empty and absent both render nothing: a clean plan turn and any pre-#212 run carry
	// [] / nil and must show no header. The present case above proves the header DOES render
	// when the list is non-empty, so this negative is non-vacuous.
	emptyList := base
	emptyList.PlanChangedFiles = []string{}
	if out := renderDetail(t, emptyList); strings.Contains(out, "files changed during planning:") {
		t.Errorf("an empty list must render no header, got:\n%s", out)
	}
	nilList := base // PlanChangedFiles left nil
	if out := renderDetail(t, nilList); strings.Contains(out, "files changed during planning:") {
		t.Errorf("a nil list must render no header, got:\n%s", out)
	}
}
