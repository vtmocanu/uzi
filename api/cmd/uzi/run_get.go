...
func newRunListCmd(env Env, gf *globalFlags) *cobra.Command {
	list := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			now := time.Now()
			for _, r := range runs {
				rows = append(rows, []string{r.ID, r.Kind, effectiveRunStatus(r.Status, r.IsPlanning, r.IsRevising), runAgeCell(r.RunDTO, now), harnessListCell(r.Harness), runTitle(r.RunDTO)})
			}
