...
// planMsg builds a plan-kind run message at seq — the producer emits kind "plan"
// (agent/src/runner.ts), which is what maxPlanSeq counts.
func planMsg(seq int32) apitypes.MessageDTO { return apitypes.MessageDTO{Seq: seq, Kind: "plan"} }

// TestRunWaitMinPlanSeqGatesStaleThenStopsFresh is the load-bearing PRD #603 test.
// The run sits at awaiting_approval the whole time; RunLogs first reports a STALE plan
// (seq == N, the pre-revise gate) and then a FRESH plan (seq > N). With --min-plan-seq N
// the wait must NOT stop on the stale gate and must stop once the fresh plan exists.
func TestRunWaitMinPlanSeqGatesStaleThenStopsFresh(t *testing.T) {
	const n = 5
	logsCalls := 0
	fc := &uzicli.FakeClient{
		GetRunHook: scriptHook(okStep("awaiting_approval")), // repeats forever
		RunLogsHook: func(string, int32) ([]apitypes.MessageDTO, error) {
			logsCalls++
			if logsCalls == 1 {
				return []apitypes.MessageDTO{planMsg(n)}, nil // stale: seq == N, not > N
			}
			return []apitypes.MessageDTO{planMsg(n + 1)}, nil // fresh: seq > N
		},
	}
	_, stderr, code := runCLI(t, fakeEnv(fc), "run", "wait", "r1",
		"--until", "awaiting_approval", "--min-plan-seq", "5", "--interval", "1ms")
	if code != uzicli.ExitOK {
		t.Fatalf("exit = %d, want 0 (stderr: %s)", code, stderr)
	}
	// It must have kept polling PAST the stale observation to reach the fresh plan. A
	// single RunLogs call means it stopped on the stale gate (gate off) or the compare
	// is inverted/off-by-one (seq==N wrongly counted as fresh via >= N).
	if logsCalls < 2 {
		t.Fatalf("wait stopped on the stale gate: RunLogs called %d times, want >= 2 (it must poll past seq %d until seq %d)", logsCalls, n, n+1)
	}
}

// TestRunWaitMinPlanSeqDoesNotDelayTerminal proves the gate is awaiting_approval-only:
// a terminal target still stops immediately even with --min-plan-seq set and no plan
// message present. --timeout 1s bounds the mutation this pins (applying the plan-seq
// gate to any target, not just awaiting_approval): a mutant would fall through and never
// stop on `completed`, so the wait would run to --timeout and return exit 7 — failing
// cleanly at the code check below, and keeping the trailing logsCalls assertion reachable
// (an unbounded wait would spin forever and never let either assertion run).
func TestRunWaitMinPlanSeqDoesNotDelayTerminal(t *testing.T) {
	logsCalls := 0
	fc := &uzicli.FakeClient{
		GetRunHook: scriptHook(okStep("completed")),
		RunLogsHook: func(string, int32) ([]apitypes.MessageDTO, error) {
			logsCalls++
			return nil, nil // no plan messages at all
		},
	}
	_, stderr, code := runCLI(t, fakeEnv(fc), "run", "wait", "r1",
		"--until", "completed", "--min-plan-seq", "5", "--interval", "1ms", "--timeout", "1s")
	if code != uzicli.ExitOK {
		t.Fatalf("exit = %d, want 0 — a terminal must not be gated by --min-plan-seq (stderr: %s)", code, stderr)
	}
	if logsCalls != 0 {
		t.Errorf("RunLogs must not be consulted for a terminal stop, called %d times", logsCalls)
	}
}
