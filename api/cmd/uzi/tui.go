...
func (m tuiModel) initCmds() []tea.Cmd {
	cmds := []tea.Cmd{m.fetchRunsCmd(m.board.admin, m.board.waitID), m.fetchSecretsCmd(),
		m.fetchRateLimitsCmd(), m.fetchCodexRateLimitsCmd(), m.fetchSettingsCmd(), tickAfter(boardPollInterval, m.board.tickGen), stripTickCmd(),
		themeTickCmd(),
...
func (m tuiModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case stripTickMsg:
		...
		return m, tea.Batch(m.fetchRateLimitsCmd(), m.fetchCodexRateLimitsCmd(), m.fetchSettingsCmd(), stripTickCmd())
