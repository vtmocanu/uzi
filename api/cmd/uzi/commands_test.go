...
func TestCommandTree(t *testing.T) {
	...
	subWant := map[string][]string{
		"run": {"list", "get", "logs", "wait", "review", "create", "approve", "reject", "revise", "cancel", "stop", "scope", "extend", "follow-up", "answer", "inputs", "expedite", "resume-now", "mr-rework", "pause", "resume", "rework", "export", "recovery", "discard"},
...
// PRD #634: the steer queue now also lists operator scope directives (kind='scope'),
// whose state IS their disposition — a scope row is never consumed. An applied ceiling
// reads "applied (finalized at the ceiling)"; a still-pending ceiling (NULL disposition
// on a live run) reads "active (scope ceiling set)". A follow_up sharing the queue keeps
// its consumed_at-derived state unchanged.
func TestRunInputsScopeDisposition(t *testing.T) {
	applied := "applied"
	appliedBody := "3"
	pendingBody := "5"
	followBody := "please add a test"
	fc := &uzicli.FakeClient{
		RunByID: map[string]apitypes.RunDTO{"r1": {ID: "r1", Status: "running", Kind: "issue"}},
		InputsByID: map[string][]apitypes.SteerInputDTO{"r1": {
			{ID: 3, Kind: "scope", Body: &pendingBody, CreatedAt: time.Now().Add(-10 * time.Second)},
			{ID: 2, Kind: "scope", Body: &appliedBody, CreatedAt: time.Now().Add(-time.Minute), Disposition: &applied},
			{ID: 1, Kind: "follow_up", Body: &followBody, CreatedAt: time.Now().Add(-2 * time.Minute)},
		}},
	}
	out, _, code := runCLI(t, fakeEnv(fc), "run", "inputs", "r1")
	if code != uzicli.ExitOK {
	...
	for _, want := range []string{
		"scope", "applied (finalized at the ceiling)", "active (scope ceiling set)",
		"follow-up", "queued",
	} {
		if !strings.Contains(out, want) {
			t.Errorf("inputs table missing %q:\n%s", want, out)
...
// PRD #634: `run scope --through N` submits a kind='scope' input whose body is N as a
// decimal string (the server parses + clamps it); a missing --through is a usage error.
func TestRunScopeSubmit(t *testing.T) {
	// inputOutcome carries a dedicated scope line (PRD #634 M3 P3a) so the confirmation
	// is not the generic "input submitted".
	if got := inputOutcome(kindScope, false); got != "scope directive sent" {
		t.Errorf("inputOutcome(kindScope,false) = %q, want %q", got, "scope directive sent")
	}

	fc := &uzicli.FakeClient{}
	out, errOut, code := runCLI(t, fakeEnv(fc), "run", "scope", "r1", "--through", "3")
	if code != uzicli.ExitOK {
		t.Fatalf("exit = %d, want 0", code)
	}
	if fc.LastInputKind != "scope" || fc.LastInputBody != "3" {
		t.Errorf("submitted kind/body = %q/%q, want scope/3", fc.LastInputKind, fc.LastInputBody)
	}
	if !strings.Contains(out, "scope directive sent") {
		t.Errorf("scope confirmation should read %q, got:\n%s", "scope directive sent", out)
	}
	if !strings.Contains(errOut, "uzi run inputs r1") {
		t.Errorf("scope confirmation should point at `uzi run inputs`, got:\n%s", errOut)
	}

	missing := &uzicli.FakeClient{}
	if _, _, code := runCLI(t, fakeEnv(missing), "run", "scope", "r1"); code != uzicli.ExitUsage {
		t.Fatalf("missing --through: exit = %d, want %d (usage)", code, uzicli.ExitUsage)
	}
	if missing.LastInputKind != "" {
		t.Errorf("a usage error must not submit an input, got kind %q", missing.LastInputKind)
	}
}
...
// The read-only agent-source leaves call the client and render the config / status
// projections. `get` shows the config (including credential_configured but never the
// value); `status` shows the sync status + staged pending flag. Both are GET-only:
// there is no sync/apply/config write verb (ADR 0602).
func TestAdminAgentSourceGetAndStatus(t *testing.T) {
	fc := &uzicli.FakeClient{AgentSourceV: apitypes.AgentSourceDTO{
		Config: apitypes.AgentSourceConfigDTO{
			URL:                  "https://git.example/roles.git",
			Ref:                  "main",
			Enabled:              true,
			Interval:             "1h0m0s",
			CredentialConfigured: true,
		},
		Status: apitypes.AgentSourceStatusDTO{
			LastSyncStatus: "ok",
			LastSyncSHA:    "deadbeef",
			LastAppliedSHA: "cafef00d",
		},
		Staged: &apitypes.AgentSourceStagedDTO{
			FetchedSHA: "deadbeef",
			Counts:     apitypes.AgentSourceCountsDTO{Staged: 3, Changed: 2, Failed: 1},
			Pending:    true,
		},
	}}

	// get: config projection, credential presence without the value.
	out, _, code := runCLI(t, fakeEnv(fc), "admin", "agent-source", "get")
	if code != uzicli.ExitOK {
		t.Fatalf("get exit = %d, want 0", code)
	}
	if !strings.Contains(out, "https://git.example/roles.git") || !strings.Contains(out, "credential_configured") {
		t.Errorf("get output missing config fields:\n%s", out)
	}

	// status: sync status + staged pending flag.
	out, _, code = runCLI(t, fakeEnv(fc), "admin", "agent-source", "status")
	if code != uzicli.ExitOK {
		t.Fatalf("status exit = %d, want 0", code)
	}
	if !strings.Contains(out, "pending") || !strings.Contains(out, "deadbeef") {
		t.Errorf("status output missing status fields:\n%s", out)
	}

	// get --json emits the config sub-object.
	out, _, code = runCLI(t, fakeEnv(fc), "admin", "agent-source", "get", "--json")
	if code != uzicli.ExitOK {
		t.Fatalf("get --json exit = %d, want 0", code)
	}
	if !strings.Contains(out, `"credential_configured": true`) {
		t.Errorf("get --json missing credential_configured:\n%s", out)
	}

	// The parent group is a container — it has no RunE, so both leaves exist under it.
	admin := findCmd(newRootCmd(fakeEnv(fc)), "admin")
	group := findCmd(admin, "agent-source")
	if group == nil {
		t.Fatal("missing `admin agent-source` group")
	}
	if group.RunE != nil || group.Run != nil {
		t.Error("`admin agent-source` must be a container group with no RunE")
	}
	for _, leaf := range []string{"get", "status"} {
		if findCmd(group, leaf) == nil {
			t.Errorf("missing `admin agent-source %s` subcommand", leaf)
		}
	}
}
...
func TestWorkerListSanitizesVersion(t *testing.T) {
	...
	// Columns are ID NAME STATUS UPTIME VERSION UPGRADE TOKEN OUTBOX. TOKEN is empty for
	// both fixtures so strings.Fields drops it, while OUTBOX renders "-" (no outbox, PRD
	// #1391 M5) and SURVIVES — so a healthy row has SEVEN fields and a blank VERSION
	// would collapse it to six, which the length check catches. Both workers are online
	// with a nil OnlineSince, so their UPTIME cell renders "-" (PRD #251) and VERSION
	// sits at index 4.
	var betaRow []string
