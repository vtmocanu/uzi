...
func (e *Engine) syncRepo(ctx context.Context, r store.ListEnabledReposWithConnectionsRow, st *repoState) {
	...
	// Board-free MR-state recorder (PRD #908): board-free sibling of SyncMRStates for every
	// card-less MR-bearing run — the scheduled lanes (prompt/self_improve) plus every
	// issue-less helper run (mr_rework, ci_fix, ...) — so their runs.mr_state is fresh for the
	// mr_rework detector reading candidates later this same tick. Reads live forge state (needs
	// the forge reachable, guaranteed by the early returns above); records mr_state and cancels
	// an in-flight rework on close/merge, but moves NO board card. Per-candidate errors are
