...
// TestSyncBoardFreeMRStatesRecordsIssuelessHelperLiveDB is the D1 regression for issue #1253:
// an issue-less MR-bearing HELPER run (here a ci_fix run sharing an issue-lane run's PR) must
// be watched by the board-free lane. Before the widen the lane gated on kind IN
// ('prompt','self_improve'), so a completed issue-less ci_fix run (issue_iid NULL) was owned by
// NEITHER watch lane — the board-coupled ListMRWatchCandidates JOINs issues and the board-free
// lane's kind filter dropped it — and its runs.mr_state stayed NULL forever. The widened
// predicate `issue_iid IS NULL OR kind IN ('prompt','self_improve')` admits it.
//
// This FAILS on the unfixed query (the ci_fix run is not a candidate, so SyncBoardFreeMRStates
// records nothing and mr_state stays NULL) and PASSES after the widen. ci_fix is the simplest
// issue-less fixture: runs_kind_shape (migration 00167) only requires pipeline_id + pipeline_ref
// non-null for it, so the seed sets those plus an mr_iid so the MR is watchable.
//
// Skipped unless UZI_TEST_DATABASE_URL points at a throwaway Postgres; `go test ./...` SKIPs.
func TestSyncBoardFreeMRStatesRecordsIssuelessHelperLiveDB(t *testing.T) {
	ctx := context.Background()
	dsn := os.Getenv("UZI_TEST_DATABASE_URL")
	if dsn == "" {
		t.Skip("UZI_TEST_DATABASE_URL not set; run via e2e/run-store-it.sh for live-DB coverage")
	...
	userID, connID, repoID := uuid.New(), uuid.New(), uuid.New()
	exec(`INSERT INTO users (id, email, password_hash) VALUES ($1, $2, 'x')`,
		userID, fmt.Sprintf("bf-cifix-%s@e2e", userID))
	exec(`INSERT INTO forge_connections (id, user_id, forge_type, base_url, bot_username, bot_forge_user_id, token_ciphertext)
	      VALUES ($1, $2, 'gitlab', 'https://forge.e2e', 'bot', 1, $3)`, connID, userID, []byte{0x1})
	exec(`INSERT INTO repos (id, connection_id, forge_project_id, path_with_namespace, web_url, default_branch, enabled)
	      VALUES ($1, $2, 1, 'g/bfcifix', 'https://forge.e2e/g/bfcifix', 'main', true)`, repoID, connID)

	const forgeProjectID = 4242
	const mrIID int64 = 9300
	branch := "uzi/ci-fix-" + uuid.New().String()

	// A completed, issue-less ci_fix run with an mr_iid (sharing an issue-lane run's PR) and
	// mr_state NULL — the pre-recorder bootstrap state. runs_kind_shape requires pipeline_id +
	// pipeline_ref for ci_fix, so both are set.
	runID := uuid.New()
	exec(`INSERT INTO runs (id, user_id, repo_id, kind, issue_iid, issue_title, issue_description, branch, pipeline_id, pipeline_ref, mr_iid, status)
	      VALUES ($1, $2, $3, 'ci_fix', NULL, 't', 'd', $4, 5150, $4, $5, 'completed')`,
		runID, userID, repoID, branch, mrIID)

	// Direction 1: the issue-less helper run IS a board-free watch candidate (the widen).
	cands, err := q.ListBoardFreeMRStateWatchCandidates(ctx, repoID)
	if err != nil {
		t.Fatalf("ListBoardFreeMRStateWatchCandidates: %v", err)
	}
	if len(cands) != 1 || cands[0].ID != runID {
		t.Fatalf("issue-less ci_fix run must be a board-free watch candidate; got %+v", cands)
	}

	// Direction 2: SyncBoardFreeMRStates records the forge-observed 'merged' state. The merge
	// edge cancels the (would-be) in-flight rework once; the run then self-evicts.
	if err := svc.SyncBoardFreeMRStates(ctx, repoID, forgeProjectID, &fakeForge{mr: forgeMR(mrIID, "merged")}); err != nil {
		t.Fatalf("SyncBoardFreeMRStates (merged): %v", err)
	}
	var got pgtype.Text
	if err := pool.QueryRow(ctx, `SELECT mr_state FROM runs WHERE id = $1`, runID).Scan(&got); err != nil {
		t.Fatalf("read mr_state: %v", err)
	}
	if !got.Valid || got.String != "merged" {
		t.Fatalf("after board-free sync, issue-less ci_fix runs.mr_state = %+v, want 'merged' persisted", got)
	}
	if len(canceller.calls) != 1 || canceller.calls[0] != mrIID {
		t.Fatalf("merge edge must cancel the rework exactly once for mr %d; got %v", mrIID, canceller.calls)
	}
