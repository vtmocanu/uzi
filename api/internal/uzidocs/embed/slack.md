...
- **Scope-capped completions** (PRD #634): if you or a teammate stopped or
  ...
  run](./run-activity.md#stopping-or-narrowing-a-run)), the run's completion
  DM in the thread is labelled **"Completed — narrowed by operator scope
  directive"** and its context line shows how many of the plan's milestones
...
- **Run judge and self-improvement**: if you've opted into the [run
  ...
  cycle is skipped because your vault is locked or the repo already has
  its maximum of open self-improvement merge requests. A started cycle
  sends no DM: the run itself and the merge request it opens already
  carry it. A judge or self-improvement
