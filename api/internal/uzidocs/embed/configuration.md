...
| `UZI_WORKER_UID_SPLIT` | `false` | Opt in to the worker/runner/runner-cmd uid-split profile Codex needs ([PRD #1493](../prds/1493-codex-k8s-uid-split-profile.md)). On, the `worker` and `seed-nix` containers start as root with a fixed capability set so the image's existing entrypoint establishes the three-identity split and then drops to `worker`; off, the pod renders byte-identical to today's single-uid start. Trades a bounded root startup window (the same posture compose has shipped since PRD #51) for OS-level containment of the Codex credential, the forge PAT and the join token from model-authorized commands — and it moves the kube-native worker namespace's PodSecurity `enforce` tier from `restricted` to `baseline` while it is on (`audit`/`warn` stay `restricted`). See [ARCHITECTURE.md](../ARCHITECTURE.md#worker-controller-k8s-only). |
| `UZI_CODEX_COMMAND_SANDBOX` | `required` | Whether a Codex command **must** have Landlock filesystem confinement to run. `required` (today's behaviour) fails a Codex command closed on a kernel without Landlock; `best-effort` runs it unconfined on such a kernel instead, relying on the uid split alone for containment — it never relaxes the split, and it still applies Landlock wherever the kernel does have it. Has no effect unless `UZI_WORKER_UID_SPLIT` is also on. |
...
| `FORGE_INTERACTIVE_RATE_MAX` | `120` | Per-**connection** outbound spend budget for interactive forge reads (the PR and CI views): a token bucket refilling at this many forge calls per minute, with a burst of the same size and a fixed one-minute window. Unlike `FORGE_RATE_LIMIT_MAX`, which caps inbound HTTP requests per user, this caps the forge calls those views actually make — and it is charged at the forge-call site, so a single list request that fans out into per-PR enrichment calls is charged for every one of them. This keeps a person clicking through PRs from starving your background board sync, which shares the same connection token. When a connection is over budget the server sheds the interactive read with an HTTP 429 and a `Retry-After` **before** making the forge call, so the shared PAT's rate budget is protected from a list→fan-out amplification. Clients honor the `Retry-After`: `uzi pr checks --watch` and `uzi ci ... --watch` (the CLI, landing in this release) back off for that interval and keep watching. A non-positive or unparseable value falls back to the default, same as the other numeric knobs here — the budget can be raised or lowered, but this knob cannot turn it off. |
...
| Var | Default | Notes |
|---|---|---|
| `AGENT_SOURCE_ALLOWED_BASE_URLS` | *(empty)* | Comma-separated SSRF allowlist for the agent-source clone URL; every entry must be an absolute `https://` URL or boot fails. Unlike `FORGE_ALLOWED_BASE_URLS`, an **empty** list is not a boot error — the feature is off by default, so an unconfigured instance still boots — but it does mean nothing is allowed until you set at least one entry here, no matter what an admin saves in the URL field. |
...
| Var | Default | Notes |
|---|---|---|
| `RUN_TIMEOUT` | `2h` | Wall-clock cap on a `running` run; the sweeper fails it past this. Also sent to the worker in the claim payload (`RunTimeoutSeconds`) for its own reference; the server's own sweeper is the actual enforcement. The admin-tunable [Run health](admin-settings.md#run-health) "near timeout" flag fires once a run's active running time (gate waits excluded) reaches a configurable share (default 85%) of this value — or of the run's persisted `budget_wall_seconds` instead, when one is set (a milestone-scaled run's frozen budget, or a non-interactive `uzi handoff` run's `HANDOFF_RUN_TIMEOUT`) — so it always warns proportionally before this ends the run, whatever you set it to. On an `issue` run with an approved (frozen) milestone list, this value is the base the effective wall clock scales from — see [Milestone-scaled budgets](#milestone-scaled-budgets-prd-122-m2) below. A run's owner can also grant that one run more time from the web or CLI ([`uzi run extend`](cli.md#commands)), up to an admin-set allowance, without changing `RUN_TIMEOUT` itself — see [Giving a run more time](run-health.md#giving-a-run-more-time). |
| `HANDOFF_RUN_TIMEOUT` | `4h` | Wall-clock cap for a **non-interactive** `uzi handoff` task run, decoupled from `RUN_TIMEOUT`. Persisted onto the run at create and enforced by the sweeper exactly as `RUN_TIMEOUT` is; clamped to the 8h wall ceiling. A non-positive value persists as NULL, so the sweeper's `COALESCE` falls back to the global `RUN_TIMEOUT`. `--interactive` handoffs persist NULL **and** are excluded from the wall-clock sweep entirely (`SweepRunningTimeout` skips `interactive` runs), so no `RUN_TIMEOUT` deadline applies to them at all — an interactive task's backstop is instead `WORKER_TASK_IDLE_TIMEOUT`, which finalizes it only once it has sat *parked without a follow-up* for that long (continued follow-ups keep the session alive), not a fixed task-wide ceiling. |
...
| `HANDOFF_RUN_MAX_ITERATIONS` | `10` | Implement ⇄ review loop cap for a non-interactive `uzi handoff`, decoupled from `RUN_MAX_ITERATIONS`, so the longer wall clock is not wasted by the iteration cap. A value that is non-positive or above the `int32` maximum persists as NULL and falls back to the global `RUN_MAX_ITERATIONS`; interactive handoffs likewise persist NULL and fall back to `RUN_MAX_ITERATIONS`. |
...
| `RUN_MAX_REQUEUES` | `1` | How many times the sweeper may re-queue a run whose worker went stale before failing it instead. A stale worker's run is re-queued after one stale window (`WORKER_HEARTBEAT_STALE`), but only **failed** after a second consecutive stale window (issue #1390) — giving a worker that briefly lost the api, rather than actually dying, time to return and have its run re-adopted (see `SWEEPER_BOOT_GRACE` and the active-run snapshot below) before the run is terminated. `RUN_MAX_REQUEUES` still caps how many times the run may be re-queued this way before that two-window rule stops protecting it and the next stale event fails it outright. `0` disables re-queueing entirely — a stale worker's run goes straight to the over-cap fail path, still subject to the two-window wait above (so it is failed roughly 2×`WORKER_HEARTBEAT_STALE` after the worker goes silent — two full stale windows plus up to one `SWEEP_INTERVAL` — not the pre-#1390 single window). |
...
| `RUN_RECOVERY_PARK_BASE` | `1m` | Wait for the **first** `recovery_wait` park a run enters when a resumed SDK turn keeps coming back empty or the provider is transiently failing, after bounded in-process retries (issue #1197; see [Recovering from a transient interruption](run-recovery-wait.md)). Each subsequent park on the same run doubles the previous wait, clamped at `RUN_RECOVERY_MAX_PARK` below. Unlike `limit_wait`, there is deliberately **no** `RUN_RECOVERY_MAX_WAITS` — a run keeps auto-resuming on this backoff until it recovers or the owner cancels it, so this knob (and the one below) shapes only the retry *cadence*, never a park budget. |
| `RUN_RECOVERY_MAX_PARK` | `30m` | Ceiling on a single `recovery_wait` park's backoff wait — the same role `RUN_LIMIT_MAX_PARK` plays for `limit_wait`, just clamped far shorter by default (30m vs. 8 days), because a transient interruption is expected to be short-lived rather than a multi-day usage-limit window. |
| `RUN_FORGE_UNREACHABLE_MAX_PARKS` | `6` | Cap on how many times one run may park at `recovery_wait` with cause `forge_unreachable` (a transient DNS/connect/5xx failure at clone or fetch, issue #1392; see [Forge unreachable at clone](run-recovery-wait.md#forge-unreachable-at-clone)) before the next such failure fails it instead, with `fail_origin=forge_unreachable` and no judge run, rather than parking again. Unlike the empty-turn park above, this cause has a lifetime cap by design — a dead forge should not hold a run open forever. `0` means unlimited, the same "off switch" convention as `RUN_LIMIT_MAX_WAITS=0`. |
...
| `SWEEPER_BOOT_GRACE` | `60s` | How long after the api starts (measured from the moment every worker-facing listener is bound, not from process start) the stale-worker sweep passes (mark-offline, over-cap fail, requeue) stay deferred (issue #1390). Covers the case where the api itself was down but its workers were fine: without the grace, every worker looks stale the instant the api comes back and the boot sweep would mark them offline and re-queue their live runs. Every other sweeper pass runs at boot as before. `0` restores the pre-#1390 behaviour of sweeping immediately. |
| `TERMINAL_PENDING_LEASE` | `1h` | How long a run stays protected from the stale-worker, timeout, and claimed-never-started sweeps while its worker has journaled an outcome for it but the api hasn't yet accepted it (or the worker has reported a pending-outcome overflow) — the lease that stops a normal recovery race from clobbering an in-flight completion (issue #1390). The run reverts to the ordinary stale-recovery path once the lease expires without the outcome landing. |
| `ACTIVE_SNAPSHOT_MAX_ENTRIES` | `256` | Server-side ceiling on how many entries a single worker's active-run snapshot (the list of run-lane attempts it reports executing on every heartbeat and claim, issue #1390) may hold; an oversized snapshot is rejected rather than truncated. |
| `WORKER_OUTBOX_MAX_PENDING` | `32` | Cap on how many terminal-pending (journaled-but-unaccepted) entries a worker may list in its active-run snapshot at once; past this the worker instead reports a pending-outcome overflow, which the api treats as a lease on the whole worker rather than per-run (issue #1390). |
| `WORKER_GAP_FILL_MAX` | `10000` | Ceiling on how many seqs may be missing from a run's stored message trace before the terminal fence gives up recovering them: once a `completed`/`failed` report's `messages_through_seq` leaves more than this many seqs missing, the api refuses the transition outright with a typed 409 (`gap_unrecoverable`) instead of the recoverable `messages_pending`, so one absurd fence value can never imply an unbounded gap-fill (PRD #1391 Run B M3). The `agent` binary reads a same-named var with the same default as its own local bound on one gap-fill pass through the message-gaps read; the two are independent knobs that happen to share a name and a default. |
| `UZI_ACTIVE_SNAPSHOT_DISABLED` | unset (off) | Rollback switch for the active-run snapshot feature (issue #1390): set it to start the api with the feature omitted from the negotiated `protocol_features`, so every connecting worker decodes and sends heartbeats exactly as it did before #1390, with no snapshot, no re-adoption, and no claim dedupe against it. A live-fleet lever to fall back to the pre-#1390 sweeper behaviour without a worker-side change. |
| `WORKER_AFFINITY_GRACE` | `2m` | **Chat lane only.** How long a re-queued chat run is claimable only by the worker that was already running it, before any of the user's other workers may claim it (gives a resume a chance to land back on the disk that still holds the session + git worktree). The run lane (`issue`/`ci_fix`/…) uses `WORKER_AFFINITY_CEILING` below instead. |
| `WORKER_AFFINITY_CEILING` | `2h` | Run lane analog of the grace above: a promoted or re-queued run stays pinned to the worker that already had it for as long as that worker is a live, non-draining claim target — no fixed window — and this ceiling only bounds the residual live-but-can't-serve edge case (a worker that stays alive but never picks the run back up). PRD #759 M3 raised it 30m→2h so a run parked on an Anthropic usage limit (see [Paused on a usage limit](run-limit-wait.md)) stays pinned to its original worker for the length of a typical park, where the SDK session is intact and any recovered work is exact; it bounds post-promotion **queue-dwell**, not the park duration itself, and stays flat rather than park-duration-aware — see [ADR-628](../adr/0628-cross-worker-resume-durability.md) D3a for why. PRD #1030 additionally holds affinity **through a worker image roll**: a cordoned/draining owner whose worker row and PVC survive keeps the pin on its parked run and resumes it on that same worker once its current work clears, instead of falling open the moment it starts draining; only a genuine **teardown** (the worker row deleted) still falls open immediately — see ADR-628's PRD #1030 amendment for the roll-vs-teardown discriminator. **Not yet wired into `docker-compose.yml`/`.env.example`** (unlike every other row in this table): setting it in `.env` on the bundled stack has no effect today, and the Go default is what actually applies. |
| `WORKER_SPREAD_GRACE` | `3× WORKER_POLL_INTERVAL` (9s) | How old a queued run must be before the fleet-aware claim spread (PRD #216) stops applying to it. Below this age, a worker already holding an active run defers a fresh queued run to a live, eligible peer that is strictly less loaded and has a free slot, rather than taking a second run while that peer sits idle; the run's own affinity leg (`WORKER_AFFINITY_CEILING` above) is checked first and always wins. Past this age the run is exempt from the spread and claimable by any eligible worker — fail-open, so a run can never be stranded waiting for a peer that isn't there. |

Invalid values for any of the above fall back to their defaults (the same lenient-parse behavior as the core table) and none is a boot guard, except `UZI_AUTOSTOP_ENABLED`, which aborts boot on a set-but-unparseable value (see its own row). All of these except `WORKER_AFFINITY_CEILING` (see its own row) — including `WORKER_SPREAD_GRACE` — are wired into the `api` service's `environment:` block in `docker-compose.yml` and documented in `.env.example`, so setting them in `.env` takes effect on the bundled stack.
...
**Mid-turn tick (issue #1597).** Both tiers above fire only at a turn boundary, so a single very long turn — the case this closes — could previously commit real work and reach no checkpoint at all until the turn ended. A repeating mid-turn tick, `CHECKPOINT_TICK_INTERVAL` (default `5m`; `0` disables it), fetches the runner clone's committed work back into the worker's bare repository at most once per tick, when the branch tip has moved and the clone is not busy, and publishes it to origin through the same credential-free broker once the branch tip differs from the last confirmed published tip and `CHECKPOINT_INTERVAL` has elapsed since the last publish attempt, not the last success — the same publish cadence as the iteration-boundary tier, just triggered without waiting for the turn to end. Worst-case loss is therefore roughly `CHECKPOINT_INTERVAL` plus one tick interval, as a best-effort bound rather than a guarantee: it holds only while the tick actually runs (the clone is not git-busy and no retained bare lock remains), the scan is trusted and finding-free, and the broker accepts the publish, so sink reachability alone does not provide it — a lingering git-busy marker defers the tick indefinitely, a finding or untrusted scan keeps that tick's work on worker-local storage (not a containment guarantee: a later unscanned park, shutdown or GitHub milestone publish may still ship it), and the tick retries a failed publish at most once per `CHECKPOINT_INTERVAL`; there is no durability claim while every sink is down, and a fetch-back with no successful remote publish is never reported as durable. A per-flight sink gate serializes the tick against every other path that moves the run's durable state (checkpoint closure, pause/wall/completion-hold parks, credential switch). A metadata-only git-busy probe (one non-blocking open of `.git`, then `lstat` of each marker; no git child) skips the tick whenever one of a fixed set of git lock files or merge/rebase/cherry-pick/revert markers is present in the runner clone, or its gitdir is unreadable, regardless of age. Scanning covers every checkpoint publish that goes through the checkpoint body without an overlay context — the mid-turn tick, the time-gated iteration-boundary tier, and milestone publishes on non-GitHub forges — over the pinned commit range being packed, minus content already public, before that range ever reaches origin; a finding or an untrusted scan skips the remote publish and keeps only the local fetch-back. On a Codex run such an overlay-less (non-GitHub) milestone publish is instead deferred (`scan_deferred`) rather than scanned inside the boundary permit. The park/shutdown/pause/capture checkpoint publishes and every GitHub milestone (`reap:true`) publish remain unscanned, unchanged from before this issue — a GitHub milestone, Codex or otherwise, always carries an overlay context and publishes unscanned, whether or not an overlay commit is actually built. Neither `CHECKPOINT_TICK_INTERVAL` nor `CHECKPOINT_INTERVAL` is exposed via the Helm chart or the controller, so hosted workers run the defaults. See [ADR-1597](../adr/1597-midturn-checkpoint-durability.md) for the full design, including the scan's size caps, merge-commit handling, and the Codex-permit deferral.
...
| `UZI_WORKER_TOKEN_FILE` | — (optional, preferred) | Path to a file containing the join token; the worker reads it once at startup. The bundled compose profile mounts it as a read-only compose file secret at `/run/secrets/worker_token`, which the root-started entrypoint forces to `0400 worker`-owned so the cap-less `runner` uid cannot read it (PRD #51; a restricted-namespace non-root single-uid start runs without the split — the PRD #58 posture); the post-read unlink no-ops on the read-only mount, so the file persists and the uid boundary — not removal — is the close. Takes precedence over `UZI_WORKER_TOKEN` when set. Keeps the token out of every process's `/proc/<pid>/environ`; see [proc-hardening.md](proc-hardening.md). A hosted worker's controller sets it; on OpenShift the Secret mounts elsewhere (`workers.secretMountPath`, see [openshift.md](openshift.md)) and the entrypoint's ownership check and the guardrails follow this path. |
...
| `CHECKPOINT_INTERVAL` | `20m` | Minimum time between the time-gated iteration-boundary checkpoint's origin publishes (PRD #267 — see [Checkpoints: two tiers](#checkpoints-two-tiers-prd-122-m8-prd-267) above); only fires when the branch tip differs from the last *confirmed* published tip, and the interval is measured from the last publish *attempt*, not the last success, so a persistent broker failure is retried at most once per interval. `0` disables the time-gated path, restoring milestone-only checkpoint behaviour. Only committed work is captured — a lead that doesn't commit isn't protected by a shorter interval. Also gates the mid-turn tick's origin publish (issue #1597) — the tick itself runs on `CHECKPOINT_TICK_INTERVAL`, but still only *publishes* once this interval has elapsed. Setting this to `0` keeps that normal publish gate permanently closed: with a nonzero `CHECKPOINT_TICK_INTERVAL` the mid-turn tick still fetches the runner clone's committed work back into the worker's bare repository, but never publishes it to origin. The one exception is an owed `scan_deferred` publish (a milestone publish deferred out of a Codex permit), which bypasses the gate once regardless of this setting; after that attempt the normal gate governs retries again. Not exposed via the Helm chart or the controller — hosted workers run the default. |
| `CHECKPOINT_TICK_INTERVAL` | `5m` | How often the mid-turn checkpoint tick (issue #1597 — see [Checkpoints: two tiers](#checkpoints-two-tiers-prd-122-m8-prd-267) above) fetches the runner clone's committed work back into the worker's bare repository while a turn is still in progress. `0` disables the mid-turn tick's fetch-back and origin publish, so checkpoints happen only at turn boundaries again (milestone and iteration-boundary, still secret-scanned when overlay-less); a non-GitHub Codex milestone's deferred (`scan_deferred`) scan+publish then runs right after the permit ends, before the milestone checkpoint returns, instead of waiting for the next iteration boundary. Otherwise, a fetch-back happens at most once per tick, when the branch tip has moved and the clone is not busy; an origin publish still additionally requires `CHECKPOINT_INTERVAL` to have elapsed and the branch tip to differ from the last confirmed published tip. With `CHECKPOINT_INTERVAL=0` the fetch-back keeps running on this interval, but the origin publish never happens except for an owed `scan_deferred` publish, which bypasses the closed gate once. Not exposed via the Helm chart or the controller — hosted workers run the default. |
| `WORKER_TRANSIENT_TRIP_MS` | `10m` (matches the batcher's built-in trip window) | How long an unbroken run of transient api-message failures may last before the batcher stops flushing to the network and starts spilling to the [message outbox](worker-setup.md#message-outbox) instead of tripping the run. Overridable so an e2e outage phase doesn't have to wait ten real minutes for the real default (PRD #1391 M2). |
| `WORKER_OUTBOX_RUN_MAX_BYTES` | `64 MiB` | Per-run byte quota for the [message outbox](worker-setup.md#message-outbox). Before a segment write would exceed it, the run's oldest segment is replaced by one compact range record instead (PRD #1391 M1). |
| `WORKER_OUTBOX_MAX_BYTES` | `512 MiB` | Worker-total byte quota across every run's outbox. When it binds, the globally-oldest segment across all runs is evicted to a range record the same way (PRD #1391 M1). |
| `WORKER_OUTBOX_SPILL_BUFFER_BYTES` | `2 MiB` | Hard cap on the batcher's in-memory buffer **while spilled** to the outbox. A message past the cap is dropped and folded into a compact range record the next spill flush writes durably, so a spilled batcher can never grow memory without bound; replay expands a range record into one gap marker per missing seq, so the stream stays contiguous (PRD #1391 M2). |
| `WORKER_OUTBOX_RETENTION` | `7d` | How long a fully-drained run's outbox subtree is kept before the retention sweep age-deletes it. Only a run whose records are all retired is eligible; a run with undrained segments is never removed by retention (PRD #1391 M1). |
| `WORKER_OUTBOX_TERMINAL_MAX_BYTES` | `1.25 MiB` | Per-field cap the terminal canonicaliser enforces on each optional string of a journaled `completed`/`failed` report (not a whole-report bound), applied identically before the first send and to the journal so first-send and replay stay byte-identical. It sits above the 1 MiB preserved patch a report can carry: the preserved patch is truncated rune-safely at a line boundary with a marker, and every other oversized optional field is dropped whole — so no value is ever byte-cut mid-token and a secret can never leak as an unrecognisable prefix past the api's scrubber (PRD #1391 Run B M3). |
| `WORKER_OUTBOX_RESERVE_TERMINALS` | `4` | How many terminal-sized journals (`WORKER_OUTBOX_TERMINAL_MAX_BYTES` each, plus per-record overhead — about 5.25 MiB at the defaults) the outbox's physical reserve is sized to always have room for, so a terminal outcome can still be journaled crash-safely on a full volume. A terminal past the reserve is sent unjournaled as it always was before this feature existed — logged and counted, never silently lost (PRD #1391 Run B M3). |
...
| `PROPOSAL_RATE_LIMIT_MAX` / `PROPOSAL_RATE_LIMIT_WINDOW` | `20` / `1m` | Per-worker limiter on the `propose_issue` worker endpoint (bounds a prompt-injected propose loop; the per-run pending-proposal cap of 10 is a separate, non-env constant). The same settings also size the separate per-worker buckets of the worker findings and recovery-settle endpoints (so a looping worker can't burn the owner's forge budget through the settle route). |
...
its recommendations feed that job.
