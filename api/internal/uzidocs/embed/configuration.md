...
| `WORKER_OUTBOX_TERMINAL_MAX_BYTES` | `1.25 MiB` | Hard cap the terminal canonicaliser enforces on a journaled `completed`/`failed` report, applied identically before the first send and to the journal so first-send and replay stay byte-identical. Above the 1 MiB preserved patch a report can carry; an optional field that would push the serialised report past this is dropped whole rather than cut mid-value, so a truncated secret can never leak past it (PRD #1391 Run B M3). |
| `WORKER_OUTBOX_RESERVE_TERMINALS` | `4` | How many terminal-sized journals (`WORKER_OUTBOX_TERMINAL_MAX_BYTES` each, plus per-record overhead — about 5.2 MiB at the defaults) the outbox's physical reserve is sized to always have room for, so a terminal outcome can still be journaled crash-safely on a full volume. A terminal past the reserve is sent unjournaled as it always was before this feature existed — logged and counted, never silently lost (PRD #1391 Run B M3). |
