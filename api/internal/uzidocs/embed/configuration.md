...
| `HANDOFF_RUN_TIMEOUT` | `4h` | Wall-clock cap for a **non-interactive** `uzi handoff` task run, decoupled from `RUN_TIMEOUT`. Persisted onto the run at create and enforced by the sweeper exactly as `RUN_TIMEOUT` is; clamped to the 8h wall ceiling. A non-positive value persists as NULL and falls back to the global `RUN_TIMEOUT`. `--interactive` handoffs are exempt — they too persist NULL and fall back to `RUN_TIMEOUT` (bounded in practice by `WORKER_TASK_IDLE_TIMEOUT`). |
