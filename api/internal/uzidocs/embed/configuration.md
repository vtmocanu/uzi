...
**Mid-turn tick (issue #1597).** Both tiers above fire only at a turn boundary, so a single very long turn — the case this closes — could previously commit real work and reach no checkpoint at all until the turn ended. A repeating mid-turn tick, `CHECKPOINT_TICK_INTERVAL` (default `5m`; `0` disables it), fetches the runner clone's committed work back into the worker's bare repository on every tick, and publishes it to origin through the same credential-free broker once `CHECKPOINT_INTERVAL` has elapsed and the branch tip has moved — the same publish cadence as the iteration-boundary tier, just triggered without waiting for the turn to end. Worst-case loss while at least one publish sink is reachable is therefore roughly `CHECKPOINT_INTERVAL` plus one tick interval; there is no durability claim while every sink is down, and a fetch-back with no successful remote publish is never reported as durable. A per-flight sink gate serializes the tick against every other path that moves the run's durable state (checkpoint closure, pause/wall/completion-hold parks, credential switch), and a stat-only busy probe skips the tick whenever a git lock or merge/rebase marker is present in the clone. Unlike the two tiers above, every mid-turn tick publish is secret-scanned over exactly the pinned commit range it packs before that range ever reaches origin; a finding or an untrusted scan skips the remote publish and keeps only the local fetch-back. The park/shutdown/pause/capture checkpoint publishes and the workflow overlay remain unscanned, unchanged from before this issue. See [ADR-1597](../adr/1597-midturn-checkpoint-durability.md) for the full design, including the scan's size caps, merge-commit handling, and the Codex-permit deferral.
...
| `CHECKPOINT_INTERVAL` | `20m` | Minimum time between the time-gated iteration-boundary checkpoint's origin publishes (PRD #267 — see [Checkpoints: two tiers](#checkpoints-two-tiers-prd-122-m8-prd-267) above); only fires when the branch tip has moved since the last publish, so an idle run costs nothing. `0` disables the time-gated path, restoring milestone-only checkpoint behaviour. Only committed work is captured — a lead that doesn't commit isn't protected by a shorter interval. Also gates the mid-turn tick's origin publish (issue #1597) — the tick itself runs on `CHECKPOINT_TICK_INTERVAL`, but still only *publishes* once this interval has elapsed. |
| `CHECKPOINT_TICK_INTERVAL` | `5m` | How often the mid-turn checkpoint tick (issue #1597 — see [Checkpoints: two tiers](#checkpoints-two-tiers-prd-122-m8-prd-267) above) fetches the runner clone's committed work back into the worker's bare repository while a turn is still in progress. `0` disables the mid-turn tick entirely, restoring turn-boundary-only checkpoint behaviour. A fetch-back happens every tick; an origin publish still additionally requires `CHECKPOINT_INTERVAL` to have elapsed and the branch tip to have moved. |
