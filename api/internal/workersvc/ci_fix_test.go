...
func TestCreateRunRefusesWhenCIFixActiveOnBranch(t *testing.T) {
	...
	fs := &fakeStore{issueByID: store.Issue{Title: "T", Labels: uziLabels(), HasPrdLink: true}, activeBranchRefRuns: 1}
	svc := New(fs, newBox(t), testParams())
	if _, err := svc.CreateRun(context.Background(), uuid.New(), uuid.New(), 9, "d", nil, nil, false, nil, nil, nil); err != ErrBranchInUse {
		t.Fatalf("err = %v, want ErrBranchInUse", err)
	}
}
