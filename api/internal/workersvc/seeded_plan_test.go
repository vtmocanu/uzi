...
func TestCreateRunSeededPlanRejectsEmptyAndWhitespace(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := svc.CreateRun(context.Background(), uuid.New(), uuid.New(), 4, "desc", nil, nil, false, &SeededPlan{PlanMD: tc.plan}, nil)
			if err != ErrPlanEmpty {
...
func TestCreateRunSeededPlanTooLarge(t *testing.T) {
	...
	_, err := svc.CreateRun(context.Background(), uuid.New(), uuid.New(), 4, "desc", nil, nil, false, &SeededPlan{PlanMD: oversize}, nil)
	...
	if _, err := svc2.CreateRun(context.Background(), uuid.New(), uuid.New(), 4, "desc", nil, nil, false, &SeededPlan{PlanMD: strings.Repeat("x", MaxSeededPlanBytes)}, nil); err != nil {
...
func TestCreateRunSeededPlanRejectsUnsafeTarget(t *testing.T) {
	...
	t.Run("benign plan is accepted", func(t *testing.T) {
		fs := seededIssueStore()
		svc := New(fs, newBox(t), testParams())
		if _, err := svc.CreateRun(context.Background(), uuid.New(), uuid.New(), 4, "desc", nil, nil, false, &SeededPlan{PlanMD: "Refactor the token store; add a test in queries_test.go"}, nil); err != nil {
...
func TestCreateRunSeededPlannedCommitValidation(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("reject: "+tc.name, func(t *testing.T) {
			fs := seededIssueStore()
			svc := New(fs, newBox(t), testParams())
			_, err := svc.CreateRun(context.Background(), uuid.New(), uuid.New(), 4, "desc", nil, nil, false, &SeededPlan{PlanMD: "# Plan\nDo it.", PlannedCommit: tc.planned}, nil)
	...
	for _, tc := range []struct {
		...
		t.Run("accept: "+tc.name, func(t *testing.T) {
			fs := seededIssueStore()
			svc := New(fs, newBox(t), testParams())
			if _, err := svc.CreateRun(context.Background(), uuid.New(), uuid.New(), 4, "desc", nil, nil, false, &SeededPlan{PlanMD: "# Plan\nDo it.", PlannedCommit: tc.planned, RequireBase: true}, nil); err != nil {
				t.Fatalf("CreateRun: %v", err)
			}
			p := fs.createRunParams
			if p == nil {
...
func TestCreateRunSeededScrubsSecretsIntoPlanMd(t *testing.T) {
	...
	const token = "glpat-SCRUBTEST00000000000000" //nolint:gosec // G101: gitleaks:allow synthetic non-credential fixture; asserts secretscrub matches the glpat- shape, never a real token
	fs := seededIssueStore()
	svc := New(fs, newBox(t), testParams())
	if _, err := svc.CreateRun(context.Background(), uuid.New(), uuid.New(), 4, "desc", nil, nil, false, &SeededPlan{PlanMD: "# Plan\nuse " + token + " to auth"}, nil); err != nil {
		t.Fatalf("CreateRun: %v", err)
	}
