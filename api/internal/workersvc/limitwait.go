...
func (s *Service) setLimitWait(ctx context.Context, run store.Run, wkr store.Worker, req StateRequest, sessionID pgtype.Text) (int64, error) {
	...
	if !d.Park {
		...
		// PRD #634 follow-up (issue #640 P2): a rate-limit opt-out is a terminal failure of
		// the run; if it carried a scope directive, settle the pending scope audit row
		// 'declined' so it does not render "active" forever. Best-effort (must never fail the
		// terminal transition) and guarded on the run carrying a directive (a 0-row no-op
		// otherwise) and on the transition actually applying (rows > 0).
