...
// traceToolUse builds a tool_use row for an arbitrary (name, input) — used to model a
// NON-execution tool (Read/Edit/Grep) whose input carries no `command`, so
// toolUseCommand yields "" for it exactly as the worker's mapper persists it.
func traceToolUse(seq int32, id, name string, input map[string]any) store.ListToolTraceForRunRow {
	p, err := json.Marshal(map[string]any{"id": id, "name": name, "input": input})
	if err != nil {
		panic(err)
	}
	return store.ListToolTraceForRunRow{Seq: seq, Kind: "tool_use", Payload: p}
	...
	cases := []struct {
		...
		},
		{
			...
			why: "ORDERING. helm ran green at 10 and was absent at 30; a tool can disappear mid-run. " +
				"The green is REAL usage, not `helm version` — a bare version probe is no longer " +
				"credited (issue #282), so it must be real work to exercise the strictly-later guard",
			rows: []store.ListToolTraceForRunRow{
				traceUse(9, "u1", "helm upgrade rel ./chart"),
				traceResult(10, "u1", "Release \"rel\" has been upgraded.", false),
		...
		},
		{
			...
			why: "helm demonstrably ran REAL work (helm upgrade) green AFTER the miss; " +
				"recording the EARLIEST green answered the wrong question and kept " +
				"reporting it. The green is real usage, NOT a bare `helm version` probe — " +
				"a probe green no longer suppresses (issue #282), so only real usage can " +
				"pin the latest-green-wins behaviour this case exists for.",
			...
			rows: []store.ListToolTraceForRunRow{
				traceUse(9, "g1", "helm upgrade rel ./chart"),
				traceResult(10, "g1", "Release \"rel\" has been upgraded.", false),
				traceUse(29, "m1", "helm template ."),
				traceResult(30, "m1", "bash: helm: command not found", true),
				traceUse(49, "g2", "helm upgrade rel ./chart"),
				traceResult(50, "g2", "Release \"rel\" has been upgraded.", false),
			},
			want: []string{},
		},
		{
			name: "11_version_probe_does_not_suppress_shim",
			why: "issue #282: `rg` resolves only to a Claude Code shell-function shim, so a " +
				"bare `rg --version` probe answers green in-session while a shim-blind " +
				"context reports `rg: command not found`. A version-probe green is no longer " +
				"proof of a resolvable binary, so the earlier genuine miss survives.",
			rows: []store.ListToolTraceForRunRow{
				traceUse(9, "m1", "rg pattern src/"),
				traceResult(10, "m1", "rg: command not found", true),
				traceUse(29, "g1", "rg --version"),
				traceResult(30, "g1", "ripgrep 14.1.0", false),
			},
			want: []string{"rg"},
		},
		{
			name: "12_real_usage_after_miss_still_suppresses",
			why: "control for issue #282: the fix is scoped to bare version PROBES only. A " +
				"REAL `rg somepattern .` green after the miss is proof the tool resolves, so " +
				"it still suppresses — proving the probe exclusion did not disable ordinary " +
				"latest-green-wins suppression.",
			rows: []store.ListToolTraceForRunRow{
				traceUse(9, "m1", "rg pattern src/"),
				traceResult(10, "m1", "rg: command not found", true),
				traceUse(29, "g1", "rg somepattern ."),
				traceResult(30, "g1", "src/main.go:1: somepattern", false),
			},
			want: []string{},
		},
	}
...
func TestPrescanSuppressionFixtureCombined(t *testing.T) {
	rows := []store.ListToolTraceForRunRow{
		...
		// 4 — helm, green BEFORE the miss (REAL usage, not a `helm version` probe — a bare
		// version probe is no longer credited green post-#282, so the green must be real work
		// for this case to exercise the strictly-later ordering guard)
		traceUse(409, "c4a", "helm upgrade rel ./chart"),
		traceResult(410, "c4a", "Release \"rel\" has been upgraded.", false),
...
// TestVersionProbeOnlyExecutables pins the issue #282 helper directly (mirroring
// TestExecutablesInParsesExecutablePosition): a basenamed executable whose ONLY
// invocation is a bare `--version`/`-V`/`version` probe is returned, because a
// shell-function shim answers such a probe and its green is not proof of a real binary.
// An executable that does real work — including one that both probes and works in the
// same command — is NOT returned. The narrow probe set deliberately excludes overloaded
// flags (`-v`, `-h`).
func TestVersionProbeOnlyExecutables(t *testing.T) {
	cases := []struct {
		command string
		want    map[string]bool
	}{
		{"rg --version", map[string]bool{"rg": true}},
		{"helm version", map[string]bool{"helm": true}},
		{"go version", map[string]bool{"go": true}},
		{"jq --version", map[string]bool{"jq": true}},
		{"kubectl -V", map[string]bool{"kubectl": true}},
		// `-v`/`-h` are overloaded, so they are NOT version probes.
		{"rm -v file", map[string]bool{}},
		{"df -h", map[string]bool{}},
		// rg both probes AND does real work → it resolved, so it is NOT returned.
		{"rg --version && rg pat", map[string]bool{}},
		// A probe with extra arguments is not a bare probe.
		{"helm version --short", map[string]bool{}},
		{"", map[string]bool{}},
	}
	for _, tc := range cases {
		t.Run(tc.command, func(t *testing.T) {
			got := versionProbeOnlyExecutables(tc.command)
			if len(got) != len(tc.want) {
				t.Fatalf("versionProbeOnlyExecutables(%q) = %v, want %v", tc.command, got, tc.want)
			}
...
// TestPrescanIgnoresCommandNotFoundInFileReadContent pins issue #326: a run that READS a
// file quoting `jq: command not found` must not report jq — the string is file content
// surfaced by a non-execution tool, not shell stderr. A genuine shell `command not found`
// for a different tool in the SAME trace is the negative control that must still report,
// so this one case covers both acceptance criteria (no false positive, no true-positive
// regression). Before the fix, jq is reported alongside kubectl.
func TestPrescanIgnoresCommandNotFoundInFileReadContent(t *testing.T) {
	rows := []store.ListToolTraceForRunRow{
		// A Read of a fixture file, its result cat -n numbered, quoting the literal.
		traceToolUse(9, "r1", "Read", map[string]any{"file_path": "agent/test/judge-runner.test.ts"}),
		traceResult(10, "r1",
			"    58\t  it(\"falls back\", () => {\n"+
				"    59\t    evidence: \"jq: command not found\",\n"+
				"    60\t  });", false),
		// A genuine shell miss for a different tool — must still be reported.
		traceUse(29, "b1", "kubectl get pods"),
		traceResult(30, "b1", "bash: kubectl: command not found", true),
	}
	assertReported(t, rows, []string{"kubectl"})
}
