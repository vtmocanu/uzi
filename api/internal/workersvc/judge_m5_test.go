...
func TestJudgeLoopThrottledByGuards(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		run := store.Run{ID: uuid.New(), UserID: burstUser, Kind: runkind.Issue, Status: "failed", Harness: string(HarnessClaude)}
		svc.maybeEnqueueJudge(context.Background(), run)
		if fs.createdJudgeRun != nil {
	...
	for i := 0; i < 4; i++ {
		fs.createdJudgeRun = nil
		run := store.Run{ID: uuid.New(), UserID: budgetUser, Kind: runkind.Issue, Status: "failed", Harness: string(HarnessClaude)}
		svc.maybeEnqueueJudge(context.Background(), run)
		if fs.createdJudgeRun != nil {
