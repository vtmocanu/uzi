...
//
// It ALSO ignores tool_result rows produced by a NON-execution tool (Read/Edit/Grep/Glob,
// indexed by nonExecToolUseIDs): that content is FILE TEXT surfaced by the tool, not shell
// stderr, so a `<tool>: command not found` quoted inside a fixture, comment or doc must not
// read as a missing worker tool (issue #326). A tool_result whose originating tool_use is
// ABSENT from the window is deliberately still scanned — the high-confidence forms must
// report even with no invocation to corroborate against.
func scanCommandNotFound(rows []store.ListToolTraceForRunRow) []missCandidate {
	...
	fileReads := nonExecToolUseIDs(rows)
	...
	for _, row := range rows {
		...
		// 🔴 issue #326: a Read/Edit/Grep/Glob result is FILE CONTENT surfaced by a
		// non-execution tool, not shell stderr — a `<tool>: command not found` quoted in a
		// fixture, comment or doc is not a missing worker tool. Skip such results. An
		// ABSENT originating tool_use is NOT skipped (the row window can start mid-run),
		// preserving the no-invocation high-confidence contract.
		if id, _ := toolResultOutcome(row.Payload); fileReads[id] {
			continue
		}
...
// nonExecToolUseIDs indexes the tool_use ids of NON-execution tools in the trace — a
// Read/Edit/Grep/Glob whose payload carries no input.command (toolUseCommand yields "").
// scanCommandNotFound skips those tools' tool_result rows: their content is FILE TEXT
// surfaced by the tool, not shell stderr, so a `<tool>: command not found` quoted inside a
// fixture, comment or doc must not read as a missing worker tool (issue #326). A
// tool_result whose originating tool_use is ABSENT from the window is deliberately NOT
// skipped — the high-confidence forms must still report with no invocation to corroborate
// against (fixture case 8, TestPrescanHighConfidenceFormsSurviveWithoutInvocation).
func nonExecToolUseIDs(rows []store.ListToolTraceForRunRow) map[string]bool {
	ids := map[string]bool{}
	for _, row := range rows {
		if row.Kind != "tool_use" {
			continue
		}
		if id, cmd := toolUseCommand(row.Payload); id != "" && cmd == "" {
			ids[id] = true
		}
	}
	return ids
...
func observedGreenTools(rows []store.ListToolTraceForRunRow) map[string]int32 {
	...
	for _, row := range rows {
		...
		case "tool_result":
			...
			// A bare `tool --version`/`-V`/`version` probe is answered by a Claude Code
			// shell-function shim, so its green is not proof of a resolvable binary
			// (issue #282). Exclude a probe-only executable so a genuine `command not
			// found` for the same shimmed name survives suppressResolved.
			probeOnly := versionProbeOnlyExecutables(cmd)
			for _, tool := range observedTools(cmd, row.Payload) {
				if vetoed[tool] || probeOnly[tool] {
					continue
				}
...
// versionProbeOnlyExecutables returns the basenamed executables whose EVERY
// invocation in `command` is a bare version/help PROBE (`tool --version`, `tool -V`,
// `tool version`) — the executable followed by exactly one version token and nothing
// else in that command segment. A Claude Code shell-function shim answers such a probe,
// so a green result for a probe-only executable is NOT proof the tool resolves to a real
// binary (issue #282): a shim-blind context — a subshell, exec.LookPath, a subagent —
// then reports `command not found` for the same name. An executable that ALSO appears in
// a non-probe segment of the same command (`rg --version && rg pat`) is NOT returned — it
// did real work, so its green stands.
//
// Deliberately CONSERVATIVE: anything ambiguous is treated as a non-probe, so the
// executable stays credited and its miss stays suppressed exactly as before this fix.
// That is NOT the report-safe direction — a credited tool has its miss dropped by
// suppressResolved when the green is later — it is chosen to bound THIS fix's blast
// radius: only a clear-cut bare-version probe is excluded, so a real tool is never newly
// reported missing on an ambiguous invocation. The probe set is narrow on purpose:
// `-v`/`-h`/`--help` are overloaded (`rm -v`, `df -h`) and are NOT version probes.
func versionProbeOnlyExecutables(command string) map[string]bool {
	inProbe := map[string]bool{}
	inNonProbe := map[string]bool{}
	...
	for i := 0; i < len(toks); i++ {
		...
		if !execNext {
			continue // an argument of the current segment, not an executable position
		}
		if envAssignment.MatchString(t) {
			continue // a FOO=bar prefix; the executable is still ahead
		}
		execNext = false
		name := basenameToken(t)
		// The segment's arguments: the tokens up to the next separator or the end. Left
		// in place for the loop to skip via execNext=false; gathered here only to classify.
		var args []string
		for j := i + 1; j < len(toks) && !shellSeparators[toks[j]]; j++ {
			args = append(args, toks[j])
		}
		if name == "" {
			continue
		}
		if len(args) == 1 && isVersionProbeToken(args[0]) {
			inProbe[name] = true
		} else {
			inNonProbe[name] = true
		}
	}
...
// isVersionProbeToken reports whether a lone argument is a bare version probe. Narrow by
// design (see versionProbeOnlyExecutables): only the three unambiguous forms.
func isVersionProbeToken(arg string) bool {
	switch arg {
