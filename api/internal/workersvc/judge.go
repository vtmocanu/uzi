...
// suppressed rather than reported as a missing worker tool. Both are anchored on a
// leading command separator so a mere CALL SITE or a substring inside another token is
// NOT matched — only a real definition. The captured group ([1]) is the function name.
var (
	// reShellFuncPosix matches the POSIX form `name() { … }` (optionally `function name() {`),
	// anchored on a statement start (`;`, `&`, `|`, `(`) or leading whitespace.
	reShellFuncPosix = regexp.MustCompile(`(?:^|[;&|(]\s*|\s)(?:function\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*\(\s*\)\s*\{`)
	// reShellFuncKeyword matches the ksh/bash keyword form `function name` (no parens).
	reShellFuncKeyword = regexp.MustCompile(`\bfunction\s+([A-Za-z_][A-Za-z0-9_]*)\b`)
)
