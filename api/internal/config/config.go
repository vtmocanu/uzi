...
type Config struct {
	...
	// AgentSourceAllowedBaseURLs is the SEPARATE SSRF allowlist for the agent-source
	// repo sync (PRD #602 M2): the only base URLs an agent-source clone may target.
	// Normalized (scheme+host, no trailing slash), https only — same shape as
	// ForgeAllowedBaseURLs but a DISTINCT list, because the forge host set and the
	// role-source host set share no concern (reusing the forge list would couple
	// them for no reason). Unlike the forge list this may be EMPTY: the feature is
	// off by default, so an unconfigured instance must still boot, and an empty list
	...
	// Ephemeral worker auto-provisioning knobs (PRD #529 M2). The feature is gated by
	// TWO switches beyond these — the instance kill-switch (app_settings
	// ephemeral_workers_enabled) and the per-user opt-in (users.ephemeral_workers_enabled)
	// — so these tune HOW MUCH provisioning happens once it is turned on, never WHETHER.

	// EphemeralMaxPerUser is the per-user cap on CONCURRENT ephemeral hosted workers
	// (UZI_EPHEMERAL_MAX_PER_USER, default 2). It is a SEPARATE cap from the standing
	// per-user hosted quota (CountHostedWorkersForUser), so an opted-in user with a full
	// persistent quota is not blocked from ephemeral auto-provisioning. Checked under the
	// same per-user advisory lock the persistent provision uses, so the cap is atomic.
	...
	// would leave a worker pending until its token expires.
...
func Load() (Config, error) {
	...
	// PRD #602 M2: the SEPARATE agent-source SSRF allowlist. Default EMPTY (the
	// feature is off by default), so unlike the forge list an empty value is NOT a
	// boot error — but a present-but-non-https/malformed entry still is (fail-closed).
	agentSourceAllowed, err := parseAgentSourceAllowedBaseURLs(getenv("AGENT_SOURCE_ALLOWED_BASE_URLS", ""))
	if err != nil {
		return Config{}, err
	}
...
// parseAgentSourceAllowedBaseURLs parses the SEPARATE agent-source SSRF allowlist
// (PRD #602 M2). Like parseAllowedBaseURLs, every PRESENT entry must be an absolute
// https URL, normalized (scheme+host) via NormalizeForgeBaseURL and deduped, and a
// non-https/malformed entry is a hard boot error (fail-closed on malformed). The
// CRITICAL difference from parseAllowedBaseURLs: an EMPTY list is TOLERATED and
// returns nil — the agent-source feature is off by default, so an unconfigured
...
// agent-source base URL (PRD #602 M2). It mirrors ForgeBaseURLAllowed but checks the
// SEPARATE AgentSourceAllowedBaseURLs list, and returns false when that list is
// empty — nothing is allowed until an admin configures the allowlist.
func (c Config) AgentSourceBaseURLAllowed(raw string) bool {
	norm, err := NormalizeForgeBaseURL(raw)
	if err != nil {
		return false
	}
	for _, a := range c.AgentSourceAllowedBaseURLs {
		if a == norm {
			return true
		}
	}
