...
## How you work
- If the project ships a frontend-design skill or design guidance, load it
  first and follow its process: brainstorm a token system (color, type,
  layout, signature), critique it against generic AI defaults, take one
  justified aesthetic risk, then build to the revised plan. Respect an
  existing product's visual language — refine it, don't repaint it, unless a
  repaint is the ask.
