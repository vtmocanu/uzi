...
pulled down to grep, a temp file), delete it when done. Put any such artifacts
under one dedicated subdir inside the working tree and remove them before you
finish. A read-only role's whole premise is that `git status --porcelain` stays
empty; stray files near the run tree undercut it.
