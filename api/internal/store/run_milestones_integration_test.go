...
func TestRunMilestonesLifecycleLiveDB(t *testing.T) {
	...
	t.Run("candidate then approve freezes", func(t *testing.T) {
		...
		// Issue #260: the GetRunMilestoneFreezeSnapshot the approve-time instrumentation
		// reads must see the same pre-approve state — candidate set, frozen still NULL,
		// updated_at populated.
		snap, err := q.GetRunMilestoneFreezeSnapshot(ctx, gateRun)
		if err != nil {
			t.Fatalf("GetRunMilestoneFreezeSnapshot (pre-approve): %v", err)
		}
		if string(snap.MilestonesCandidate) != `[{"id": "m1", "title": "Only one now"}]` {
			t.Fatalf("snapshot candidate pre-approve = %s, want the revised list", snap.MilestonesCandidate)
		}
		if snap.MilestonesFrozen != nil {
			t.Fatalf("snapshot frozen must be NULL before approve, got %s", snap.MilestonesFrozen)
		}
		if !snap.UpdatedAt.Valid {
			t.Fatalf("snapshot updated_at must be populated, got invalid")
		}
		...
		// Issue #260: the same snapshot read now reflects the frozen list the log captures
		// as after_frozen.
		snap, err = q.GetRunMilestoneFreezeSnapshot(ctx, gateRun)
		if err != nil {
			t.Fatalf("GetRunMilestoneFreezeSnapshot (post-approve): %v", err)
		}
		if string(snap.MilestonesFrozen) != `[{"id": "m1", "title": "Only one now"}]` {
			t.Fatalf("snapshot frozen post-approve = %s, want a copy of the candidate", snap.MilestonesFrozen)
		}
