...
// versions:
//   sqlc v1.31.1
// source: finding_issue_close.sql
...
type ListFindingIssueCloseEdgesRow struct {
...
func (q *Queries) ListFindingIssueCloseEdges(ctx context.Context, repoID uuid.UUID) ([]ListFindingIssueCloseEdgesRow, error) {
	...
	defer rows.Close()
	items := []ListFindingIssueCloseEdgesRow{}
	for rows.Next() {
		var i ListFindingIssueCloseEdgesRow
		if err := rows.Scan(&i.ID, &i.FiledIssueIid); err != nil {
			return nil, err
		}
