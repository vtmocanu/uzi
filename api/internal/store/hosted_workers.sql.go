...
const countEphemeralHostedWorkersForUser = `-- name: CountEphemeralHostedWorkersForUser :one
SELECT count(*) FROM workers WHERE user_id = $1 AND kind = 'hosted' AND ephemeral
`
...
func (q *Queries) CountEphemeralHostedWorkersForUser(ctx context.Context, userID uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countEphemeralHostedWorkersForUser, userID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
SELECT count(*) FROM workers WHERE user_id = $1 AND kind = 'hosted' AND NOT ephemeral
`
...
const deleteEphemeralWorkerForRun = `-- name: DeleteEphemeralWorkerForRun :execrows
DELETE FROM workers w
WHERE w.ephemeral
  AND w.ephemeral_run_id = $1::uuid
  AND NOT EXISTS (
