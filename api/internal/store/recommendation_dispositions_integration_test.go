...
func TestRecommendationDispositionsLiveDB(t *testing.T) {
	...
	// Isolation: this test deliberately ends with an OPEN improve_uzi/'' rec (the tail Undo
	// re-includes it). The self-improve backlog query is now owner-scoped
	// (ListOpenImproveUziRecommendationsForUser — PRD #590 M2 deleted the instance-wide one),
	// so a leftover row only leaks into a later assertion that shares this user; the cleanup
	// keeps that robust regardless. Deleting the review cascades its recommendations,
	// dispositions, and filed links. Runs before the deferred pool.Close (LIFO), and on
	// failure too (Fatalf runs defers).
	defer func() {
