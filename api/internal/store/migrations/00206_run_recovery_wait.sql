...
-- together. The promotion index is built concurrently in 00208.

-- runs.status: widen to a thirteenth value. The twelve existing values are carried
-- from the live constraint (00204_run_paused.sql, validated by 00205). A DROP+ADD that
...
-- Validate separately in 00207 so the backlog scan does not retain this migration's
-- ACCESS EXCLUSIVE lock. Integration verified against 00204/00205 on 2026-09-08.
...
-- Narrow the status CHECK back to the twelve values that existed immediately before this
-- migration (00204's set). DATA-DEPENDENT and best-effort, copying 00170/00091's honesty:
