...
-- Validate the seven CHECKs added NOT VALID in 00224 (PRD #1332 M5A / C1). VALIDATE
...
-- Split from 00224 so the adds are lock-cheap and the validation is non-blocking, per the
-- standard two-step pattern (00219/00220, 00216/00217) for a CHECK on a live table.
--
-- The backlog satisfies every CHECK by construction: 00224 backfilled runs.harness from
...
-- pre-00225 state — each constraint present but NOT VALID — by dropping and re-adding it
-- NOT VALID. The CHECK bodies match 00224's Up verbatim. 00224's Down then drops them with
