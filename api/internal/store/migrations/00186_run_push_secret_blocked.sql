...
-- `remote rejected` the run ends `failed` with this typed origin. The pre-push diff
-- is DELIBERATELY NOT written to runs.preserved_patch on this path (it may carry the
-- detected secret); the committed work stays recoverable from the run branch/PVC. This
-- mirrors the TYPED-origin shape of the workflow_scope_missing guard in
-- agent/src/ci-config-guard.ts, but NOT its diff preservation, now for the secret-in-push class.
--
...
-- failed run.
--
-- preserved_patch ALREADY EXISTS (added by 00137) and is reused unchanged by the OTHER
-- origins (workflow_scope_missing / finalize_base_align_conflict) — this migration does
-- NOT touch that column, in either direction (00137 owns it), and push_secret_blocked
-- deliberately does not use it (see above).
--
