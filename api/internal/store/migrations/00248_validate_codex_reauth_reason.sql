...
-- Validate the CHECK added NOT VALID in 00247 (issue #1594). VALIDATE CONSTRAINT scans
...
-- hold. Split from 00247 so the add is lock-cheap and the validation non-blocking, per the
-- two-step pattern (00239/00240) for a CHECK on a live table.
--
-- NOTE (goose numbering): drafted as 00248, immediately after 00247; renumber the PAIR
-- above the live head together at landing via `task migration:renumber` if another
-- migration lands first.
--
-- The backlog satisfies the CHECK by construction: 00247 added reauth_reason nullable
...
-- pre-00248 state — the constraint present but NOT VALID — by dropping and re-adding it
-- NOT VALID. The CHECK body matches 00247's Up verbatim; 00247's Down then drops it with
