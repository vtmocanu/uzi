...
-- Validate the CHECK added NOT VALID in 00238 (PRD #1209 M1). VALIDATE CONSTRAINT scans
...
-- would hold. Split from 00238 so the add is lock-cheap and the validation non-blocking,
-- per the two-step pattern (00226/00227, 00233/00234) for a CHECK on a live table.
--
-- NOTE (goose numbering): drafted as 00239, immediately after 00238; renumber the PAIR
-- above the live head together at landing via `task migration:renumber` if another
-- migration lands first.
--
-- The backlog satisfies the CHECK by construction: 00238 added reauth_required NOT NULL
...
-- pre-00239 state — the constraint present but NOT VALID — by dropping and re-adding it
-- NOT VALID. The CHECK body matches 00238's Up verbatim; 00238's Down then drops it with
