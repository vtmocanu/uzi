...
// through the model struct. The ONLY queries whose *source text* contains the identifier
// `online_since` are therefore the three liveness WRITES. So the guard is: every query
// whose body names `online_since` must be in the allowed write set. Any new reader added
// in a WHERE/ORDER BY/JOIN — the scheduling shapes we are defending against — names the
// column literally and trips this. A future explicit-projection SELECT that legitimately
// reads the column for the DTO would need adding to allowedOnlineSinceQueries with a note.
func TestOnlineSinceIsWriteOnlyAndNeverScheduled(t *testing.T) {
	// The queries permitted to reference online_since at all. All three are WRITES on the
	// liveness path; none READ it in a predicate/order/join.
