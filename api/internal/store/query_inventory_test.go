...
var queryInventory = []queryPin{
	...
	// ── selfimprove.sql — the self_improve run producer (PRD #590 M2 deleted the engine;
	//    the relocated schedsvc.fireSelfImprove and the run kind stay) ──────────────────
	{"CountActiveSelfImproveRunsForRepo", "selfimprove.sql", unpinnedPin,
		"PRD #590 M1 — the per-repo active-run pre-check the relocated schedsvc.fireSelfImprove " +
			"path uses. No live-DB test in internal/store or internal/handler executes it: its only " +
			"caller is schedsvc.fireSelfImprove, exercised by the schedsvc unit tests against a fake " +
			"Store (schedsvc is not in inventoryPackages and has no live-DB test). Its `repo_id = @repo_id " +
			"AND status NOT IN (completed,failed,cancelled)` predicate is the per-repo scoping that the " +
			"00158 partial index also enforces at insert; a live pin belongs with a future integration " +
			"test, recorded here rather than left silent"},
	{"ListOpenImproveUziRecommendationsForUser", "selfimprove.sql", "TestRecommendationFiledIssuesLiveDB",
		"PRD #590 M2 made this the sole improve_uzi backlog query (the instance-wide " +
			"ListOpenImproveUziRecommendations was deleted with the engine). Reached through the " +
			"openImproveUziTargets(userID) helper — NOT visible to a body scan. That helper's calls in " +
			"TestRecommendationFiledIssuesLiveDB pin the PRD #68 Decision 12 exclusion in both " +
			"directions (present before any claim, absent while CLAIMED, absent after a re-judge); " +
			"TestRecommendationDispositionsLiveDB pins the PRD #94 Decision 9 exclusion through the same " +
			"helper; and TestFiledIssueCloseAutoDonesOnceLiveDB calls it directly at " +
	...
	{"CreateSelfImproveRun", "selfimprove.sql", unpinnedPin,
		"No live test executes it. Every caller path is faked: schedsvc/scheduler_test.go drives " +
			...
			"(self_improve.go) is reached from no live-DB test. So the INSERT has never run against " +
			"the real schema — including the self_improve kind-shape CHECK and the per-repo " +
			"uq_runs_one_active_self_improve partial index (00158), which is the guard that stops a " +
			"double-create onto the fixed branch. The 23505 handling is asserted against a fake that " +
			"RETURNS 23505; nothing has made Postgres produce one"},
	{"MarkImproveUziRecommendationsAddressed", "selfimprove.sql", unpinnedPin,
		"No live test executes it — only the fake Store in schedsvc/scheduler_test.go. It is a WRITE " +
	...
	{"DeleteAppSettings", "settings.sql", unpinnedPin,
		"PRD #590 M2 — retires the legacy selfimprove_* keys after the boot-migration materializes " +
			"the self_improve schedule row. Its only caller is schedsvc.MigrateLegacySelfImprove, which " +
			"no live-DB test in inventoryPackages drives (schedsvc is not an inventory package and the " +
			"migration is exercised only by future LiveDB tests). Its `key = ANY(@keys)` delete has " +
			"never run against the real schema; a live pin belongs with a boot-migration integration test"},
