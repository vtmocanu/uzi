...
--
-- stop_kind (PRD #634) is selected FROM THE RUNS ROW so the notifier can tell a
-- scope-capped completion (status='completed' AND stop_kind='scope_capped', written by
-- m3 when an operator scope directive narrows the run) from an ordinary one and render
