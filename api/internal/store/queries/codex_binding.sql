...
WHERE r.id = @id
  ...
  AND CASE WHEN pg_input_is_valid(r.codex_account_key, 'jsonb')
           ...
           ELSE false END
  AND r.codex_account_key = @account_key::text
...
-- name: MarkCodexReauthRequired :execrows
...
WHERE id = @id AND user_id = @user_id
    AND coord_state IN ('idle', 'committed')
    AND recovery_sealed IS NULL
    AND generation = @observed_generation::bigint
    AND credential_revision = @observed_credential_revision::bigint;
