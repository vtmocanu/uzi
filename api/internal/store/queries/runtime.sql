...
-- SyncBoardFreeMRStates (issue-less helper runs plus prompt/self_improve, board-free) —
-- PRD #908; each records the state for its own lane through this one statement (they can
-- overlap only on the newest self_improve run when its shared tracking issue is cached, which is
