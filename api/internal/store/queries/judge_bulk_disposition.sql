...
-- name: ListRecommendationsForCoordsAll :many
...
-- name: UpsertAdminDispositionsForResolvedCoords :execrows
...
SELECT m.review_id, m.category, m.target, 'done', m.rationale_hash, 'admin', @admin_user_id
FROM members m
WHERE NOT EXISTS (
...
-- name: DeleteAdminDispositionsForCoords :execrows
