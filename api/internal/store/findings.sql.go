...
func (q *Queries) ListFindingEvidenceForDispositions(ctx context.Context, ids []uuid.UUID) ([]ListFindingEvidenceForDispositionsRow, error) {
	rows, err := q.db.Query(ctx, listFindingEvidenceForDispositions, ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
