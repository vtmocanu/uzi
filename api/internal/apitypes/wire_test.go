...
func TestUserDTOTags(t *testing.T) {
	assertTags(t, "UserDTO", UserDTO{},
		...
		"wait_on_limit",
		// PRD #1020: the per-user opt-in to an early-limit-reset alert. Default true;
		// mirrors wait_on_limit as a required, always-on-the-wire per-user flag.
...
var runDTOKeys = []string{
	...
	"codex_secret_id", "codex_secret_label",
...
func TestSelfUsageDTOTags(t *testing.T) {
	assertTags(t, "SelfUsageDTO", SelfUsageDTO{}, "lifetime", "last_7_days", "run_count",
		// PRD #1293: failed-run rate windows wrapper.
		"outcomes",
