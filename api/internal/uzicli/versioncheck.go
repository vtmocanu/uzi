...
// after the leading-v normalization both sides need — the CLI ships `v0.63.0`
// (Formula's -X main.version=v#{version}) and the server serves bare `0.63.0`
// (apitypes.BuildInfoDTO.Version). cmp is <0 when the CLI is behind, 0 when equal,
// >0 when ahead. ok is false when either side is not valid semver — a `dev` CLI or
// an empty/garbage server — in which case cmp is 0 and the caller renders the client
// version alone rather than a comparison.
//
// SkewWarning answers only behind-or-not (and fails closed to silent); this is the
// three-way seam the TUI footer readout (issue #687) needs. Both share normSemver's
// re-prefix so the two never disagree about what "equal" means.
func CompareServerVersion(cliVersion, serverVersion string) (cmp int, ok bool) {
	cli, srv := normSemver(cliVersion), normSemver(serverVersion)
	if !semver.IsValid(cli) || !semver.IsValid(srv) {
		return 0, false
	}
	return semver.Compare(cli, srv), true
}
