...
// sign collapses an int to -1/0/+1 so a row asserts on the DIRECTION of the compare,
// not on semver.Compare's exact magnitude (which is always ±1 but need not be).
func sign(n int) int {
	switch {
	case n < 0:
		return -1
...
// TestCompareServerVersion pins the three-way seam: the sign of cmp AND ok, with the
// invariant that ok==false always carries cmp==0. The CLI side ships `v`-prefixed and
// the server side bare, so the equal/behind/ahead rows also prove normSemver is
// applied symmetrically to both operands.
func TestCompareServerVersion(t *testing.T) {
	for _, tc := range []struct {
		...
		{"equal", "v0.63.0", "0.63.0", 0, true},
		{"behind", "v0.1.0", "0.63.0", -1, true},
		{"ahead", "v0.64.0", "0.63.0", 1, true},
		{"normalization symmetric", "0.63.0", "v0.63.0", 0, true},
		{"dev cli", "dev", "0.63.0", 0, false},
		{"empty server", "v0.63.0", "", 0, false},
		{"garbage server", "v0.63.0", "not-a-version", 0, false},
		{"both dev", "dev", "dev", 0, false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			cmp, ok := CompareServerVersion(tc.cli, tc.srv)
			if ok != tc.wantOK {
				t.Fatalf("CompareServerVersion(%q, %q) ok = %v, want %v", tc.cli, tc.srv, ok, tc.wantOK)
			}
			if got := sign(cmp); got != tc.wantSign {
				t.Errorf("CompareServerVersion(%q, %q) sign(cmp) = %d, want %d (cmp=%d)", tc.cli, tc.srv, got, tc.wantSign, cmp)
			}
			if !ok && cmp != 0 {
				t.Errorf("CompareServerVersion(%q, %q) returned cmp=%d with ok=false, want cmp=0", tc.cli, tc.srv, cmp)
			}
		})
	}
}
