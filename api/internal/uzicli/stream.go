...
// 'pool_wait' by PRD #754, and with 'recovery_wait' by issue #1197). A value
...
var knownRunStatuses = map[string]struct{}{
	...
	// while its token pool is empty, resumed when a token is pooled. recovery_wait
	// (issue #1197): a transient-recovery park that auto-resumes on a capped backoff
	// until it recovers or the owner cancels. ALL FIVE are NON-terminal and are
	// deliberately absent from terminalRunStatuses below.
	"limit_wait": {}, "awaiting_input": {}, "awaiting_followup": {}, "pool_wait": {}, "recovery_wait": {},
