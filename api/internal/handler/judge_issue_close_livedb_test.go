...
func TestFiledIssueCloseAutoDonesOnceLiveDB(t *testing.T) {
	...
	// ListOpenImproveUziRecommendationsForUser selects `WHERE rr.category = 'improve_uzi'`
	// across ALL of this owner's reviews — no review scope — and the fixture's decoy below
	// shares the same owner, so within this user `target` is still not unique: every
	...
	//
	// (PRD #590 M2 deleted the instance-wide ListOpenImproveUziRecommendations; the
	// owner-scoped sibling shares the exact same two coordinate exclusions, so this probe of
	// the self-improve backlog's exclusion behaviour is unchanged, only owner-scoped.)
	...
	after, err := q.ListOpenImproveUziRecommendationsForUser(ctx, store.ListOpenImproveUziRecommendationsForUserParams{
		UserID: f.userID,
		Lim:    50,
	})
	if err != nil {
