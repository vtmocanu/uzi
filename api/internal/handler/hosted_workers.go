...
func (h *Handler) HostedConfig(w http.ResponseWriter, r *http.Request) {
	...
	// The ephemeral admin kill-switch (PRD #649). Unlike the quota read above, a
	// settings blip here must HIDE the auto-provision toggle, not error the whole
	// config fetch: the typed getter already returns false on error (fails closed
	// by construction), so the error is deliberately discarded.
