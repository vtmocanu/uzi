...
//
// 197 as of this commit (PRD #1349 M1 added GET /api/worker/runs/{id}/recovery-holds — the
...
// the cookie-only admin ls-remote that resolves the latest semver tag for a typed, unsaved
// source URL).
// It was 162 until then (PRD #636 M1 added POST /api/schedules/{id}/add-repo — the
// custom multi-repo "add another repo" endpoint that replicates a schedule's config onto
// another owned repo as a grouped sibling).
// It was 161 until then (PRD #602 M4 added GET /api/admin/agent-source, POST
// /api/admin/agent-source/sync and POST /api/admin/agent-source/apply — the admin
// agent-source config/status read plus the "Sync now" and approve-and-apply writes).
// It was 158 until then (PRD #576 M6 added POST
...
var wantRouteMounts = []routeMount{
	...
	{"GET", "/api/worker/runs/{id}/forge/latest-pipeline", noLimiter},
	{"GET", "/api/worker/runs/{id}/forge/merge-requests/{iid}", noLimiter},
	...
	{"POST", "/api/admin/agent-source/apply", noLimiter},
	// PRD #702 M2: resolve the latest semver tag for a typed, unsaved source URL —
	// cookie-only admin, single bounded ls-remote round trip, no per-user spend → noLimiter.
	{"POST", "/api/admin/agent-source/resolve-latest", noLimiter},
	...
	// PRD #1183 M3: bulk dismiss and undo-dismiss are LOCAL writes too — no forge call, no
	...
	// Enable a default scheduled job (PRD #589 M2): computes next_fire from the catalog
	// cron and inserts, no forge read → noLimiter (mirrors schedule create).
	{"POST", "/api/repos/{id}/schedule-catalog/{slug}", noLimiter},
	// Sweep-label guardrail (PRD #589 M4): both read/write the repo's forge
	// (ListLabels / EnsureLabels), so they carry the per-user forge budget like the
	...
	// Reset a default schedule (PRD #589 M2): recomputes next_fire from the catalog cron
	// and updates editable fields, no forge read → noLimiter.
	{"POST", "/api/schedules/{id}/reset", noLimiter},
	// Clone a schedule (PRD #589 M3): owner-scoped DB copy, recomputes next_fire from the
	// copied timing, no forge read → noLimiter.
	{"POST", "/api/schedules/{id}/clone", noLimiter},
	// Add a repo to a custom schedule (PRD #636 M1): owner-scoped transactional DB copy
	// (coalesce group + insert sibling), no forge read → noLimiter.
	{"POST", "/api/schedules/{id}/add-repo", noLimiter},
	...
	{"PUT", "/api/me/ephemeral-workers", noLimiter},
