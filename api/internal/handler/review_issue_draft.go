...
import (
	...
	"gitlab.example.com/vtmocanu/uzi/api/internal/issuedraft"
...
// judged run's repo isn't the caller's (an admin viewing another user's run), or
// selfimprove_repo is unset / not the caller's — the draft opens with an empty picker and
// a reason (mock state D) rather than guessing.
func (h *Handler) resolveDefaultRepo(ctx context.Context, callerID uuid.UUID, category string, runRepo pgtype.UUID) (string, string) {
	...
	default: // improve_uzi, install_worker_tool, adjust_template, enable_tool, cost_efficiency
		...
		okNote = "Defaulted from the category to uzi's own repo. Pick any repo you have connected."
	}
	if have {
