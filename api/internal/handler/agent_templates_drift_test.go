...
// TestTemplateDTOCarriesOrigin pins the PRD #602 M5 provenance field on the wire:
// a builtin row's store `origin` (embedded|synced|admin) reaches the DTO as a
// JSON string, and a non-builtin row's NULL origin serializes as JSON null (the
// pointer's zero value) so the web can distinguish "unset" from any value.
func TestTemplateDTOCarriesOrigin(t *testing.T) {
	originJSON := func(t *testing.T, row store.AgentTemplate) (any, bool) {
		t.Helper()
		b, err := json.Marshal(templateDTO(row))
		if err != nil {
			t.Fatalf("marshal dto: %v", err)
		...
		v, present := got["origin"]
		if !present {
			t.Fatalf("dto is missing the origin key entirely: %s", b)
		}
		return v, present
	}

	t.Run("synced builtin serializes origin=synced", func(t *testing.T) {
		row := seededRow(t, "coder")
		row.Origin = pgtype.Text{String: "synced", Valid: true}
		if v, _ := originJSON(t, row); v != "synced" {
			t.Errorf("origin = %v, want %q", v, "synced")
		}
	})

	t.Run("embedded builtin serializes origin=embedded", func(t *testing.T) {
		row := seededRow(t, "coder")
		row.Origin = pgtype.Text{String: "embedded", Valid: true}
		if v, _ := originJSON(t, row); v != "embedded" {
			t.Errorf("origin = %v, want %q", v, "embedded")
		}
	})

	t.Run("non-synced global row serializes origin=null", func(t *testing.T) {
		row := store.AgentTemplate{
			ID:          uuid.New(),
			Name:        "release-notes",
			...
			PromptBody:  "x\n",
			Scope:       "global",
			// Origin left zero: NULL pgtype.Text.
		}
		if v, _ := originJSON(t, row); v != nil {
			t.Errorf("origin = %v, want JSON null for a NULL-origin row", v)
		}
	})
}
