...
// consistent (PRD #308 M3): the closed set has exactly the five expected members with no
// duplicates, every benign seam sentinel maps into that set, an unrelated error maps to
// no reason, and the two reasons the seam does not map (fetch_failed is recorded at the
// sweep site, already_running also at the prompt site) are still enumerated. The
